...
#if 0	// To remove compiler warning ... -Wcomment
//REMOVED: #if !defined(unix) && !defined(__unix__) && !defined(__unix) && !defined(__APPLE__) && !defined(_WIN32)
//REMOVED: #error "This module only works on Unix and Windows, see MBEDTLS_NET_C in config.h"
//REMOVED: #endif
#endif	// 0	: To remove compiler warning ... -Wcomment
...
#if 0	// To remove compiler warning ... -Wcomment
//REMOVED: #if (defined(_WIN32) || defined(_WIN32_WCE)) && !defined(EFIX64) && !defined(EFI32)
//REMOVED:
 ...
 */
int mbedtls_net_bsd_connect( mbedtls_net_context *ctx, const char *host,
										...
										const char *port, int proto, unsigned int timeout)
{
	...
	for (cur = addr_list ; cur != NULL ; cur = cur->ai_next) {
		...
		if (select(ctx->fd + 1, NULL, &write_fds, NULL, &tv) == 1) {
			...
			getsockopt(ctx->fd, SOL_SOCKET, SO_ERROR, &sockerr, &sockerr_len);
...
static int net_would_block_dpm(const mbedtls_net_context_dpm *ctx)
{
    /*
     * Never return 'WOULD BLOCK' on a non-blocking socket
     */
...
{
    int ret;
    int fd = ((mbedtls_net_context_dpm *)ctx)->fd;
...
void mbedtls_net_bsd_free_dpm(mbedtls_net_context_dpm *ctx)
{
	...
	ctx->fd = -1;
	memset(ctx->name, 0x00, sizeof(ctx->name));
}
#endif /* MBEDTLS_NET_C */
