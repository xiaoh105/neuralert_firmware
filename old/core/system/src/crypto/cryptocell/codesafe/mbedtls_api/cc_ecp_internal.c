             ...
             const mbedtls_mpi *m, const mbedtls_ecp_point *P )
{
	...
	resPoint = (uint8_t *)mbedtls_calloc(1, (CC_EC_MONT_MOD_SIZE_IN_BYTES*sizeof(uint8_t)) );
	scalar = (uint8_t *)mbedtls_calloc(1, (MBEDTLS_ECP_MAX_BYTES*sizeof(uint8_t)) );
	px = (uint8_t *)mbedtls_calloc(1, (CC_EC_MONT_MOD_SIZE_IN_BYTES*sizeof(uint8_t)) );

	if( (resPoint == NULL) || (scalar == NULL) || (px == NULL) ){
                     ...
                     void *p_rng )
{
	...
	if (G != NULL)
	{
		rc = CC_EcMontKeyPairBase(resPoint, &resPointSize, scalar, &scalarSize, px, (size_t)CC_EC_MONT_MOD_SIZE_IN_BYTES, pRndContext, ecMontTempBuff);
	}
	else
	{
