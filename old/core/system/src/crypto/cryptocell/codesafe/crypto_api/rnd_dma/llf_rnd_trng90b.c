    ...
    * from fast to slow Rosc                  */
    ...
    for (i = 0; i < LLF_RND_NUM_OF_ROSCS; ++i) {
        *sourceOutSize_ptr = trng90bRequiredBytes;
		...
		//save_with_run_time("LLF_RND_TRNG_ReadMultipleEHR");
#endif
        ...
        if (error == CC_OK) {
#if defined(__RUNTIME_CALCULATION__) && defined(XIP_CACHE_BOOT)
			//save_with_run_time("runContinuousTesting");
#endif
...
#if defined(__RUNTIME_CALCULATION__) && defined(XIP_CACHE_BOOT)
				//save_with_run_time("LLF_RND_StartTrngHW");
#endif
...
#else
#if defined(__RUNTIME_CALCULATION__) && defined(XIP_CACHE_BOOT)
	//save_with_run_time("LLF_RND_TurnOffTrng");
#endif
    LLF_RND_TurnOffTrng();
#endif
...
#if defined(__RUNTIME_CALCULATION__) && defined(XIP_CACHE_BOOT)
	//save_with_run_time("Start Loop LLF_RND_TRNG_ReadEhrData");
#endif
    while (inSize) {
        LLF_RND_TurnOffTrng();
...
#if defined(__RUNTIME_CALCULATION__) && defined(XIP_CACHE_BOOT)
	//sprintf(tmpString, "End loop (%d)", counter);
	//save_with_run_time(tmpString);
#endif
    return CC_OK;
}
