                ...
                mbedtls_srp_context *pCtx)
{
    ...
    // verify input
    if ((ephemPrivSize < CC_SRP_PRIV_NUM_MIN_SIZE) || (ephemPrivSize > CC_SRP_PRIV_NUM_MAX_SIZE) ||
        (pwdVerifier == NULL) ||
        (hostPubKeyB == NULL) ||
        (pCtx == NULL) || (pCtx->pRndCtx->rndGenerateVectFunc == NULL)) {
        return CC_SRP_PARAM_INVALID_ERROR;
                ...
                mbedtls_srp_context *pCtx)
{
    ...
    // Use PKA to calculate S=(((A*v^u)^b)%N
    rc = SrpHostSessionKeyCalc(userPubKeyA,
                pwdVerifier,
                digBuff,
                sessionKey,
                pCtx);
    ...
    rc = SRP_SharedSecretCalc((uint8_t *)sessionKey,
...
end_with_error:
    ...
    CC_PalMemSetZero(sharedSecret, sizeof(mbedtls_srp_secret));
    CC_PalMemSetZero(pCtx, sizeof(mbedtls_srp_context));
    return rc;
}
                ...
                mbedtls_srp_context *pCtx)
{
    ...
    rc = SrpUserSessionKeyCalc(hostPubKeyB,
                xBuff,
                uScramble,
                sessionKey,
                pCtx);
