...
static const int ciphersuite_da16x_preference[] =
{
    /* Using HW Engine */
    /* All AES-256 ephemeral suites */
    MBEDTLS_TLS_ECDHE_ECDSA_WITH_AES_256_CCM,
...
static const int ciphersuite_da16x_hw_preference[] =
{
    /* All AES-256 ephemeral suites */
    MBEDTLS_TLS_ECDHE_ECDSA_WITH_AES_256_CCM,
    MBEDTLS_TLS_DHE_RSA_WITH_AES_256_CCM,
...
{
    return( ciphersuite_da16x_preference );
}

const int *mbedtls_ssl_list_da16x_hw_ciphersuites( void )
{
    return( ciphersuite_da16x_hw_preference );
}
...
const int *mbedtls_ssl_list_ciphersuites( void )
{
    ...
    if( supported_init == 0 )
    {
             ...
             p++ )
        {
            ...
            { // DEBUG:
#if 0 /* Blocked debug log. */
                mbedtls_printf("Cipher-Suite: %04x, %s %s%s%s\n"
                        , cs_info->id
                        , cs_info->name
                        , (cs_info->flags & MBEDTLS_CIPHERSUITE_WEAK) ? ", \"Weak\"" : ""
                        , (cs_info->flags & MBEDTLS_CIPHERSUITE_SHORT_TAG) ? ", \"Short-Auth-Tag\"" : ""
                        , (cs_info->flags & MBEDTLS_CIPHERSUITE_NODTLS) ? ", \"No-DTLS\"" : ""
                        );
#endif
                *(q++) = *p;
            }
...
const int *mbedtls_ssl_list_da16x_ciphersuites( void )
{
    ...
    if( supported_init == 0 )
    {
        ...
        for( p = ciphersuite_da16x_preference, q = supported_ciphersuites;
             *p != 0 && q < supported_ciphersuites + MAX_CIPHERSUITES - 1;
             p++ )
        {
#if defined(MBEDTLS_REMOVE_ARC4_CIPHERSUITES)
...
const int *mbedtls_ssl_list_da16x_hw_ciphersuites( void )
{
    ...
    if( supported_init == 0 )
    {
        ...
        for( p = ciphersuite_da16x_hw_preference, q = supported_ciphersuites;
             *p != 0 && q < supported_ciphersuites + MAX_CIPHERSUITES - 1;
             p++ )
        {
#if defined(MBEDTLS_REMOVE_ARC4_CIPHERSUITES)
