...
void da16x_des_isr(void)
{
	if (g_evt != NULL)
		OAL_SET_EVENTS_HISR((g_evt), (DES_INT_EVT) , OAL_OR );
}
static void da16x_des_irq( void )
{
	INTR_CNTXT_CALL(da16x_des_isr);
}
int alt_des3_init( alt_des3_context *des )
{
	...
	if( OAL_CREATE_SEMAPHORE( (OAL_SEMAPHORE *)(&(des->des_sema)), "des.mut", 1, OAL_SUSPEND) == OAL_SUCCESS ){
		if( _sys_nvic_write(DES_INT, (void *)da16x_des_irq, 0x7) == TRUE ){
			if (OAL_CREATE_EVENT_GROUP(&(des->des_event), "desevt") == OAL_SUCCESS){
				g_evt = &des->des_event;
				return 1;
			}
			else {
				OAL_DELETE_SEMAPHORE((&(des->des_sema)));
				_sys_nvic_write(DES_INT, NULL, 0x7);
				return 0;
			}
		}
		else {
...
int alt_des3_free( alt_des3_context *des )
{
	if (des == NULL)
		return 0;
	OAL_DELETE_SEMAPHORE((&(des->des_sema)));
	OAL_DELETE_EVENT_GROUP(&(des->des_event));
	_sys_nvic_write(DES_INT, NULL, 0x7);
	g_evt = NULL;
	return 1;
}
int alt_des3_setkey(alt_des3_context *des, const uint8_t key[8])
{
	uint32_t *ukey = (uint32_t*)key;
	uint32_t timeOut = 10;
	if (des == NULL)
		return 0;
	if( OAL_OBTAIN_SEMAPHORE( (OAL_SEMAPHORE *)(&(des->des_sema)), timeOut ) == OAL_SUCCESS ){
		...
		OAL_RELEASE_SEMAPHORE( (OAL_SEMAPHORE *)(&(des->des_sema)));
		return 1;
	}
	else
		return 0;
}
int alt_des3_set2key(alt_des3_context *des, const uint8_t key[16])
{
	uint32_t *ukey = (uint32_t*)key;
	uint32_t timeOut = 10;
	if (des == NULL)
...
int alt_des3_set3key(alt_des3_context *des, const uint8_t key[24])
{
	uint32_t *ukey = (uint32_t*)key;
	uint32_t timeOut = 10;
	if (des == NULL)
						...
						uint8_t *output )
{
	...
	if (mode == MBEDTLS_DES_ENCRYPT)
	{
		...
		// wait
#ifdef POLLING
		while ( ((*((volatile unsigned int *)(0x50020000)) ) & 0x01000000) == 0);
#else
		if (OAL_RETRIEVE_EVENTS(&(des->des_event), (DES_INT_EVT) , OAL_OR, &evt_mask, timeOut ) == OAL_SUCCESS ){
#endif
		// get 64bit data
		...
		OAL_SET_EVENTS(&(des->des_event),~(DES_INT_EVT),OAL_AND);
#endif
		}
#ifdef POLLING
#else
...
{
    int i, j, u, v, ret = 0;
    alt_des3_context ctx3;
    unsigned char buf[8], result[8];
    unsigned char prv[8];
    unsigned char iv[8];
    ...
    for( i = 0; i < 6; i++ )
    {
        ...
        case 0:
		case 1:
            alt_des3_setkey( &ctx3, des3_test_keys );
            break;

        case 2:
		case 3:
            alt_des3_set2key( &ctx3, des3_test_keys );
            break;

        case 4:
		case 5:
            alt_des3_set3key( &ctx3, des3_test_keys );
            break;
        default:
            return( 1 );
        }
        ...
        if( v == MBEDTLS_DES_DECRYPT )
        {
            for( j = 0; j < 10000; j++ )
            {
                alt_des3_crypt_cbc( &ctx3, v, 8, iv, buf, result );
				memcpy(buf, result, 8);
            }
        }
        else
