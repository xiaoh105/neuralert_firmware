...
{
    unsigned char *p = buf;
    const unsigned char *end = ssl->out_msg + ssl->conf->out_content_len;
    size_t hostname_len;
                                    ...
                                    size_t pms_offset )
{
    ...
    if( offset + len_bytes > ssl->conf->out_content_len )
    {
    ...
    if( ( ret = mbedtls_pk_encrypt( &ssl->session_negotiate->peer_cert->pk,
                            ...
                            ssl->conf->out_content_len - offset - len_bytes,
...
static int ssl_write_client_key_exchange( mbedtls_ssl_context *ssl )
{
    ...
    if( mbedtls_ssl_ciphersuite_uses_psk( ciphersuite_info ) )
    {
        ...
        if( i + 2 + n > ssl->conf->out_content_len )
        {
        ...
        if( ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_PSK )
        {
            ...
            ret = mbedtls_ecdh_make_public( &ssl->handshake->ecdh_ctx, &n,
                    &ssl->out_msg[i], ssl->conf->out_content_len - i,
                    ssl->conf->f_rng, ssl->conf->p_rng );
            if( ret != 0 )
            {
                MBEDTLS_SSL_DEBUG_RET( 1, "mbedtls_ecdh_make_public", ret );
    ...
    if( ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECJPAKE )
    {
        ...
        ret = mbedtls_ecjpake_write_round_two( &ssl->handshake->ecjpake_ctx,
                ssl->out_msg + i, ssl->conf->out_content_len - i, &n,
                ssl->conf->f_rng, ssl->conf->p_rng );
        if( ret != 0 )
        {
