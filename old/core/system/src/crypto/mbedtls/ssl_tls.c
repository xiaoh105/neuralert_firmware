...
#define	mbedtls_zeroize(v, n)	memset(v, 0, n)

#if defined(MBEDTLS_SSL_USE_STATIC_CONTENT_BUFFER)
typedef struct _mbedtls_peer_buf_crt {
	unsigned char flag;	//0: not used. 1: used.
	unsigned char in_buf[MBEDTLS_SSL_IN_BUFFER_LEN];
	unsigned char out_buf[MBEDTLS_SSL_OUT_BUFFER_LEN];
} mbedtls_peer_buf_crt;

static mbedtls_peer_buf_crt	mbedtls_peer_buf[MBEDTLS_SSL_MAX_PEER];

static int mbedtls_ssl_alloc_peer_buf(unsigned char **in_buf, unsigned char **out_buf)
{
    for (int idx = 0 ; idx < MBEDTLS_SSL_MAX_PEER ; idx++) {
        if (mbedtls_peer_buf[idx].flag == 0) {
            mbedtls_peer_buf[idx].flag = 1;
            *in_buf = mbedtls_peer_buf[idx].in_buf;
            *out_buf = mbedtls_peer_buf[idx].out_buf;
            return 0;
        }
    }
...
static int mbedtls_ssl_free_buf(unsigned char **in_buf, unsigned char **out_buf)
{
    for (int idx = 0 ; idx < MBEDTLS_SSL_MAX_PEER ; idx++) {
        if (mbedtls_peer_buf[idx].flag == 1
            && *in_buf == mbedtls_peer_buf[idx].in_buf
            && *out_buf == mbedtls_peer_buf[idx].out_buf) {

            mbedtls_zeroize(&mbedtls_peer_buf[idx], sizeof(mbedtls_peer_buf_crt));
        }
    }
...
#endif	/* MBEDTLS_SSL_USE_STATIC_CONTENT_BUFFER */
 ...
 */
static unsigned int ssl_mfl_code_to_length( const mbedtls_ssl_config *conf, int mfl )
{
    const unsigned int max_len = (16 * 1024);

    if( !conf ) {
        return( MBEDTLS_SSL_DEF_FRAG_LEN );
    }
    ...
    if( conf->in_content_len > conf->out_content_len )
    {
        return( ( conf->out_content_len > max_len ) ? max_len : conf->out_content_len );
    }

    return( ( conf->in_content_len > max_len ) ? max_len : conf->in_content_len );
}
...
#if defined(MBEDTLS_SSL_REDUCE_PEER_CERT_CHAIN)
static int ssl_reduce_peer_cert_chain( mbedtls_ssl_context *ssl )
{
    int ret = 0;
    mbedtls_x509_crt *del_crt = NULL;

    if( (ssl->state != MBEDTLS_SSL_HANDSHAKE_OVER) ||
        (ssl->session->peer_cert == NULL) ||
        (ssl->session->peer_cert->next == NULL) )
    {
        return( 0 );
    }

    /* skip the first certificate */
    del_crt = ssl->session->peer_cert->next;

    if (del_crt) {
        mbedtls_x509_crt_free(del_crt);
        mbedtls_free(del_crt);
    }
...
#endif /* MBEDTLS_SSL_REDUCE_PEER_CERT_CHAIN */
                            ...
                            unsigned char *dstbuf, size_t dlen )
{
    size_t nb;
    size_t i, j, k, md_len;
#if 0 /* increased buffer size because of sha384. 20181203 */
    unsigned char tmp[128];
#else
...
int mbedtls_ssl_derive_keys( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->conf->f_export_keys_for_eap_tls != NULL )
    {
        ssl->conf->f_export_keys_for_eap_tls(ssl->conf->p_export_keys_for_eap_tls,
                                             session->master,
                                             handshake->randbytes,
                                             session->ticket, session->ticket_len,
                                             handshake->tls_prf);
    }
#endif /* defined(MBEDTLS_SSL_EAP_TLS) */
...
#if defined(MBEDTLS_SSL_DPM_SUPPORT)
    memcpy(transform->keyblk, keyblk, 256);
...
static int ssl_encrypt_buf( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->out_msglen > ssl->conf->out_content_len )
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "Record content %u too large, maximum %d",
                                    (unsigned) ssl->out_msglen,
                                    ssl->conf->out_content_len ) );
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
    }
...
static inline uint64_t ssl_get_be64(unsigned char *a)
{
...
static int ssl_decrypt_buf( mbedtls_ssl_context *ssl )
{
        ...
        mode == MBEDTLS_MODE_CCM )
    {
        ...
        if( ssl->conf->force_sync_iv ) {
            if( ssl_get_be64( ssl->in_ctr ) > 0 ) {
                const uint32_t max_allowed_gap = ssl->conf->force_sync_iv_gap;
                uint64_t diff = ssl_get_be64( ssl->in_iv ) - ssl_get_be64( ssl->prev_in_iv );

                //increase message seq
                if( ( diff != 1 ) && ( diff < max_allowed_gap ) ) {
                    for( uint64_t inc = 1; inc < diff; inc++ ) {
            ...
            memcpy(ssl->prev_in_iv, ssl->in_iv, 8);
        }
    ...
    defined(MBEDTLS_SSL_PROTO_TLS1_2)
        if( ssl->minor_ver > MBEDTLS_SSL_MINOR_VERSION_0 )
        {
            ...
            correct &= ( padding_idx < ssl->conf->in_content_len +
                                       ssl->transform_in->maclen );
...
int mbedtls_ssl_fetch_input( mbedtls_ssl_context *ssl, size_t nb_want )
{
    ...
    if( nb_want > ssl->in_buflen - (size_t)( ssl->in_hdr - ssl->in_buf ) )
    {
    ...
    if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
    {
        ...
        {
            len = ssl->in_buflen - ( ssl->in_hdr - ssl->in_buf );
...
int mbedtls_ssl_write_record( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->out_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE )
    {
        ...
        if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
        {
            ...
            if( ssl->conf->out_content_len - ssl->out_msglen < 8 )
            {
                MBEDTLS_SSL_DEBUG_MSG( 1, ( "DTLS handshake message too large: "
                              "size %u, maximum %u",
                               (unsigned) ( ssl->in_hslen - 4 ),
                               (unsigned) ( ssl->conf->out_content_len - 12 ) ) );
                return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
            }
...
static int ssl_reassemble_dtls_handshake( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->handshake->hs_msg == NULL )
    {
        ...
        if( ssl->in_hslen > ssl->conf->in_content_len )
        {
    ...
    if( ssl->in_left > ssl->next_record_offset )
    {
        ...
        if( ssl->in_left > ssl->in_buflen - (size_t)( ssl->in_hdr - ssl->in_buf ) )
        {
...
static int ssl_handle_possible_reconnect( mbedtls_ssl_context *ssl )
{
    ...
    ret = ssl_check_dtls_clihlo_cookie(
            ...
            ssl->out_buf, ssl->conf->out_content_len, &len );
...
static int ssl_parse_record_header( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->in_msglen > ssl->in_buflen - (size_t)( ssl->in_msg - ssl->in_buf ) )
    {
    ...
    {
        if( ssl->in_msglen < 1 ||
            ssl->in_msglen > ssl->conf->in_content_len )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad message length" ) );
            return( MBEDTLS_ERR_SSL_INVALID_RECORD );
        }
...
#if defined(MBEDTLS_SSL_PROTO_SSL3)
        if( ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0 &&
            ssl->in_msglen > ssl->transform_in->minlen + ssl->conf->in_content_len )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad message length" ) );
            return( MBEDTLS_ERR_SSL_INVALID_RECORD );
        }
...
static int ssl_prepare_record_content( mbedtls_ssl_context *ssl )
{
    ...
    if( !done && ssl->transform_in != NULL )
    {
        ...
        if( ssl->in_msglen > ssl->conf->in_content_len )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad message length" ) );
            return( MBEDTLS_ERR_SSL_INVALID_RECORD );
        }
...
int mbedtls_ssl_write_certificate( mbedtls_ssl_context *ssl )
{
    ...
    {
        n = crt->raw.len;
        if( n > ssl->conf->out_content_len - 3 - i )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "certificate too large, %d > %d",
                           i + 3 + n, ssl->conf->out_content_len ) );
                       ...
                       const mbedtls_ssl_config *conf )
{
    int ret;
#if !defined(MBEDTLS_SSL_USE_STATIC_CONTENT_BUFFER)
    const size_t inlen = conf->in_content_len 
   ...
   const size_t outlen =  conf->out_content_len 
                            + MBEDTLS_SSL_COMPRESSION_ADD
                            + MBEDTLS_MAX_IV_LENGTH 
                            + MBEDTLS_SSL_MAC_ADD 
                            + MBEDTLS_SSL_PADDING_ADD
     ...
     */
#if defined(MBEDTLS_SSL_USE_STATIC_CONTENT_BUFFER)
    ret = mbedtls_ssl_alloc_peer_buf(&ssl->in_buf, &ssl->out_buf);
    if (ret) {
        return( ret );
    }

    ssl->in_buflen = MBEDTLS_SSL_IN_BUFFER_LEN;
    ssl->out_buflen = MBEDTLS_SSL_OUT_BUFFER_LEN;
#else

    if( ( ssl-> in_buf = mbedtls_calloc( 1, inlen ) ) == NULL ||
            ( ssl->out_buf = mbedtls_calloc( 1, outlen ) ) == NULL )
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "alloc(tx:%d,rx:%d bytes) failed", inlen, outlen ) );
        if (ssl->in_buf)
...
static int ssl_session_reset_int( mbedtls_ssl_context *ssl, int partial )
{
    ...
    memset( ssl->out_buf, 0, ssl->out_buflen );
    if( partial == 0 )
        memset( ssl->in_buf, 0, ssl->in_buflen );
                ...
                const unsigned char *psk_identity, size_t psk_identity_len )
{
    ...
    if( ( psk_identity_len >> 16 ) != 0 ||
        psk_identity_len > conf->in_content_len )
    {
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
    }
...
int mbedtls_ssl_conf_max_frag_len( mbedtls_ssl_config *conf, unsigned char mfl_code )
{
    ...
    if( mfl_code >= MBEDTLS_SSL_MAX_FRAG_LEN_INVALID ||
        ssl_mfl_code_to_length( conf, mfl_code ) > max_len )
    {
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
    }
...
#if defined(MBEDTLS_SSL_EAP_TLS)
void mbedtls_ssl_conf_export_keys_cb_for_eap_tls( mbedtls_ssl_config *conf,
		mbedtls_ssl_export_keys_for_eap_tls_t *f_export_keys,
		void *p_export_keys)
{
    conf->f_export_keys_for_eap_tls = f_export_keys;
    conf->p_export_keys_for_eap_tls = p_export_keys;
}
#endif /* deinfed(MBEDTLS_SSL_EAP_TLS) */
...
size_t mbedtls_ssl_get_max_frag_len( const mbedtls_ssl_context *ssl )
{
     ...
     */
    max_len = ssl_mfl_code_to_length( ssl->conf, ssl->conf->mfl_code );

    /* Check if a smaller max length was negotiated */
    if( ssl->session_out != NULL &&
        ssl_mfl_code_to_length( ssl->conf, ssl->session_out->mfl_code ) < max_len )
    {
        max_len = ssl_mfl_code_to_length( ssl->conf, ssl->session_out->mfl_code );
    }

    /* During a handshake, use the value being negotiated */
    if( ssl->session_negotiate != NULL &&
        ssl_mfl_code_to_length( ssl->conf, ssl->session_negotiate->mfl_code ) < max_len )
    {
        max_len = ssl_mfl_code_to_length( ssl->conf, ssl->session_negotiate->mfl_code );
    }
...
#if defined(MBEDTLS_SSL_REDUCE_PEER_CERT_CHAIN)
    if (ret == 0 && ssl->state == MBEDTLS_SSL_HANDSHAKE_OVER) {
        ssl_reduce_peer_cert_chain(ssl);
    }
#endif /* MBEDTLS_SSL_REDUCE_PEER_CERT_CHAIN */
...
#else
    size_t max_len = ssl->conf->out_content_len;
#endif /* MBEDTLS_SSL_MAX_FRAGMENT_LENGTH */
    if( len > max_len )
    {
#if defined(MBEDTLS_SSL_PROTO_DTLS)
...
#if defined(MBEDTLS_SSL_USE_STATIC_CONTENT_BUFFER)
    mbedtls_ssl_free_buf(&ssl->in_buf, &ssl->out_buf);
#else
    if( ssl->out_buf != NULL )
    {
        mbedtls_zeroize( ssl->out_buf, ssl->out_buflen );
        mbedtls_free( ssl->out_buf );
    }

    if( ssl->in_buf != NULL )
    {
        mbedtls_zeroize( ssl->in_buf, ssl->in_buflen );
        mbedtls_free( ssl->in_buf );
    }
#endif	/* MBEDTLS_SSL_USE_STATIC_CONTENT_BUFFER */
                                 ...
                                 int endpoint, int transport, int preset )
{
    ...
    switch( preset )
    {
        ...
        case MBEDTLS_SSL_PRESET_DA16X:
            ...
            conf->ciphersuite_list[MBEDTLS_SSL_MINOR_VERSION_2] =
            conf->ciphersuite_list[MBEDTLS_SSL_MINOR_VERSION_3] =
                                   mbedtls_ssl_list_da16x_ciphersuites();
        ...
        /*
         * DA16X Suite only using HW engine
         */
        case MBEDTLS_SSL_PRESET_DA16X_ONLY_HW:
            conf->min_major_ver = ( MBEDTLS_SSL_MIN_MAJOR_VERSION >
                                    MBEDTLS_SSL_MIN_VALID_MAJOR_VERSION ) ?
                                    MBEDTLS_SSL_MIN_MAJOR_VERSION :
                                    MBEDTLS_SSL_MIN_VALID_MAJOR_VERSION;
            ...
            conf->ciphersuite_list[MBEDTLS_SSL_MINOR_VERSION_2] =
            conf->ciphersuite_list[MBEDTLS_SSL_MINOR_VERSION_3] =
                                   mbedtls_ssl_list_da16x_hw_ciphersuites();
    ...
    conf->in_content_len = MBEDTLS_SSL_MAX_IN_CONTENT_LEN;
    conf->out_content_len = MBEDTLS_SSL_MAX_OUT_CONTENT_LEN;
...
int mbedtls_ssl_set_pac_info( mbedtls_ssl_context *ssl,
                              const unsigned char *pac, size_t pac_len )
{
...
int mbedtls_ssl_is_decrypting( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl && ssl->state == MBEDTLS_SSL_HANDSHAKE_OVER ){
        hdr_len = mbedtls_ssl_hdr_len( ssl );
