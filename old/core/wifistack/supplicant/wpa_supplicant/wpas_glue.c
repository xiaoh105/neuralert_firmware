					   ...
					   struct wpa_config_blob *blob)
{
	...
	if (wpa_s->conf->update_config) {
		...
		if (ret) {
			wpa_printf_dbg(MSG_DEBUG, "Failed to update config after "
				   "blob set");
		}
	}
}
...
static int wpa_ether_send(struct wpa_supplicant *wpa_s, const u8 *dest,
			  u16 proto, const u8 *buf, size_t len, int is_hs_ack)
{
				     ...
				     size_t len)
{
	...
	if (wpa_key_mgmt_wpa_psk(wpa_s->key_mgmt) ||
		 ...
		 * machines. */
		wpa_printf_dbg(MSG_DEBUG, "WPA: drop TX EAPOL in non-IEEE 802.1X "
	...
	if (is_zero_ether_addr(wpa_s->bssid)) {
		wpa_printf_dbg(MSG_DEBUG, "BSSID not set when trying to send an "
			   "EAPOL frame");
		if (wpa_drv_get_bssid(wpa_s, bssid) == 0 &&
		    !is_zero_ether_addr(bssid)) {
			dst = bssid;
			wpa_printf_dbg(MSG_DEBUG, "Using current BSSID " MACSTR
				   " from the driver as the EAPOL destination",
				   MAC2STR(dst));
		} else {
			dst = wpa_s->last_eapol_src;
			wpa_printf_dbg(MSG_DEBUG, "Using the source address of the"
	...
	wpa_printf_dbg(MSG_DEBUG, "TX EAPOL: dst=" MACSTR, MAC2STR(dst));
	wpa_hexdump(MSG_MSGDUMP, "TX EAPOL", msg, msglen);
	res = wpa_ether_send(wpa_s, dst, ETH_P_EAPOL, msg, msglen, 0);
	os_free(msg);
	return res;
}
				    ...
				    void *ctx)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAPOL authentication completed - result=%s",
		   result_str(result));
	...
	wpa_printf_dbg(MSG_DEBUG, "Configure PMK for driver-based RSN 4-way "
		   "handshake");
	...
	if (wpa_key_mgmt_ft(wpa_s->key_mgmt)) {
		...
		u8 buf[2 * PMK_LEN];
		wpa_printf_dbg(MSG_DEBUG, "RSN: Use FT XXKey as PMK for "
	...
	if (res) {
		wpa_printf_dbg(MSG_DEBUG, "Failed to get PMK from EAPOL state "
			   "machines");
		return;
	}
	...
	if (wpa_drv_set_key(wpa_s, WPA_ALG_PMK, NULL, 0, 0, NULL, 0, pmk,
			    pmk_len)) {
		wpa_printf_dbg(MSG_DEBUG, "Failed to set PMK to the driver");
	}
...
{
	struct wpa_supplicant *wpa_s = ctx;
	wpa_dbg(wpa_s, MSG_DEBUG, "WPA: EAPOL processing complete");
	if (wpa_key_mgmt_wpa_ieee8021x(wpa_s->key_mgmt)) {
...
{
	return wpa_ether_send(wpa_s, dest, proto, buf, len, is_hs_ack);
}
					   ...
					   enum eap_proxy_sim_state sim_state)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "eap_proxy: SIM card status %u", sim_state);
	switch (sim_state) {
	case SIM_STATE_ERROR:
		wpa_sm_sim_state_error_handler(wpa_s);
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "eap_proxy: SIM card status unknown");
		break;
	}
}
...
#ifdef CONFIG_AP
	if (wpa_s->ap_iface) {
		wpa_printf_dbg(MSG_DEBUG, "AP mode active - skip EAPOL Supplicant "
			   "port status: %s",
			   authorized ? "Authorized" : "Unauthorized");
		return;
	}
#endif /* CONFIG_AP */
	wpa_printf_dbg(MSG_DEBUG, "EAPOL: Supplicant port status: %s",
		   authorized ? "Authorized" : "Unauthorized");
...
{
	da16x_eapol_prt("[%s] " WPA_EVENT_EAP_STATUS "status='%s' parameter='%s'\n",
			__func__, status, parameter);
}


static void wpa_supplicant_eap_error_cb(void *ctx, int error_code)
{
	da16x_eapol_prt("[%s] " WPA_EVENT_EAP_STATUS "error_code=%d\n",
			__func__, error_code);
}
...
#else
		res = wpa_config_write(wpa_s->conf, wpa_s->ifname);
...
int wpa_supplicant_init_wpa(struct wpa_supplicant *wpa_s)
{
	...
	ctx = os_zalloc(sizeof(*ctx));
	if (ctx == NULL) {
		da16x_eapol_prt("[%s] Failed to allocate WPA context\n", __func__);
		return -1;
	}
	...
	if (wpa_s->wpa == NULL) {
		da16x_eapol_prt("[%s] Failed to initialize WPA state machine",
					__func__);
		return -1;
	}
