				      ...
				      u16 status_code)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "ANQP: Response callback dst=" MACSTR
				      ...
				      struct wpa_bss *bss)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Interworking: ANQP Query Request to " MACSTR,
		   MAC2STR(bss->bssid));
#ifdef CONFIG_GAS
	...
	if (res < 0) {
		wpa_printf_dbg(MSG_DEBUG, "ANQP: Failed to send Query Request");
		wpabuf_free(buf);
		ret = -1;
		da16x_eloop_register_timeout(0, 0, interworking_continue_anqp, wpa_s,
				       NULL);
	} else
		wpa_printf_dbg(MSG_DEBUG, "ANQP: Query started with dialog token "
			   "%u", res);
#endif /* CONFIG_GAS */
				      ...
				      const u8 *end)
{
	...
	if (pos + 3 > end) {
		wpa_printf_dbg(MSG_DEBUG, "No room for EAP Method fixed fields");
		return NULL;
	}

	elen = *pos++;
	if (pos + elen > end || elen < 2) {
		wpa_printf_dbg(MSG_DEBUG, "No room for EAP Method subfield");
		return NULL;
	}
	e_end = pos + elen;
	e->method = *pos++;
	auth_count = *pos++;
	wpa_printf_dbg(MSG_DEBUG, "EAP Method: len=%u method=%u auth_count=%u",
	...
	for (a = 0; a < auth_count; a++) {
		...
		if (pos + 2 > end || pos + 2 + pos[1] > end) {
			wpa_printf_dbg(MSG_DEBUG, "No room for Authentication "
				   "Parameter subfield");
			return NULL;
		}
		...
		case NAI_REALM_EAP_AUTH_NON_EAP_INNER_AUTH:
			...
			case NAI_REALM_INNER_NON_EAP_PAP:
				wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/PAP");
				break;
			case NAI_REALM_INNER_NON_EAP_CHAP:
				wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/CHAP");
				break;
			case NAI_REALM_INNER_NON_EAP_MSCHAP:
				wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/MSCHAP");
				break;
			case NAI_REALM_INNER_NON_EAP_MSCHAPV2:
				wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/MSCHAPV2");
				break;
			}
			break;
		case NAI_REALM_EAP_AUTH_INNER_AUTH_EAP_METHOD:
			if (len < 1)
				break;
			e->inner_method = *pos;
			wpa_printf_dbg(MSG_DEBUG, "Inner EAP method: %u",
				   e->inner_method);
			break;
		case NAI_REALM_EAP_AUTH_CRED_TYPE:
			if (len < 1)
				break;
			e->cred_type = *pos;
			wpa_printf_dbg(MSG_DEBUG, "Credential Type: %u",
				   e->cred_type);
			break;
		case NAI_REALM_EAP_AUTH_TUNNELED_CRED_TYPE:
			...
			wpa_printf_dbg(MSG_DEBUG, "Tunneled EAP Method Credential "
				   "Type: %u", e->tunneled_cred_type);
			break;
		default:
			wpa_printf_dbg(MSG_DEBUG, "Unsupported Authentication "
					...
					const u8 *end)
{
	...
	if (end - pos < 4) {
		wpa_printf_dbg(MSG_DEBUG, "No room for NAI Realm Data "
			   "fixed fields");
		return NULL;
	}
	...
	if (pos + realm_len > f_end) {
		wpa_printf_dbg(MSG_DEBUG, "No room for NAI Realm "
			   "(len=%u; left=%u)",
	...
	if (pos + 1 > f_end) {
		wpa_printf_dbg(MSG_DEBUG, "No room for EAP Method Count");
		return NULL;
	}
	r->eap_count = *pos++;
	wpa_printf_dbg(MSG_DEBUG, "EAP Count: %u", r->eap_count);
	if (pos + r->eap_count * 3 > f_end) {
		wpa_printf_dbg(MSG_DEBUG, "No room for EAP Methods");
		return NULL;
	}
...
static struct nai_realm * nai_realm_parse(struct wpabuf *anqp, u16 *count)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "NAI Realm Count: %u", num);
	pos += 2;

	if (num * 5 > end - pos) {
		wpa_printf_dbg(MSG_DEBUG, "Invalid NAI Realm Count %u - not "
...
static int plmn_id_match(struct wpabuf *anqp, const char *imsi, int mnc_len)
{
	...
	if (pos + 2 > end)
		return 0;
	if (*pos != 0) {
		wpa_printf_dbg(MSG_DEBUG, "Unsupported GUD version 0x%x", *pos);
		return 0;
	}
	pos++;
	udhl = *pos++;
	if (pos + udhl > end) {
		wpa_printf_dbg(MSG_DEBUG, "Invalid UDHL");
		return 0;
	}
	end = pos + udhl;

	wpa_printf_dbg(MSG_DEBUG, "Interworking: Matching against MCC/MNC alternatives: %02x:%02x:%02x or %02x:%02x:%02x (IMSI %s, MNC length %d)",
			  ...
			  size_t mnc_len, char prefix)
{
	...
	if (imsi == NULL || os_strlen(imsi) > 16) {
		wpa_printf_dbg(MSG_DEBUG, "No valid IMSI available");
		return -1;
	}
	sep = os_strchr(imsi, '-');
	if (sep) {
				     ...
				     struct wpa_bss *bss)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Interworking: Remove duplicate network entry for the same credential");
					...
					struct wpa_ssid *ssid)
{
	...
	if (wpa_config_set(ssid, "key_mgmt", key_mgmt) < 0)
		return -1;
	if (wpa_config_set(ssid, "proto", "RSN") < 0)
		return -1;
	if (wpa_config_set(ssid, "pairwise", "CCMP") < 0)
		return -1;
	return 0;
}
				     ...
				     struct wpa_bss *bss)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Interworking: Connect with " MACSTR " (3GPP)",
		   MAC2STR(bss->bssid));
	...
	case EAP_TYPE_AKA:
		prefix = '0';
		res = wpa_config_set(ssid, "eap", "AKA");
		break;
	...
	if (res < 0) {
		wpa_printf_dbg(MSG_DEBUG, "Selected EAP method (%d) not supported",
			   eap_type);
		goto fail;
	}

	if (!cred->pcsc && set_root_nai(ssid, cred->imsi, prefix) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "Failed to set Root NAI");
		goto fail;
	}
				       ...
				       struct wpa_cred *cred, int ttls)
{
	...
	if (cred->password && cred->password[0]) {
		if (cred->ext_password &&
		    wpa_config_set(ssid, "password", cred->password) < 0)
			return -1;
		if (!cred->ext_password &&
	...
	if (cred->private_key && cred->private_key[0] &&
	    wpa_config_set_quoted(ssid, "p_key", cred->private_key) < 0)
		return -1;

	if (cred->private_key_passwd && cred->private_key_passwd[0] &&
	    wpa_config_set_quoted(ssid, "p_key_pw",
	...
	struct wpa_bss *bss)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Interworking: Connect with " MACSTR " based on "
	...
	if (cred->eap_method == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "Interworking: No EAP method set for "
				       ...
				       struct wpa_bss *bss, int allow_excluded)
{
	...
	if (disallowed_bssid(wpa_s, bss->bssid) ||
		...
		wpa_printf_dbg(MSG_DEBUG, "Interworking: Reject connection to disallowed BSS "
			   MACSTR, MAC2STR(bss->bssid));
		return -1;
	}

	wpa_printf_dbg(MSG_DEBUG, "Interworking: Considering BSS " MACSTR
	...
	if (!wpa_bss_get_ie(bss, WLAN_EID_RSN)) {
		 ...
		 */
		wpa_printf_dbg(MSG_DEBUG, "Interworking: Network does not use "
			   "RSN");
		return -1;
	}
	...
	cred_rc = interworking_credentials_available_roaming_consortium(
		wpa_s, bss, 0, excl);
	if (cred_rc) {
		wpa_printf_dbg(MSG_DEBUG, "Interworking: Highest roaming "
	...
	if (cred) {
		wpa_printf_dbg(MSG_DEBUG, "Interworking: Highest NAI Realm list "
	...
	cred_3gpp = interworking_credentials_available_3gpp(wpa_s, bss, 0,
							    excl);
	if (cred_3gpp) {
		wpa_printf_dbg(MSG_DEBUG, "Interworking: Highest 3GPP matching "
	...
	if (!cred_rc && !cred && !cred_3gpp) {
		wpa_printf_dbg(MSG_DEBUG, "Interworking: No full credential matches - consider options without BW(etc.) limits");
		cred_rc = interworking_credentials_available_roaming_consortium(
			wpa_s, bss, 1, excl);
		if (cred_rc) {
			wpa_printf_dbg(MSG_DEBUG, "Interworking: Highest roaming "
		...
		cred = interworking_credentials_available_realm(wpa_s, bss, 1,
								excl);
		if (cred) {
			wpa_printf_dbg(MSG_DEBUG, "Interworking: Highest NAI Realm "
		...
		cred_3gpp = interworking_credentials_available_3gpp(wpa_s, bss,
								    1, excl);
		if (cred_3gpp) {
			wpa_printf_dbg(MSG_DEBUG, "Interworking: Highest 3GPP "
	...
	if (cred == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "Interworking: No matching credentials "
	...
	realm = nai_realm_parse(bss->anqp ? bss->anqp->nai_realm : NULL,
				&count);
	if (realm == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "Interworking: Could not parse NAI "
	...
	wpa_printf_dbg(MSG_DEBUG, "Interworking: Connect with " MACSTR,
		   MAC2STR(bss->bssid));
	...
	case EAP_TYPE_TTLS:
		if (eap->inner_method) {
			os_snprintf(buf, sizeof(buf), "\"autheap=%s\"",
				    eap_get_name(EAP_VENDOR_IETF,
						 eap->inner_method));
			if (wpa_config_set(ssid, "phase2", buf) < 0)
				goto fail;
			break;
		}
		...
		case NAI_REALM_INNER_NON_EAP_CHAP:
			...
			break;
		case NAI_REALM_INNER_NON_EAP_MSCHAP:
			if (wpa_config_set(ssid, "phase2", "\"auth=MSCHAP\"") < 0)
				goto fail;
			break;
		case NAI_REALM_INNER_NON_EAP_MSCHAPV2:
			if (wpa_config_set(ssid, "phase2", "\"auth=MSCHAPV2\"") < 0)
				goto fail;
			break;
		default:
	...
	case EAP_TYPE_PEAP:
	case EAP_TYPE_FAST:
		if (wpa_config_set(ssid, "phase1", "\"fast_provisioning=2\"") < 0)
			goto fail;
		if (wpa_config_set(ssid, "pac_file",
				   "\"blob://pac_interworking\"") < 0)
			goto fail;
	...
	int *excluded)
{
	...
	if (!wpa_s->imsi[0]) {
		size_t len;
		wpa_printf_dbg(MSG_DEBUG, "Interworking: IMSI not available - try to read again through eap_proxy");
...
#if defined(CONFIG_PCSC_FUNCS) || defined(CONFIG_EAP_PROXY)
	compare:
#endif /* CONFIG_PCSC_FUNCS || CONFIG_EAP_PROXY */
		wpa_printf_dbg(MSG_DEBUG, "Interworking: Parsing 3GPP info from "
			   MACSTR, MAC2STR(bss->bssid));
		ret = plmn_id_match(bss->anqp->anqp_3gpp, imsi, mnc_len);
		wpa_printf_dbg(MSG_DEBUG, "PLMN match %sfound", ret ? "" : "not ");
		if (ret) {
			if (cred_no_required_oi_match(cred, bss))
				continue;
			if (!ignore_bw &&
	...
	int *excluded)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Interworking: Parsing NAI Realm list from "
		   MACSTR, MAC2STR(bss->bssid));
	...
	int *excluded)
{
	...
	if (disallowed_bssid(wpa_s, bss->bssid) ||
	    disallowed_ssid(wpa_s, bss->ssid, bss->ssid_len)) {
		wpa_printf_dbg(MSG_DEBUG, "Interworking: Ignore disallowed BSS "
			   MACSTR, MAC2STR(bss->bssid));
		return NULL;
	}
			      ...
			      struct wpabuf *domain_names)
{
	...
	if (domain_names &&
		...
		if (realm)
			realm++;
		wpa_printf_dbg(MSG_DEBUG, "Interworking: Search for match "
	...
	for (i = 0; i < cred->num_domain; i++) {
		wpa_printf_dbg(MSG_DEBUG, "Interworking: Search for match with "
...
static int roaming_partner_match(struct wpa_supplicant *wpa_s,
				 ...
				 struct wpabuf *domain_names)
{
	wpa_printf_dbg(MSG_DEBUG, "Interworking: Comparing roaming_partner info fqdn='%s' exact_match=%d priority=%u country='%s'",
		       ...
		       struct wpa_bss *bss)
{
	...
	if (bss->anqp == NULL || bss->anqp->domain_name == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "Interworking: No ANQP domain name info -> use default roaming partner priority 128");
		return 128; /* cannot check preference with domain name */
	}

	if (interworking_home_sp_cred(wpa_s, cred, bss->anqp->domain_name) > 0)
	{
		wpa_printf_dbg(MSG_DEBUG, "Interworking: Determined to be home SP -> use maximum preference 0 as roaming partner priority");
	...
	for (i = 0; i < cred->num_roaming_partner; i++) {
		if (roaming_partner_match(wpa_s, &cred->roaming_partner[i],
			...
			wpa_printf_dbg(MSG_DEBUG, "Interworking: Roaming partner preference match - priority %u",
				   cred->roaming_partner[i].priority);
			return cred->roaming_partner[i].priority;
		}
	}

	wpa_printf_dbg(MSG_DEBUG, "Interworking: No roaming partner preference match - use default roaming partner priority 128");
	return 128;
}
						  ...
						  struct wpa_cred *cred)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Interworking: roaming_prio=%u for selected BSS "
	...
	dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) {
		...
		wpa_printf_dbg(MSG_DEBUG, "Interworking: roaming_prio=%u for BSS "
		...
		if (prio < best_prio) {
			...
			wpa_printf_dbg(MSG_DEBUG, "Interworking: old: %d %d %d  new: %d %d %d",
				   bh1, load1, conn1, bh2, load2, conn2);
			if (bh1 || load1 || conn1 || !(bh2 || load2 || conn2)) {
				wpa_printf_dbg(MSG_DEBUG, "Interworking: Better roaming partner " MACSTR " selected", MAC2STR(bss->bssid));
				best_prio = prio;
				selected = bss;
			}
		}
...
static void interworking_select_network(struct wpa_supplicant *wpa_s)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Interworking: Select network (auto_select=%d)",
		   wpa_s->auto_select);
	dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) {
		int excluded = 0;
		...
		if (!wpa_bss_get_ie(bss, WLAN_EID_RSN)) {
			 ...
			 */
			wpa_printf_dbg(MSG_DEBUG, "Interworking: Credential match "
		...
		if (wpa_s->auto_select ||
			...
			if (bh || bss_load || conn_capab) {
				if (selected2_cred == NULL ||
					...
					wpa_printf_dbg(MSG_DEBUG, "Interworking: Mark as selected2");
					selected2 = bss;
					selected2_cred = cred;
				}
				if (res > 0 &&
				    (selected2_home_cred == NULL ||
				     ...
				     0)) {
					wpa_printf_dbg(MSG_DEBUG, "Interworking: Mark as selected2_home");
					selected2_home = bss;
				...
				if (selected_cred == NULL ||
					...
					wpa_printf_dbg(MSG_DEBUG, "Interworking: Mark as selected");
					selected = bss;
					selected_cred = cred;
				}
				if (res > 0 &&
				    (selected_home_cred == NULL ||
				     ...
				     0)) {
					wpa_printf_dbg(MSG_DEBUG, "Interworking: Mark as selected_home");
					selected_home = bss;
	...
	if (selected_home && selected_home != selected &&
		...
		wpa_printf_dbg(MSG_DEBUG, "Interworking: Overrided selected with selected_home");
	...
	if (!selected) {
		if (selected2_home) {
			wpa_printf_dbg(MSG_DEBUG, "Interworking: Use home BSS with BW limit mismatch since no other network could be selected");
			selected = selected2_home;
			selected_cred = selected2_home_cred;
		} else if (selected2) {
			wpa_printf_dbg(MSG_DEBUG, "Interworking: Use visited BSS with BW limit mismatch since no other network could be selected");
			selected = selected2;
	...
	if (count == 0) {
		...
		if (interworking_find_network_match(wpa_s)) {
			wpa_printf_dbg(MSG_DEBUG, "Interworking: Possible BSS "
		...
		if (wpa_s->auto_network_select) {
			wpa_printf_dbg(MSG_DEBUG, "Interworking: Continue "
	...
	if (selected) {
		wpa_printf_dbg(MSG_DEBUG, "Interworking: Selected " MACSTR,
...
interworking_match_anqp_info(struct wpa_supplicant *wpa_s, struct wpa_bss *bss)
{
	...
	dl_list_for_each(other, &wpa_s->bss, struct wpa_bss, list) {
		...
		wpa_printf_dbg(MSG_DEBUG, "Interworking: Share ANQP data with "
...
static void interworking_next_anqp_fetch(struct wpa_supplicant *wpa_s)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Interworking: next_anqp_fetch - "
	...
	if (eloop_terminated() || !wpa_s->fetch_anqp_in_progress) {
		wpa_printf_dbg(MSG_DEBUG, "Interworking: Stop next-ANQP-fetch");
		return;
	}

	if (wpa_s->fetch_osu_icon_in_progress) {
		wpa_printf_dbg(MSG_DEBUG, "Interworking: Next icon (in progress)");
	...
	if (found == 0) {
		if (wpa_s->fetch_osu_info) {
			if (wpa_s->num_prov_found == 0 &&
				...
				wpa_printf_dbg(MSG_DEBUG, "HS 2.0: No OSU providers seen - try to scan again");
				hs20_start_osu_scan(wpa_s);
				return;
			}
			wpa_printf_dbg(MSG_DEBUG, "Interworking: Next icon");
		  ...
		  u16 info_ids[], size_t num_ids, u32 subtypes)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "ANQP: Query Request to " MACSTR " for %u id(s)",
					    ...
					    const u8 *data, size_t slen)
{
	...
	case ANQP_VENDOR_SPECIFIC:
		...
		default:
			wpa_printf_dbg(MSG_DEBUG, "Interworking: Unsupported "
		...
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "Interworking: Unsupported ANQP Info ID "
			   "%u", info_id);
		break;
	}
}
		  ...
		  const struct wpabuf *resp, u16 status_code)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Interworking: anqp_resp_cb dst=" MACSTR
		   " dialog_token=%u result=%d status_code=%u",
		   MAC2STR(dst), dialog_token, result, status_code);
	...
	if (wpabuf_len(adv_proto) < 4 || pos[0] != WLAN_EID_ADV_PROTO ||
		...
		wpa_printf_dbg(MSG_DEBUG, "ANQP: Unexpected Advertisement "
	...
	while (pos < end) {
		if (pos + 4 > end) {
			wpa_printf_dbg(MSG_DEBUG, "ANQP: Invalid element");
			break;
		}
		...
		pos += 2;
		if (pos + slen > end) {
			wpa_printf_dbg(MSG_DEBUG, "ANQP: Invalid element length "
...
{
	wpa_printf_dbg(MSG_DEBUG, "Interworking: Scan results available - start "
		   "ANQP fetch");
	interworking_start_fetch_anqp(wpa_s);
}
			...
			int *freqs)
{
	...
	wpa_s->fetch_osu_info = 0;
	wpa_printf_dbg(MSG_DEBUG, "Interworking: Start scan for network "
		   "selection");
		     ...
		     const struct wpabuf *query)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "GAS request to " MACSTR " (freq %d MHz)",
		   MAC2STR(dst), freq);
	...
	if (res < 0) {
		wpa_printf_dbg(MSG_DEBUG, "GAS: Failed to send Query Request");
		wpabuf_free(buf);
		ret = -1;
	} else
		wpa_printf_dbg(MSG_DEBUG, "GAS: Query started with dialog token "
			   "%u", res);
