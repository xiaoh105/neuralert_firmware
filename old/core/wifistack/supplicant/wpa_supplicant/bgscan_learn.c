...
static int bgscan_learn_load(struct bgscan_learn_data *data)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "bgscan learn: Loading data from %s",
		   data->fname);
	...
	while (fgets(buf, sizeof(buf), f)) {
		if (os_strncmp(buf, "BSS ", 4) == 0) {
			...
			wpa_printf_dbg(MSG_DEBUG, "bgscan learn: Loaded BSS "
				   "entry: " MACSTR " freq=%d",
...
static void bgscan_learn_save(struct bgscan_learn_data *data)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "bgscan learn: Saving data to %s",
		   data->fname);
					 ...
					 int *freqs, size_t count)
{
	...
	do {
		if (!in_array(freqs, data->supp_freqs[idx])) {
			wpa_printf_dbg(MSG_DEBUG, "bgscan learn: Probe new freq "
...
static void bgscan_learn_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (data->ssid->scan_freq)
		...
		wpa_printf_dbg(MSG_DEBUG, "bgscan learn: BSSes in this ESS have "
		...
		pos[0] = '\0';
		wpa_printf_dbg(MSG_DEBUG, "bgscan learn: Scanning frequencies:%s",
			   msg);
		params.freqs = freqs;
	}

	wpa_printf_dbg(MSG_DEBUG, "bgscan learn: Request a background scan");
	if (wpa_supplicant_trigger_scan(wpa_s, &params)) {
		wpa_printf_dbg(MSG_DEBUG, "bgscan learn: Failed to trigger scan");
		da16x_eloop_register_timeout(data->scan_interval, 0,
				...
				const struct wpa_ssid *ssid)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "bgscan learn: Signal strength threshold %d  "
	...
	da16x_eloop_register_timeout(data->scan_interval, 0, bgscan_learn_timeout,
			       data, NULL);
...
static void bgscan_learn_deinit(void *priv)
{
	...
	da16x_eloop_cancel_timeout(bgscan_learn_timeout, data, NULL);
	if (data->signal_threshold)
...
#define MAX_BSS 50
	...
	wpa_printf_dbg(MSG_DEBUG, "bgscan learn: scan result notification");
	...
	wpa_printf_dbg(MSG_DEBUG, "bgscan learn: %u matching BSSes in scan "
	...
	for (i = 0; i < scan_res->num; i++) {
		...
		if (bss && bss->freq != res->freq) {
			wpa_printf_dbg(MSG_DEBUG, "bgscan learn: Update BSS "
			   MACSTR " freq %d -> %d",
				   MAC2STR(res->bssid), bss->freq, res->freq);
			bss->freq = res->freq;
		} else if (!bss) {
			wpa_printf_dbg(MSG_DEBUG, "bgscan learn: Add BSS " MACSTR
...
{
	wpa_printf_dbg(MSG_DEBUG, "bgscan learn: beacon loss");
					      ...
					      int current_txrate)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "bgscan learn: signal level changed "
		   "(above=%d current_signal=%d current_noise=%d "
		   "current_txrate=%d)", above, current_signal,
		   current_noise, current_txrate);
	if (data->scan_interval == data->long_interval && !above) {
		wpa_printf_dbg(MSG_DEBUG, "bgscan learn: Start using short bgscan "
			   "interval");
		data->scan_interval = data->short_interval;
		os_get_reltime(&now);
		...
		wpa_printf_dbg(MSG_DEBUG, "bgscan learn: Start using long bgscan "
			   "interval");
		data->scan_interval = data->long_interval;
		da16x_eloop_cancel_timeout(bgscan_learn_timeout, data, NULL);
		da16x_eloop_register_timeout(data->scan_interval, 0,
	...
	if (scan) {
		wpa_printf_dbg(MSG_DEBUG, "bgscan learn: Trigger immediate scan");
		da16x_eloop_cancel_timeout(bgscan_learn_timeout, data, NULL);
		da16x_eloop_register_timeout(0, 0, bgscan_learn_timeout, data, NULL);
	}
}
