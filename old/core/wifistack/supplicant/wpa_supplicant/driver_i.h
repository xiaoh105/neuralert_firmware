...
static inline int wpa_drv_switch_change_mesh(struct wpa_supplicant *wpa_s , int new_channel)
{
	if (wpa_s->driver->chan_ch_mesh)
		return wpa_s->driver->chan_ch_mesh(wpa_s->drv_priv , new_channel);
	return -1;
...
static inline int wpa_drv_free_umac_scan_results(
	struct wpa_supplicant *wpa_s)
{
	if (wpa_s->driver->free_umac_scan_results)
		return wpa_s->driver->free_umac_scan_results(wpa_s->drv_priv);
	return NULL;
}
...
{
	if (wpa_s->driver->remain_on_ch)
		return wpa_s->driver->remain_on_ch(wpa_s->drv_priv, freq,
							duration);
	return -1;
}
...
static inline int wpa_drv_cancel_remain_on_channel(
	struct wpa_supplicant *wpa_s)
{
	if (wpa_s->driver->cancel_remain_on_ch)
		return wpa_s->driver->cancel_remain_on_ch(
			wpa_s->drv_priv);
	return -1;
}
...
static inline int wpa_drv_set_p2p_powersave(struct wpa_supplicant *wpa_s,
					    int is_noa, u8 count, int duration,
					    int interval, int start, int ctwindow)
{
	if (!wpa_s->driver->set_p2p_powersave)
		return -1;
	return wpa_s->driver->set_p2p_powersave(wpa_s->drv_priv, is_noa, count,
						duration, interval, start,
						ctwindow);
}
				...
				struct wpa_driver_scan_params *params)
{
	if (wpa_s->driver->abort_scan)
		return wpa_s->driver->abort_scan(wpa_s->drv_priv, params);
...
static inline int wpa_drv_get_rtctimegap(struct wpa_supplicant *wpa_s,
					      u64 *rtctimegap)
{
	if (!wpa_s->driver->get_rtctime_gap)
		return -1;
	return wpa_s->driver->get_rtctime_gap(wpa_s->drv_priv, rtctimegap);
}
#endif /* DRIVER_I_H */
