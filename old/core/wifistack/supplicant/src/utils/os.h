...
static inline void os_time_sub(struct os_reltime *a, struct os_reltime *b,
 ...
 */
#ifdef DEBUG_SUPP_MALLOC
#define	os_calloc(...) _os_calloc(__VA_ARGS__, __func__)
static inline void * _os_calloc(size_t nmemb, size_t size, const char* caller)
#else
...
extern void *profile_supp_malloc(char *name, int line, size_t size);
extern void profile_supp_free(char *name, int line, void *f);

#define	os_malloc(...) profile_supp_malloc( __FILE__, __LINE__, __VA_ARGS__ )
#define os_free(...) profile_supp_free( __FILE__, __LINE__, __VA_ARGS__ )

#else
#ifdef DEBUG_SUPP_MALLOC
#define	os_malloc(...) supp_malloc(__VA_ARGS__, __func__)
extern void 	*supp_malloc(size_t size, const char* caller);
#else
#define	os_malloc(...) supp_malloc(__VA_ARGS__ )
extern void 	*supp_malloc(size_t size);
#endif /* DEBUG_SUPP_MALLOC */

#define os_free(...) supp_free(__VA_ARGS__ )
extern void 	supp_free(void *f);

#endif /* SUPPORT_MEM_PROFILE */

#if 1 //def OS_NO_C_LIB_DEFINES
 ...
 */
static inline void * os_realloc(void *ptr, size_t size)
{
	void *result;
...
static inline void * supp_calloc(size_t nmemb, size_t size)
{
	if (size && nmemb > (~(size_t) 0) / size)
		return NULL;
	return os_zalloc(nmemb * size);
 ...
 * Caller is responsible for freeing the returned buffer with os_free().
 */
static inline char * os_strdup(const char *str)
{
 ...
 * characters will be compared.
 */
static inline int os_strncasecmp(const char *s1, const char *s2, size_t n)
{
	if (n != 0) {
		register const unsigned char *cm = charmap,
				*us1 = (const unsigned char *)s1,
 ...
 * Returns: Pointer to the matched character or %NULL if not found
 */
static inline char *os_strrchr(const char *cp, int ch)
{
	char *save;
	char c;
...
static inline char *os_strstr(const char *in, const char *str)
{
	char c;
	size_t len;
