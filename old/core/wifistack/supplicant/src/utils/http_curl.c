			 ...
			 void *userdata)
{
	...
	case CURLINFO_SSL_DATA_IN:
		wpa_printf_dbg(MSG_DEBUG, "debug - CURLINFO_SSL_DATA_IN - %d",
			   (int) len);
		break;
	case CURLINFO_SSL_DATA_OUT:
		wpa_printf_dbg(MSG_DEBUG, "debug - CURLINFO_SSL_DATA_OUT - %d",
			   (int) len);
		break;
	case CURLINFO_END:
		wpa_printf_dbg(MSG_DEBUG, "debug - CURLINFO_END - %d",
			   (int) len);
		break;
	}
	return 0;
			    ...
			    LogotypeData *data)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (!found_hash) {
			wpa_printf_dbg(MSG_DEBUG, "OpenSSL: No SHA256 hash found for the logo");
			continue;
		}
...
static int curl_cb_ssl_verify(int preverify_ok, X509_STORE_CTX *x509_ctx)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "curl_cb_ssl_verify");
...
static void tls_show_errors(const char *func, const char *txt)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "OpenSSL: %s - %s %s",
		   func, txt, ERR_error_string(ERR_get_error(), NULL));

	while ((err = ERR_get_error())) {
		wpa_printf_dbg(MSG_DEBUG, "OpenSSL: pending error: %s",
...
static int ocsp_resp_cb(SSL *s, void *arg)
{
	...
	if (!p) {
		wpa_printf_dbg(MSG_DEBUG, "OpenSSL: No OCSP response received");
	...
	if (ctx->peer_issuer) {
		wpa_printf_dbg(MSG_DEBUG, "OpenSSL: Add issuer");
	...
	wpa_printf_dbg(MSG_DEBUG, "OpenSSL: OCSP response verification succeeded");

	if (!ctx->peer_cert) {
		wpa_printf_dbg(MSG_DEBUG, "OpenSSL: Peer certificate not available for OCSP status check");
		OCSP_BASICRESP_free(basic);
		OCSP_RESPONSE_free(rsp);
	...
	if (!ctx->peer_issuer) {
		wpa_printf_dbg(MSG_DEBUG, "OpenSSL: Peer issuer certificate not available for OCSP status check");
		OCSP_BASICRESP_free(basic);
		OCSP_RESPONSE_free(rsp);
	...
	if (!id) {
		wpa_printf_dbg(MSG_DEBUG, "OpenSSL: Could not create OCSP certificate identifier");
		OCSP_BASICRESP_free(basic);
		OCSP_RESPONSE_free(rsp);
	...
	wpa_printf_dbg(MSG_DEBUG, "OpenSSL: OCSP status for server certificate: %s",
	...
	if (status == V_OCSP_CERTSTATUS_REVOKED)
		...
		return 0;
	if (ctx->ocsp == MANDATORY_OCSP) {
		wpa_printf_dbg(MSG_DEBUG, "OpenSSL: OCSP status unknown, but OCSP required");
		ctx->last_err = "OCSP status unknown";
		return 0;
	}
	wpa_printf_dbg(MSG_DEBUG, "OpenSSL: OCSP status unknown, but OCSP was not required, so allow connection to continue");
	return 1;
}
...
static CURLcode curl_cb_ssl(CURL *curl, void *sslctx, void *parm)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "curl_cb_ssl");
			      ...
			      const char *client_key)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Start HTTP client: address=%s ca_fname=%s "
...
xml_node_t * soap_send_receive(struct http_ctx *ctx, xml_node_t *node)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "SOAP: Sending message");
	...
	wpa_printf_dbg(MSG_DEBUG, "SOAP: Server response code %ld", http);
	if (http != 200) {
	...
	wpa_printf_dbg(MSG_DEBUG, "SOAP body localname: '%s'",
		       ...
		       const char *fname, const char *ca_fname)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "curl: Download file from %s to %s (ca=%s)",
		   url, fname, ca_fname);
	curl = curl_easy_init();
	if (curl == NULL)
		return -1;
	...
	wpa_printf_dbg(MSG_DEBUG, "curl: Server response code %ld", http);
	if (http != 200) {
		ctx->last_err = "HTTP download failed";
		wpa_printf(MSG_INFO, "HTTP download failed - code %ld", http);
		curl_easy_cleanup(curl);
		 ...
		 size_t *resp_len)
{
	...
	ctx->last_err = NULL;
	wpa_printf_dbg(MSG_DEBUG, "curl: HTTP POST to %s", url);
