...
#define HOSTAPD_CHAN_PASSIVE_SCAN 0x00000002 // FC9000 old
#define HOSTAPD_CHAN_NO_IR 0x00000002	// supplicant 2.7
...
//#ifdef CONFIG_ACS - 201601_trinity, blocked because of connection fail
	/**
...
struct wpa_driver_scan_params {
	 ...
	 */
	u8 num_ssids; //trinity.song size_t -> u8
...
#ifdef	UNUSED_CODE
	/**
	 * promiscuous - Request driver to sniff all channels
	 *
	 * This is used to request the driver to sniff all channels (ch.1 ~ ch.14)
	 */
	/* FEATURE_SMART_CONFIG */
	int promiscuous;
#endif /* UNUSED_CODE */
...
struct hostapd_freq_params {
	...
	/**
	 * cent_freq1 - Segment 0 center frequency in MHz
	 *
	...
	/**
	 * cent_freq2 - Segment 1 center frequency in MHz
	 *
...
#define WPA_DRIVER_MESH_CONF_FLAG_RSSI_THRESHOLD	0x00000010
	 ...
	 * See NLD11_MESHCONF_* for all the mesh config parameters.
	 */
	unsigned int flags;
	int auto_plinks;
...
struct hostapd_acl_params {
	...
	struct hostapd_mac_address mac_acl[0];
};
...
struct csa_settings {
	...
	struct hostapd_beacon_data beacon_csa;
	struct hostapd_beacon_data beacon_after;
...
struct wpa_driver_ops {
	 ...
	 */
	int (*set_tx_queue_params)(void *priv, int queue, int aifs, int cwmin,
	...
	/**
	 * remain_on_ch - Remain awake on a channel
	 * @priv: Private driver interface data
	 * @freq: Frequency (in MHz) of the channel
	 ...
	 * executed.
	 */
	int (*remain_on_ch)(void *priv, unsigned int freq,
				 unsigned int duration);

	/**
	 * cancel_remain_on_ch - Cancel remain-on-channel operation
	 * @priv: Private driver interface data
	 *
	 ...
	 * used, e.g., when remain_on_ch() is used to request extra time
	 ...
	 */
	int (*cancel_remain_on_ch)(void *priv);
	 ...
	 * Returns: 0 on success or -1 on failure
	 */
	int (*set_p2p_powersave)(void *priv, int is_noa, u8 count,
				 int duration, int interval, int start,
				 int ctwindow);
#endif	/* CONFIG_P2P */
	 ...
	 * an example, this will be converted to a vendor specific cfgd11
	 * command in case of the nld11 driver interface.
	 */
	...
	int (*ap_pwrsave)(void *priv, int ps_state, int timeout); // FC9000 only ??
#endif	/* CONFIG_AP */
...
//#ifdef CONFIG_MESH_PORTAL
	/**
	 * chan_ch_mesh - Change Channel in MP
	 * @chanel: Private driver interface data
	 * Returns 0 on success, -1 on failure
	 */
	int (*chan_ch_mesh)(void *priv , int new_channel);
//#endif
	/**
	 ...
	 *	when the vendor interface QCA_NLD11_VENDOR_SUBCMD_TRIGGER_SCAN
	 *	was used to trigger scan. Otherwise, 0 is used.
	 * Returns 0 on success, -1 on failure
	 */
#if 1 /* FC9000 Only */
	int (*abort_scan)(void *priv, struct wpa_driver_scan_params *params);
#else
...
enum wpa_event_type {
	 ...
	 * operation is included in union wpa_event_data::remain_on_ch.
	 */
...
#if 1 //def CONFIG_RECEIVE_DHCP_EVENT /* FC9000 Only */
...
union wpa_event_data {
	...
	/**
	 * struct remain_on_ch - Data for EVENT_REMAIN_ON_CHANNEL events
	 *
...
/* driver_nl80211.c */
extern const struct wpa_driver_ops wpa_driver_nld11_ops;
