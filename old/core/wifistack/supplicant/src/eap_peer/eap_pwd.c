...
{
	wpa_printf_dbg(MSG_DEBUG, "EAP-PWD: %s -> %s",
...
static void * eap_pwd_init(struct eap_sm *sm)
{
	...
	if (password == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD: No password configured!");
		return NULL;
	}

	identity = eap_get_config_identity(sm, &identity_len);
	if (identity == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD: No identity configured!");
		return NULL;
	}

	if ((data = os_zalloc(sizeof(*data))) == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD: memory allocation data fail");
		return NULL;
	}

	if ((data->id_peer = os_malloc(identity_len)) == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD: memory allocation id fail");
		os_free(data);
		return NULL;
	}
	...
	if ((data->password = os_malloc(password_len)) == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD: memory allocation psk fail");
			    ...
			    const u8 *payload, size_t payload_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PWD (peer): using group %d",
		   data->group_num);
	...
	if (data->id_server || data->grp) {
		wpa_printf_dbg(MSG_INFO, "EAP-pwd: data was already allocated");
		eap_pwd_state(data, FAILURE);
		return;
	}
	...
	if (data->grp == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD: failed to allocate memory for "
			   "group");
		eap_pwd_state(data, FAILURE);
		return;
	}
				...
				const u8 *payload, size_t payload_len)
{
	...
	case EAP_PWD_PREP_SSHA1:
		...
		if (payload_len < 1 || *ptr == 0) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: Invalid Salt-len");
			goto fin;
		}
		salt_len = *ptr++;
	...
	if (res) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD (peer): unable to compute PWE");
		eap_pwd_state(data, FAILURE);
		return;
	}

	wpa_printf_dbg(MSG_DEBUG, "EAP-PWD (peer): computed %d bit PWE...",
	...
	if (!data->private_value || !data->my_element || !cofactor ||
	    !data->my_scalar || !mask) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD (peer): scalar allocation fail");
		goto fin;
	}

	if (crypto_ec_cofactor(data->grp->group, cofactor) < 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-pwd (peer): unable to get cofactor "
			   "for curve");
		goto fin;
	}
	...
	if (crypto_ec_point_mul(data->grp->group, data->grp->pwe, mask,
				data->my_element) < 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD (peer): element allocation "
			   "fail");
		eap_pwd_state(data, FAILURE);
		goto fin;
	}
	...
	if (crypto_ec_point_invert(data->grp->group, data->my_element) < 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD (peer): element inversion fail");
		goto fin;
	}
	...
	if (!data->k || !K || !point) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD (peer): peer data allocation "
			   "fail");
		goto fin;
	}
	...
	if (!data->server_element) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD (peer): setting peer element "
			   "fail");
		goto fin;
	}
	ptr += prime_len * 2;
	...
	if (!crypto_bignum_is_one(cofactor)) {
		if (crypto_ec_point_mul(data->grp->group, data->server_element,
					cofactor, point) < 0) {
			wpa_printf_dbg(MSG_INFO, "EAP-PWD (peer): cannot multiply "
				   "server element by order!\n");
			goto fin;
		}
		if (crypto_ec_point_is_at_infinity(data->grp->group, point)) {
			wpa_printf_dbg(MSG_INFO, "EAP-PWD (peer): server element "
				   "is at infinity!\n");
			goto fin;
		}
	}
	...
	if (crypto_ec_point_mul(data->grp->group, data->grp->pwe,
	    ...
	    crypto_ec_point_mul(data->grp->group, K, data->private_value,
				K) < 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD (peer): computing shared key "
			   "fail");
		goto fin;
	}
	...
	if (crypto_ec_point_is_at_infinity(data->grp->group, K)) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD (peer): shared key point is at "
			   "infinity!\n");
		goto fin;
	}

	if (crypto_ec_point_x(data->grp->group, K, data->k) < 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD (peer): unable to extract "
	...
	if (!scalar || !element) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD (peer): data allocation fail");
		goto fin;
	}
	...
	if (crypto_ec_point_to_bin(data->grp->group, data->my_element, element,
				   element + prime_len) != 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD (peer): point assignment fail");
		goto fin;
	}
				 ...
				 const u8 *payload, size_t payload_len)
{
	...
	*ptr = EAP_PWD_DEFAULT_PRF;
	...
	if (!cruft) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD (server): confirm allocation "
			   "fail");
		goto fin;
	}
	...
	if (crypto_ec_point_to_bin(data->grp->group, data->server_element,
		...
		wpa_printf_dbg(MSG_INFO, "EAP-PWD (server): confirm point "
			   "assignment fail");
		goto fin;
	}
	...
	if (os_memcmp_const(conf, ptr, SHA256_MAC_LEN)) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD (peer): confirm did not verify");
		goto fin;
	}

	wpa_printf_dbg(MSG_DEBUG, "EAP-pwd (peer): confirm verified");
	...
	if (crypto_ec_point_to_bin(data->grp->group, data->my_element, cruft,
				   cruft + prime_len) != 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-PWD (peer): confirm point "
			   "assignment fail");
		goto fin;
	}
	eap_pwd_h_update(hash, cruft, prime_len * 2);
	...
	if (compute_keys(data->grp, data->k,
		...
		wpa_printf_dbg(MSG_INFO, "EAP-PWD (peer): unable to compute MSK | "
			   "EMSK");
		goto fin;
	}
		...
		const struct wpabuf *reqData)
{
	...
	if ((pos == NULL) || (len < 1)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: Got a frame but pos is %s and "
			   "len is %d",
	...
	if (data->out_frag_pos) {
		 ...
		 */
		if (len)
			wpa_printf_dbg(MSG_INFO, "Bad Response! Fragmenting but "
				   "not an ACK");

		wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: Got an ACK for a fragment");
		/*
		...
		if (resp == NULL) {
			wpa_printf_dbg(MSG_INFO, "Unable to allocate memory for "
				   "next fragment!");
			return NULL;
		}
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: Send %s fragment of %d bytes",
	...
	if (EAP_PWD_GET_LENGTH_BIT(lm_exch)) {
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: Incoming fragments whose "
		...
		if (data->inbuf == NULL) {
			wpa_printf_dbg(MSG_INFO, "Out of memory to buffer "
				   "fragments!");
			return NULL;
		}
		data->in_frag_pos = 0;
	...
	if (EAP_PWD_GET_MORE_BIT(lm_exch) || data->in_frag_pos) {
		...
		if (data->in_frag_pos > wpabuf_size(data->inbuf)) {
			wpa_printf_dbg(MSG_INFO, "EAP-pwd: Buffer overflow attack "
				   "detected (%d vs. %d)!",
	...
	if (EAP_PWD_GET_MORE_BIT(lm_exch)) {
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: ACKing a %d byte fragment",
			   (int) len);
		return resp;
	}
	/*
	 ...
	 */
	if (data->in_frag_pos) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: Last fragment, %d bytes",
			   (int) len);
		pos = wpabuf_head_u8(data->inbuf);
		len = data->in_frag_pos;
	}
	wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: processing frame: exch %d, len %d",
	...
	case EAP_PWD_OPCODE_CONFIRM_EXCH:
		...
		break;
	default:
		wpa_printf_dbg(MSG_INFO, "EAP-pwd: Ignoring message with unknown "
			   "opcode %d", lm_exch);
		break;
	}
	/*
	...
	if ((len + EAP_PWD_HDR_SIZE) > data->mtu) {
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: Fragmenting output, total "
			   "length = %d", tot_len);
	} else {
		resp = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_PWD,
				     EAP_PWD_HDR_SIZE + len,
