...
static void * eap_pax_init(struct eap_sm *sm)
{
	...
	if (!identity || !password) {
		wpa_printf_dbg(MSG_INFO, "EAP-PAX: CID (nai) or key (password) "
			   "not configured");
		return NULL;
	}

	if (password_len != EAP_PAX_AK_LEN) {
		wpa_printf_dbg(MSG_INFO, "EAP-PAX: Invalid PSK length");
		return NULL;
	}
					     ...
					     size_t req_plen)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PAX: PAX_STD-1 (received)");

	if (data->state != PAX_INIT) {
		wpa_printf_dbg(MSG_INFO, "EAP-PAX: PAX_STD-1 received in "
	...
	if (req->flags & EAP_PAX_FLAGS_CE) {
		wpa_printf_dbg(MSG_INFO, "EAP-PAX: PAX_STD-1 with CE flag set - "
			   "ignored");
		ret->ignore = TRUE;
		return NULL;
	}
	...
	if (left < 2 + EAP_PAX_RAND_LEN) {
		wpa_printf_dbg(MSG_INFO, "EAP-PAX: PAX_STD-1 with too short "
			   "payload");
		ret->ignore = TRUE;
		return NULL;
	}
	...
	if (WPA_GET_BE16(pos) != EAP_PAX_RAND_LEN) {
		wpa_printf_dbg(MSG_INFO, "EAP-PAX: PAX_STD-1 with incorrect A "
			   "length %d (expected %d)",
	...
	if (random_get_bytes(data->rand.r.y, EAP_PAX_RAND_LEN)) {
		wpa_printf_dbg(MSG_ERROR, "EAP-PAX: Failed to get random data");
		ret->ignore = TRUE;
		return NULL;
	}
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PAX: PAX_STD-2 (sending)");
					     ...
					     size_t req_plen)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PAX: PAX_STD-3 (received)");

	if (data->state != PAX_STD_2_SENT) {
		wpa_printf_dbg(MSG_INFO, "EAP-PAX: PAX_STD-3 received in "
			   "unexpected state (%d) - ignored", data->state);
		ret->ignore = TRUE;
		return NULL;
	}
	...
	if (req->flags & EAP_PAX_FLAGS_CE) {
		wpa_printf_dbg(MSG_INFO, "EAP-PAX: PAX_STD-3 with CE flag set - "
			   "ignored");
		ret->ignore = TRUE;
		return NULL;
	}
	...
	if (left < 2 + EAP_PAX_MAC_LEN) {
		wpa_printf_dbg(MSG_INFO, "EAP-PAX: PAX_STD-3 with too short "
			   "payload");
		ret->ignore = TRUE;
		return NULL;
	}
	...
	if (WPA_GET_BE16(pos) != EAP_PAX_MAC_LEN) {
		wpa_printf_dbg(MSG_INFO, "EAP-PAX: PAX_STD-3 with incorrect "
	...
	if (os_memcmp_const(pos, mac, EAP_PAX_MAC_LEN) != 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-PAX: Invalid MAC_CK(B, CID) "
			   "received");
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PAX: PAX-ACK (sending)");
				       ...
				       const struct wpabuf *reqData)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PAX: received frame: op_code 0x%x "
	...
	if (data->state != PAX_INIT && data->mac_id != req->mac_id) {
		wpa_printf_dbg(MSG_INFO, "EAP-PAX: MAC ID changed during "
	...
	if (data->state != PAX_INIT && data->dh_group_id != req->dh_group_id) {
		wpa_printf_dbg(MSG_INFO, "EAP-PAX: DH Group ID changed during "
			   "authentication (was 0x%d, is 0x%d)",
			   data->dh_group_id, req->dh_group_id);
		ret->ignore = TRUE;
		return NULL;
	...
	if (data->state != PAX_INIT &&
	    data->public_key_id != req->public_key_id) {
		wpa_printf_dbg(MSG_INFO, "EAP-PAX: Public Key ID changed during "
			   "authentication (was 0x%d, is 0x%d)",
			   data->public_key_id, req->public_key_id);
		ret->ignore = TRUE;
		return NULL;
	...
	if (req->mac_id != EAP_PAX_MAC_HMAC_SHA1_128) {
		wpa_printf_dbg(MSG_INFO, "EAP-PAX: Unsupported MAC ID 0x%x",
			   req->mac_id);
		ret->ignore = TRUE;
		return NULL;
	}
	...
	if (req->dh_group_id != EAP_PAX_DH_GROUP_NONE) {
		wpa_printf_dbg(MSG_INFO, "EAP-PAX: Unsupported DH Group ID 0x%x",
			   req->dh_group_id);
		ret->ignore = TRUE;
		return NULL;
	}
	...
	if (req->public_key_id != EAP_PAX_PUBLIC_KEY_NONE) {
		wpa_printf_dbg(MSG_INFO, "EAP-PAX: Unsupported Public Key ID 0x%x",
			   req->public_key_id);
		ret->ignore = TRUE;
		return NULL;
	}
	...
	if (req->flags & EAP_PAX_FLAGS_MF) {
		...
		wpa_printf_dbg(MSG_INFO, "EAP-PAX: fragmentation not supported - "
			   "ignored packet");
		ret->ignore = TRUE;
		return NULL;
	}
	...
	if (os_memcmp_const(icv, icvbuf, EAP_PAX_ICV_LEN) != 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PAX: invalid ICV - ignoring the "
			   "message");
	...
	case EAP_PAX_OP_STD_3:
		...
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-PAX: ignoring message with unknown "
