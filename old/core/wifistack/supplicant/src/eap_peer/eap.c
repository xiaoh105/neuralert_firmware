...
{
	wpa_printf_dbg(MSG_DEBUG, "EAP: Status notification: %s (param=%s)",
		   status, parameter);
...
{
	wpa_printf_dbg(MSG_DEBUG, "EAP: Error notification: %d", error_code);
...
static void eap_deinit_prev_method(struct eap_sm *sm, const char *txt)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP: deinitialize previously used EAP method "
...
SM_STATE(EAP, INITIALIZE)
{
	...
	if (sm->fast_reauth && sm->m && sm->m->has_reauth_data &&
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP: maintaining EAP method data for "
...
#if 1	/* by Shingu 20160609 (Optimize) */
	eap_deinit_prev_method(sm, "DISCONNECT");
#endif	/* 1 */
}
...
SM_STATE(EAP, GET_METHOD)
{
	...
	if (!eap_sm_allowMethod(sm, sm->reqVendor, method)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: vendor %u method %u not allowed",
			   sm->reqVendor, method);
	...
	if (sm->fast_reauth &&
	    ...
	    sm->m->has_reauth_data(sm, sm->eap_method_priv)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: Using previous method data"
	...
	if (sm->m == NULL)
		sm->m = eap_method;
	if (!sm->m) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: Could not find selected method: "
			   "vendor %d method %d",
			   sm->reqVendor, method);
		goto nak;
	}
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP: Initialize selected EAP method: "
		   "vendor %u method %u (%s)",
	...
	if (sm->eap_method_priv == NULL) {
		...
		if (sm->reqMethod == EAP_TYPE_TLS && config &&
			 ...
			 * current EAP packet.
			 */
			wpa_printf_dbg(MSG_DEBUG, "EAP: Pending PIN/passphrase "
				   "request - skip Nak");
			return;
		}
...
static char * eap_get_realm(struct eap_sm *sm, struct eap_peer_config *config)
{
	...
	if (!config->identity && !config->anonymous_identity &&
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP: Build realm from IMSI (eap_proxy)");
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP: Generated realm '%s'", realm);
		return realm;
	}
...
static void eap_erp_remove_keys_realm(struct eap_sm *sm, const char *realm)
{
	...
	while ((erp = eap_erp_get_key(sm, realm)) != NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: Delete old ERP key %s",
			   erp->keyname_nai);
		       ...
		       size_t ext_emsk_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP: Realm for ERP keyName-NAI: %s", realm);
	...
	if (hmac_sha256_kdf(session_id, session_id_len, "EMSK", len,
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP: Could not derive EMSKname");
		goto fail;
	}
	...
	if (hmac_sha256_kdf(emsk, emsk_len,
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP: Could not derive rRK for ERP");
		goto fail;
	}
	...
	if (hmac_sha256_kdf(erp->rRK, erp->rRK_len,
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP: Could not derive rIK for ERP");
		goto fail;
	}
	erp->rIK_len = erp->rRK_len;
	wpa_hexdump_key(MSG_DEBUG, "EAP: ERP rIK", erp->rIK, erp->rIK_len);

	wpa_printf_dbg(MSG_DEBUG, "EAP: Stored ERP keys %s", erp->keyname_nai);
...
struct wpabuf * eap_peer_build_erp_reauth_start(struct eap_sm *sm, u8 eap_id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP: Valid ERP key found %s (SEQ=%u)",
...
static int eap_peer_erp_reauth_start(struct eap_sm *sm, u8 eap_id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP: Sending EAP-Initiate/Re-auth");
	wpabuf_free(sm->eapRespData);
	sm->eapRespData = msg;
	sm->reauthInit = TRUE;
	return 0;
...
SM_STATE(EAP, METHOD)
{
	...
	sm->eapRespData = sm->m->process(sm, sm->eap_method_priv, &ret,
					 eapReqData);
	wpa_printf_dbg(MSG_DEBUG, "EAP: method process -> ignore=%s "
...
SM_STATE(EAP, SEND_RESPONSE)
{
	...
	if (sm->eapRespData) {
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP: No eapRespData available");
...
static int eap_success_workaround(struct eap_sm *sm, int reqId, int lastId)
{
	...
	if (sm->workaround && (reqId == ((lastId + 1) & 0xff) ||
			       reqId == ((lastId + 2) & 0xff))) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: Workaround for unexpected "
			   "identifier field in EAP Success: "
			   "reqId=%d lastId=%d (these are supposed to be "
			   "same)", reqId, lastId);
		return 1;
	}
	wpa_printf_dbg(MSG_DEBUG, "EAP: EAP-Success Id mismatch - reqId=%d "
...
static int eap_peer_req_is_duplicate(struct eap_sm *sm)
{
	...
	if (sm->workaround && duplicate &&
		 ...
		 */
		wpa_printf_dbg(MSG_DEBUG, "EAP: AS used the same Id again, but "
			   "EAP packets were not identical");
		wpa_printf_dbg(MSG_DEBUG, "EAP: workaround - assume this is not a "
			   "duplicate packet");
		duplicate = 0;
	}
				  ...
				  EapType method)
{
	if (!eap_allowed_method(sm, vendor, method)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: configuration does not allow: "
			   "vendor %u method %u", vendor, method);
		return FALSE;
	}
	if (eap_peer_get_eap_method(vendor, method))
		return TRUE;
	wpa_printf_dbg(MSG_DEBUG, "EAP: not included in build: "
		   "vendor %u method %u", vendor, method);
	return FALSE;
}
	...
	size_t count)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP: Building expanded EAP-Nak");
	...
	for (m = methods; m; m = m->next) {
		...
		if (eap_allowed_method(sm, m->vendor, m->method)) {
			wpa_printf_dbg(MSG_DEBUG, "EAP: allowed type: "
				   "vendor=%u method=%u",
				   m->vendor, m->method);
	...
	if (!found) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: no more allowed methods");
		wpabuf_put_u8(resp, EAP_TYPE_EXPANDED);
		wpabuf_put_be24(resp, EAP_VENDOR_IETF);
...
static struct wpabuf * eap_sm_buildNak(struct eap_sm *sm, int id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP: Building EAP-Nak (requested type %u "
...
static struct wpabuf * eap_sm_buildNotify(int id)
{
	wpa_printf_dbg(MSG_DEBUG, "EAP: Generating EAP-Response Notification");
			      ...
			      size_t len)
{
	...
	if (len < sizeof(*hdr) + 1) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: Ignored too short EAP-Initiate");
		return;
	}
...
void eap_peer_finish(struct eap_sm *sm, const struct eap_hdr *hdr, size_t len)
{
	...
	if (len < sizeof(*hdr) + 1) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: Ignored too short EAP-Finish");
		return;
	}
	...
	pos += 2;
	wpa_printf_dbg(MSG_DEBUG, "EAP: Flags=0x%x SEQ=%u", flags, seq);
	...
	if (!erp) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: No matching ERP key found for %s",
			   nai);
		return;
	}
	...
	if (end[-17] != EAP_ERP_CS_HMAC_SHA256_128) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: Different Cryptosuite used");
		if (flags & 0x80)
			goto no_auth_tag;
		return;
	}
...
no_auth_tag:
	...
	if (hmac_sha256_kdf(erp->rRK, erp->rRK_len,
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP: Could not derive rMSK for ERP");
		eap_sm_free_key(sm);
		return;
	}
...
static void eap_sm_parseEapReq(struct eap_sm *sm, const struct wpabuf *req)
{
	...
	if (plen > wpabuf_len(req)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: Ignored truncated EAP-Packet "
			   "(len=%lu plen=%lu)",
	...
	case EAP_CODE_REQUEST:
		if (plen < sizeof(*hdr) + 1) {
			wpa_printf_dbg(MSG_DEBUG, "EAP: Too short EAP-Request - "
				   "no Type field");
			return;
		}
		sm->rxReq = (Boolean)TRUE;
		pos = (const u8 *) (hdr + 1);
		sm->reqMethod = (EapType)*pos++;
		if (sm->reqMethod == EAP_TYPE_EXPANDED) {
			if (plen < sizeof(*hdr) + 8) {
				wpa_printf_dbg(MSG_DEBUG, "EAP: Ignored truncated "
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP: Received EAP-Request id=%d "
	...
	case EAP_CODE_RESPONSE:
		if (sm->selectedMethod == EAP_TYPE_LEAP) {
			 ...
			 */
			if (plen < sizeof(*hdr) + 1) {
				wpa_printf_dbg(MSG_DEBUG, "EAP: Too short "
					   "EAP-Response - no Type field");
				return;
			}
			sm->rxResp = (Boolean)TRUE;
			pos = (const u8 *) (hdr + 1);
			sm->reqMethod = (EapType)*pos;
			wpa_printf_dbg(MSG_DEBUG, "EAP: Received EAP-Response for "
				   "LEAP method=%d id=%d",
				   sm->reqMethod, sm->reqId);
			break;
		}
		wpa_printf_dbg(MSG_DEBUG, "EAP: Ignored EAP-Response");
		break;
	case EAP_CODE_SUCCESS:
		wpa_printf_dbg(MSG_DEBUG, "EAP: Received EAP-Success");
		eap_notify_status(sm, "completion", "success");
		sm->rxSuccess = TRUE;
		break;
	case EAP_CODE_FAILURE:
		wpa_printf_dbg(MSG_DEBUG, "EAP: Received EAP-Failure");
	...
	case EAP_CODE_FINISH:
		...
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP: Ignored EAP-Packet with unknown "
			   "code %d", hdr->code);
		break;
	}
}
