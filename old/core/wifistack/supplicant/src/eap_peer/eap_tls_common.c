			      ...
			      const u8 **data, size_t *data_len)
{
	...
	if (blob == NULL) {
		da16x_eap_prt("[%s] %s: Named configuration blob '%s' not "
			     "found\n", __func__, __func__, *name + 7);
		return -1;
	}
...
static int eap_tls_params_read_cert_blob(int module, int type, unsigned char **out, size_t *outlen)
{
    ...
    buf = os_calloc(buflen, sizeof(unsigned char));
    if (!buf) {
        return -1;
    }

    err = da16x_cert_read(module, type, &format, buf, &buflen);
...
static int eap_tls_params_set_cert_blob(struct tls_connection_params *params)
{
    ...
    err = eap_tls_params_read_cert_blob(DA16X_CERT_MODULE_WPA_ENTERPRISE, DA16X_CERT_TYPE_CA_CERT, &data, &data_len);
    if (err) {
        goto err;
    }
    ...
    err = eap_tls_params_read_cert_blob(DA16X_CERT_MODULE_WPA_ENTERPRISE, DA16X_CERT_TYPE_CERT, &data, &data_len);
    if (err) {
        goto err;
    }
    ...
    err = eap_tls_params_read_cert_blob(DA16X_CERT_MODULE_WPA_ENTERPRISE, DA16X_CERT_TYPE_PRIVATE_KEY, &data, &data_len);
    if (err) {
        goto err;
    }
    ...
    err = eap_tls_params_read_cert_blob(DA16X_CERT_MODULE_WPA_ENTERPRISE, DA16X_CERT_TYPE_DH_PARAMS, &data, &data_len);
    if (err) {
        goto err;
    }
				    ...
				    struct eap_peer_config *config, int phase2)
{
	...
	if (phase2) {
		da16x_eap_prt("[%s]TLS: using phase2 config options\n", __func__);
		eap_tls_params_from_conf2(params, config);
	} else {
		da16x_eap_prt("[%s]TLS: using phase1 config options\n", __func__);
	...
	if (eap_tls_check_blob(sm, &params->ca_cert, &params->ca_cert_blob,
	    ...
	    eap_tls_check_blob(sm, &params->dh_file, &params->dh_blob,
			       &params->dh_blob_len)) {
		da16x_eap_prt("[%s] SSL: Failed to get configuration blobs\n",
			     __func__);
		return -1;
	}
#else
				   ...
				   struct tls_connection_params *params)
{
	...
	if (data->conn == NULL) {
		da16x_eap_prt("[%s]SSL: Failed to initialize new TLS "
			   "connection\n", __func__);
		return -1;
	}
	...
	if (res == TLS_SET_PARAMS_ENGINE_PRV_BAD_PIN) {
		 ...
		 */
		da16x_eap_prt("[%s]TLS: Bad PIN provided, requesting a new one\n", __func__);
		os_free(config->pin);
		config->pin = NULL;
		eap_sm_request_pin(sm);
		sm->ignore = TRUE;
	} else if (res == TLS_SET_PARAMS_ENGINE_PRV_INIT_FAILED) {
		da16x_eap_prt("[%s]TLS: Failed to initialize engine\n", __func__);
	} else if (res == TLS_SET_PARAMS_ENGINE_PRV_VERIFY_FAILED) {
		da16x_eap_prt("[%s]TLS: Failed to load private key\n", __func__);
		sm->ignore = TRUE;
	}
	if (res) {
		da16x_eap_prt("[%s]TLS: Failed to set TLS connection "
			   "parameters\n", __func__);
			  ...
			  struct eap_peer_config *config, u8 eap_type)
{
	...
	if (config->phase1 &&
		...
		da16x_eap_prt("[%s]TLS: Include TLS Message Length in "
					    ...
					    const struct wpabuf *in_data)
{
	...
	if (tls_in_len + in_len == 0) {
		...
		da16x_eap_prt("[%s]SSL: Invalid reassembly state: "
	...
	if (tls_in_len + in_len > 65536) {
		 ...
		 * memory allocations.
		 */
		da16x_eap_prt("[%s]SSL: Too long TLS fragment (size over "
			   "64 kB)\n", __func__);
		eap_peer_tls_reset_input(data);
		return -1;
	}
	...
	if (in_len > data->tls_in_left) {
		...
		da16x_eap_prt("[%s]SSL: more data than TLS message length "
			   "indicated\n", __func__);
		eap_peer_tls_reset_input(data);
		return -1;
	}
	...
	if (wpabuf_resize(&data->tls_in, in_len) < 0) {
		da16x_eap_prt("[%s]SSL: Could not allocate memory for TLS "
			   "data\n", __func__);
		eap_peer_tls_reset_input(data);
		return -1;
	}
	...
	if (data->tls_in_left > 0) {
		da16x_eap_prt("[%s]SSL: Need %lu bytes more input "
			   "data\n", __func__, (unsigned long) data->tls_in_left);
		return 1;
	}
				 ...
				 struct wpabuf **out_data)
{
	...
	if (data->tls_out) {
		...
		da16x_eap_prt("[%s]SSL: eap_tls_process_input - pending "
				  ...
				  struct wpabuf **out_data)
{
	...
	da16x_eap_prt("[%s]SSL: %lu bytes left to be sent out (of total "
		   "%lu bytes)\n", __func__,
		   (unsigned long) len,
	...
	if (len > data->tls_out_limit) {
		...
		da16x_eap_prt("[%s]SSL: sending %lu bytes, more fragments "
				...
				struct wpabuf **out_data)
{
	...
	*out_data = NULL;
	...
	if (data->tls_out && wpabuf_len(data->tls_out) > 0 &&
		...
		da16x_eap_prt("[%s]SSL: Received non-ACK when output "
	...
	if (data->tls_out == NULL || wpabuf_len(data->tls_out) == 0) {
		...
		if (tls_get_version(data->ssl_ctx, data->conn,
				    buf, sizeof(buf)) == 0) {
			da16x_eap_prt("[%s]SSL: Using TLS version %s\n", __func__, buf);
	...
	if (tls_connection_get_failed(data->ssl_ctx, data->conn)) {
		...
		da16x_eap_prt("[%s]SSL: Failed - tls_out available to "
			   "report error (len=%u)\n", __func__,
	...
	if (wpabuf_len(data->tls_out) == 0) {
		 ...
		 */
		da16x_eap_prt("[%s]SSL: No data to be sent out\n", __func__);
		wpabuf_free(data->tls_out);
		data->tls_out = NULL;
		return 1;
	}
				       ...
				       int peap_version)
{
	...
	if (resp == NULL)
		return NULL;
	da16x_eap_prt("[%s]SSL: Building ACK (type=%d id=%d ver=%d)\n",
		   __func__, (int) eap_type, id, peap_version);
				     ...
				     size_t *len, u8 *flags)
{
	...
	if (tls_get_errors(data->ssl_ctx)) {
		da16x_eap_prt("[%s]SSL: TLS errors detected\n", __func__);
		ret->ignore = TRUE;
		return NULL;
	}
	...
	if (left == 0) {
		da16x_eap_prt("[%s]SSL: Invalid TLS message: no Flags "
			   "octet included\n", __func__);
		if (!sm->workaround) {
			ret->ignore = TRUE;
			return NULL;
		...
		da16x_eap_prt("[%s]SSL: Workaround - assume no Flags "
			   "indicates ACK frame\n", __func__);
		*flags = 0;
	} else {
		*flags = *pos++;
	...
	da16x_eap_prt("[%s] SSL: Received packet(len=%lu) - "
		     "Flags 0x%02x\n", __func__,
		     (unsigned long) wpabuf_len(reqData), *flags);
	if (*flags & EAP_TLS_FLAGS_LENGTH_INCLUDED) {
		if (left < 4) {
			da16x_eap_prt("[%s] SSL: Short frame with TLS "
				     "length\n", __func__);
			ret->ignore = TRUE;
			return NULL;
		}
		tls_msg_len = WPA_GET_BE32(pos);
		da16x_eap_prt("[%s] SSL: TLS Message Length: %d\n", __func__,
			     tls_msg_len);
		if (data->tls_in_left == 0) {
			 ...
			 struct wpabuf **in_decrypted)
{
	...
	*in_decrypted = tls_connection_decrypt(data->ssl_ctx, data->conn, msg);
	...
	if (*in_decrypted == NULL) {
		da16x_eap_prt("[%s] SSL: Failed to decrypt Phase 2 data\n",
			     __func__);
		return -1;
	}
	return 0;
			 ...
			 struct wpabuf **out_data)
{
	if (in_data) {
		...
		data->tls_out = tls_connection_encrypt(data->ssl_ctx,
						       data->conn, in_data);
		if (data->tls_out == NULL) {
			da16x_eap_prt("[%s] SSL: Failed to encrypt Phase 2 "
				     "data (in_len=%lu)\n", __func__,
				   ...
				   size_t *num_types)
{
	...
	while (start && *start != '\0') {
		...
		if (vendor == EAP_VENDOR_IETF && method == EAP_TYPE_NONE) {
			da16x_eap_prt("[%s]TLS: Unsupported Phase2 EAP "
				   "method '%s'\n", __func__, start);
			os_free(methods);
			os_free(buf);
			return -1;
...
get_defaults:
	...
	if (methods == NULL) {
		da16x_eap_prt("[%s] TLS: No Phase2 EAP methods available\n",
			     __func__);
		return -1;
	}
	da16x_dump("TLS: Phase2 EAP types",
		  (u8 *) methods, num_methods * sizeof(struct eap_method_type));
			    ...
			    struct eap_hdr *hdr, struct wpabuf **resp)
{
	...
	da16x_eap_prt("[%s] TLS: Phase 2 Request: Nak type=%d\n",
		     __func__, hdr->identifier);
	da16x_dump("TLS: Allowed Phase2 EAP types",
