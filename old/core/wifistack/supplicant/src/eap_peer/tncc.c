	...
	TNC_UInt32 typeCount)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_TNCC_ReportMessageTypes(imcID=%lu "
		   "typeCount=%lu)",
		   (unsigned long) imcID, (unsigned long) typeCount);

	for (i = 0; i < typeCount; i++) {
		wpa_printf_dbg(MSG_DEBUG, "TNC: supportedTypes[%lu] = %lu",
			   i, supportedTypes[i]);
	}
	...
	TNC_MessageType messageType)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_TNCC_SendMessage(imcID=%lu "
...
{
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_TNCC_RequestHandshakeRetry");
...
static TNC_Result TNC_9048_LogMessage(TNC_IMCID imcID, TNC_UInt32 severity,
				      const char *message)
{
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_9048_LogMessage(imcID=%lu "
...
static TNC_Result TNC_9048_UserMessage(TNC_IMCID imcID,
				       ...
				       const char *message)
{
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_9048_UserMessage(imcID=%lu "
...
{
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_TNCC_BindFunction(imcID=%lu, "
...
static int tncc_imc_resolve_funcs(struct tnc_if_imc *imc)
{
	...
	if (imc->Initialize == NULL) {
		wpa_printf_dbg(MSG_ERROR, "TNC: IMC does not export "
			   "TNC_IMC_Initialize");
		return -1;
	}
...
static int tncc_imc_initialize(struct tnc_if_imc *imc)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "TNC: Calling TNC_IMC_Initialize for IMC '%s'",
		   imc->name);
	res = imc->Initialize(imc->imcID, TNC_IFIMC_VERSION_1,
			      TNC_IFIMC_VERSION_1, &imc_ver);
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_IMC_Initialize: res=%lu imc_ver=%lu",
...
static int tncc_imc_terminate(struct tnc_if_imc *imc)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "TNC: Calling TNC_IMC_Terminate for IMC '%s'",
		   imc->name);
	res = imc->Terminate(imc->imcID);
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_IMC_Terminate: %lu",
		   (unsigned long) res);
...
static int tncc_imc_provide_bind_function(struct tnc_if_imc *imc)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "TNC: Calling TNC_IMC_ProvideBindFunction for "
		   "IMC '%s'", imc->name);
	res = imc->ProvideBindFunction(imc->imcID, TNC_TNCC_BindFunction);
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_IMC_ProvideBindFunction: res=%lu",
		   (unsigned long) res);
					     ...
					     TNC_ConnectionState state)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "TNC: Calling TNC_IMC_NotifyConnectionChange(%d)"
		   " for IMC '%s'", (int) state, imc->name);
	res = imc->NotifyConnectionChange(imc->imcID, imc->connectionID,
					  state);
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_IMC_NotifyConnectionChange: %lu",
		   (unsigned long) res);
...
static int tncc_imc_begin_handshake(struct tnc_if_imc *imc)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "TNC: Calling TNC_IMC_BeginHandshake for IMC "
		   "'%s'", imc->name);
	res = imc->BeginHandshake(imc->imcID, imc->connectionID);
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_IMC_BeginHandshake: %lu",
		   (unsigned long) res);
...
{
	if (imc->path == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "TNC: No IMC configured");
		return -1;
	}

	wpa_printf_dbg(MSG_DEBUG, "TNC: Opening IMC: %s (%s)",
		   imc->name, imc->path);
#ifdef CONFIG_NATIVE_WINDOWS
#ifdef UNICODE
	{
	...
	if (imc->dlhandle == NULL) {
		wpa_printf_dbg(MSG_ERROR, "TNC: Failed to open IMC '%s' (%s): %d",
	...
	if (imc->dlhandle == NULL) {
		wpa_printf_dbg(MSG_ERROR, "TNC: Failed to open IMC '%s' (%s): %s",
	...
	if (tncc_imc_resolve_funcs(imc) < 0) {
		wpa_printf_dbg(MSG_ERROR, "TNC: Failed to resolve IMC functions");
		return -1;
	}

	if (tncc_imc_initialize(imc) < 0 ||
	    tncc_imc_provide_bind_function(imc) < 0) {
		wpa_printf_dbg(MSG_ERROR, "TNC: Failed to initialize IMC");
		return -1;
	}
			      ...
			      const u8 *msg, size_t len)
{
	...
	for (imc = tncc->imc; imc; imc = imc->next) {
		...
		wpa_printf_dbg(MSG_DEBUG, "TNC: Call ReceiveMessage for IMC '%s'",
			   imc->name);
		res = imc->ReceiveMessage(imc->imcID, imc->connectionID,
					  (TNC_BufferReference) msg, len,
					  type);
		wpa_printf_dbg(MSG_DEBUG, "TNC: ReceiveMessage: %lu",
			   (unsigned long) res);
	}
}
...
static unsigned char * tncc_get_base64(char *start, size_t *decoded_len)
{
	...
	*pos2 = '<';
	if (decoded == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "TNC: Failed to decode Base64 data");
	}
...
static enum tncc_process_res tncc_get_recommendation(char *start)
{
	...
	saved = *pos2;
	*pos2 = '\0';
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNCCS-Recommendation: '%s'", pos);
					    ...
					    const u8 *msg, size_t len)
{
	...
	*end = '\0';
	...
	if (*pos == '"')
		pos++;
	batch_id = atoi(pos);
	wpa_printf_dbg(MSG_DEBUG, "TNC: Received IF-TNCCS BatchId=%u",
		   batch_id);
	if (batch_id != tncc->last_batchid + 1) {
		wpa_printf_dbg(MSG_DEBUG, "TNC: Unexpected IF-TNCCS BatchId "
			   "%u (expected %u)",
	...
	while (*start) {
		...
		*end = '\0';
		...
		wpa_printf_dbg(MSG_DEBUG, "TNC: IMC-IMV-Message Type 0x%x", type);
	...
	while (*start) {
		...
		*end = '\0';
		...
		wpa_printf_dbg(MSG_DEBUG, "TNC: TNCC-TNCS-Message Type 0x%x",
			   type);
...
static int tncc_read_config_reg(struct tncc_data *tncc, HKEY hive)
{
	...
	for (i = 0; ; i++) {
		...
		if (ret != ERROR_SUCCESS) {
			wpa_printf_dbg(MSG_DEBUG, "TNC: RegEnumKeyEx failed: 0x%x",
				   (unsigned int) ret);
			break;
		}
		...
		wpa_printf_dbg(MSG_DEBUG, "TNC: IMC '" TSTR "'", name);

		ret = RegOpenKeyEx(hk, name, 0, KEY_QUERY_VALUE, &hk2);
		if (ret != ERROR_SUCCESS) {
			wpa_printf_dbg(MSG_DEBUG, "Could not open IMC key '" TSTR
				   "'", name);
			continue;
		}
		...
		ret = RegQueryValueEx(hk2, TEXT("Path"), NULL, NULL, NULL,
				      &buflen);
		if (ret != ERROR_SUCCESS) {
			wpa_printf_dbg(MSG_DEBUG, "TNC: Could not read Path from "
				   "IMC key '" TSTR "'", name);
			RegCloseKey(hk2);
			continue;
		}
		...
		wpa_printf_dbg(MSG_DEBUG, "TNC: IMC Path '%s'", (char *) val);
		...
		if (j >= TNC_MAX_IMC_ID) {
			wpa_printf_dbg(MSG_DEBUG, "TNC: Too many IMCs");
			os_free(val);
			continue;
		}
...
static struct tnc_if_imc * tncc_parse_imc(char *start, char *end, int *error)
{
	...
	pos = start;
	wpa_printf_dbg(MSG_DEBUG, "TNC: Configured IMC: %s", pos);
	if (pos + 1 >= end || *pos != '"') {
		wpa_printf_dbg(MSG_ERROR, "TNC: Ignoring invalid IMC line '%s' "
	...
	*pos2 = '\0';
	wpa_printf_dbg(MSG_DEBUG, "TNC: Name: '%s'", pos);
	...
	pos++;
	wpa_printf_dbg(MSG_DEBUG, "TNC: IMC file: '%s'", pos);
...
static int tncc_read_config(struct tncc_data *tncc)
{
	...
	if (config == NULL) {
		wpa_printf_dbg(MSG_ERROR, "TNC: Could not open TNC configuration "
...
struct tncc_data * tncc_init(void)
{
	...
	if (tncc_read_config(tncc) < 0) {
		wpa_printf_dbg(MSG_ERROR, "TNC: Failed to read TNC configuration");
		goto failed;
	}

	for (imc = tncc->imc; imc; imc = imc->next) {
		if (tncc_load_imc(imc)) {
			wpa_printf_dbg(MSG_ERROR, "TNC: Failed to load IMC '%s'",
				   imc->name);
			goto failed;
		}
	}
...
struct wpabuf * tncc_process_soh_request(int ver, const u8 *data, size_t len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "TNC: SoH Request TLV received");
