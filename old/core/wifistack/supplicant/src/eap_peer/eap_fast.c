...
#if 1	/* by Shingu 20170131 (TTA Certificate) */
#define EAP_FAST_UNKNOWN_CERT_A_ID	0x02cdab8967452301
#endif	/* 1 */
				      ...
				      u8 *master_secret)
{
	...
	da16x_eap_prt("[%s] EAP-FAST: SessionTicket callback\n", __func__);

	if (client_random == NULL || server_random == NULL ||
	    master_secret == NULL) {
		da16x_eap_prt("[%s] EAP-FAST: SessionTicket failed - fall "
			     "back to full TLS handshake\n", __func__);
		data->session_ticket_used = 0;
		if (data->provisioning_allowed) {
			da16x_eap_prt("[%s] EAP-FAST: Try to provision a "
				     "new PAC-Key\n", __func__);
			data->provisioning = 1;
	...
	da16x_dump("EAP-FAST: SessionTicket", ticket, len);

	if (data->current_pac == NULL) {
		da16x_eap_prt("[%s] EAP-FAST: No PAC-Key available for "
			     "using SessionTicket\n", __func__);
		data->session_ticket_used = 0;
		return 0;
	}
				  ...
				  const char *phase1)
{
	...
	if (pos) {
		...
		da16x_eap_prt("[%s] EAP-FAST: Automatic PAC provisioning "
			     "mode: %d\n", __func__,
	...
	if (pos) {
		...
		da16x_eap_prt("[%s] EAP-FAST: Maximum PAC list length: %lu\n",
			     __func__, (unsigned long) data->max_pac_list_len);
	}

	pos = os_strstr(phase1, "fast_pac_format=binary");
	if (pos) {
		data->use_pac_binary_format = 1;
		da16x_eap_prt("[%s] EAP-FAST: Using binary format for PAC "
			     "list\n", __func__);
	}
}
...
static void * eap_fast_init(struct eap_sm *sm)
{
	...
	if (config->phase1) {
		...
		if (data->provisioning_allowed == 1) {
			da16x_err_prt("EAP-FAST: fast_provisioning=1 is not supported.\n");
			os_free(data);
			return NULL;
		}
	}
	...
	if (eap_peer_tls_ssl_init(sm, &data->ssl, config, EAP_TYPE_FAST)) {
		da16x_eap_prt("[%s] EAP-FAST: Failed to initialize SSL.\n",
			     __func__);
		eap_fast_deinit(sm, data);
		return NULL;
	}
	...
	if (tls_connection_set_session_ticket_cb(sm->ssl_ctx, data->ssl.conn,
						 ...
						 data) < 0) {
		da16x_eap_prt("[%s] EAP-FAST: Failed to set SessionTicket "
			     "callback\n", __func__);
		eap_fast_deinit(sm, data);
		return NULL;
	}
	...
	if (tls_connection_enable_workaround(sm->ssl_ctx, data->ssl.conn)) {
		da16x_eap_prt("[%s] EAP-FAST: Failed to enable TLS "
			     "workarounds\n", __func__);
	}

#ifdef	CONFIG_FAST_FILE
	if (!config->pac_file) {
		da16x_eap_prt("[%s] EAP-FAST: No PAC file configured\n",
			     __func__);
		eap_fast_deinit(sm, data);
		return NULL;
	}
	...
	if (data->use_pac_binary_format &&
		...
		da16x_eap_prt("[%s] EAP-FAST: Failed to load PAC file\n",
			     __func__);
		eap_fast_deinit(sm, data);
		return NULL;
	}
	...
	if (data->pac == NULL && !data->provisioning_allowed) {
		da16x_eap_prt("[%s] EAP-FAST: No PAC configured and "
				    ...
				    struct eap_fast_data *data)
{
	...
	sks = eap_fast_derive_key(sm->ssl_ctx, data->ssl.conn,
				  EAP_FAST_SKS_LEN);
	if (sks == NULL) {
		da16x_eap_prt("[%s] EAP-FAST: Failed to derive "
			     "session_key_seed\n", __func__);
		return -1;
	}
	 ...
	 */
	da16x_dump("EAP-FAST: session_key_seed (SKS = S-IMCK[0])",
					    ...
					    struct eap_fast_data *data)
{
	...
	if (data->key_block_p == NULL) {
		da16x_eap_prt("[%s] EAP-FAST: Failed to derive key block\n",
			     __func__);
		return -1;
	}
	/*
...
static int eap_fast_select_phase2_method(struct eap_fast_data *data, u8 type)
{
	...
	if (data->anon_provisioning && type != EAP_TYPE_MSCHAPV2) {
		da16x_eap_prt("[%s] EAP-FAST: Only EAP-MSCHAPv2 is allowed "
	...
	if (type == EAP_TYPE_TNC) {
		...
		da16x_eap_prt("[%s] EAP-FAST: Selected Phase 2 EAP "
				   ...
				   struct wpabuf **resp)
{
	...
	if (len <= sizeof(struct eap_hdr)) {
		da16x_eap_prt("[%s] EAP-FAST: too short "
			     "Phase 2 request (len=%lu)\n", __func__,
			     (unsigned long) len);
		return -1;
	}
	pos = (u8 *) (hdr + 1);
	da16x_eap_prt("[%s] EAP-FAST: Phase 2 Request: type=%d\n",
		     __func__, *pos);
	...
	if (data->phase2_priv && data->phase2_method &&
	    *pos != data->phase2_type.method) {
		da16x_eap_prt("[%s] EAP-FAST: Phase 2 EAP sequence - "
	...
	if ((data->phase2_priv == NULL &&
	    ...
	    data->phase2_method == NULL) {
		da16x_eap_prt("[%s] EAP-FAST: Failed to initialize "
...
static struct wpabuf * eap_fast_tlv_result(int status, int intermediate)
{
	...
	if (buf == NULL)
		return NULL;
	da16x_eap_prt("[%s] EAP-FAST: Add %sResult TLV(status=%d)\n", __func__,
...
static struct wpabuf * eap_fast_tlv_pac_ack(void)
{
	...
	da16x_eap_prt("[%s] EAP-FAST: Add PAC TLV (ack)\n", __func__);
	...
	u8 *eap_payload_tlv, size_t eap_payload_tlv_len)
{
	...
	if (eap_payload_tlv_len < sizeof(*hdr)) {
		da16x_eap_prt("[%s] EAP-FAST: too short EAP "
			     "Payload TLV (len=%lu)\n", __func__,
	...
	if (be_to_host16(hdr->length) > eap_payload_tlv_len) {
		da16x_eap_prt("[%s] EAP-FAST: EAP packet overflow in "
			     "EAP Payload TLV\n", __func__);
		return NULL;
	}

	if (hdr->code != EAP_CODE_REQUEST) {
		da16x_eap_prt("[%s] EAP-FAST: Unexpected code=%d in "
			     "Phase 2 EAP header\n", __func__, hdr->code);
		return NULL;
	}

	if (eap_fast_phase2_request(sm, data, ret, hdr, &resp)) {
		da16x_eap_prt("[%s] EAP-FAST: Phase2 Request processing "
			     "failed\n", __func__);
		return NULL;
	}
...
{
	da16x_eap_prt("[%s] EAP-FAST: Crypto-Binding TLV: Version %d "
		     "Received Version %d SubType %d\n", __func__,
		     _bind->version, _bind->received_version, _bind->subtype);
	da16x_dump("EAP-FAST: NONCE", _bind->nonce, sizeof(_bind->nonce));
	da16x_dump("EAP-FAST: Compound MAC",
	...
	if (_bind->version != EAP_FAST_VERSION ||
		...
		da16x_eap_prt("[%s] EAP-FAST: Invalid version/subtype in "
	...
	struct eap_tlv_crypto_binding_tlv *_bind, const u8 *cmk)
{
	...
	da16x_eap_prt("[%s] EAP-FAST: Reply Crypto-Binding TLV: Version %d "
		     "Received Version %d SubType %d\n", __func__,
		     rbind->version, rbind->received_version, rbind->subtype);
	da16x_dump("EAP-FAST: NONCE", rbind->nonce, sizeof(rbind->nonce));
	da16x_dump("EAP-FAST: Compound MAC",
				   ...
				   u8 *isk, size_t isk_len)
{
	...
	if (data->phase2_method == NULL || data->phase2_priv == NULL) {
		da16x_eap_prt("[%s] EAP-FAST: Phase 2 method not "
			     "available\n", __func__);
		return -1;
	}
	...
	if (!data->phase2_method->isKeyAvailable(sm, data->phase2_priv) ||
	    (key = data->phase2_method->getKey(sm, data->phase2_priv,
					       &key_len)) == NULL) {
		da16x_eap_prt("[%s] EAP-FAST: Could not get key material "
			     "from Phase 2\n", __func__);
		return -1;
	}
			    ...
			    u8 *cmk)
{
	...
	da16x_eap_prt("[%s] EAP-FAST: Determining CMK[%d] for Compound MIC "
		     "calculation\n", __func__, data->simck_idx + 1);
	...
	if (eap_fast_get_phase2_key(sm, data, isk, sizeof(isk)) < 0)
		return -1;
	da16x_dump("EAP-FAST: ISK[j]", isk, sizeof(isk));
	...
	da16x_dump("EAP-FAST: S-IMCK[j]", data->simck, EAP_FAST_SIMCK_LEN);
	os_memcpy(cmk, imck + EAP_FAST_SIMCK_LEN, EAP_FAST_CMK_LEN);
	da16x_dump("EAP-FAST: CMK[j]", cmk, EAP_FAST_CMK_LEN);
	...
	struct eap_tlv_crypto_binding_tlv *_bind, size_t bind_len)
{
	...
	da16x_dump("EAP-FAST: Crypto-Binding TLV for Compound "
	...
	da16x_dump("EAP-FAST: Received Compound MAC", cmac, sizeof(cmac));
	da16x_dump("EAP-FAST: Calculated Compound MAC",
		  _bind->compound_mac, sizeof(cmac));
	if (res != 0) {
		da16x_eap_prt("[%s] EAP-FAST: Compound MAC did not match\n",
			     __func__);
	...
	if (!data->anon_provisioning && data->phase2_success &&
		...
		da16x_eap_prt("[%s] EAP-FAST: Failed to generate MSK\n",
			     __func__);
		ret->methodState = METHOD_DONE;
		ret->decision = DECISION_FAIL;
	...
	if (!data->anon_provisioning && data->phase2_success) {
		...
		if (data->session_id) {
			da16x_dump("EAP-FAST: Derived Session-Id",
				  data->session_id, data->id_len);
		} else {
			da16x_eap_prt("EAP-FAST: Failed to derive "
				     "Session-Id\n", __func__);
			wpabuf_free(resp);
			return NULL;
		}
				   ...
				   u8 *pos, size_t len, int *pac_key_found)
{
	...
	case PAC_TYPE_PAC_KEY:
		da16x_dump("EAP-FAST: PAC-Key", pos, len);
		if (len != EAP_FAST_PAC_KEY_LEN) {
			da16x_eap_prt("[%s] EAP-FAST: Invalid PAC-Key "
				     "length %lu\n", __func__,
				     (unsigned long) len);
			break;
		}
	...
	case PAC_TYPE_PAC_OPAQUE:
		da16x_dump("EAP-FAST: PAC-Opaque", pos, len);
		entry->pac_opaque = pos;
		entry->pac_opaque_len = len;
		break;
	case PAC_TYPE_PAC_INFO:
		da16x_dump("EAP-FAST: PAC-Info", pos, len);
		entry->pac_info = pos;
		entry->pac_info_len = len;
		break;
	default:
		da16x_eap_prt("[%s] EAP-FAST: Ignored unknown PAC type %d\n",
			     __func__, type);
		break;
	}
}
				    ...
				    u8 *pac, size_t pac_len)
{
	...
	while (left > sizeof(*hdr)) {
		...
		left -= sizeof(*hdr);
		if (len > left) {
			da16x_eap_prt("[%s] EAP-FAST: PAC TLV overrun "
	...
	if (!pac_key_found || !entry->pac_opaque || !entry->pac_info) {
		da16x_eap_prt("[%s] EAP-FAST: PAC TLV does not include "
				   ...
				   u8 *pos, size_t len)
{
	...
	case PAC_TYPE_CRED_LIFETIME:
		...
		os_get_time(&now);
		da16x_eap_prt("EAP-FAST: PAC-Info - CRED_LIFETIME %d "
			   "(%d days)",
			   lifetime, (lifetime - (u32) now.sec) / 86400);
		break;
	case PAC_TYPE_A_ID:
		da16x_dump("EAP-FAST: PAC-Info - A-ID", pos, len);
		entry->a_id = pos;
		entry->a_id_len = len;
		break;
	case PAC_TYPE_I_ID:
		da16x_dump("EAP-FAST: PAC-Info - I-ID", pos, len);
		entry->i_id = pos;
		entry->i_id_len = len;
		break;
	case PAC_TYPE_A_ID_INFO:
		da16x_dump("EAP-FAST: PAC-Info - A-ID-Info", pos, len);
		entry->a_id_info = pos;
	...
	case PAC_TYPE_PAC_TYPE:
		...
		if (len != 2) {
			da16x_eap_prt("[%s] EAP-FAST: Invalid PAC-Type "
				     "length %lu (expected 2)\n", __func__,
				     (unsigned long) len);
			da16x_dump("EAP-FAST: PAC-Info - PAC-Type", pos, len);
			return -1;
		}
		...
		if (pac_type != PAC_TYPE_TUNNEL_PAC &&
			...
			da16x_eap_prt("[%s] EAP-FAST: Unsupported PAC Type "
				     "%d\n", __func__, pac_type);
			return -1;
		}

		da16x_eap_prt("[%s] EAP-FAST: PAC-Info - PAC-Type %d\n",
			     __func__, pac_type);
		entry->pac_type = pac_type;
		break;
	default:
		da16x_eap_prt("[%s] EAP-FAST: Ignored unknown PAC-Info "
			     "type %d\n", __func__, type);
		break;
	}
...
static int eap_fast_process_pac_info(struct eap_fast_pac *entry)
{
	...
	for (int i = 0; i < 3; i++) {
		...
		left -= sizeof(*hdr);
		if (len > left) {
			da16x_eap_prt("[%s] EAP-FAST: PAC-Info overrun "
				     "(type=%d len=%lu left=%lu)\n", __func__,
				     type, (unsigned long) len,
				     (unsigned long) left);
			return -1;
	...
	if (entry->a_id == NULL && entry->a_id_info == NULL) {
#endif	/* 0 */
		da16x_eap_prt("[%s] EAP-FAST: PAC-Info does not include "
			     "all the required fields\n", __func__);
		return -1;
	}

#if 0	/* by Shingu 20170131 (TTA Certificate) */
	long long a_id_tmp = EAP_FAST_UNKNOWN_CERT_A_ID;
	if (entry->a_id != NULL &&
	    os_memcmp(entry->a_id, &a_id_tmp, 8) == 0) {
		return -1;
	}
#endif	/* 1 */
					    ...
					    u8 *pac, size_t pac_len)
{
	...
	if (data->provisioning) {
		...
		da16x_eap_prt("[%s] EAP-FAST: Send PAC-Acknowledgement TLV "
				    ...
				    struct wpabuf **resp)
{
	...
	while (end - pos > 4) {
		...
		da16x_eap_prt("[%s] EAP-FAST: Received Phase 2: "
		...
		if (res < 0) {
			if (mandatory) {
				da16x_eap_prt("[%s] EAP-FAST: Nak unknown "
					     "mandatory TLV type %d\n",
					     __func__, tlv_type);
				*resp = eap_fast_tlv_nak(0, tlv_type);
				break;
			} else {
				da16x_eap_prt("[%s] EAP-FAST: ignored "
				     ...
				     u8 identifier, struct wpabuf **out_data)
{
	...
	if (eap_peer_tls_encrypt(sm, &data->ssl, EAP_TYPE_FAST,
				 ...
				 resp, out_data)) {
		da16x_eap_prt("[%s] EAP-FAST: Failed to encrypt a Phase 2 "
			     "frame\n", __func__);
	}
	wpabuf_free(resp);
				      ...
				      struct wpabuf **out_data)
{
	...
	if (tlv.pac && tlv.result != EAP_TLV_RESULT_SUCCESS) {
		da16x_eap_prt("[%s] EAP-FAST: PAC TLV without Result TLV "
	...
	if (data->current_pac == NULL && data->provisioning &&
		 ...
		 * provisioning.
		 */
		da16x_eap_prt("[%s] EAP-FAST: Request Tunnel PAC\n", __func__);
	...
	if (resp && tlv.result == EAP_TLV_RESULT_SUCCESS && !failed &&
		...
		if (data->anon_provisioning) {
			da16x_eap_prt("[%s] EAP-FAST: Unauthenticated "
				     "provisioning completed successfully.\n",
				     __func__);
			ret->methodState = METHOD_DONE;
			ret->decision = DECISION_FAIL;
			...
			da16x_eap_prt("[%s] EAP-FAST: Authentication "
	...
	if (resp == NULL) {
		da16x_eap_prt("[%s] EAP-FAST: No recognized TLVs - send "
			    ...
			    struct wpabuf **out_data)
{
	...
	da16x_eap_prt("[%s] EAP-FAST: Received %lu bytes encrypted data for"
		     " Phase 2\n", __func__,
		     (unsigned long) wpabuf_len(in_data));

	if (data->pending_phase2_req) {
		da16x_eap_prt("[%s] EAP-FAST: Pending Phase 2 request - "
...
continue_req:
	if (wpabuf_len(in_decrypted) < 4) {
		da16x_eap_prt("[%s] EAP-FAST: Too short Phase 2 "
			     "TLV frame (len=%lu)\n", __func__,
	...
	*id_len = len;
	if (len > sizeof(*hdr)) {
		...
		if (be_to_host16(hdr->type) == PAC_TYPE_A_ID &&
		    sizeof(*hdr) + tlen <= len) {
			da16x_eap_prt("[%s] EAP-FAST: A-ID was in TLV "
				     "(Start)\n", __func__);
			a_id = (const u8 *) (hdr + 1);
			*id_len = tlen;
		}
	}
	da16x_dump("EAP-FAST: A-ID", a_id, *id_len);
				...
				const u8 *a_id, size_t a_id_len)
{
	...
	if (data->current_pac) {
		da16x_eap_prt("[%s] EAP-FAST: PAC found for this A-ID "
			     "(PAC-Type %d)\n", __func__,
			     data->current_pac->pac_type);
		da16x_dump("EAP-FAST: A-ID-Info",
				   ...
				   struct eap_fast_pac *pac)
{
	...
	if (tlv == NULL ||
	    tls_connection_client_hello_ext(sm->ssl_ctx, data->ssl.conn,
					    TLS_EXT_PAC_OPAQUE,
					    tlv, tlv_len) < 0) {
		da16x_eap_prt("[%s] EAP-FAST: Failed to add PAC-Opaque TLS "
			     "extension\n", __func__);
		os_free(tlv);
		return -1;
	}
					 ...
					 struct eap_fast_data *data)
{
	if (tls_connection_client_hello_ext(sm->ssl_ctx, data->ssl.conn,
		...
		da16x_eap_prt("[%s] EAP-FAST: Failed to remove PAC-Opaque "
			     "TLS extension\n", __func__);
		return -1;
	}
	return 0;
					     ...
					     struct eap_fast_data *data)
{
	...
	if (tls_connection_set_cipher_list(sm->ssl_ctx, data->ssl.conn,
					   ciphers)) {
		da16x_eap_prt("[%s] EAP-FAST: Could not configure TLS "
				  ...
				  const u8 *pos, size_t left)
{
	...
	da16x_eap_prt("[%s] EAP-FAST: Start (server ver=%d, own ver=%d)\n",
		     __func__, flags & EAP_TLS_VERSION_MASK, data->fast_version);
	if ((flags & EAP_TLS_VERSION_MASK) < data->fast_version)
		data->fast_version = flags & EAP_TLS_VERSION_MASK;
	da16x_eap_prt("[%s] EAP-FAST: Using FAST version %d\n", __func__,
		     data->fast_version);
	...
	if (data->resuming && data->current_pac) {
		da16x_eap_prt("[%s] EAP-FAST: Trying to resume session - "
		...
		if (!data->provisioning_allowed) {
			da16x_eap_prt("[%s] EAP-FAST: No PAC found and "
				     "provisioning disabled\n", __func__);
			return -1;
		}
		da16x_eap_prt("[%s] EAP-FAST: No PAC found - "
					...
					const struct wpabuf *reqData)
{
	...
	if (tls_connection_established(sm->ssl_ctx, data->ssl.conn) &&
		...
		if (tls_connection_established(sm->ssl_ctx, data->ssl.conn)) {
			char cipher[80];
			da16x_eap_prt("[%s] EAP-FAST: TLS done, proceed to "
				     "Phase 2\n", __func__);
			if (data->provisioning &&
			    (!(data->provisioning_allowed &
			       EAP_FAST_PROV_AUTH) ||
				...
				da16x_eap_prt("[%s] EAP-FAST: Using "
