...
{
	wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: %s -> %s",
...
static void * eap_gpsk_init(struct eap_sm *sm)
{
	...
	if (password == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-GPSK: No key (password) configured");
		return NULL;
	}
	...
	if (phase1) {
		...
		if (pos) {
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Forced cipher %u",
				   data->forced_cipher);
		}
	}
					     ...
					     const u8 *pos, const u8 *end)
{
	...
	if (end - pos < 2) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Too short GPSK-1 packet");
		return NULL;
	}
	alen = WPA_GET_BE16(pos);
	pos += 2;
	if (end - pos < alen) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: ID_Server overflow");
		return NULL;
	}
	os_free(data->id_server);
	data->id_server = os_memdup(pos, alen);
	if (data->id_server == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: No memory for ID_Server");
		return NULL;
	}
					       ...
					       const u8 *pos, const u8 *end)
{
	...
	if (end - pos < EAP_GPSK_RAND_LEN) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: RAND_Server overflow");
		return NULL;
	}
				  ...
				  size_t csuite_list_len)
{
	...
	for (i = 0; i < count; i++) {
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: CSuite[%d]: %d:%d",
			   i, vendor, specifier);
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Selected ciphersuite %d:%d",
					       ...
					       const u8 *pos, const u8 *end)
{
	...
	if (len > (size_t) (end - pos)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: CSuite_List overflow");
		return NULL;
	}
	if (len == 0 || (len % sizeof(struct eap_gpsk_csuite))) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Invalid CSuite_List len %lu",
			   (unsigned long) len);
		return NULL;
	}
					       ...
					       size_t payload_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Received Request/GPSK-1");
					    ...
					    size_t csuite_list_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Sending Response/GPSK-2");
	...
	if (random_get_bytes(data->rand_peer, EAP_GPSK_RAND_LEN)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Failed to get random data "
			   "for RAND_Peer");
		eap_gpsk_state(data, FAILURE);
		wpabuf_free(resp);
		return NULL;
	...
	if (eap_gpsk_derive_keys(data->psk, data->psk_len,
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Failed to derive keys");
		eap_gpsk_state(data, FAILURE);
		wpabuf_free(resp);
		return NULL;
	}
	...
	if (eap_gpsk_derive_session_id(data->psk, data->psk_len,
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Failed to derive Session-Id");
		eap_gpsk_state(data, FAILURE);
		wpabuf_free(resp);
		return NULL;
	}
...
{
	if (end - pos < EAP_GPSK_RAND_LEN) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Message too short for "
			   "RAND_Peer");
		return NULL;
	}
	if (os_memcmp(pos, data->rand_peer, EAP_GPSK_RAND_LEN) != 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: RAND_Peer in GPSK-2 and "
	...
	if (os_memcmp(pos, data->rand_server, EAP_GPSK_RAND_LEN) != 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: RAND_Server in GPSK-1 and "
			   "GPSK-3 did not match");
					      ...
					      const u8 *pos, const u8 *end)
{
	...
	if (len != data->id_server_len ||
		...
		wpa_printf_dbg(MSG_INFO, "EAP-GPSK: ID_Server did not match with "
					   ...
					   const u8 *pos, const u8 *end)
{
	...
	if (vendor != data->vendor || specifier != data->specifier) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: CSuite_Sel (%d:%d) does not "
					       ...
					       const u8 *pos, const u8 *end)
{
	...
	if (end - pos < (int) miclen) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Message too short for MIC "
			   "(left=%lu miclen=%lu)",
	...
	if (eap_gpsk_compute_mic(data->sk, data->sk_len, data->vendor,
	    ...
	    < 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Failed to compute MIC");
		return NULL;
	}
	if (os_memcmp_const(mic, pos, miclen) != 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-GPSK: Incorrect MIC in GPSK-3");
					       ...
					       size_t payload_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Received Request/GPSK-3");
	...
	if (pos != end) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Ignored %lu bytes of extra "
					    ...
					    u8 identifier)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Sending Response/GPSK-4");
					...
					const struct wpabuf *reqData)
{
	...
	opcode = *pos++;
	len--;
	wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Received frame: opcode %d", opcode);
