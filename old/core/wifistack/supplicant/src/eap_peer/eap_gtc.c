...
static void * eap_gtc_init(struct eap_sm *sm)
{
	...
	if (sm->m && sm->m->vendor == EAP_VENDOR_IETF &&
		...
		da16x_eap_prt("[%s] EAP-GTC: EAP-FAST tunnel - use prefix "
				       ...
				       const struct wpabuf *reqData)
{
	...
	da16x_dump("EAP-GTC: Request message", pos, len);
	if (data->prefix &&
	    (len < 10 || os_memcmp(pos, "CHALLENGE=", 10) != 0)) {
		da16x_eap_prt("[%s] EAP-GTC: Challenge did not start with "
			     "expected prefix\n", __func__);
	...
	if (password == NULL) {
		da16x_eap_prt("[%s] EAP-GTC: Password not configured\n",
			     __func__);
	...
	da16x_dump("EAP-GTC: Response",
		  wpabuf_head_u8(resp) + sizeof(struct eap_hdr) + 1, plen);

	if (otp) {
		da16x_eap_prt("[%s] EAP-GTC: Forgetting used password\n",
			     __func__);
