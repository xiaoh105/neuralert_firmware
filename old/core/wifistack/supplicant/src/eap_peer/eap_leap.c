						...
						const struct wpabuf *reqData)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-LEAP: Processing EAP-Request");
	...
	if (pos == NULL || len < 3) {
		wpa_printf_dbg(MSG_INFO, "EAP-LEAP: Invalid EAP-Request frame");
		ret->ignore = TRUE;
		return NULL;
	}

	if (*pos != LEAP_VERSION) {
		wpa_printf_dbg(MSG_WARNING, "EAP-LEAP: Unsupported LEAP version "
			   "%d", *pos);
		ret->ignore = TRUE;
		return NULL;
	}
	...
	if (challenge_len != LEAP_CHALLENGE_LEN || challenge_len > len - 3) {
		wpa_printf_dbg(MSG_INFO, "EAP-LEAP: Invalid challenge "
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-LEAP: Generating Challenge Response");
	...
	if ((pwhash && challenge_response(challenge, password, rpos)) ||
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-LEAP: Failed to derive response");
		ret->ignore = TRUE;
		wpabuf_free(resp);
		return NULL;
	}
						...
						const struct wpabuf *reqData)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-LEAP: Processing EAP-Success");
	...
	if (data->state != LEAP_WAIT_SUCCESS) {
		wpa_printf_dbg(MSG_INFO, "EAP-LEAP: EAP-Success received in "
	...
	if (random_get_bytes(pos, LEAP_CHALLENGE_LEN)) {
		wpa_printf_dbg(MSG_WARNING, "EAP-LEAP: Failed to read random data "
			   "for challenge");
		wpabuf_free(resp);
		ret->ignore = TRUE;
		return NULL;
						 ...
						 const struct wpabuf *reqData)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-LEAP: Processing EAP-Response");
	...
	pos = eap_hdr_validate(EAP_VENDOR_IETF, EAP_TYPE_LEAP, reqData, &len);
	if (pos == NULL || len < 3) {
		wpa_printf_dbg(MSG_INFO, "EAP-LEAP: Invalid EAP-Response frame");
		ret->ignore = TRUE;
		return NULL;
	}
	...
	if (response_len != LEAP_RESPONSE_LEN || response_len > len - 3) {
		wpa_printf_dbg(MSG_INFO, "EAP-LEAP: Invalid response "
	...
	if (os_memcmp_const(pos, expected, LEAP_RESPONSE_LEN) != 0) {
		wpa_printf_dbg(MSG_WARNING, "EAP-LEAP: AP sent an invalid "
					...
					const struct wpabuf *reqData)
{
	...
	if (password == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-LEAP: Password not configured");
	...
	if (wpabuf_len(reqData) < sizeof(*eap) ||
		...
		wpa_printf_dbg(MSG_INFO, "EAP-LEAP: Invalid frame");
		ret->ignore = TRUE;
		return NULL;
	}
	...
	default:
		wpa_printf_dbg(MSG_INFO, "EAP-LEAP: Unexpected EAP code (%d) - "
			   "ignored", eap->code);
		ret->ignore = TRUE;
		return NULL;
	}
