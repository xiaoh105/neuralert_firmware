				       ...
				       const struct wpabuf *reqData)
{
	...
	if (password == NULL) {
		da16x_eap_prt("[%s] EAP-MD5: Password not configured\n",
			     __func__);
	...
	if (pos == NULL || len == 0) {
		da16x_eap_prt("[%s] EAP-MD5: Invalid frame (pos=%p len=%lu)\n",
			     __func__, pos, (unsigned long) len);
		ret->ignore = TRUE;
		return NULL;
	}
	...
	if (challenge_len == 0 || challenge_len > len - 1) {
		da16x_eap_prt("[%s] EAP-MD5: Invalid challenge "
	...
	ret->ignore = FALSE;
	challenge = pos;
	da16x_dump("EAP-MD5: Challenge", challenge, challenge_len);

	da16x_eap_prt("[%s] EAP-MD5: Generating Challenge Response\n", __func__);
	...
	if (chap_md5(id, password, password_len, challenge, challenge_len,
		     rpos)) {
		da16x_eap_prt("[%s] EAP-MD5: CHAP MD5 operation failed\n",
			     __func__);
		ret->ignore = TRUE;
		wpabuf_free(resp);
		return NULL;
	}
	da16x_dump("EAP-MD5: Response", rpos, CHAP_MD5_LEN);
