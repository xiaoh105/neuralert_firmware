				  ...
				  struct eap_fast_read_ctx *rc)
{
	...
	if (os_strncmp(pac_file, "blob://", 7) == 0) {
		...
		if (blob == NULL) {
			da16x_eap_prt("[%s] EAP-FAST: No PAC blob '%s' - "
		...
		if (rc->f == NULL) {
			da16x_eap_prt("[%s] EAP-FAST: No PAC file '%s' - "
				     "assume no PAC entries have been "
					     "provisioned\n", __func__,
					     pac_file);
			os_free(rc->buf);
		      ...
		      const char *pac_file)
{
...
#ifndef	CONFIG_FAST_FILE
	pac_buf = wpa_config_read_nvram_string(ENV_FAST_PAC);
	pac_len = strlen(pac_buf);
	if (pac_len <= 0) {
	...
	if (err) {
		da16x_eap_prt("[%s] EAP-FAST: %s in '%s:%d'\n", __func__,
			     err, pac_file, rc.line);
		return -1;
	}

	da16x_eap_prt("[%s] EAP-FAST: Read %d PAC entries\n",
		   __func__, count);
			      ...
			      char *buf, size_t len)
{
	if (os_strncmp(pac_file, "blob://", 7) == 0) {
		...
		f = fopen(pac_file, "wb");
		if (f == NULL) {
			da16x_eap_prt("[%s] EAP-FAST: Failed to open PAC "
				   "file '%s' for writing\n",
				   __func__, pac_file);
			return -1;
		}
		if (fwrite(buf, 1, len, f) != len) {
			da16x_eap_prt("[%s] EAP-FAST: Failed to write all "
				     "PACs into '%s'\n", __func__, pac_file);
			fclose(f);
			return -1;
		}
				 ...
				 char **pos, size_t *buf_len)
{
	...
	*pos += ret;
	...
	if (*buf == NULL) {
		da16x_eap_prt("[%s] EAP-FAST: No memory for PAC "
			     "data\n", __func__);
		return -1;
	}
		      ...
		      const char *pac_file)
{
	...
	wpa_config_write_nvram_string(ENV_FAST_PAC, buf);
	wpa_config_write_nvram_int(ENV_FAST_PAC_LEN, pos - buf, 0);
#endif	/* CONFIG_FAST_FILE */

	da16x_eap_prt("[%s] EAP-FAST: Wrote %d PAC entries\n", __func__, count);
			  ...
			  const char *pac_file)
{
	...
	*pac_root = NULL;
	...
	if (os_strncmp(pac_file, "blob://", 7) == 0) {
		...
		da16x_eap_prt("[%s] What do I do for eloop timer ???\n",
			     __func__);
		return 0;
	}
	...
	if (len < 6 || WPA_GET_BE32(buf) != EAP_FAST_PAC_BINARY_MAGIC ||
		...
		da16x_eap_prt("[%s] EAP-FAST: Invalid PAC file '%s' (bin)\n",
			     __func__, pac_file);
		if (blob == NULL)
			os_free(buf);
		return -1;
	...
	da16x_eap_prt("[%s] EAP-FAST: Read %lu PAC entries (bin)\n",
		     __func__, (unsigned long) count);

	return 0;

parse_fail:
	da16x_eap_prt("[%s] EAP-FAST: Failed to parse PAC file '%s' (bin)\n",
		     __func__, pac_file);
	if (blob == NULL)
		os_free(buf);
	if (pac)
			  ...
			  const char *pac_file)
{
	...
	da16x_eap_prt("[%s] EAP-FAST: Wrote %lu PAC entries into '%s' "
		     "(bin)\n", __func__, (unsigned long) count, pac_file);
