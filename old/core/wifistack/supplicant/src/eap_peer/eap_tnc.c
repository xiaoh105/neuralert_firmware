...
static struct wpabuf * eap_tnc_build_frag_ack(u8 id, u8 code)
{
	...
	if (msg == NULL) {
		wpa_printf_dbg(MSG_ERROR, "EAP-TNC: Failed to allocate memory "
			   "for fragment ack");
		return NULL;
	}
					 ...
					 struct eap_method_ret *ret, u8 id)
{
	...
	ret->ignore = FALSE;
	wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Generating Response");
				...
				const u8 *buf, size_t len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Received %lu bytes, waiting for "
				       ...
				       const struct wpabuf *reqData)
{
	...
	if (pos == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-TNC: Invalid frame (pos=%p len=%lu)",
	...
	if (data->state == WAIT_FRAG_ACK) {
		if (len > 1) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Unexpected payload in "
				   "WAIT_FRAG_ACK state");
			ret->ignore = TRUE;
			return NULL;
		}
	...
	if (data->state == WAIT_START) {
		if (!(flags & EAP_TNC_FLAGS_START)) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Server did not use "
		...
		if (flags & EAP_TNC_FLAGS_START) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Server used start "
				   "flag again");
			ret->ignore = TRUE;
			goto fail;
		}
		...
		case TNCCS_RECOMMENDATION_ERROR:
			wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: No "
	...
	if (tncs_done) {
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: TNCS done - reply with an "
			   "empty ACK message");
		return resp;
	}
