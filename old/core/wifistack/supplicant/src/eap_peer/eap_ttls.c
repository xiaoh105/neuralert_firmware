...
static void * eap_ttls_init(struct eap_sm *sm)
{
	...
	if (config && config->phase2) {
		...
		while ((token = cstr_token(config->phase2, " \t", &last))) {
			...
			if (last - token == 8 &&
				...
				da16x_eap_prt("[%s]EAP-TTLS: Unsupported Phase2 type '%s'\n",
					   __func__, token);
			...
			if (selected_non_eap) {
				da16x_eap_prt("[%s]EAP-TTLS: Only one Phase2 type can be specified\n", __func__);
				eap_ttls_deinit(sm, data);
				return NULL;
			}
		...
		if (os_strstr(config->phase2, "autheap=")) {
			if (selected_non_eap) {
				da16x_eap_prt("[%s]EAP-TTLS: Both auth= and autheap= params cannot be specified\n", __func__);
				eap_ttls_deinit(sm, data);
				return NULL;
			}
			selected = "EAP";
	...
	da16x_eap_prt("[%s]EAP-TTLS: Phase2 type: %s\n", __func__, selected);
	...
	if (eap_peer_tls_ssl_init(sm, &data->ssl, config, EAP_TYPE_TTLS)) {
		da16x_eap_prt("[%s]EAP-TTLS: Failed to initialize SSL.\n", __func__);
		eap_ttls_deinit(sm, data);
		return NULL;
	}
				  ...
				  struct eap_ttls_data *data)
{
	...
	data->key_data = eap_peer_tls_derive_key(sm, &data->ssl,
						 ...
						 EAP_EMSK_LEN);
	if (!data->key_data) {
		da16x_eap_prt("[%s] EAP-TTLS: Failed to derive key\n", __func__);
		return -1;
	}

	da16x_dump("EAP-TTLS: Derived key", data->key_data, EAP_TLS_KEY_LEN);
	...
	data->session_id = eap_peer_tls_derive_session_id(sm, &data->ssl,
							  EAP_TYPE_TTLS,
	                                                  &data->id_len);
	if (data->session_id) {
		da16x_dump("EAP-TTLS: Derived Session-Id",
			  data->session_id, data->id_len);
	} else {
		da16x_eap_prt("[%s] EAP-TTLS: Failed to derive Session-Id\n",
			     __func__);
	}
					      ...
					      u8 method)
{
	...
	for (i = 0; i < data->num_phase2_eap_types; i++) {
		...
		da16x_eap_prt("[%s]EAP-TTLS: Selected "
					      ...
					      u8 method, struct wpabuf **resp)
{
	...
	if (data->ready_for_tnc && !data->tnc_started &&
	    method == EAP_TYPE_TNC) {
		da16x_eap_prt("[%s]EAP-TTLS: Start TNC after completed "
			   "EAP method\n", __func__);
		data->tnc_started = 1;
	}
	...
	if (data->tnc_started) {
		if (data->phase2_eap_type.vendor != EAP_VENDOR_IETF ||
		    data->phase2_eap_type.method == EAP_TYPE_TNC) {
			da16x_eap_prt("[%s]EAP-TTLS: Unexpected EAP "
				   "type %d for TNC\n", __func__, method);
			return -1;
		}
	...
	if (data->phase2_priv == NULL || data->phase2_method == NULL) {
		da16x_eap_prt("[%s]EAP-TTLS: failed to initialize "
				       ...
				       struct wpabuf **resp)
{
	...
	if (len <= sizeof(struct eap_hdr)) {
		da16x_eap_prt("[%s]EAP-TTLS: too short "
			   "Phase 2 request (len=%lu)\n",
			   __func__, (unsigned long) len);
		return -1;
	}
	pos = (u8 *) (hdr + 1);
	da16x_eap_prt("[%s]EAP-TTLS: Phase 2 EAP Request: type=%d\n", __func__, *pos);
	switch (*pos) {
...
{
#ifdef CONFIG_FIPS
	da16x_eap_prt("[%s]EAP-TTLS: MSCHAPV2 not supported in FIPS build\n", __func__);
	return -1;
#else /* CONFIG_FIPS */
#ifdef EAP_MSCHAPv2
	struct wpabuf *msg;
	...
	da16x_eap_prt("[%s] EAP-TTLS: Phase 2 MSCHAPV2 Request\n", __func__);
	...
	if (msg == NULL) {
		da16x_eap_prt("[%s] EAP-TTLS/MSCHAPV2: Failed to allocate "
			     "memory\n", __func__);
		return -1;
	}
	...
	if (challenge == NULL) {
		wpabuf_free(msg);
		da16x_eap_prt("[%s] EAP-TTLS/MSCHAPV2: Failed to derive "
			     "implicit challenge\n", __func__);
		return -1;
	}
	...
	*pos++ = 0; /* Flags */
	if (os_get_random(pos, EAP_TTLS_MSCHAPV2_CHALLENGE_LEN) < 0) {
		os_free(challenge);
		wpabuf_free(msg);
		da16x_eap_prt("[%s] EAP-TTLS/MSCHAPV2: Failed to get "
	...
	*resp = msg;
	...
	return 0;
#else /* EAP_MSCHAPv2 */
	da16x_eap_prt("[%s]EAP-TTLS: MSCHAPv2 not included in the build\n", __func__);
	return -1;
#endif /* EAP_MSCHAPv2 */
#endif /* CONFIG_FIPS */
}
...
{
#ifdef CONFIG_FIPS
	da16x_eap_prt("[%s]EAP-TTLS: MSCHAP not supported in FIPS build\n", __func__);
	return -1;
#else /* CONFIG_FIPS */
	struct wpabuf *msg;
	...
	da16x_eap_prt("[%s]EAP-TTLS: Phase 2 MSCHAP Request\n", __func__);
	...
	msg = wpabuf_alloc(identity_len + 1000);
	if (msg == NULL) {
		da16x_eap_prt("[%s]EAP-TTLS/MSCHAP: Failed to allocate memory\n", __func__);
		return -1;
	}
	pos = buf = wpabuf_mhead(msg);
	...
	if (challenge == NULL) {
		wpabuf_free(msg);
		da16x_eap_prt("[%s]EAP-TTLS/MSCHAP: Failed to derive "
			   "implicit challenge\n", __func__);
		return -1;
	}
	...
	*pos++ = 1; /* Flags: Use NT style passwords */
	...
	if (pwhash) {
		...
		if (challenge_response(challenge, password, pos)) {
			da16x_eap_prt("[%s]EAP-TTLS/MSCHAP: Failed derive password hash\n", __func__);
			wpabuf_free(msg);
			os_free(challenge);
			return -1;
		}
		...
		if (nt_challenge_response(challenge, password, password_len,
					  pos)) {
			da16x_eap_prt("[%s]EAP-TTLS/MSCHAP: Failed derive password\n", __func__);
			wpabuf_free(msg);
			os_free(challenge);
			return -1;
		}
				       ...
				       struct wpabuf **resp)
{
	...
	da16x_eap_prt("[%s]EAP-TTLS: Phase 2 PAP Request\n", __func__);
	...
	if (msg == NULL) {
		da16x_eap_prt("[%s]EAP-TTLS/PAP: Failed to allocate memory\n", __func__);
		return -1;
	}
	pos = buf = wpabuf_mhead(msg);
...
{
#ifdef CONFIG_FIPS
	da16x_eap_prt("[%s]EAP-TTLS: CHAP not supported in FIPS build\n", __func__);
	return -1;
#else /* CONFIG_FIPS */
	struct wpabuf *msg;
	u8 *buf, *pos, *challenge;
	...
	da16x_eap_prt("[%s]EAP-TTLS: Phase 2 CHAP Request\n", __func__);
	...
	msg = wpabuf_alloc(identity_len + 1000);
	if (msg == NULL) {
		da16x_eap_prt("[%s]EAP-TTLS/CHAP: Failed to allocate memory\n", __func__);
		return -1;
	}
	pos = buf = wpabuf_mhead(msg);
	...
	if (challenge == NULL) {
		wpabuf_free(msg);
		da16x_eap_prt("[%s]EAP-TTLS/CHAP: Failed to derive "
			   "implicit challenge\n", __func__);
		return -1;
	}
				   ...
				   struct wpabuf **resp)
{
...
#ifdef EAP_TNC
	if (data->tnc_started) {
		da16x_eap_prt("[%s]EAP-TTLS: Processing TNC\n", __func__);
	...
	if (phase2_type == EAP_TTLS_PHASE2_MSCHAPV2 ||
		...
		if (eap_get_config_identity(sm, &len) == NULL) {
			da16x_eap_prt("[%s] EAP-TTLS: Identity not configured\n",
				     __func__);
		...
		if (eap_get_config_password(sm, &len) == NULL) {
			da16x_eap_prt("[%s] EAP-TTLS: Password not configured\n",
				     __func__);
			eap_sm_request_password(sm);
			return 0;
		}
	...
	case EAP_TTLS_PHASE2_CHAP:
		...
		break;
	default:
		da16x_eap_prt("[%s] EAP-TTLS: Phase 2 - Unknown\n", __func__);
		res = -1;
		break;
	}
...
{
	da16x_eap_prt("[%s] EAP-TTLS: AVP - EAP Message\n", __func__);
	if (parse->eapdata == NULL) {
		parse->eapdata = os_memdup(dpos, dlen);
		if (parse->eapdata == NULL) {
			da16x_eap_prt("[%s] EAP-TTLS: Failed to allocate "
			      ...
			      struct ttls_parse_avp *parse)
{
	...
	avp_length &= 0xffffff;
	da16x_eap_prt("[%s] EAP-TTLS: AVP: code=%d flags=0x%02x "
		     "length=%d\n", __func__, (int) avp_code, avp_flags,
		     (int) avp_length);

	if (avp_length > left) {
		da16x_eap_prt("[%s] EAP-TTLS: AVP overflow "
	...
	if (avp_length < sizeof(*avp)) {
		da16x_eap_prt("[%s] EAP-TTLS: Invalid AVP length %d\n", __func__,
			     avp_length);
		return -1;
	}
	...
	if (avp_flags & AVP_FLAGS_VENDOR) {
		if (dlen < 4) {
			da16x_eap_prt("[%s] EAP-TTLS: Vendor AVP "
				     "underflow\n", __func__);
			return -1;
		}
		vendor_id = WPA_GET_BE32(dpos);
		da16x_eap_prt("[%s] EAP-TTLS: AVP vendor_id %d\n", __func__,
			     (int) vendor_id);
		dpos += 4;
		dlen -= 4;
	}

	da16x_dump("EAP-TTLS: AVP data", dpos, dlen);
	...
	if (vendor_id == 0 && avp_code == RADIUS_ATTR_EAP_MESSAGE) {
		 ...
		 * the user. */
		da16x_eap_prt("[%s] EAP-TTLS: AVP - Reply-Message\n", __func__,
			     dpos, dlen);
	} else if (vendor_id == RADIUS_VENDOR_ID_MICROSOFT &&
		   avp_code == RADIUS_ATTR_MS_CHAP2_SUCCESS) {
		da16x_eap_prt("[%s] EAP-TTLS: MS-CHAP2-Success\n", __func__,
			     dpos, dlen);
		if (dlen != 43) {
			da16x_eap_prt("[%s] EAP-TTLS: Unexpected "
				     "MS-CHAP2-Success length "
					     "(len=%lu, expected 43)\n",
					     __func__, (unsigned long) dlen);
			return -1;
		}
		parse->mschapv2 = dpos;
	} else if (vendor_id == RADIUS_VENDOR_ID_MICROSOFT &&
		   avp_code == RADIUS_ATTR_MS_CHAP_ERROR) {
		da16x_eap_prt("[%s] EAP-TTLS: MS-CHAP-Error\n", __func__,
			     dpos, dlen);
		parse->mschapv2_error = 1;
	} else if (avp_flags & AVP_FLAGS_MANDATORY) {
		da16x_eap_prt("[%s] EAP-TTLS: Unsupported mandatory AVP "
			     "code %d vendor_id %d - dropped\n", __func__,
			     (int) avp_code, (int) vendor_id);
		return -1;
	} else {
		da16x_eap_prt("[%s] EAP-TTLS: Ignoring unsupported AVP "
			     "code %d vendor_id %d\n", __func__,
			     (int) avp_code, (int) vendor_id);
	}
			       ...
			       struct ttls_parse_avp *parse)
{
	...
	da16x_dump("EAP-TTLS: Decrypted Phase 2 AVPs", pos, left);
	if (left < sizeof(struct ttls_avp)) {
		da16x_eap_prt("[%s] EAP-TTLS: Too short Phase 2 AVP frame"
			     " len=%lu expected %lu or more - dropped\n",
			     __func__, (unsigned long) left,
...
static u8 * eap_ttls_fake_identity_request(void)
{
	...
	da16x_eap_prt("[%s] EAP-TTLS: empty data in beginning of Phase 2 - "
		     "use fake EAP-Request Identity\n", __func__);
	buf = os_malloc(sizeof(*hdr) + 1);
	if (buf == NULL) {
		da16x_eap_prt("[%s] EAP-TTLS: failed to allocate memory for "
			     "fake EAP-Identity Request\n", __func__);
		return NULL;
	}
				     ...
				     struct wpabuf **out_data)
{
	...
	if (eap_peer_tls_encrypt(sm, &data->ssl, EAP_TYPE_TTLS,
				 ...
				 resp, out_data)) {
		da16x_eap_prt("[%s] EAP-TTLS: Failed to encrypt a Phase 2 "
			     "frame\n", __func__);
		return -1;
	}
	wpabuf_free(resp);
				       ...
				       struct wpabuf **resp)
{
	...
	if (parse->eapdata == NULL) {
		da16x_eap_prt("[%s]EAP-TTLS: No EAP Message in the "
			   "packet - dropped\n", __func__);
		return -1;
	}
	...
	if (parse->eap_len < sizeof(*hdr)) {
		da16x_eap_prt("[%s]EAP-TTLS: Too short Phase 2 EAP "
	...
	if (len > parse->eap_len) {
		da16x_eap_prt("[%s]EAP-TTLS: Length mismatch in Phase 2 "
	...
	da16x_eap_prt("[%s]EAP-TTLS: received Phase 2: code=%d "
		   "identifier=%d length=%lu\n", __func__,
		   hdr->code, hdr->identifier, (unsigned long) len);
	switch (hdr->code) {
	case EAP_CODE_REQUEST:
		if (eap_ttls_phase2_request(sm, data, ret, hdr, resp)) {
			da16x_eap_prt("[%s]EAP-TTLS: Phase2 Request "
				   "processing failed\n", __func__);
			return -1;
		}
		break;
	default:
		da16x_eap_prt("[%s]EAP-TTLS: Unexpected code=%d in "
			   "Phase 2 EAP header\n", __func__, hdr->code);
		return -1;
	}
					    ...
					    struct ttls_parse_avp *parse)
{
	...
	if (parse->mschapv2_error) {
		da16x_eap_prt("[%s] EAP-TTLS/MSCHAPV2: Received "
...
#endif /* EAP_TNC */
		da16x_eap_prt("[%s] EAP-TTLS: no MS-CHAP2-Success AVP "
			     "received for Phase2 MSCHAPV2\n", __func__);
		return -1;
	}
	if (parse->mschapv2[0] != data->ident) {
		da16x_eap_prt("[%s] EAP-TTLS: Ident mismatch for Phase 2 "
			     "MSCHAPV2 (received Ident 0x%02x, "
				     "expected 0x%02x)\n", __func__,
	...
	if (!data->auth_response_valid ||
	    mschapv2_verify_auth_response(data->auth_response,
					  parse->mschapv2 + 1, 42)) {
		da16x_eap_prt("[%s] EAP-TTLS: Invalid authenticator response in "
			     "Phase 2 MSCHAPV2 success request\n", __func__);
		return -1;
	}

	da16x_eap_prt("[%s] EAP-TTLS: Phase 2 MSCHAPV2 "
				      ...
				      struct wpabuf **resp)
{
	...
	if (parse->eapdata == NULL) {
		da16x_eap_prt("[%s] EAP-TTLS: Phase 2 received "
			     "unexpected tunneled data (no EAP)\n", __func__);
		return -1;
	}

	if (!data->ready_for_tnc) {
		da16x_eap_prt("[%s] EAP-TTLS: Phase 2 received EAP after "
			     "non-EAP, but not ready for TNC\n", __func__);
		return -1;
	}
				      ...
				      struct wpabuf **out_data)
{
	...
	case EAP_TTLS_PHASE2_CHAP:
		...
		da16x_eap_prt("[%s]EAP-TTLS: Phase 2 received unexpected "
			   "tunneled data\n", __func__);
		return -1;
#endif /* EAP_TNC */
	}
				 ...
				 struct wpabuf **out_data)
{
	...
	if (data->reauth &&
		...
		da16x_eap_prt("[%s] EAP-TTLS: Session resumption - "
			     "skip phase 2\n", __func__);
			    ...
			    struct wpabuf **out_data)
{
	...
	da16x_eap_prt("[%s] EAP-TTLS: received %lu bytes encrypted data for"
		     " Phase 2\n", __func__,
		     in_data ? (unsigned long) wpabuf_len(in_data) : 0);

	if (data->pending_phase2_req) {
		da16x_eap_prt("[%s] EAP-TTLS: Pending Phase 2 request - "
				      ...
				      struct wpabuf **out_data)
{
	...
	if (sm->waiting_ext_cert_check && data->pending_resp) {
		...
		if (config->pending_ext_cert_check == EXT_CERT_CHECK_GOOD) {
			da16x_eap_prt("[%s]EAP-TTLS: External certificate check succeeded - continue handshake\n", __func__);
		...
		if (config->pending_ext_cert_check == EXT_CERT_CHECK_BAD) {
			da16x_eap_prt("[%s]EAP-TTLS: External certificate check failed - force authentication failure\n", __func__);
			ret->methodState = METHOD_DONE;
			ret->decision = DECISION_FAIL;
			sm->waiting_ext_cert_check = 0;
			return 0;
		...
		da16x_eap_prt("[%s]EAP-TTLS: Continuing to wait external server certificate validation\n", __func__);
		return 0;
	}
	...
	res = eap_peer_tls_process_helper(sm, &data->ssl, EAP_TYPE_TTLS,
					  ...
					  in_data, out_data);
	if (res < 0) {
		da16x_eap_prt("[%s]EAP-TTLS: TLS processing failed\n", __func__);
		ret->methodState = METHOD_DONE;
		ret->decision = DECISION_FAIL;
		return -1;
	}
	...
	if (sm->waiting_ext_cert_check) {
		da16x_eap_prt("[%s]EAP-TTLS: Waiting external server certificate validation\n", __func__);
		wpabuf_free(data->pending_resp);
	...
	if (tls_connection_established(sm->ssl_ctx, data->ssl.conn)) {
		da16x_eap_prt("[%s] EAP-TTLS: TLS done, proceed to "
			     "Phase 2\n", __func__);
		if (data->resuming) {
			da16x_eap_prt("[%s] EAP-TTLS: fast reauth - may "
				     "skip Phase 2\n", __func__);
			ret->decision = DECISION_COND_SUCC;
			ret->methodState = METHOD_MAY_CONT;
		}
				       ...
				       struct eap_method_ret *ret)
{
	if (ret->methodState == METHOD_DONE) {
		...
		if (ret->decision == DECISION_UNCOND_SUCC ||
		    ret->decision == DECISION_COND_SUCC) {
			da16x_eap_prt("[%s] EAP-TTLS: Authentication "
		   ...
		   (ret->decision == DECISION_UNCOND_SUCC ||
		    ret->decision == DECISION_COND_SUCC)) {
			da16x_eap_prt("[%s] EAP-TTLS: Authentication completed "
				     "successfully (MAY_CONT)\n", __func__);
			data->phase2_success = 1;
			data->decision_succ = ret->decision;
		 ...
		 */
		da16x_eap_prt("[%s]EAP-TTLS: Restore success decision after fragmented frame sent completely\n", __func__);
					...
					const struct wpabuf *reqData)
{
	...
	if (flags & EAP_TLS_FLAGS_START) {
		da16x_eap_prt("[%s] EAP-TTLS: Start (server ver=%d, own "
			     "ver=%d)\n", __func__,
