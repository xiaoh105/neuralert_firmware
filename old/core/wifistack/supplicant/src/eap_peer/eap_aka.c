...
{
	wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: %s -> %s",
...
static int eap_aka_ext_sim_req(struct eap_sm *sm, struct eap_aka_data *data)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: Use external USIM processing");
	pos = req;
	end = pos + sizeof(req);
				  ...
				  struct eap_peer_config *conf)
{
	...
	if (os_strncmp(resp, "UMTS-AUTH:", 10) != 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: Unrecognized external USIM processing response");
		os_free(resp);
		return -1;
	}
...
invalid:
	wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: Invalid external USIM processing UMTS-AUTH response");
	os_free(resp);
	return -1;
}
...
static int eap_aka_umts_auth(struct eap_sm *sm, struct eap_aka_data *data)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: UMTS authentication algorithm");
	...
	if (conf->password) {
		...
		const char *pos;
		wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: Use internal Milenage "
			   "implementation for UMTS authentication");
		if (conf->password_len < 78) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: invalid Milenage "
				   "password");
			return -1;
		}
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: Use hardcoded Kc and SRES values for "
		   "testing");
	...
	os_memset(data->ck, '4', EAP_AKA_CK_LEN);
	{
		...
		if (os_memcmp_const(autn, data->autn, EAP_AKA_AUTN_LEN) != 0) {
			wpa_printf_dbg(MSG_WARNING, "EAP-AKA: AUTN did not match "
				   "with expected value");
			return -1;
		}
	}
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: No UMTS authentication algorithm "
		   "enabled");
	return -1;
				     ...
				     struct eap_aka_data *data, int id)
{
	if ((id & CLEAR_PSEUDONYM) && data->pseudonym) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: forgetting old pseudonym");
		os_free(data->pseudonym);
		data->pseudonym = NULL;
	...
	if ((id & CLEAR_REAUTH_ID) && data->reauth_id) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: forgetting old reauth_id");
		os_free(data->reauth_id);
		data->reauth_id = NULL;
		data->reauth_id_len = 0;
	}
	if ((id & CLEAR_EAP_ID) && data->last_eap_identity) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: forgetting old eap_id");
		os_free(data->last_eap_identity);
			     ...
			     struct eap_sim_attrs *attr)
{
	if (attr->next_pseudonym) {
		...
		if (data->pseudonym == NULL) {
			wpa_printf_dbg(MSG_INFO, "EAP-AKA: (encr) No memory for "
				   "next pseudonym");
			data->pseudonym_len = 0;
			return -1;
		}
				  ...
				  struct eap_sim_msg *msg)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "   AT_CHECKCODE");
				    ...
				    const u8 *checkcode, size_t checkcode_len)
{
	...
	if (data->id_msgs == NULL) {
		if (checkcode_len != 0) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: Checkcode from server "
	...
	if (os_memcmp_const(hash, checkcode, hash_len) != 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: Mismatch in AT_CHECKCODE");
		return -1;
	}
					    ...
					    int err)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: Send Client-Error (error code %d)",
		   err);
						     ...
						     u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Generating EAP-AKA Authentication-Reject "
		   "(id=%d)", id);
	msg = eap_sim_msg_init(EAP_CODE_RESPONSE, id, data->eap_method,
	...
	struct eap_aka_data *data, u8 id, struct eap_sim_attrs *attr)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Generating EAP-AKA Synchronization-Failure "
		   "(id=%d)", id);
	msg = eap_sim_msg_init(EAP_CODE_RESPONSE, id, data->eap_method,
			       EAP_AKA_SUBTYPE_SYNCHRONIZATION_FAILURE);
	wpa_printf_dbg(MSG_DEBUG, "   AT_AUTS");
	...
	if (data->eap_method == EAP_TYPE_AKA_PRIME) {
		...
		for (i = 0; i < attr->kdf_count; i++) {
			wpa_printf_dbg(MSG_DEBUG, "   AT_KDF");
						 ...
						 enum eap_sim_id_req id_req)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Generating EAP-AKA Identity (id=%d)", id);
	msg = eap_sim_msg_init(EAP_CODE_RESPONSE, id, data->eap_method,
						  ...
						  u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Generating EAP-AKA Challenge (id=%d)", id);
	msg = eap_sim_msg_init(EAP_CODE_RESPONSE, id, data->eap_method,
			       EAP_AKA_SUBTYPE_CHALLENGE);
	wpa_printf_dbg(MSG_DEBUG, "   AT_RES");
	eap_sim_msg_add(msg, EAP_SIM_AT_RES, data->res_len * 8,
			data->res, data->res_len);
	eap_aka_add_checkcode(data, msg);
	if (data->use_result_ind) {
		wpa_printf_dbg(MSG_DEBUG, "   AT_RESULT_IND");
		eap_sim_msg_add(msg, EAP_SIM_AT_RESULT_IND, 0, NULL, 0);
	}
	wpa_printf_dbg(MSG_DEBUG, "   AT_MAC");
					       ...
					       const u8 *nonce_s)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Generating EAP-AKA Reauthentication (id=%d)",
		   id);
	msg = eap_sim_msg_init(EAP_CODE_RESPONSE, id, data->eap_method,
			       EAP_AKA_SUBTYPE_REAUTHENTICATION);
	wpa_printf_dbg(MSG_DEBUG, "   AT_IV");
	wpa_printf_dbg(MSG_DEBUG, "   AT_ENCR_DATA");
	eap_sim_msg_add_encr_start(msg, EAP_SIM_AT_IV, EAP_SIM_AT_ENCR_DATA);

	if (counter_too_small) {
		wpa_printf_dbg(MSG_DEBUG, "   *AT_COUNTER_TOO_SMALL");
		eap_sim_msg_add(msg, EAP_SIM_AT_COUNTER_TOO_SMALL, 0, NULL, 0);
		counter = data->counter_too_small;
	} else
		counter = data->counter;

	wpa_printf_dbg(MSG_DEBUG, "   *AT_COUNTER %d", counter);
	eap_sim_msg_add(msg, EAP_SIM_AT_COUNTER, counter, NULL, 0);

	if (eap_sim_msg_add_encr_end(msg, data->k_encr, EAP_SIM_AT_PADDING)) {
		wpa_printf_dbg(MSG_WARNING, "EAP-AKA: Failed to encrypt "
			   "AT_ENCR_DATA");
						     ...
						     u8 id, u16 notification)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Generating EAP-AKA Notification (id=%d)", id);
	msg = eap_sim_msg_init(EAP_CODE_RESPONSE, id, data->eap_method,
	...
	if (k_aut && data->reauth) {
		...
		wpa_printf_dbg(MSG_DEBUG, "   *AT_COUNTER %d", data->counter);
						...
						struct eap_sim_attrs *attr)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: subtype Identity");
	...
	if (id_error) {
		wpa_printf_dbg(MSG_INFO, "EAP-AKA: Too many ID requests "
						...
						u8 id, u16 kdf)
{
	...
	data->kdf = kdf;
	wpa_printf_dbg(MSG_DEBUG, "Generating EAP-AKA Challenge (id=%d) (KDF "
		   "select)", id);
	msg = eap_sim_msg_init(EAP_CODE_RESPONSE, id, data->eap_method,
			       EAP_AKA_SUBTYPE_CHALLENGE);
	wpa_printf_dbg(MSG_DEBUG, "   AT_KDF");
				   ...
				   struct eap_sim_attrs *attr)
{
	...
	if (data->kdf_negotiation) {
		...
		if (attr->kdf[0] != data->kdf) {
			wpa_printf_dbg(MSG_WARNING, "EAP-AKA': The server did not "
	...
	for (i = data->kdf ? 1 : 0; i < attr->kdf_count; i++) {
		for (j = i + 1; j < attr->kdf_count; j++) {
			if (attr->kdf[i] == attr->kdf[j]) {
				wpa_printf_dbg(MSG_WARNING, "EAP-AKA': The server "
						 ...
						 struct eap_sim_attrs *attr)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: subtype Challenge");

	if (attr->checkcode &&
	    eap_aka_verify_checkcode(data, attr->checkcode,
				     attr->checkcode_len)) {
		wpa_printf_dbg(MSG_WARNING, "EAP-AKA: Invalid AT_CHECKCODE in the "
			   "message");
		return eap_aka_client_error(data, id,
					    EAP_AKA_UNABLE_TO_PROCESS_PACKET);
	}
	...
	if (data->eap_method == EAP_TYPE_AKA_PRIME) {
		if (!attr->kdf_input || attr->kdf_input_len == 0) {
			wpa_printf_dbg(MSG_WARNING, "EAP-AKA': Challenge message "
		...
		if (data->network_name == NULL) {
			wpa_printf_dbg(MSG_WARNING, "EAP-AKA': No memory for "
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-AKA': KDF %d selected", data->kdf);
	}
	...
	if (data->eap_method == EAP_TYPE_AKA && attr->bidding) {
		...
		if ((flags & EAP_AKA_BIDDING_FLAG_D) &&
		    eap_allowed_method(sm, EAP_VENDOR_IETF,
				       EAP_TYPE_AKA_PRIME)) {
			wpa_printf_dbg(MSG_WARNING, "EAP-AKA: Bidding down from "
				   "AKA' to AKA detected");
			/* Fail authentication as if AUTN had been incorrect */
			return eap_aka_authentication_reject(data, id);
		}
	...
	if (res == -1) {
		wpa_printf_dbg(MSG_WARNING, "EAP-AKA: UMTS authentication "
			   "failed (AUTN)");
		return eap_aka_authentication_reject(data, id);
	} else if (res == -2) {
		wpa_printf_dbg(MSG_WARNING, "EAP-AKA: UMTS authentication "
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: Wait for external USIM processing");
		return NULL;
	} else if (res) {
		wpa_printf_dbg(MSG_WARNING, "EAP-AKA: UMTS authentication failed");
		return eap_aka_client_error(data, id,
					    EAP_AKA_UNABLE_TO_PROCESS_PACKET);
	}
#ifdef EAP_AKA_PRIME
	if (data->eap_method == EAP_TYPE_AKA_PRIME) {
		...
		if (!(amf & 0x8000)) {
			wpa_printf_dbg(MSG_WARNING, "EAP-AKA': AMF separation bit "
					       ...
					       struct eap_sim_attrs *attr)
{
	...
	if (attr->encr_data == NULL || attr->iv == NULL) {
		wpa_printf_dbg(MSG_WARNING, "EAP-AKA: Notification message after "
	...
	decrypted = eap_sim_parse_encr(data->k_encr, attr->encr_data,
				       ...
				       0);
	if (decrypted == NULL) {
		wpa_printf_dbg(MSG_WARNING, "EAP-AKA: Failed to parse encrypted "
			   "data from notification message");
		return -1;
	}

	if (eattr.counter < 0 || (size_t) eattr.counter != data->counter) {
		wpa_printf_dbg(MSG_WARNING, "EAP-AKA: Counter in notification "
...
{
	if (attr->mac == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-AKA: no AT_MAC in after_auth "
			   "Notification message");
		return -1;
	}

	if (eap_aka_verify_mac(data, reqData, attr->mac, (u8 *) "", 0)) {
		wpa_printf_dbg(MSG_WARNING, "EAP-AKA: Notification message "
			   "used invalid AT_MAC");
		return -1;
	}
	...
	if (data->reauth &&
		...
		wpa_printf_dbg(MSG_WARNING, "EAP-AKA: Invalid notification "
...
{
	wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: subtype Notification");
	if (data->num_notification > 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-AKA: too many notification "
	...
	if (attr->notification == -1) {
		wpa_printf_dbg(MSG_INFO, "EAP-AKA: no AT_NOTIFICATION in "
			   "Notification message");
		return eap_aka_client_error(data, id,
					    EAP_AKA_UNABLE_TO_PROCESS_PACKET);
	}
	...
	const struct wpabuf *reqData, struct eap_sim_attrs *attr)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: subtype Reauthentication");
	...
	if (data->reauth_id == NULL) {
		wpa_printf_dbg(MSG_WARNING, "EAP-AKA: Server is trying "
	...
	data->reauth = 1;
	if (eap_aka_verify_mac(data, reqData, attr->mac, (u8 *) "", 0)) {
		wpa_printf_dbg(MSG_WARNING, "EAP-AKA: Reauthentication "
	...
	if (eattr.nonce_s == NULL || eattr.counter < 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-AKA: (encr) No%s%s in reauth packet",
	...
	if (eattr.counter < 0 || (size_t) eattr.counter <= data->counter) {
		struct wpabuf *res;
		wpa_printf_dbg(MSG_INFO, "EAP-AKA: (encr) Invalid counter "
	...
	if (data->counter > EAP_AKA_MAX_FAST_REAUTHS) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: Maximum number of "
				       ...
				       const struct wpabuf *reqData)
{
	...
	if (eap_get_config_identity(sm, &len) == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-AKA: Identity not configured");
	...
	subtype = *pos++;
	wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: Subtype=%d", subtype);
	pos += 2; /* Reserved */
	...
	case EAP_AKA_SUBTYPE_CLIENT_ERROR:
		wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: subtype Client-Error");
		res = eap_aka_client_error(data, id,
					   EAP_AKA_UNABLE_TO_PROCESS_PACKET);
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: Unknown subtype=%d", subtype);
		res = eap_aka_client_error(data, id,
					   EAP_AKA_UNABLE_TO_PROCESS_PACKET);
		break;
	}
