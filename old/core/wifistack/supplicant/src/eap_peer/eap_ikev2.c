...
static void * eap_ikev2_init(struct eap_sm *sm)
{
	...
	if (identity == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-IKEV2: No identity available");
		return NULL;
	}
					   ...
					   struct eap_method_ret *ret, u8 id)
{
	...
	ret->ignore = FALSE;
	wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Generating Response");
	...
	if (data->out_used == wpabuf_len(data->out_buf)) {
		...
		case IKEV2_DONE:
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Authentication "
				 ...
				 int frag_ack)
{
	if (flags & IKEV2_FLAGS_ICV_INCLUDED) {
		...
		*end -= icv_len;
		...
		wpa_printf_dbg(MSG_INFO, "EAP-IKEV2: The message should have "
				  ...
				  const u8 *buf, size_t len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Received %lu bytes, waiting "
					 ...
					 const struct wpabuf *reqData)
{
	...
	if (data->out_buf == NULL) {
		...
		if (data->out_buf == NULL) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Failed to generate "
				   "IKEv2 message");
			return NULL;
		}
		data->out_used = 0;
