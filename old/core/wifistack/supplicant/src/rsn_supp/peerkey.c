					 ...
					 u16 mui, u16 error_type, int ver)
{
	...
	if (peer) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: Sending EAPOL-Key SMK Error (peer "
			   MACSTR " mui %d error_type %d)",
			   MAC2STR(peer), mui, error_type);
	} else {
		wpa_printf_dbg(MSG_DEBUG, "RSN: Sending EAPOL-Key SMK Error "
			   "(mui %d error_type %d)", mui, error_type);
	}

	wpa_eapol_key_send(sm, sm->ptk.kck, ver, dst, ETH_P_EAPOL,
			   rbuf, rlen, err->key_mic, 0);
				      ...
				      int ver, struct wpa_peerkey *peerkey)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "RSN: Sending EAPOL-Key SMK M3");
	...
	const struct wpa_eapol_key *key, size_t extra_len, int ver)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "RSN: Received SMK M2");
	...
	if (wpa_supp_parse_ies((const u8 *) (key + 1), extra_len, &kde) < 0) {
	...
	wpa_printf_dbg(MSG_DEBUG, "RSN: SMK M2 - SMK initiator " MACSTR,
	...
	wpa_printf_dbg(MSG_DEBUG, "RSN: Using %s for PeerKey",
					   ...
					   struct wpa_peerkey *peerkey)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "RSN: Sending EAPOL-Key STK 1/4 to " MACSTR,
					   ...
					   struct wpa_peerkey *peerkey)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "RSN: Sending EAPOL-Key STK 3/4 to " MACSTR,
		   MAC2STR(peerkey->addr));
	wpa_eapol_key_send(sm, peerkey->stk.kck, ver, peerkey->addr,
			   ETH_P_EAPOL, mbuf, mlen, msg->key_mic, 0);
}
...
static int wpa_supplicant_process_smk_m4(struct wpa_peerkey *peerkey,
					 struct wpa_eapol_ie_parse *kde)
{
	wpa_printf_dbg(MSG_DEBUG, "RSN: Received SMK M4 (Initiator " MACSTR ")",
		   MAC2STR(kde->mac_addr));
					 ...
					 struct wpa_eapol_ie_parse *kde)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "RSN: Received SMK M5 (Peer " MACSTR ")",
		   MAC2STR(kde->mac_addr));
	...
	const struct wpa_eapol_key *key, size_t extra_len, int ver)
{
	...
	if (!sm->peerkey_enabled || sm->proto != WPA_PROTO_RSN) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: SMK handshake not allowed for "
			   "the current network");
		return -1;
	}
	...
	wpa_printf_dbg(MSG_DEBUG, "RSN: SMK lifetime %u seconds", lifetime);
	...
	const struct wpa_eapol_key *key, size_t extra_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "RSN: Received SMK Error");
	...
	if (kde.mac_addr &&
		...
		if (peerkey == NULL) {
			wpa_printf_dbg(MSG_DEBUG, "RSN: No matching SMK handshake "
				   "found for SMK Error");
			return -1;
		}
					      ...
					      u16 ver)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "RSN: RX message 1 of STK 4-Way Handshake from "
	...
	if (wpa_supp_parse_ies(kde, len, &ie) < 0 || ie.pmkid == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: No SMKID in STK 1/4");
		return;
	}
					       ...
					       struct wpa_eapol_ie_parse *kde)
{
	...
	if (lifetime >= peerkey->lifetime) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: Peer used SMK lifetime %u seconds "
			   "which is larger than or equal to own value %u "
			   "seconds - ignored", lifetime, peerkey->lifetime);
		return;
	}

	wpa_printf_dbg(MSG_DEBUG, "RSN: Peer used shorter SMK lifetime %u seconds "
					      ...
					      u16 ver)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "RSN: RX message 2 of STK 4-Way Handshake from "
		   MACSTR " (ver=%d)", MAC2STR(peerkey->addr), ver);
	...
	if (wpa_supp_parse_ies(keydata, len, &kde) < 0 ||
	    kde.pmkid == NULL || kde.rsn_ie == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: No SMKID or RSN IE in STK 2/4");
		return;
	}
					      ...
					      u16 ver)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "RSN: RX message 3 of STK 4-Way Handshake from "
		   MACSTR " (ver=%d)", MAC2STR(peerkey->addr), ver);
	...
	if (wpa_supp_parse_ies(keydata, len, &kde) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: Failed to parse key data in "
			   "STK 3/4");
		return;
	}
					      ...
					      u16 ver)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "RSN: RX message 4 of STK 4-Way Handshake from "
		   MACSTR " (ver=%d)", MAC2STR(peerkey->addr), ver);
...
int wpa_sm_stkstart(struct wpa_sm *sm, const u8 *peer)
{
	...
	if (sm->ap_rsn_ie &&
		...
		wpa_printf_dbg(MSG_DEBUG, "RSN: Current AP does not support STK");
		return -1;
	}
