...
static void pmksa_cache_expire(void *eloop_ctx, void *timeout_ctx)
{
	...
	while (pmksa->pmksa && pmksa->pmksa->expiration <= now.sec) {
		...
		da16x_info_prt("[%s] RSN: expired PMKSA cache entry for "
			   MACSTR "\n", __func__, MAC2STR(entry->aa));
...
static void pmksa_cache_set_expiration(struct rsn_pmksa_cache *pmksa)
{
	...
	da16x_eloop_cancel_timeout(pmksa_cache_expire, pmksa, NULL);
	da16x_eloop_cancel_timeout(pmksa_cache_reauth, pmksa, NULL);
	...
	if (sec < 0)
		sec = 0;
	da16x_eloop_register_timeout(sec + 1, 0, pmksa_cache_expire, pmksa, NULL);
	...
	if (entry) {
		...
		if (sec < 0)
			sec = 0;
		da16x_eloop_register_timeout(sec, 0, pmksa_cache_reauth, pmksa,
				       NULL);
	}
}
		      ...
		      struct rsn_pmksa_cache_entry *entry)
{
		...
		if (os_memcmp(entry->aa, pos->aa, ETH_ALEN) == 0) {
			if (pos->pmk_len == entry->pmk_len &&
			    ...
			    os_memcmp_const(pos->pmkid, entry->pmkid,
					    PMKID_LEN) == 0) {
				wpa_printf_dbg(MSG_DEBUG, "WPA: reusing previous "
					   "PMKSA entry");
				os_free(entry);
				return pos;
			}
			 ...
			 */
			wpa_printf_dbg(MSG_DEBUG, "RSN: Replace PMKSA entry for "
	...
	if (pmksa->pmksa_count >= pmksa_cache_max_entries && pmksa->pmksa) {
		...
		if (pos) {
			wpa_printf_dbg(MSG_DEBUG, "RSN: removed the oldest idle "
	...
	pmksa->pmksa_count++;
	wpa_printf_dbg(MSG_DEBUG, "RSN: Added PMKSA cache entry for " MACSTR
		...
		const u8 *aa, const u8 *spa, void *network_ctx, int akmp)
{
	...
	while (pos) {
		if (os_memcmp(aa, pos->aa, ETH_ALEN) == 0) {
			if (pos->pmk_len == pmk_len &&
			    ...
			    os_memcmp(pos->pmkid, entry->pmkid, PMKID_LEN) == 0)
			{
				da16x_p2p_prt("[%s] WPA: reusing previous "
					   "PMKSA entry\n", __func__);
				os_free(entry);
				return pos;
			}
			 ...
			 * been cleared in wpa_supplicant_key_neg_complete().
			 */
			da16x_p2p_prt("[%s] RSN: Replace PMKSA entry for "
				   "the current AP and any PMKSA cache entry "
				   "that was based on the old PMK\n", __func__);
	...
	if (pmksa->pmksa_count >= pmksa_cache_max_entries && pmksa->pmksa) {
		...
		if (pos) {
			da16x_p2p_prt("[%s] RSN: removed the oldest idle "
				   "PMKSA cache entry (for " MACSTR ") to "
				   "make room for new one\n",
				   __func__, MAC2STR(pos->aa));
	...
	da16x_p2p_prt("[%s] RSN: Added PMKSA cache entry for " MACSTR
		   " network_ctx=%p\n",
			__func__, MAC2STR(entry->aa), network_ctx);
		       ...
		       const u8 *pmk, size_t pmk_len)
{
	...
	while (entry) {
		if ((entry->network_ctx == network_ctx ||
			...
			da16x_info_prt("[%s] RSN: Flush PMKSA cache entry "
				   "for " MACSTR "\n",
				__func__, MAC2STR(entry->aa));
			      ...
			      const u8 *aa, int akmp)
{
	...
	da16x_p2p_prt("[%s] RSN: Consider " MACSTR " for OKC\n",
			__func__, MAC2STR(aa));
...
int pmksa_cache_set_current(struct wpa_sm *sm, const u8 *pmkid,
			    ...
			    const u8 *fils_cache_id, int akmp)
{
	struct rsn_pmksa_cache *pmksa = sm->pmksa;
	TX_FUNC_START("        ");

	da16x_wpa_prt("        [%s] PMKSA cache search - network_ctx=%p "
		   "try_opportunistic=%d akmp=0x%x\n",
		   __func__, network_ctx, try_opportunistic, akmp);

	if (pmkid)
		da16x_dump("RSN: Search for PMKID", pmkid, PMKID_LEN);

	if (bssid)
		da16x_wpa_prt("        [%s] Search for BSSID " MACSTR "\n",
			   __func__, MAC2STR(bssid));
	if (fils_cache_id)
		da16x_wpa_prt("        [%s] Search for FILS Cache Identifier %02x%02x\n",
			   __func__, fils_cache_id[0], fils_cache_id[1]);
	...
	if (sm->cur_pmksa == NULL && fils_cache_id)
		sm->cur_pmksa = pmksa_cache_get_fils_cache_id(pmksa,
							      ...
							      fils_cache_id);
	if (sm->cur_pmksa) {
		da16x_dump("RSN: PMKSA cache entry found - PMKID",
			    sm->cur_pmksa->pmkid, PMKID_LEN);
		return 0;
	}

	da16x_wpa_prt("        [%s] No PMKSA cache entry found\n", __func__);
			    ...
			    int akmp)
{
	...
	da16x_wpa_prt("        [%s] PMKSA cache search - network_ctx=%p "
		   "try_opportunistic=%d\n",
			__func__, network_ctx, try_opportunistic);
	if (pmkid)
		da16x_dump("RSN: Search for PMKID", pmkid, PMKID_LEN);
