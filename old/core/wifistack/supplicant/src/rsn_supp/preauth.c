				...
				const u8 *buf, size_t len)
{
	...
	da16x_prt("<%s> RX pre-auth from " MACSTR "\n",
				__func__, MAC2STR(src_addr));
	da16x_dump("RX pre-auth", buf, len);

	if (sm->preauth_eapol == NULL ||
	    is_zero_ether_addr(sm->preauth_bssid) ||
	    os_memcmp(sm->preauth_bssid, src_addr, ETH_ALEN) != 0) {
		da16x_prt("<%s> RSN pre-auth frame received from "
			   "unexpected source " MACSTR " - dropped", __func__, 
			   MAC2STR(src_addr));
		return;
	}
				 ...
				 void *ctx)
{
	...
	if (result == EAPOL_SUPP_RESULT_SUCCESS) {
		...
		if (res == 0) {
			da16x_dump("RSN: PMK from pre-auth", pmk, pmk_len);
			...
			da16x_prt("<%s> RSN: failed to get master session key from "
				"pre-auth EAPOL state machines", __func__);
			result = EAPOL_SUPP_RESULT_FAILURE;
		}
	}

	da16x_prt("<%s> RSN: pre-authentication with " MACSTR " %s", 
		 __func__, MAC2STR(sm->preauth_bssid),
...
static void rsn_preauth_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	da16x_prt("<%s> RSN: pre-authentication with " MACSTR " timed out", 
		 __func__, MAC2STR(sm->preauth_bssid));
	rsn_preauth_deinit(sm);
	rsn_preauth_candidate_process(sm);
}
				  ...
				  size_t len)
{
	...
	da16x_dump("TX EAPOL (preauth)", msg, msglen);
	res = l2_packet_send(sm->l2_preauth, sm->preauth_bssid,
			     ETH_P_RSN_PREAUTH, msg, msglen, 0);
	os_free(msg);
	return res;
}
		     ...
		     struct eap_peer_config *eap_conf)
{
	...
	da16x_prt("<%s> RSN: starting pre-authentication with " MACSTR, 
	...
	if (sm->l2_preauth == NULL) {
		da16x_prt("<%s> RSN: Failed to initialize L2 packet "
			   "processing for pre-authentication", __func__);
		return -2;
	}
	...
	if (sm->bridge_ifname) {
		...
		if (sm->l2_preauth_br == NULL) {
			da16x_prt("<%s> RSN: Failed to initialize L2 "
				   "packet processing (bridge) for "
				   "pre-authentication", __func__);
			return -2;
		}
	}
	...
	if (ctx == NULL) {
		da16x_prt("<%s> Failed to allocate EAPOL context.", __func__);
		return -4;
	}
	ctx->ctx = sm->ctx->ctx;
	...
	if (sm->preauth_eapol == NULL) {
		os_free(ctx);
		da16x_prt("<%s> RSN: Failed to initialize EAPOL "
			   "state machines for pre-authentication", __func__);
		return -3;
	}
	...
	da16x_eloop_register_timeout(sm->dot11RSNAConfigSATimeout, 0,
...
void rsn_preauth_deinit(struct wpa_sm *sm)
{
	...
	da16x_eloop_cancel_timeout(rsn_preauth_timeout, sm, NULL);
...
void rsn_preauth_candidate_process(struct wpa_sm *sm)
{
	...
	da16x_prt("<%s> RSN: processing PMKSA candidate list", __func__);
	if (sm->preauth_eapol ||
	    sm->proto != WPA_PROTO_RSN ||
	    wpa_sm_get_state(sm) != WPA_COMPLETED ||
	    (sm->key_mgmt != WPA_KEY_MGMT_IEEE8021X &&
	     sm->key_mgmt != WPA_KEY_MGMT_IEEE8021X_SHA256)) {
		da16x_prt("<%s> RSN: not in suitable "
			"state for new pre-authentication", __func__);
	...
	dl_list_for_each_safe(candidate, n, &sm->pmksa_candidates,
		...
		if (os_memcmp(sm->bssid, candidate->bssid, ETH_ALEN) != 0 &&
			...
			da16x_prt("<%s> RSN: PMKSA "
				"candidate " MACSTR
				" selected for pre-authentication",
				__func__, MAC2STR(candidate->bssid));
		...
		da16x_prt("<%s> SN: PMKSA candidate "
	...
	da16x_prt("<%s> RSN: no more pending PMKSA "
		"candidates", __func__);
}
			 ...
			 int prio, int preauth)
{
	...
	if (!preauth) {
		da16x_prt("<%s> RSN: Ignored PMKID candidate without "
			   "preauth flag\n", __func__);
		return;
	}
	...
	da16x_prt("<%s> RSN: added PMKSA cache "
		"candidate " MACSTR " prio %d", 
		__func__, MAC2STR(bssid), prio);
	rsn_preauth_candidate_process(sm);
}
