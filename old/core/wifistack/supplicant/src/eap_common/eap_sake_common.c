				   ...
				   u8 attr_id, u8 len, const u8 *data)
{
	...
	case EAP_SAKE_AT_RAND_S:
		wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Parse: AT_RAND_S");
		if (len != EAP_SAKE_RAND_LEN) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: AT_RAND_S with "
	...
	case EAP_SAKE_AT_RAND_P:
		wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Parse: AT_RAND_P");
		if (len != EAP_SAKE_RAND_LEN) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: AT_RAND_P with "
				   "invalid payload length %d", len);
			return -1;
		}
		attr->rand_p = data;
	...
	case EAP_SAKE_AT_MIC_S:
		wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Parse: AT_MIC_S");
		if (len != EAP_SAKE_MIC_LEN) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: AT_MIC_S with "
				   "invalid payload length %d", len);
			return -1;
		}
		attr->mic_s = data;
	...
	case EAP_SAKE_AT_MIC_P:
		wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Parse: AT_MIC_P");
		if (len != EAP_SAKE_MIC_LEN) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: AT_MIC_P with "
				   "invalid payload length %d", len);
			return -1;
		}
		attr->mic_p = data;
	...
	case EAP_SAKE_AT_SERVERID:
		wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Parse: AT_SERVERID");
		attr->serverid = data;
		attr->serverid_len = len;
		break;
	case EAP_SAKE_AT_PEERID:
		wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Parse: AT_PEERID");
		attr->peerid = data;
		attr->peerid_len = len;
		break;
	case EAP_SAKE_AT_SPI_S:
		wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Parse: AT_SPI_S");
		attr->spi_s = data;
		attr->spi_s_len = len;
		break;
	case EAP_SAKE_AT_SPI_P:
		wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Parse: AT_SPI_P");
		attr->spi_p = data;
		attr->spi_p_len = len;
		break;
	case EAP_SAKE_AT_ANY_ID_REQ:
		wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Parse: AT_ANY_ID_REQ");
		if (len != 2) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Invalid AT_ANY_ID_REQ"
	...
	case EAP_SAKE_AT_PERM_ID_REQ:
		wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Parse: AT_PERM_ID_REQ");
		if (len != 2) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Invalid "
	...
	case EAP_SAKE_AT_ENCR_DATA:
		wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Parse: AT_ENCR_DATA");
		attr->encr_data = data;
		attr->encr_data_len = len;
		break;
	case EAP_SAKE_AT_IV:
		wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Parse: AT_IV");
		attr->iv = data;
		attr->iv_len = len;
		break;
	case EAP_SAKE_AT_PADDING:
		wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Parse: AT_PADDING");
		for (i = 0; i < len; i++) {
			if (data[i]) {
				wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: AT_PADDING "
	...
	case EAP_SAKE_AT_NEXT_TMPID:
		wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Parse: AT_NEXT_TMPID");
		attr->next_tmpid = data;
		attr->next_tmpid_len = len;
		break;
	case EAP_SAKE_AT_MSK_LIFE:
		wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Parse: AT_MSK_LIFE");
		if (len != 4) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Invalid "
	...
	default:
		if (attr_id < 128) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Unknown non-skippable"
				   " attribute %d", attr_id);
			return -1;
		}
		wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Ignoring unknown skippable "
			   "attribute %d", attr_id);
		break;
	}

	if (attr->iv && !attr->encr_data) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: AT_IV included without "
			   "AT_ENCR_DATA");
		return -1;
	}
			      ...
			      struct eap_sake_parse_attr *attr)
{
	...
	while (pos < end) {
		if (end - pos < 2) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Too short attribute");
			return -1;
		}

		if (pos[1] < 2) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Invalid attribute "
				   "length (%d)", pos[1]);
			return -1;
		}

		if (pos + pos[1] > end) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Attribute underflow");
			return -1;
		}
			  ...
			  u8 *emsk)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Deriving keys");
