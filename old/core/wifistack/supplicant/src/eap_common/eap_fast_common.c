...
struct wpabuf * eap_fast_tlv_eap_payload(struct wpabuf *buf)
{
	...
	da16x_eap_prt("[%s] EAP-FAST: Add EAP-Payload TLV\n", __func__);
	e = wpabuf_alloc(sizeof(struct pac_tlv_hdr) + wpabuf_len(buf));
	if (e == NULL) {
		da16x_eap_prt("[%s] EAP-FAST: Failed to allocate memory "
...
#define TLS_MASTER_SECRET_LEN 48
	...
	da16x_dump("EAP-FAST: client_random", client_random, TLS_RANDOM_LEN);
	da16x_dump("EAP-FAST: server_random", server_random, TLS_RANDOM_LEN);
	...
	da16x_dump("EAP-FAST: master_secret",
		       ...
		       int tlv_type, u8 *pos, size_t len)
{
	...
	case EAP_TLV_EAP_PAYLOAD_TLV:
		da16x_dump("EAP-FAST: EAP-Payload TLV", pos, len);
		if (tlv->eap_payload_tlv) {
			da16x_eap_prt("[%s] EAP-FAST: More than one EAP-Payload "
				     "TLV in the message\n", __func__);
	...
	case EAP_TLV_RESULT_TLV:
		da16x_dump("EAP-FAST: Result TLV", pos, len);
		if (tlv->result) {
			da16x_eap_prt("[%s] EAP-FAST: More than one "
		...
		if (tlv->result != EAP_TLV_RESULT_SUCCESS &&
		    tlv->result != EAP_TLV_RESULT_FAILURE) {
			da16x_eap_prt("[%s] EAP-FAST: Unknown Result %d\n",
				     __func__, tlv->result);
			tlv->result = EAP_TLV_RESULT_FAILURE;
		}
		da16x_eap_prt("[%s] EAP-FAST: Result: %s\n", __func__,
			     tlv->result == EAP_TLV_RESULT_SUCCESS ?
				     "Success" : "Failure");
		break;
	case EAP_TLV_INTERMEDIATE_RESULT_TLV:
		da16x_dump("EAP-FAST: Intermediate Result TLV", pos, len);
		if (len < 2) {
			da16x_eap_prt("[%s] EAP-FAST: Too short "
		...
		if (tlv->iresult != EAP_TLV_RESULT_SUCCESS &&
		    tlv->iresult != EAP_TLV_RESULT_FAILURE) {
			da16x_eap_prt("[%s] EAP-FAST: Unknown Intermediate "
				     "Result %d\n", __func__, tlv->iresult);
			tlv->iresult = EAP_TLV_RESULT_FAILURE;
		}
		da16x_eap_prt("[%s] EAP-FAST: Intermediate Result: %s\n", __func__,
			     tlv->iresult == EAP_TLV_RESULT_SUCCESS ?
				     "Success" : "Failure");
		break;
	case EAP_TLV_CRYPTO_BINDING_TLV:
		da16x_dump("EAP-FAST: Crypto-Binding TLV", pos, len);
	...
	case EAP_TLV_REQUEST_ACTION_TLV:
		da16x_dump("EAP-FAST: Request-Action TLV", pos, len);
		...
		da16x_eap_prt("[%s] EAP-FAST: Request-Action: %d\n", __func__,
			     tlv->request_action);
		break;
	case EAP_TLV_PAC_TLV:
		da16x_dump("EAP-FAST: PAC TLV", pos, len);
		if (tlv->pac) {
			da16x_eap_prt("[%s] EAP-FAST: More than one "
