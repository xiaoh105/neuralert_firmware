...
struct wpabuf * eap_ikev2_build_frag_ack(u8 id, u8 code)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Send fragment ack");
			   ...
			   const u8 *pos, const u8 *end)
{
	...
	if (end - pos < (int) icv_len) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Not enough room in the "
			   "message for Integrity Checksum Data");
		return -1;
	}

	if (SK_a == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: No SK_a for ICV validation");
		return -1;
	}
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Valid Integrity Checksum Data in "
