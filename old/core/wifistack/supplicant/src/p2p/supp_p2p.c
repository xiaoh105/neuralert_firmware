...
#define P2P_PEER_EXPIRATION_AGE 20
...
#endif	/* 0 */
		da16x_p2p_prt("[%s] Expiring old peer entry " MACSTR "\n",
...
static void p2p_expiration_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	da16x_eloop_register_timeout(P2P_PEER_EXPIRATION_INTERVAL, 0,
...
{
	da16x_p2p_prt("[%s] State %s -> %s\n",
		__func__, p2p_state_txt(p2p->state), p2p_state_txt(new_state));
...
{
	da16x_p2p_prt("[%s] Set timeout (state=%s): %u.%06u sec\n",
		__func__, p2p_state_txt(p2p->state), sec, usec);

	da16x_eloop_cancel_timeout(p2p_state_timeout, p2p, NULL);
	da16x_eloop_register_timeout(sec, usec, p2p_state_timeout, p2p, NULL);
}


void p2p_clear_timeout(struct p2p_data *p2p)
{
	da16x_p2p_prt("[%s] Clear timeout (state=%s)\n",
				__func__, p2p_state_txt(p2p->state));
...
static void p2p_listen_in_find(struct p2p_data *p2p, int dev_disc)
{
	...
	da16x_p2p_prt("[%s] Starting short listen state (state=%s)\n",
			__func__, p2p_state_txt(p2p->state));

	if (p2p->pending_listen_freq) {
		/* We have a pending p2p_listen request */
		da16x_p2p_prt("[%s] p2p_listen command pending already\n",
			__func__);
		return;
	}
	...
	if (tu == 0) {
		da16x_p2p_prt("[%s] Skip listen state since duration "
			"was 0 TU\n", __func__);
...
#if 0
	/*
	 * Wi-Fi Direct Technical Spec. RECOMMEND that 
	 * the P2P FIND cycle is 100, 200, or 300ms randomly. 
	 */
	if (p2p->cfg->start_listen(p2p->cfg->cb_ctx, freq,
				   1024 * tu / 1000, ies) < 0)
		da16x_p2p_prt("[%s] Failed to start listen mode\n", __func__);
	else {
		p2p->in_listen = 1;
		p2p_set_timeout(p2p, 0, 1024 * tu);
	}
#else
...
int p2p_listen(struct p2p_data *p2p, unsigned int timeout)
{
	...
	da16x_p2p_prt("[%s] Going to listen(only) state\n", __func__);
	...
	if (p2p->p2p_scan_running) {
		if (p2p->start_after_scan == P2P_AFTER_SCAN_CONNECT) {
			da16x_p2p_prt("[%s] p2p_scan running - connect is "
				"already pending - skip listen\n",
					__func__);
			return 0;
		}
		da16x_p2p_prt("[%s] p2p_scan running - delay start of "
				"listen state\n", __func__);
	...
	p2p->in_listen = 1;
	p2p_set_timeout(p2p, timeout / 1000, 0);
...
void p2p_remove_device(struct p2p_data *p2p, const u8 *addr, int all)
{
	...
	dl_list_for_each_safe(dev, prev, &p2p->devices,
		...
		if (all ||
		    os_memcmp(dev->info.p2p_device_addr, addr, ETH_ALEN) == 0) {
			dl_list_del(&dev->list);
			p2p_device_free(p2p, dev);
		}
	}
					      ...
					      int is_essential)
{
	...
	if (count + 1 > p2p->cfg->max_peers && oldest) {
		if (is_essential) {
			da16x_p2p_prt("[%s] Remove oldest peer entry to make room "
				     "for a new peer\n", __func__);
				 ...
				 const u8 *gi, size_t gi_len)
{
	...
	for (c = 0; c < info.num_clients; c++) {
		...
		if (dev) {
			...
			dev = p2p_create_device(p2p, cli->p2p_device_addr, 1);
			if (dev == NULL)
				continue;
			      ...
			      int probe_req, const struct p2p_message *msg)
{
	...
	if (!probe_req) {
		...
		if (new_config_methods &&
			...
			da16x_p2p_prt("[%s] Update peer " MACSTR
		   ...
		   size_t ies_len, int scan_res)
{
...
#if 0	/* by Shingu : duplicated DA16X P2P_peer_result */
	...
	if (p2p_parse_ies(ies, ies_len, &msg)) {
		da16x_p2p_prt("[%s] Failed to parse P2P IE for "
			"a device entry\n", __func__);
	...
	if (msg.p2p_device_addr) {
		...
		da16x_p2p_prt("[%s] Ignore scan data without P2P Device Info "
			"or P2P Device Id\n", __func__);
	...
	if (!is_zero_ether_addr(p2p->peer_filter) &&
		...
		da16x_p2p_prt("[%s] Do not add peer filter for " MACSTR
			" due to peer filter\n",
	...
	dev = p2p_create_device(p2p, p2p_dev_addr, 0);
	if (dev == NULL) {
		p2p_parse_free(&msg);
		return -1;
	}
	...
	if (dev->last_seen.sec > 0 &&
		...
		da16x_p2p_prt("[%s] Do not update peer entry based on old "
			"frame (rx_time=%u.%06u last_seen=%u.%06u)\n",
			__func__,
	...
	if (freq >= 2412 && freq <= 2484 && msg.ds_params &&
	    *msg.ds_params >= 1 && *msg.ds_params <= 14) {
		...
		if (freq != ds_freq) {
			da16x_p2p_prt("[%s] Update Listen frequency based on "
				"DS Parameter Set IE: %d -> %d MHz\n",
				__func__,
				freq, ds_freq);
	...
	if (dev->listen_freq && dev->listen_freq != freq && scan_res) {
		da16x_p2p_prt("[%s] Update Listen frequency based on scan "
...
#if 0	/* by Shingu : duplicated DA16X P2P_peer_result */
	dev_name_changed = os_strncmp(dev->info.device_name, msg.device_name, 32) != 0;
#endif
	...
	p2p_parse_free(&msg);
#if 0	/* by Shingu : duplicated DA16X P2P_peer_result */
	if (dev->flags & P2P_DEV_REPORTED && !dev_name_changed)
		return 0;
#else
	...
	da16x_p2p_prt("[%s] Peer found with Listen frequency %d MHz "
		"(rx_time=%u.%06u)\n",
		__func__,
		freq, (unsigned int) rx_time->sec,
		(unsigned int) rx_time->usec);
	if (dev->flags & P2P_DEV_USER_REJECTED) {
		da16x_p2p_prt("[%s] Do not report rejected device\n", __func__);
		return 0;
	}
	...
	if (dev->info.config_methods == 0 &&
		 ...
		 * information.
		 */
		da16x_p2p_prt("[%s] Do not report peer " MACSTR
...
static int p2p_get_next_prog_freq(struct p2p_data *p2p)
{
	...
	da16x_p2p_prt("[%s] Next progressive search channel: reg_class %u "
			"channel %u -> %d MHz\n",
			__func__,
...
static void p2p_search(struct p2p_data *p2p)
{
	...
	if (res < 0) {
		da16x_p2p_prt("[%s] Scan request failed\n", __func__);
		p2p_continue_find(p2p);
	} else {
		da16x_p2p_prt("[%s] Running p2p_scan\n", __func__);

#ifdef	CONFIG_P2P_UNUSED_CMD
		p2p->p2p_scan_running = 1;

		da16x_eloop_cancel_timeout(p2p_scan_timeout, p2p, NULL);
		da16x_eloop_register_timeout(P2P_SCAN_TIMEOUT, 0,
...
static void p2p_find_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	da16x_notice_prt("P2P_FIND timeout\n");
...
static int p2p_run_after_scan(struct p2p_data *p2p)
{
	...
	if (p2p->after_scan_tx) {
		...
		da16x_p2p_prt("[%s] Send pending Action frame at p2p_scan "
			     "completion\n", __func__);
	...
	case P2P_AFTER_SCAN_LISTEN:
		da16x_p2p_prt("[%s] Start previously requested Listen state\n",
				__func__);
	...
	case P2P_AFTER_SCAN_CONNECT:
		da16x_p2p_prt("[%s] Start previously requested connect with "
			"" MACSTR "\n",
			__func__, MAC2STR(p2p->after_scan_peer));

		dev = p2p_get_device(p2p, p2p->after_scan_peer);
		if (dev == NULL) {
			da16x_p2p_prt("[%s] Peer not known anymore\n",
				__func__);
			break;
		}
...
static void p2p_scan_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	da16x_p2p_prt("[%s] p2p_scan timeout (running=%d)\n",
			__func__, p2p->p2p_scan_running);
	     ...
	     enum p2p_discovery_type type)
{
	...
	da16x_p2p_prt("[%s] Starting find (type=%d)\n", __func__, type);
	os_get_reltime(&p2p->find_start);

	if (p2p->state == P2P_SEARCH) {
		da16x_err_prt("P2P_FIND is now running\n");
		return -1;
	}
	...
	da16x_eloop_cancel_timeout(p2p_find_timeout, p2p, NULL);

	p2p->p2p_find_timeout = timeout;

	if (timeout) {
		p2p->p2p_find_timeout = timeout;
		da16x_eloop_register_timeout(timeout, 0, p2p_find_timeout, p2p,
					    NULL);
	} else {
		p2p->p2p_find_timeout = DEFAULT_P2P_FIND_TIMEOUT;
		da16x_eloop_register_timeout(DEFAULT_P2P_FIND_TIMEOUT, 0,
	...
	if (res == 0) {
		...
		da16x_p2p_prt("[%s] Failed to start p2p_scan - another p2p_scan "
			"was already running\n", __func__);
		...
		da16x_p2p_prt("[%s] Failed to start p2p_scan\n", __func__);
		p2p_set_state(p2p, P2P_IDLE);
...
void p2p_stop_listen(struct p2p_data *p2p)
{
	if (p2p->state != P2P_LISTEN_ONLY) {
		da16x_p2p_prt("[%s] Skip stop_listen since not in listen_only "
				"state.\n", __func__);
		return;
	}
				    ...
				    unsigned int pref_freq, int go)
{
	...
	da16x_p2p_prt("[%s] Prepare channel pref - force_freq=%u pref_freq=%u"
		     "go=%d\n",	__func__, force_freq, pref_freq, go);

	if (p2p_freq_to_channel(freq, &op_class, &op_channel) < 0) {
		da16x_err_prt("[%s] Unsupported frequency %u MHz", __func__, freq);
		return -1;
	}
	...
	if (!p2p_channels_includes(&p2p->cfg->channels, op_class, op_channel) &&
	    (go || !p2p_channels_includes(&p2p->cfg->cli_channels, op_class,
					  op_channel))) {
		da16x_p2p_prt("Frequency %u MHz (oper_class %u channel %u) "
			     "not allowed for P2P\n",
...
static void p2p_prepare_channel_best(struct p2p_data *p2p)
{
	...
	da16x_p2p_prt("[%s] Prepare channel best\n", __func__);
	if (!p2p->cfg->cfg_op_channel && p2p->best_freq_overall > 0 &&
	    p2p_supported_freq(p2p, p2p->best_freq_overall) &&
	    p2p_freq_to_channel(p2p->best_freq_overall, &op_class, &op_channel)
	    == 0) {
		da16x_p2p_prt("[%s] Select best overall channel as operating "
			     "channel preference\n", __func__);
		...
		p2p->op_channel = op_channel;
		   ...
		   == 0) {
		da16x_p2p_prt("[%s] Select best 5 GHz channel as operating "
			     "channel preference\n", __func__);
		...
		p2p->op_channel = op_channel;
		   ...
		   p2p_freq_to_channel(p2p->best_freq_24, &op_class,
				       &op_channel) == 0) {
		da16x_p2p_prt("[%s] Select best 2.4 GHz channel as operating "
			     "channel preference\n", __func__);
		...
		da16x_p2p_prt("[%s] Select first pref_chan entry as "
		...
		da16x_p2p_prt("[%s] Select possible VHT channel (op_class %u "
			     "channel %u) as operating channel preference\n",
			     __func__, p2p->op_reg_class, p2p->op_channel);
	} else if (p2p_channel_select(&p2p->cfg->channels, op_classes_ht40,
			&p2p->op_reg_class, &p2p->op_channel) == 0) {
		da16x_p2p_prt("[%s] Select possible HT40 channel (op_class %u "
			     "channel %u) as operating channel preference\n",
			     __func__, p2p->op_reg_class, p2p->op_channel);
	} else if (p2p_channel_select(&p2p->cfg->channels, op_classes_5ghz,
			&p2p->op_reg_class, &p2p->op_channel) == 0) {
		da16x_p2p_prt("[%s] Select possible 5 GHz channel (op_class %u "
			     "channel %u) as operating channel preference\n",
			     __func__, p2p->op_reg_class, p2p->op_channel);
	}
#endif	/* CONFIG_P2P_UNUSED_CMD */
	if (p2p_channels_includes(&p2p->cfg->channels, p2p->cfg->op_reg_class,
				  p2p->cfg->op_channel)) {
		da16x_p2p_prt("[%s] Select pre-configured channel as operating "
			     "channel preference\n", __func__);
		...
		p2p->op_channel = p2p->cfg->op_channel;
					     ...
					     &p2p->op_reg_class,
					     &p2p->op_channel) == 0) {
		da16x_p2p_prt("[%s] Select random available social "
			     "channel %d from 2.4 GHz band as operating "
				     "channel preference\n",
				     __func__, p2p->op_channel);
	} else {
		...
		p2p_channel_select(&p2p->cfg->channels, NULL,
				   &p2p->op_reg_class,
				   &p2p->op_channel);
		da16x_p2p_prt("[%s] Select random available channel %d from "
			"operating class %d as operating channel preference\n",
			__func__, p2p->op_channel, p2p->op_reg_class);
	}
 ...
 * is available.
 */
int p2p_prepare_channel(struct p2p_data *p2p, struct p2p_device *dev, int go)
{
#ifdef	CONFIG_P2P_UNUSED_CMD
	da16x_p2p_prt("[%s] Prepare channel - force_freq=%u pref_freq=%u "
		     "go=%d\n",	__func__, go);
...
#ifdef	CONFIG_P2P_UNUSED_CMD
	da16x_p2p_prt("[%s] Own preference for operation channel: Operating "
		     "Class %u Channel %u%s\n", __func__, p2p->op_reg_class,
...
int p2p_connect(struct p2p_data *p2p, const u8 *peer_addr,
		...
		const u8 *own_interface_addr, int pd_before_go_neg)
{
	struct p2p_device *dev;

	da16x_p2p_prt("[%s] Request to start group negotiation - peer=" MACSTR
		     "  GO Intent=%d  Intended Interface Address=" MACSTR
		     " wps_method=%d pd_before_go_neg=%d\n",
		     __func__, MAC2STR(peer_addr), go_intent,
		     MAC2STR(own_interface_addr), wps_method,
		     pd_before_go_neg);
	...
	if (p2p_prepare_channel(p2p, dev, go_intent == 15) < 0)
		return -1;

	if (dev->flags & P2P_DEV_GROUP_CLIENT_ONLY) {
		if (!(dev->info.dev_capab &
		      P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY)) {
			da16x_p2p_prt("[%s] Cannot connect to P2P Device "
				     MACSTR " that is in a group and is not "
					     "discoverable\n",
					     __func__, MAC2STR(peer_addr));
		...
		if (dev->oper_freq <= 0) {
			da16x_p2p_prt("[%s] Cannot connect to P2P Device "
				     MACSTR " with incomplete information\n",
				     __func__, MAC2STR(peer_addr));
	...
	p2p->ssid_set = 0;
	if (force_ssid) {
		da16x_ascii_dump("P2P: Forced SSID", force_ssid, force_ssid_len);
	...
	if (p2p->after_scan_tx) {
		 ...
		 */
		da16x_p2p_prt("[%s] Dropped previous pending Action frame "
			     "TX that was waiting for p2p_scan completion\n",
			     __func__);
...
#ifdef	CONFIG_P2P_UNUSED_CMD
	if (p2p->p2p_scan_running) {
		da16x_p2p_prt("[%s] p2p_scan running - delay connect send\n",
			     __func__);
...
int p2p_authorize(struct p2p_data *p2p, const u8 *peer_addr,
		  ...
		  const u8 *force_ssid, size_t force_ssid_len, u16 oob_pw_id)
{
	struct p2p_device *dev;

	da16x_p2p_prt("[%s] Request to authorize group negotiation - peer="
		MACSTR "  GO Intent=%d  Intended Interface Address=" MACSTR
	...
	dev = p2p_get_device(p2p, peer_addr);
	if (dev == NULL) {
		da16x_p2p_prt("[%s] Cannot authorize unknown P2P Device "
			MACSTR "\n",
			__func__, MAC2STR(peer_addr));
	...
	if (p2p_prepare_channel(p2p, dev, force_freq, go_intent == 15) < 0)
		return -1;
		      ...
		      struct p2p_device *dev, struct p2p_message *msg)
{
	...
	if (msg->listen_channel) {
		...
		if (freq < 0) {
			da16x_p2p_prt("[%s] Unknown peer Listen channel: "
				     "country=%c%c(0x%02x) reg_class=%u "
					     "channel=%u\n", __func__,
	...
	if (dev->flags & P2P_DEV_PROBE_REQ_ONLY) {
		dev->flags &= ~P2P_DEV_PROBE_REQ_ONLY;
		da16x_p2p_prt("[%s] Completed device entry based on data "
			     "from GO Negotiation Request\n", __func__);
	} else {
		da16x_p2p_prt("[%s] Created device entry based on GO Neg Req: "
...
void p2p_go_complete(struct p2p_data *p2p, struct p2p_device *peer)
{
	...
	da16x_p2p_prt("[%s] GO Negotiation with " MACSTR " completed "
			"(%s will be GO)\n",
		__func__,
...
{
	da16x_p2p_prt("[%s] RX P2P Public Action from " MACSTR "\n",
			__func__, MAC2STR(sa));

	da16x_dump("P2P: P2P Public Action contents", data, len);
	...
	case P2P_DEV_DISC_RESP:
		...
		break;
	default:
		da16x_p2p_prt("[%s] Unsupported P2P Public Action "
			"frame type %d\n",
			__func__, data[0]);
		break;
	}
		   ...
		   const u8 *data, size_t len, int freq)
{
	...
	/* P2P action frame */
	da16x_p2p_prt("[%s] RX P2P Action from " MACSTR "\n",
			__func__, MAC2STR(sa));

	da16x_dump("P2P: P2P Action contents", data, len);
...
#ifdef	CONFIG_P2P_POWER_SAVE
	case P2P_NOA:
		da16x_p2p_prt("[%s] Received P2P Action - Notice of Absence\n",
				__func__);
		/* TODO */
		break;
	...
	case P2P_GO_DISC_REQ:
		...
		break;
	default:
		da16x_p2p_prt("[%s] Received P2P Action - unknown type %u\n",
				__func__, data[0]);
		break;
	}
}
				       ...
				       const u8 *ie, size_t ie_len)
{
	...
	dev = p2p_create_device(p2p, addr, 0);
	if (dev == NULL) {
		p2p_parse_free(&msg);
		return;
	}
	...
	da16x_p2p_prt("[%s] Created device entry based on Probe Req: " MACSTR
						...
						struct p2p_message *msg)
{
	...
	dev = p2p_create_device(p2p, addr, 1);
	if (dev == NULL)
		return NULL;
...
da16x_p2p_prt("[%s] Have to include CONFIG_WPS features ...\n", __func__);
	memset((unsigned char *)&attr, 0, sizeof (struct wps_parse_attr));
#endif	/* CONFIG_WPS */
...
struct wpabuf * p2p_build_probe_resp_ies(struct p2p_data *p2p)
{
	...
	if (p2p_build_wps_ie(p2p, buf, pw_id, 1) < 0) {
		da16x_p2p_prt("[%s] Failed to build WPS IE for Probe Response\n",
				__func__);
		wpabuf_free(buf);
		return NULL;
	}
		...
		const u8 *bssid, const u8 *ie, size_t ie_len)
{
	...
	if (!p2p->in_listen) {
		...
		da16x_p2p_prt("[%s] Not in Listen state (in_listen=%d) "
			"- ignore Probe Request\n",
			__func__, p2p->in_listen);

		return P2P_PREQ_NOT_LISTEN;
	}

	if (ieee802_11_parse_elements((u8 *) ie, ie_len, &elems, 0) == ParseFailed) {
		/* Ignore invalid Probe Request frames */
		da16x_p2p_prt("[%s] Could not parse Probe Request frame "
			"- ignore it\n", __func__);
	...
	if (elems.p2p == NULL) {
		...
		da16x_p2p_prt("[%s] Not a P2P probe - ignore it\n", __func__);
	...
	if (dst && !is_broadcast_ether_addr(dst) &&
		 ...
		 */
		da16x_p2p_prt("[%s] Probe Req DA " MACSTR " not ours - "
			"ignore it\n",
	...
	if (bssid && !is_broadcast_ether_addr(bssid)) {
		...
		da16x_p2p_prt("[%s] Probe Req BSSID " MACSTR " not wildcard - "
			"ignore it\n",
	...
	if (elems.ssid == NULL || elems.ssid_len != P2P_WILDCARD_SSID_LEN ||
		...
		da16x_p2p_prt("[%s] Probe Req not using P2P Wildcard SSID - "
			"ignore it\n", __func__);
		return P2P_PREQ_NOT_PROCESSED;
	}
	...
	if (supp_rates_11b_only(&elems)) {
		...
		da16x_p2p_prt("[%s] Probe Req with 11b rates only supported - "
			"ignore it\n", __func__);
		return P2P_PREQ_NOT_P2P;
	}
	...
	if (p2p_parse_ies(ie, ie_len, &msg) < 0) {
		...
		da16x_p2p_prt("[%s] Could not parse P2P attributes in Probe "
			"Req - ignore it\n", __func__);
		return P2P_PREQ_NOT_P2P;
	}
	...
	if (msg.device_id &&
		...
		da16x_p2p_prt("[%s] Probe Req requested Device ID " MACSTR
	...
	if (msg.wps_attributes &&
		...
		da16x_p2p_prt("[%s] Probe Req requestred Device Type did not "
			"match - ignore it\n", __func__);
		p2p_parse_free(&msg);
		return P2P_PREQ_NOT_PROCESSED;
	}
	...
	if (!p2p->cfg->send_probe_resp) {
		...
		da16x_p2p_prt("[%s] Probe Resp generated elsewhere - "
			"do not generate additional response\n", __func__);
		return P2P_PREQ_NOT_PROCESSED;
	}

	da16x_p2p_prt("[%s] Reply to P2P Probe Request in Listen state\n",
			__func__);
...
#else
	resp = (struct _ieee80211_mgmt *)wpabuf_put(buf, sizeof(struct _ieee80211_mgmt));
#endif	/* 0 */
		 ...
		 const u8 *bssid, const u8 *ie, size_t ie_len)
{
	...
	if ((p2p->state == P2P_CONNECT || p2p->state == P2P_CONNECT_LISTEN) &&
		...
		da16x_p2p_prt("[%s] Found GO Negotiation peer - try to "
			"start GO negotiation from timeout\n", __func__);

		da16x_eloop_cancel_timeout(p2p_go_neg_start, p2p, NULL);
		da16x_eloop_register_timeout(0, 0, p2p_go_neg_start, p2p, NULL);
	...
	if ((p2p->state == P2P_INVITE || p2p->state == P2P_INVITE_LISTEN) &&
		...
		da16x_p2p_prt("[%s] Found Invite peer - try to start Invite "
			"from timeout\n", __func__);

		da16x_eloop_cancel_timeout(p2p_invite_start, p2p, NULL);
		da16x_eloop_register_timeout(0, 0, p2p_invite_start, p2p, NULL);
...
{
	if (p2p->go_neg_peer == NULL) {
		da16x_p2p_prt("[%s] No pending Group Formation - ignore WPS "
	...
	if (os_memcmp(mac_addr, p2p->go_neg_peer->intended_addr, ETH_ALEN) !=
	    0) {
		da16x_p2p_prt("[%s] Ignore WPS registration success "
			"notification for "
	...
	da16x_p2p_prt("[%s] Group Formation completed successfully with "
			MACSTR "\n",
...
{
	if (p2p->go_neg_peer == NULL) {
		da16x_p2p_prt("[%s] No pending Group Formation - ignore "
			"group formation failure notification\n", __func__);
		return; /* No pending Group Formation */
	}

	da16x_p2p_prt("[%s[ Group Formation failed with " MACSTR "\n",
		__func__, MAC2STR(p2p->go_neg_peer->intended_addr));
...
void p2p_deinit(struct p2p_data *p2p)
{
	...
	da16x_eloop_cancel_timeout(p2p_expiration_timeout, p2p, NULL);
#ifdef	CONFIG_P2P_UNUSED_CMD
	da16x_eloop_cancel_timeout(p2p_ext_listen_timeout, p2p, NULL);
	da16x_eloop_cancel_timeout(p2p_scan_timeout, p2p, NULL);
#endif	/* CONFIG_P2P_UNUSED_CMD */
	da16x_eloop_cancel_timeout(p2p_go_neg_start, p2p, NULL);
...
int p2p_unauthorize(struct p2p_data *p2p, const u8 *addr)
{
	...
	da16x_p2p_prt("[%s] Unauthorizing " MACSTR "\n",
			__func__, MAC2STR(addr));
...
void p2p_continue_find(struct p2p_data *p2p)
{
	...
	dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
		...
		if (dev->req_config_methods &&
			...
			da16x_p2p_prt("[%s] Send pending Provision Discovery "
				"Request to " MACSTR " (config methods 0x%x)\n",
				__func__,
				MAC2STR(dev->info.p2p_device_addr),
...
{
	da16x_p2p_prt("[%s] Service Discovery Query TX callback: success=%d\n",
		__func__, success);
	...
	if (p2p->sd_peer == NULL) {
		da16x_p2p_prt("[%s] No SD peer entry known\n", __func__);
...
static void p2p_retry_pd(struct p2p_data *p2p)
{
	...
	dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
		...
		da16x_p2p_prt("[%s] Send pending Provision Discovery Request to "
...
{
	da16x_p2p_prt("[%s] Provision Discovery Request TX callback: "
		"success=%d\n", __func__, success);
			 ...
			 size_t ies_len)
{
	if (os_reltime_before(rx_time, &p2p->find_start)) {
		 ...
		 */
		da16x_p2p_prt("[%s] Ignore old scan result for " MACSTR
			" (rx_time=%u.%06u)\n",
			__func__,
...
#ifdef	CONFIG_P2P_UNUSED_CMD
	if (!p2p->p2p_scan_running) {
		da16x_p2p_prt("[%s] p2p_scan was not running, but scan "
			"results received\n", __func__);
	}
	p2p->p2p_scan_running = 0;
...
void p2p_scan_ie(struct p2p_data *p2p, struct wpabuf *ies, const u8 *dev_id)
{
	...
	if (p2p->cfg->reg_class && p2p->cfg->channel)
		...
		p2p_buf_add_listen_channel(ies, p2p->cfg->country, 81, 1);
...
static void p2p_go_neg_req_cb(struct p2p_data *p2p, int success)
{
	...
	da16x_p2p_prt("[%s] GO Negotiation Request TX callback: success=%d\n",
			__func__, success);

	if (dev == NULL) {
		da16x_p2p_prt("[%s] No pending GO Negotiation\n", __func__);
		return;
	}
	...
	if (!success &&
		...
		da16x_p2p_prt("[%s] Peer " MACSTR " did not acknowledge request"
...
{
	da16x_p2p_prt("[%s] GO Negotiation Response TX callback: success=%d\n",
		__func__, success);
	if (!p2p->go_neg_peer && p2p->state == P2P_PROVISIONING) {
		da16x_p2p_prt("[%s] Ignore TX callback event - GO Negotiation "
...
static void p2p_go_neg_resp_failure_cb(struct p2p_data *p2p, int success,
				       const u8 *addr)
{
	da16x_p2p_prt("[%s] GO Negotiation Response (failure) TX callback: "
		"success=%d\n", __func__, success);
			       ...
			       enum p2p_send_action_result result)
{
	...
	da16x_p2p_prt("[%s] GO Negotiation Confirm TX callback: result=%d\n",
			__func__, result);
	...
	if (result == P2P_SEND_ACTION_NO_ACK) {
		...
		if (dev && dev->go_neg_conf &&
			...
			da16x_p2p_prt("[%s] GO Negotiation Confirm retry %d\n",
				__func__, dev->go_neg_conf_sent);
			...
			da16x_p2p_prt("[%s] Failed to re-send Action frame\n",
				__func__);
		 ...
		 * the TX status.
		 */
		da16x_p2p_prt("[%s] Assume GO Negotiation Confirm TX was "
			"actually received by the peer even though Ack was "
			"not reported\n", __func__);
	}
			...
			enum p2p_send_action_result result)
{
	...
	da16x_p2p_prt("[%s] Action frame TX callback (state=%d freq=%u dst="
		MACSTR " src=" MACSTR " bssid=" MACSTR " result=%d\n",
		__func__,
	...
	case P2P_NO_PENDING_ACTION:
		...
		if (p2p->after_scan_tx_in_progress) {
			...
			if (p2p->start_after_scan != P2P_AFTER_SCAN_NOTHING &&
				...
				break;
			if (p2p->state == P2P_SEARCH) {
				da16x_p2p_prt("[%s] Continue find after "
		   ...
		   unsigned int duration)
{
	if (freq == p2p->pending_client_disc_freq) {
		da16x_p2p_prt("[%s] Client discoverability remain-awake "
			"completed\n", __func__);
	...
	if (freq != p2p->pending_listen_freq) {
		da16x_p2p_prt("[%s] Unexpected listen callback for freq=%u "
	...
	da16x_p2p_prt("[%s] Starting Listen timeout(%u,%u) on freq=%u "
		"based on callback\n",
		__func__,
...
{
	da16x_p2p_prt("[%s] Driver ended Listen state (freq=%u)\n",
			__func__, freq);
	...
	if (p2p->state == P2P_CONNECT_LISTEN && p2p->go_neg_peer) {
		if (p2p->go_neg_peer->connect_reqs >= 120) {
			da16x_p2p_prt("[%s] Timeout on sending GO Negotiation "
		...
		if (p2p->p2p_scan_running) {
			  ...
			  * restart a scan here.
			  */
			da16x_p2p_prt("[%s] p2p_scan already in progress - "
				"do not try to start a new one\n", __func__);
			return 1;
		}
#endif	/* CONFIG_P2P_UNUSED_CMD */
		...
		if (p2p->pending_listen_freq) {
			 ...
			 */
			da16x_p2p_prt("[%s] Listen operation did not seem to "
				"start - delay search phase to avoid "
				"busy loop\n", __func__);
...
static void p2p_timeout_connect(struct p2p_data *p2p)
{
	...
	if (p2p->go_neg_peer &&
	    (p2p->go_neg_peer->flags & P2P_DEV_WAIT_GO_NEG_CONFIRM)) {
		da16x_p2p_prt("[%s] Wait for GO Negotiation Confirm timed out "
			"- assume GO Negotiation failed\n", __func__);
	...
	if (p2p->go_neg_peer &&
		...
		da16x_p2p_prt("[%s] Peer expected to wait our response - "
			"skip listen\n", __func__);
	...
	if (p2p->go_neg_peer && p2p->go_neg_peer->oob_go_neg_freq > 0) {
		da16x_p2p_prt("[%s] Skip connect-listen since GO Neg channel "
			"known (OOB)\n", __func__);
...
static void p2p_timeout_wait_peer_idle(struct p2p_data *p2p)
{
	...
	if (dev == NULL) {
		da16x_p2p_prt("[%s] Unknown GO Neg peer - stop GO Neg wait\n",
				__func__);
		return;
	}
	...
	if (os_reltime_expired(&now, &dev->go_neg_wait_started, 120)) {
		da16x_p2p_prt("[%s] Timeout on waiting peer to become ready "
			"for GO Negotiation\n", __func__);
	...
	da16x_p2p_prt("[%s] Go to Listen state while waiting for the "
		"peer to become ready for GO Negotiation\n",
		__func__);
...
{
	da16x_p2p_prt("[%s] Service Discovery Query timeout\n",
			__func__);
...
{
	da16x_p2p_prt("[%s] Provision Discovery Request timeout\n",
			__func__);
...
static void p2p_timeout_prov_disc_req(struct p2p_data *p2p)
{
	...
	da16x_p2p_prt("[%s] User initiated Provision Discovery "
		"Request timeout\n", __func__);
...
static void p2p_timeout_invite(struct p2p_data *p2p)
{
	...
	if (p2p->inv_role == P2P_INVITE_ROLE_ACTIVE_GO) {
		 ...
		 */
		da16x_p2p_prt("[%s] Inviting in active GO role - wait on "
			"operating channel\n", __func__);
...
static void p2p_timeout_invite_listen(struct p2p_data *p2p)
{
	if (p2p->invite_peer && p2p->invite_peer->invitation_reqs < 100) {
		...
		if (p2p->invite_peer) {
			da16x_p2p_prt("[%s] Invitation Request retry limit "
				"reached\n", __func__);
...
static void p2p_state_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	da16x_p2p_prt("[%s] Timeout (state=%s)\n",
		__func__, p2p_state_txt(p2p->state));
	...
	case P2P_LISTEN_ONLY:
		...
		if (p2p->ext_listen_only) {
			da16x_p2p_prt("[%s] Extended Listen Timing - Listen "
				"State completed\n", __func__);
...
#endif	/* CONFIG_P2P_OPTION */
	default:
		da16x_p2p_prt("[%s:d] Unknown State\n",__func__,__LINE__);
		break;
	}
}
...
int p2p_reject(struct p2p_data *p2p, const u8 *peer_addr)
{
	...
	da16x_p2p_prt("[%s] Local request to reject connection attempts "
		"by peer " MACSTR "\n",
		__func__, MAC2STR(peer_addr));

	if (dev == NULL) {
		da16x_p2p_prt("[%s] Peer " MACSTR " unknown\n",
			__func__, MAC2STR(peer_addr));
		return -1;
	}
...
{
	if (enabled) {
		da16x_p2p_prt("[%s] Client discoverability enabled\n", __func__);

		p2p->dev_capab |= P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY;
	} else {
		da16x_p2p_prt("[%s] Client discoverability disabled\n",
				__func__);
		     ...
		     u32 interval2)
{
	...
	da16x_p2p_prt("[%s] Send Presence Request to GO " MACSTR
...
#if 0	/* by Shingu 20170209 (TTA Certificate) */
	if (noa) {
				     ...
				     int rx_freq)
{
	...
	da16x_p2p_prt("[%s] Received P2P Action - P2P Presence Request\n",
			__func__);
	...
	if (group == NULL) {
		da16x_p2p_prt("[%s] Ignore P2P Presence Request for "
			"unknown group " MACSTR "\n",
			__func__, MAC2STR(da));
		return;
	}

	if (p2p_parse(data, len, &msg) < 0) {
		da16x_p2p_prt("[%s] Failed to parse P2P Presence Request\n",
			__func__);
	...
	if (msg.noa == NULL) {
		da16x_p2p_prt("[%s] No NoA attribute in P2P Presence Request\n",
				__func__);
...
fail:
	resp = p2p_build_presence_resp(status, msg.noa_len > 0 ? msg.noa : NULL,
				       msg.noa_len > 0 ? msg.noa_len : 0,
				       msg.dialog_token);
	if (parsed)
		p2p_parse_free(&msg);
	if (resp == NULL)
				      ...
				      const u8 *sa, const u8 *data, size_t len)
{
	...
	da16x_p2p_prt("[%s] Received P2P Action - P2P Presence Response\n",
			__func__);

	if (p2p_parse(data, len, &msg) < 0) {
		da16x_p2p_prt("[%s] Failed to parse P2P Presence Response\n",
			__func__);
		return;
	}

	if (msg.status == NULL || msg.noa == NULL) {
		da16x_p2p_prt("[%s] No Status or NoA attribute in P2P Presence "
			"Response\n", __func__);
	...
	if (*msg.status) {
		da16x_p2p_prt("[%s] P2P Presence Request was rejected: "
			"status %u\n",
			__func__, *msg.status);
	...
	da16x_p2p_prt("[%s] P2P Presence Request was accepted\n", __func__);

	da16x_dump("P2P: P2P Presence Response - NoA", msg.noa, msg.noa_len);
...
static void p2p_ext_listen_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (p2p->ext_listen_interval) {
		...
		da16x_eloop_register_timeout(p2p->ext_listen_interval_sec,
	...
	if ((p2p->cfg->is_p2p_in_progress &&
	    ...
	    (p2p->pending_action_state == P2P_PENDING_PD &&
	     p2p->pd_retries > 0)) {
		da16x_p2p_prt("[%s] Operation in progress - skip Extended "
			"Listen timeout (%s)\n",
			__func__, p2p_state_txt(p2p->state));
	...
	if (p2p->state == P2P_LISTEN_ONLY && p2p->ext_listen_only) {
		 ...
		 */
		da16x_p2p_prt("[%s] Previous Extended Listen operation "
			"had not been completed - try again\n",
			__func__);
	...
	if (p2p->state != P2P_IDLE) {
		da16x_p2p_prt("[%s] Skip Extended Listen timeout in "
			"active state (%s)\n",
			__func__, p2p_state_txt(p2p->state));
		return;
	}

	da16x_p2p_prt("[%s] Extended Listen timeout\n", __func__);

	p2p->ext_listen_only = 1;
	if (p2p_listen(p2p, p2p->ext_listen_period) < 0) {
		da16x_p2p_prt("[%s] Failed to start Listen state for "
		   ...
		   unsigned int interval)
{
	if (period > 65535 || interval > 65535 || period > interval ||
		...
		da16x_p2p_prt("[%s] Invalid Extended Listen Timing request: "
			"period=%u interval=%u\n",
			__func__, period, interval);
	...
	if (interval == 0) {
		da16x_p2p_prt("[%s] Disabling Extended Listen Timing\n",
				__func__);
	...
	da16x_p2p_prt("[%s] Enabling Extended Listen Timing: period %u msec, "
			"interval %u msec\n",
			__func__, period, interval);
		      ...
		      const u8 *ie, size_t ie_len)
{
	...
	da16x_p2p_prt("[%s] Deauthentication notification BSSID " MACSTR
			...
			const u8 *ie, size_t ie_len)
{
	...
	da16x_p2p_prt("[%s] Disassociation notification BSSID " MACSTR
		" reason_code=%u minor_reason_code=%u\n",
		__func__,
		MAC2STR(bssid), reason_code, *msg.minor_reason_code);
...
int p2p_set_listen_channel(struct p2p_data *p2p, u8 reg_class, u8 channel)
{
	...
	da16x_p2p_prt("[%s] Set Listen channel: reg_class %u channel %u\n",
		__func__, reg_class, channel);
			 ...
			 int cfg_op_channel)
{
	...
	da16x_p2p_prt("[%s] Set Operating channel: reg_class %u channel %u\n",
		__func__, op_reg_class, op_channel);
...
{
	da16x_p2p_prt("[%s] New SSID postfix: %s\n",
			__func__, wpa_ssid_txt(postfix, len));
	if (postfix == NULL) {
...
{
	if (enabled) {
		da16x_p2p_prt("[%s] Managed P2P Device operations enabled\n",
				__func__);
		p2p->dev_capab |= P2P_DEV_CAPAB_INFRA_MANAGED;
	} else {
		da16x_p2p_prt("[%s] Managed P2P Device operations disabled\n",
				__func__);
		p2p->dev_capab &= ~P2P_DEV_CAPAB_INFRA_MANAGED;
	}
}
...
{
	da16x_p2p_prt("[%s] Cross connection %s\n",
		       ...
		       const struct wpa_freq_range_list *list)
{
	...
	da16x_p2p_prt("[%s] Updated no GO chan list\n", __func__);
...
void p2p_set_peer_filter(struct p2p_data *p2p, const u8 *addr)
{
	...
	if (is_zero_ether_addr(p2p->peer_filter)) {
		da16x_p2p_prt("[%s] Disable peer filter\n", __func__);
	} else {
		da16x_p2p_prt("[%s] Enable peer filter for " MACSTR "\n",
			__func__, MAC2STR(p2p->peer_filter));
	}
}
#endif	/* CONFIG_P2P_UNUSED_CMD */
...
{
	da16x_p2p_prt("[%s] Intra BSS distribution %s\n",
		__func__, enabled ? "enabled" : "disabled");
...
{
	da16x_p2p_prt("[%s] Update channel list\n", __func__);
...
#ifdef	CONFIG_P2P_UNUSED_CMD
	if (p2p->p2p_scan_running) {
		da16x_p2p_prt("[%s] Delay Action frame TX until p2p_scan "
			"completes\n", __func__);

		if (p2p->after_scan_tx) {
			da16x_p2p_prt("[%s] Dropped previous pending Action "
				"frame TX\n", __func__);
...
void p2p_set_best_channels(struct p2p_data *p2p, int freq_24, int freq_5,
			   int freq_overall)
{
	da16x_p2p_prt("[%s] Best channel: 2.4 GHz: %d, 5 GHz: %d, overall: %d\n",
		__func__, freq_24, freq_5, freq_overall);
...
{
	da16x_p2p_prt("[%s] Own frequency preference: %d MHz\n",
			__func__, freq);
		     ...
		     int max_disc_tu)
{
	...
	da16x_p2p_prt("[%s] Set discoverable interval: "
			"min=%d max=%d max_tu=%d\n",
			__func__,
