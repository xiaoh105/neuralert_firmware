			       ...
			       const u8 *data, size_t len, int rx_freq)
{
	...
	if (p2p->state == P2P_IDLE) {
		...
		da16x_notice_prt(">>> P2P Join Request from " MACSTR "\n",
				MAC2STR(sa));
	}
#endif	/* 1 */

	da16x_p2p_prt("[%s] Received Provision Discovery Request from " MACSTR
		" with config methods 0x%x (freq=%d)\n",
		__func__, MAC2STR(sa), msg.wps_config_methods, rx_freq);

	dev = p2p_get_device(p2p, sa);
	if (dev == NULL || (dev->flags & P2P_DEV_PROBE_REQ_ONLY)) {
		da16x_p2p_prt("[%s] Provision Discovery Request from "
			"unknown peer " MACSTR "\n",
			__func__, MAC2STR(sa));

		if (p2p_add_device(p2p, sa, rx_freq, NULL, 0, data + 1, len - 1,
				   0)) {
			da16x_p2p_prt("[%s] Provision Discovery Request add "
				"device failed " MACSTR "\n",
				__func__, MAC2STR(sa));
		}
	...
	if (!(msg.wps_config_methods &
		...
		da16x_p2p_prt("[%s] Unsupported Config Methods in Provision "
			"Discovery Request\n", __func__);
		goto out;
	}
	...
	if (msg.group_id) {
		...
		if (i == p2p->num_groups) {
			da16x_p2p_prt("[%s] PD request for unknown P2P Group "
				"ID - reject\n", __func__);
			goto out;
		}
	}
	...
	if (msg.wps_config_methods & WPS_CONFIG_DISPLAY) {
		da16x_p2p_prt("[%s] Peer " MACSTR " requested us to show "
			"a PIN on display\n", __func__, MAC2STR(sa));
		if (dev)
			dev->flags |= P2P_DEV_PD_PEER_KEYPAD;
	} else if (msg.wps_config_methods & WPS_CONFIG_KEYPAD) {
		da16x_p2p_prt("[%s] Peer " MACSTR " requested us to write "
			"its PIN using keypad\n",
			__func__, MAC2STR(sa));
...
out:
	...
	da16x_p2p_prt("[%s] Sending Provision Discovery Response\n", __func__);
	if (rx_freq > 0)
		freq = rx_freq;
	else
	...
	if (p2p_send_action(p2p, freq, sa, p2p->cfg->dev_addr,
		...
		da16x_p2p_prt("[%] Failed to send Action frame\n", __func__);
	} else
				...
				const u8 *data, size_t len)
{
	...
	da16x_p2p_prt("[%s] Received Provision Discovery Response from " MACSTR
		" with config methods 0x%x\n",
		__func__, MAC2STR(sa), msg.wps_config_methods);

	dev = p2p_get_device(p2p, sa);
	if (dev == NULL || !dev->req_config_methods) {
		da16x_p2p_prt("[%s] Ignore Provision Discovery Response from "
			MACSTR " with no pending request\n",
			__func__, MAC2STR(sa));
	...
	if (dev->dialog_token != msg.dialog_token) {
		da16x_p2p_prt("[%s] Ignore Provision Discovery Response with "
	...
	if (msg.wps_config_methods != req_config_methods) {
		da16x_p2p_prt("[%s] Peer rejected our Provision Discovery "
			"Request (received config_methods 0x%x expected 0x%x\n",
			__func__,
	...
	if (req_config_methods & WPS_CONFIG_DISPLAY) {
		da16x_p2p_prt("[%s] Peer " MACSTR " accepted to show a PIN "
			"on display\n", __func__, MAC2STR(sa));

		dev->flags |= P2P_DEV_PD_PEER_DISPLAY;
	} else if (msg.wps_config_methods & WPS_CONFIG_KEYPAD) {
		da16x_p2p_prt("[%s] Peer " MACSTR " accepted to write our "
			"PIN using keypad\n",
			__func__, MAC2STR(sa));
...
out:
	...
	if (dev->flags & P2P_DEV_PD_BEFORE_GO_NEG) {
		da16x_p2p_prt("[%s] Start GO Neg after the PD-before-GO-Neg "
			   ...
			   int join, int force_freq)
{
	...
	if (freq <= 0) {
		da16x_p2p_prt("[%s] No Listen/Operating frequency known for "
			"the peer " MACSTR " to send Provision Discovery "
			"Reques\nt",
			__func__,
	...
	if (dev->flags & P2P_DEV_GROUP_CLIENT_ONLY) {
		if (!(dev->info.dev_capab &
			...
			da16x_p2p_prt("[%s] Cannot use PD with P2P Device "
		      ...
		      int user_initiated_pd)
{
	...
	if (dev == NULL || (dev->flags & P2P_DEV_PROBE_REQ_ONLY)) {
		da16x_p2p_prt("[%s] Provision Discovery Request destination "
			MACSTR " not yet known\n",
			__func__, MAC2STR(peer_addr));

		return -1;
	}

	da16x_p2p_prt("[%s] Provision Discovery Request with " MACSTR
		" (config methods 0x%x)\n",
	...
	if (p2p->state != P2P_IDLE && p2p->state != P2P_SEARCH &&
		...
		da16x_p2p_prt("[%s] Busy with other operations; postpone "
