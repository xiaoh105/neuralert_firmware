...
u8 * p2p_buf_add_ie_hdr(struct wpabuf *buf)
{
	...
	da16x_p2p_prt("[%s] P2P: * P2P IE header\n", __func__);
...
void p2p_buf_add_capability(struct wpabuf *buf, u8 dev_capab, u8 group_capab)
{
	...
	da16x_p2p_prt("[%s] P2P: * Capability dev=%02x group=%02x\n",
		   __func__, dev_capab, group_capab);
}
...
void p2p_buf_add_go_intent(struct wpabuf *buf, u8 go_intent)
{
	...
	da16x_p2p_prt("[%s] P2P: * GO Intent: Intent %u Tie breaker %u\n",
		   __func__, go_intent >> 1, go_intent & 0x01);
}
				...
				u8 reg_class, u8 channel)
{
	...
	da16x_p2p_prt("[%s] P2P: * Listen Channel: Regulatory Class %u "
				   ...
				   u8 reg_class, u8 channel)
{
	...
	da16x_p2p_prt("[%s] P2P: * Operating Channel: Regulatory Class %u "
		   "Channel %u\n", __func__, reg_class, channel);
}
			     ...
			     struct p2p_device *peer)
{
	...
	da16x_p2p_prt("[%s] P2P: * Device Info\n", __func__);
}
				...
				u8 client_timeout)
{
	...
	da16x_p2p_prt("[%s] P2P: * Configuration Timeout: GO %d (*10ms)  "
		   "client %d (*10ms)\n",
			__func__, go_timeout, client_timeout);
}
...
void p2p_buf_add_intended_addr(struct wpabuf *buf, const u8 *interface_addr)
{
	...
	da16x_p2p_prt("[%s] P2P: * Intended P2P Interface Address " MACSTR "\n",
		   __func__, MAC2STR(interface_addr));
}
			  ...
			  const u8 *ssid, size_t ssid_len)
{
	...
	da16x_p2p_prt("[%s] P2P: * P2P Group ID " MACSTR "\n",
		   __func__, MAC2STR(dev_addr));

	da16x_ascii_dump("P2P: P2P Group ID SSID", ssid, ssid_len);
}
...
void p2p_buf_add_noa_def(struct wpabuf *buf, int ps)
{
	struct p2p_noa_desc desc, *ptr = NULL;
	...
	desc.interval = 102400;
	desc.start_time = 0x00056477;
	ptr = &desc;

	wpabuf_put_u8(buf, P2P_ATTR_NOTICE_OF_ABSENCE);
	wpabuf_put_le16(buf, 15);
	wpabuf_put_u8(buf, 17);
	//wpabuf_put_u8(buf, 0xad); //45, 50   
	wpabuf_put_u8(buf, 0xd5); // 85, 10

	p2p_buf_add_noa_desc(buf, ptr);
}
				   ...
				   u16 interval)
{
	...
	da16x_p2p_prt("[%s] P2P: * Extended Listen Timing (period %u msec  "
			"interval %u msec)\n",
				    ...
				    enum p2p_role_indication role)
{
	...
	da16x_p2p_prt("[%s] P2P: * OOB GO Negotiation Channel: Operating "
		   "Class %u Channel %u Role %d\n",
		   __func__, oper_class, channel, role);
}
...
da16x_p2p_prt("[%s] Have to include CONFIG_WPS features ...\n", __func__);
	...
	if (pw_id >= 0) {
		...
		da16x_p2p_prt("[%s] P2P: WPS IE Device Password ID: %d\n",
			   __func__, pw_id);
