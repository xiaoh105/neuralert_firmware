...
static void * eap_pwd_init(struct eap_sm *sm)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: Selected group number %d",
		   data->group_num);
...
static void eap_pwd_build_id_req(struct eap_sm *sm, struct eap_pwd_data *data,
				 u8 id)
{
	wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: ID/Request");
	/*
				     ...
				     struct eap_pwd_data *data, u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: Commit/Request");
	/*
				      ...
				      struct eap_pwd_data *data, u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: Confirm/Request");
	/*
...
eap_pwd_build_req(struct eap_sm *sm, void *priv, u8 id)
{
	 ...
	 */
	if (data->in_frag_pos) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: ACKing a fragment!!");
	...
	if ((len + EAP_PWD_HDR_SIZE) > data->mtu) {
		...
		if (data->out_frag_pos == 0) {
			...
			len -= sizeof(u16);
			wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: Fragmenting output, "
				   "total length = %d", totlen);
		}
		wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: Send a %d byte fragment",
			   (int) len);
	}
			     ...
			     struct wpabuf *respData)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: Received frame: exch = %d, len = %d",
				    ...
				    const u8 *payload, size_t payload_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PWD (server): computed %d bit PWE...",
			    ...
			    const u8 *payload, size_t payload_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: Received commit response");
			     ...
			     const u8 *payload, size_t payload_len)
{
	...
	*ptr = EAP_PWD_DEFAULT_PRF;
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-pwd (server): confirm verified");
			    ...
			    struct wpabuf *respData)
{
	...
	if (data->out_frag_pos) {
		if (len > 1) {
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: received ACK from "
				   "peer");
		}
	...
	if (EAP_PWD_GET_LENGTH_BIT(lm_exch)) {
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: Incoming fragments, total "
			   "length = %d", tot_len);
		if (tot_len > 15000)
			return;
		if (data->inbuf) {
	...
	if (EAP_PWD_GET_MORE_BIT(lm_exch) || data->in_frag_pos) {
		if ((data->in_frag_pos + len) > wpabuf_size(data->inbuf)) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: Buffer overflow "
	...
	if (EAP_PWD_GET_MORE_BIT(lm_exch)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-pwd: Got a %d byte fragment", (int) len);
		return;
	}
	/*
