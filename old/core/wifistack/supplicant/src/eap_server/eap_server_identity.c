					     ...
					     u8 id)
{
	...
	req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_IDENTITY, req_data_len,
			    EAP_CODE_REQUEST, id);
	if (req == NULL) {
		da16x_eap_prt("     [%s] EAP-Identity: Failed to allocate "
			   "memory for request\n", __func__);
		data->state = FAILURE;
		return NULL;
	}
				  ...
				  struct wpabuf *respData)
{
	...
	pos = eap_hdr_validate(EAP_VENDOR_IETF, EAP_TYPE_IDENTITY,
			       respData, &len);
	if (pos == NULL) {
		da16x_eap_prt("     [%s] EAP-Identity: Invalid frame\n", __func__);
		return TRUE;
	}
				 ...
				 struct wpabuf *respData)
{
	...
	if (data->pick_up) {
		if (eap_identity_check(sm, data, respData)) {
			da16x_eap_prt("     [%s] EAP-Identity: failed to pick "
	...
	da16x_dump("EAP-Identity: Peer identity", pos, len);
	buf = os_malloc(len * 4 + 1);
	if (buf) {
		printf_encode(buf, len * 4 + 1, pos, len);
		da16x_eap_prt("     [%s] EAP-Response/Identity '%s'\n", __func__, buf);
		os_free(buf);
	}
	if (sm->identity)
		sm->update_user = TRUE;
