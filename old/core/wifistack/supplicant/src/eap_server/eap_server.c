	...
	if (eap_sm_Policy_doPickUp(sm, sm->respMethod)) {
		...
		if (sm->m && sm->m->initPickUp) {
			...
			if (sm->eap_method_priv == NULL) {
				wpa_printf_dbg(MSG_DEBUG, "EAP: Failed to "
...
SM_STATE(EAP, METHOD_REQUEST)
{
	...
	if (sm->m == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: method not initialized");
		return;
	}

	sm->currentId = eap_sm_nextId(sm, sm->currentId);
	wpa_printf_dbg(MSG_DEBUG, "EAP: building EAP-Request: Identifier %d",
		   sm->currentId);
...
static void eap_server_erp_init(struct eap_sm *sm)
{
	...
	if (eap_erp_add_key(sm, erp) == 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: Stored ERP keys %s",
			   erp->keyname_nai);
		erp = NULL;
	}
...
try_another_method:
	...
	if (sm->m) {
		...
		if (sm->eap_method_priv == NULL) {
			wpa_printf_dbg(MSG_DEBUG, "EAP: Failed to initialize EAP "
	...
	if (sm->m == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: Could not find suitable EAP method");
...
SM_STATE(EAP, SELECT_ACTION)
{
	...
	sm->decision = (enum _decision) eap_sm_Policy_getDecision(sm);
}
				   ...
				   u8 flags, u16 seq, const char *nai)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP: Send EAP-Finish/Re-auth (%s)",
...
SM_STATE(EAP, INITIATE_RECEIVED)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP: ERP key %s SEQ updated to %u",
		   erp->keyname_nai, seq);
	erp->recv_seq = seq;
...
SM_STEP(EAP)
{
	if (sm->eap_if.eapRestart && sm->eap_if.portEnabled)
		...
		if (sm->num_rounds == EAP_MAX_AUTH_ROUNDS + 1) {
			wpa_printf_dbg(MSG_DEBUG, "EAP: more than %d "
	...
	case EAP_RECEIVED:
		if (sm->rxResp && (sm->respId == sm->currentId) &&
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP: RECEIVED->DISCARD: "
				   ...
				   int methodTimeout)
{
	...
	if (methodTimeout) {
		 ...
		 */
		wpa_printf_dbg(MSG_DEBUG, "EAP: retransmit timeout %d seconds "
...
static void eap_sm_parseEapResp(struct eap_sm *sm, const struct wpabuf *resp)
{
	...
	if (resp == NULL || wpabuf_len(resp) < sizeof(*hdr)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: parseEapResp: invalid resp=%p "
			   "len=%lu", resp,
...
static int eap_sm_getId(const struct wpabuf *data)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP: getId: id=%d", hdr->identifier);
...
static struct wpabuf * eap_sm_buildSuccess(struct eap_sm *sm, u8 id)
{
	struct wpabuf *msg;
	struct eap_hdr *resp;
	wpa_printf_dbg(MSG_DEBUG, "EAP: Building EAP-Success (id=%d)", id);
...
static struct wpabuf * eap_sm_buildFailure(struct eap_sm *sm, u8 id)
{
	struct wpabuf *msg;
	struct eap_hdr *resp;
	wpa_printf_dbg(MSG_DEBUG, "EAP: Building EAP-Failure (id=%d)", id);
				 ...
				 size_t len)
{
	...
	if (sm->user->phase2) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: EAP-Nak received after Phase2 user"
...
static EapType eap_sm_Policy_getNextMethod(struct eap_sm *sm, int *vendor)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP: getNextMethod: vendor %d type %d",
		   *vendor, next);
	return next;
}
...
static int eap_sm_Policy_getDecision(struct eap_sm *sm)
{
	if (!sm->eap_server && sm->identity && !sm->start_reauth) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: getDecision: -> PASSTHROUGH");
		return DECISION_PASSTHROUGH;
	}

	if (sm->m && sm->currentMethod != EAP_TYPE_IDENTITY &&
	    sm->m->isSuccess(sm, sm->eap_method_priv)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: getDecision: method succeeded -> "
			   "SUCCESS");
		sm->update_user = TRUE;
	...
	if (sm->m && sm->m->isDone(sm, sm->eap_method_priv) &&
	    !sm->m->isSuccess(sm, sm->eap_method_priv)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: getDecision: method failed -> "
			   "FAILURE");
		sm->update_user = TRUE;
	...
	if ((sm->user == NULL || sm->update_user) && sm->identity &&
		...
		if (eap_user_get(sm, sm->identity, sm->identity_len, 0) != 0) {
			wpa_printf_dbg(MSG_DEBUG, "EAP: getDecision: user not "
		...
		if (id_req && sm->user &&
		    sm->user->methods[0].vendor == EAP_VENDOR_IETF &&
		    sm->user->methods[0].method == EAP_TYPE_IDENTITY) {
			wpa_printf_dbg(MSG_DEBUG, "EAP: getDecision: stop "
	...
	if (sm->user && sm->user_eap_method_index < EAP_MAX_METHODS &&
	    (sm->user->methods[sm->user_eap_method_index].vendor !=
	     ...
	     EAP_TYPE_NONE)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: getDecision: another method "
			   "available -> CONTINUE");
	...
	if (sm->identity == NULL || sm->currentId == -1) {
		wpa_printf_dbg(MSG_DEBUG, "EAP: getDecision: no identity known "
			   "yet -> CONTINUE");
		return DECISION_CONTINUE;
	}

	wpa_printf_dbg(MSG_DEBUG, "EAP: getDecision: no more methods available -> "
		   "FAILURE");
	return DECISION_FAILURE;
}
				   ...
				   struct eap_config *conf)
{
	...
	sm->eap_fast_prov = (enum _eap_fast_prov) conf->eap_fast_prov;
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP: Server state machine created");
...
void eap_server_sm_deinit(struct eap_sm *sm)
{
	if (sm == NULL)
		return;
	wpa_printf_dbg(MSG_DEBUG, "EAP: Server state machine removed");
	if (sm->m && sm->eap_method_priv)
		sm->m->reset(sm, sm->eap_method_priv);
	wpabuf_free(sm->eap_if.eapReqData);
#ifdef	CONFIG_SUPP27_BIN_CLR_FREE
...
void eap_sm_pending_cb(struct eap_sm *sm)
{
	if (sm == NULL)
		return;
	wpa_printf_dbg(MSG_DEBUG, "EAP: Callback for pending request received");
	if (sm->method_pending == METHOD_PENDING_WAIT)
				   ...
				   const u8 *challenge, const u8 *response)
{
	...
	wpa_snprintf_hex_sep(hex_response, sizeof(hex_response), response, 24,
			     ':');
	wpa_printf_dbg(MSG_DEBUG, "[%s/user=%s] asleap -C %s -R %s",
