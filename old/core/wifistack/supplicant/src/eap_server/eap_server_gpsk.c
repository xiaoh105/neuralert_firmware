					     ...
					     struct eap_gpsk_data *data, u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Request/GPSK-1");
					     ...
					     struct eap_gpsk_data *data, u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Request/GPSK-3");
...
static struct wpabuf * eap_gpsk_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Unknown state %d in buildReq",
			   data->state);
		break;
	}
	return NULL;
			      ...
			      struct wpabuf *respData)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Received frame: opcode=%d", *pos);
				    ...
				    const u8 *payload, size_t payloadlen)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Received Response/GPSK-2");

	pos = payload;
	end = payload + payloadlen;

	if (end - pos < 2) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Too short message for "
			   "ID_Peer length");
		eap_gpsk_state(data, FAILURE);
		return;
	}
	...
	if (data->id_peer == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Not enough memory to store "
	...
	if (alen != sm->server_id_len ||
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: ID_Server in GPSK-1 and "
	...
	if (alen != data->csuite_count * sizeof(struct eap_gpsk_csuite) ||
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: CSuite_List in GPSK-1 and "
			   "GPSK-2 did not match");
		eap_gpsk_state(data, FAILURE);
		return;
	}
	...
	if (i == data->csuite_count) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Peer selected unsupported "
			   "ciphersuite %d:%d",
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: CSuite_Sel %d:%d",
				    ...
				    const u8 *payload, size_t payloadlen)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Received Response/GPSK-4");
