			     ...
			     const u8 *ker, size_t ker_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: KEr DH Group #%u", group);

	if (group != data->proposal.dh) {
		wpa_printf_dbg(MSG_DEBUG, "IKEV2: KEr DH Group #%u does not match "
				 ...
				 struct ikev2_payloads *pl)
{
	...
	if (pl->encrypted) {
		wpa_printf_dbg(MSG_DEBUG, "IKEV2: Encrypted payload in SA_INIT - "
			     ...
			     const u8 *idr, size_t idr_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: IDr ID Type %d", id_type);
	...
	if (data->IDr) {
		if (id_type != data->IDr_type || idr_len != data->IDr_len ||
			...
			wpa_printf(MSG_INFO, "IKEV2: IDr differs from the one "
				   "received earlier");
			wpa_printf_dbg(MSG_DEBUG, "IKEV2: Previous IDr ID Type %d",
				   id_type);
				     ...
				     size_t auth_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: Peer authenticated successfully "
		   "using shared keys");
				 ...
				 struct ikev2_payloads *pl)
{
	...
	if (ret == 0 && !data->unknown_user) {
		wpa_printf_dbg(MSG_DEBUG, "IKEV2: Authentication completed");
			    ...
			    struct wpabuf *msg, u8 next_payload)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: Adding SAi payload");
			   ...
			   struct wpabuf *msg, u8 next_payload)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: Adding KEi payload");
			  ...
			  struct wpabuf *msg, u8 next_payload)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: Adding Ni payload");
			   ...
			   struct wpabuf *msg, u8 next_payload)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: Adding IDi payload");
