					      ...
					      size_t *secret_len)
{
	...
	if (IDr == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: No IDr received - default "
	...
	if (eap_user_get(sm, IDr, IDr_len, 0) < 0 || sm->user == NULL ||
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: No user entry found");
		return NULL;
	}
...
{
	wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: %s -> %s",
...
static struct wpabuf * eap_ikev2_build_msg(struct eap_ikev2_data *data, u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Generating Request");
	...
	if (data->keys_ready) {
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Add Integrity Checksum "
			   "Data");
		flags |= IKEV2_FLAGS_ICV_INCLUDED;
		integ = ikev2_get_integ(data->ikev2.proposal.integ);
		if (integ == NULL) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Unknown INTEG "
	...
	if (data->out_used == wpabuf_len(data->out_buf)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Sending out %lu bytes "
...
static struct wpabuf * eap_ikev2_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	case MSG:
		if (data->out_buf == NULL) {
			...
			if (data->out_buf == NULL) {
				wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Failed to "
	...
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Unexpected state %d in "
			   "buildReq", data->state);
		return NULL;
	}
}
				  ...
				  const u8 *buf, size_t len)
{
	...
	if (len > wpabuf_tailroom(data->in_buf)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Fragment overflow");
		eap_ikev2_state(data, FAIL);
		return -1;
	}

	wpabuf_put_data(data->in_buf, buf, len);
	wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Received %lu bytes, waiting for %lu "
				      ...
				      const u8 *buf, size_t len)
{
	...
	if (data->in_buf == NULL && !(flags & IKEV2_FLAGS_LENGTH_INCLUDED)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: No Message Length field in "
			   "a fragmented packet");
		return -1;
	}
	...
	if (data->in_buf == NULL) {
		...
		if (data->in_buf == NULL) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: No memory for "
				   "message");
			return -1;
		}
		wpabuf_put_data(data->in_buf, buf, len);
		wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Received %lu bytes in first "
...
static int eap_ikev2_server_keymat(struct eap_ikev2_data *data)
{
	if (eap_ikev2_derive_keymat(
		    ...
		    data->keymat) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Failed to derive "
			   "key material");
		return -1;
	}
	data->keymat_ok = 1;
			      ...
			      struct wpabuf *respData)
{
	...
	if (flags & IKEV2_FLAGS_LENGTH_INCLUDED) {
		if (end - pos < 4) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Message underflow");
			eap_ikev2_state(data, FAIL);
			return;
		}
		...
		if (message_length < (u32) (end - pos)) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Invalid Message "
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Received packet: Flags 0x%x "
		   "Message Length %u", flags, message_length);

	if (data->state == WAIT_FRAG_ACK) {
		if (len != 0) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Unexpected payload "
				   "in WAIT_FRAG_ACK state");
			eap_ikev2_state(data, FAIL);
			return;
		}
		wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Fragment acknowledged");
	...
	case IKEV2_DONE:
		if (data->state == FAIL)
			break;
		wpa_printf_dbg(MSG_DEBUG, "EAP-IKEV2: Authentication completed "
			   "successfully");
