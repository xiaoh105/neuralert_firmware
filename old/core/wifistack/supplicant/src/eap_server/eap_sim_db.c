...
static int db_table_create_pseudonym(sqlite3 *db)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: Adding database table for "
				     ...
				     const char *imsi, char *buf)
{
	...
	if (entry == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: No pending entry for the "
	...
	if (os_strncmp(start, "FAILURE", 7) == 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: External server reported "
			   "failure");
		entry->state = FAILURE;
	...
	entry->state = SUCCESS;
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: Authentication data parsed "
...
parse_fail:
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: Failed to parse response string");
	eap_sim_db_free_pending(data, entry);
}
				     ...
				     const char *imsi, char *buf)
{
	...
	*end = '\0';
	...
	if (entry->u.aka.res_len > EAP_AKA_RES_MAX_LEN) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: Too long RES");
...
static void eap_sim_db_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	if (data->get_complete_cb == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: No get_complete_cb "
			   "registered");
		return;
	}
	...
	*pos = '\0';
	...
	if (pos == NULL)
		goto parse_fail;
	*pos = '\0';
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: External response=%s for IMSI %s",
		   cmd, imsi);
		...
		void *ctx)
{
	...
	if (os_strncmp(data->fname, "unix:", 5) == 0) {
		if (eap_sim_db_open_socket(data)) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: External database "
			   ...
			   size_t len)
{
	...
	if (_errno == ENOTCONN || _errno == EDESTADDRREQ || _errno == EINVAL ||
		...
		if (eap_sim_db_open_socket(data) < 0)
			return -1;
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: Reconnected to the "
			   "external server");
		if (send(data->sock, msg, len, 0) < 0) {
			wpa_printf(MSG_INFO, "send[EAP-SIM DB UNIX]: %s",
				   strerror(errno));
				...
				void *cb_session_ctx)
{
	...
	if (username == NULL || username[0] != EAP_SIM_PERMANENT_PREFIX ||
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: unexpected username '%s'",
			   username);
		return EAP_SIM_DB_FAILURE;
	}
	imsi = username + 1;
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: Get GSM triplets for IMSI '%s'",
		   imsi);
	...
	if (entry) {
		...
		if (entry->state == FAILURE) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: Pending entry -> "
				   "failure");
			eap_sim_db_free_pending(data, entry);
			return EAP_SIM_DB_FAILURE;
		}
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: requesting SIM authentication "
			     ...
			     const char *permanent, char *pseudonym)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: Add pseudonym '%s' for permanent "
	...
	if (p) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: Replacing previous "
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: Added new pseudonym entry");
	return 0;
}
			   ...
			   char *reauth_id, u16 counter)
{
	...
	if (r) {
		...
		r->reauth_id = reauth_id;
		data->reauths = r;
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: Added new reauth entry");
	}
			  ...
			  char *reauth_id, u16 counter, const u8 *mk)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: Add reauth_id '%s' for permanent "
			    ...
			    u8 *res, size_t *res_len, void *cb_session_ctx)
{
	...
	imsi = username + 1;
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: Get AKA auth for IMSI '%s'",
		   imsi);
	...
	if (entry) {
		if (entry->state == FAILURE) {
			eap_sim_db_free_pending(data, entry);
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: Failure");
			return EAP_SIM_DB_FAILURE;
		}

		if (entry->state == PENDING) {
			eap_sim_db_add_pending(data, entry);
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: Pending");
			return EAP_SIM_DB_PENDING;
		}

		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: Returning successfully "
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: requesting AKA authentication "
		    "data for IMSI '%s'", imsi);
	if (eap_sim_db_send(data, msg, len) < 0)
		return EAP_SIM_DB_FAILURE;
			     ...
			     const u8 *auts, const u8 *_rand)
{
	...
	if (data->sock >= 0) {
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM DB: reporting AKA AUTS for "
			   "IMSI '%s'", imsi);
		if (eap_sim_db_send(data, msg, len) < 0)
			return -1;
	}
