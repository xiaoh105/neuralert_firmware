...
{
	wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: %s -> %s",
...
static int eap_ttls_avp_parse(struct wpabuf *buf, struct eap_ttls_avp *parse)
{
	...
	while (left > 0) {
		...
		avp_length &= 0xffffff;
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: AVP: code=%d flags=0x%02x "
		...
		if (avp_flags & AVP_FLAGS_VENDOR) {
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: AVP vendor_id %d",
				   (int) vendor_id);
			dpos += 4;
			dlen -= 4;
		}
		...
		if (vendor_id == 0 && avp_code == RADIUS_ATTR_EAP_MESSAGE) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: AVP - EAP Message");
			if (parse->eap == NULL) {
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: Ignoring unsupported "
	...
	struct eap_sm *sm, struct eap_ttls_data *data, u8 id)
{
	...
	if (buf == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/EAP: Failed to encapsulate "
			   "packet");
		return NULL;
	}
...
static struct wpabuf * eap_ttls_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	case PHASE1:
		if (tls_connection_established(sm->ssl_ctx, data->ssl.conn)) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: Phase1 done, "
				   "starting Phase2");
	...
	case PHASE2_MSCHAPV2_RESP:
		...
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: %s - unexpected state %d",
			   __func__, data->state);
		return NULL;
	}
					...
					size_t user_password_len)
{
	if (!sm->user || !sm->user->password || sm->user->password_hash ||
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/PAP: No plaintext user "
			   "password configured");
		eap_ttls_state(data, FAILURE);
		return;
	}
	...
	if (sm->user->password_len != user_password_len ||
	    os_memcmp_const(sm->user->password, user_password,
			    user_password_len) != 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/PAP: Invalid user password");
		eap_ttls_state(data, FAILURE);
		return;
	}

	wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/PAP: Correct user password");
					 ...
					 size_t password_len)
{
	...
	if (challenge == NULL || password == NULL ||
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/CHAP: Invalid CHAP attributes "
	...
	if (!sm->user || !sm->user->password || sm->user->password_hash ||
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/CHAP: No plaintext user "
			   "password configured");
		eap_ttls_state(data, FAILURE);
		return;
	}
	...
	if (chal == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/CHAP: Failed to generate "
	...
	if (os_memcmp_const(challenge, chal, EAP_TTLS_CHAP_CHALLENGE_LEN)
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/CHAP: Challenge mismatch");
		os_free(chal);
		eap_ttls_state(data, FAILURE);
		return;
	}
	...
	if (os_memcmp_const(hash, password + 1, EAP_TTLS_CHAP_PASSWORD_LEN) ==
	    0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/CHAP: Correct user password");
		eap_ttls_state(data, SUCCESS);
		eap_ttls_valid_session(sm, data);
	} else {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/CHAP: Invalid user password");
		eap_ttls_state(data, FAILURE);
	}
}
					   ...
					   u8 *response, size_t response_len)
{
	...
	if (challenge == NULL || response == NULL ||
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/MSCHAP: Invalid MS-CHAP "
	...
	if (!sm->user || !sm->user->password ||
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/MSCHAP: No user password "
			   "configured");
		eap_ttls_state(data, FAILURE);
		return;
	}
	...
	if (chal == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/MSCHAP: Failed to generate "
			   "challenge from TLS data");
		eap_ttls_state(data, FAILURE);
		return;
	}
	...
	if (os_memcmp_const(challenge, chal, EAP_TTLS_MSCHAP_CHALLENGE_LEN)
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/MSCHAP: Challenge mismatch");
		os_free(chal);
		eap_ttls_state(data, FAILURE);
		return;
	}
	...
	if (os_memcmp_const(nt_response, response + 2 + 24, 24) == 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/MSCHAP: Correct response");
		eap_ttls_state(data, SUCCESS);
		eap_ttls_valid_session(sm, data);
	} else {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/MSCHAP: Invalid NT-Response");
					     ...
					     u8 *response, size_t response_len)
{
	...
	if (challenge == NULL || response == NULL ||
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/MSCHAPV2: Invalid MS-CHAP2 "
			   "attributes (challenge len %lu response len %lu)",
			   (unsigned long) challenge_len,
			   (unsigned long) response_len);
		eap_ttls_state(data, FAILURE);
	...
	if (!sm->user || !sm->user->password ||
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/MSCHAPV2: No user password "
			   "configured");
		eap_ttls_state(data, FAILURE);
		return;
	}
	...
	if (sm->identity == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/MSCHAPV2: No user identity "
			   "known");
		eap_ttls_state(data, FAILURE);
		return;
	}
	...
	if (chal == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/MSCHAPV2: Failed to generate "
			   "challenge from TLS data");
		eap_ttls_state(data, FAILURE);
		return;
	}
	...
	if (os_memcmp_const(challenge, chal, EAP_TTLS_MSCHAPV2_CHALLENGE_LEN)
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/MSCHAPV2: Challenge mismatch");
		os_free(chal);
		eap_ttls_state(data, FAILURE);
		return;
	}
	...
	if (os_memcmp_const(nt_response, rx_resp, 24) == 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/MSCHAPV2: Correct "
			   "NT-Response");
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/MSCHAPV2: Invalid "
			   "NT-Response");
						 ...
						 u8 *in_data, size_t in_len)
{
	...
	if (priv == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/EAP: %s - Phase2 not "
	...
	if (in_len > sizeof(*hdr) && *pos == EAP_TYPE_NAK) {
		...
		if (sm->user && sm->user_eap_method_index < EAP_MAX_METHODS &&
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: try EAP type %d",
				   next_type);
			if (eap_ttls_phase2_eap_init(sm, data, next_type)) {
				wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: Failed to "
	...
	if (m->check(sm, priv, &buf)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/EAP: Phase2 check() asked to "
			   "ignore the packet");
		return;
	}
	...
	if (sm->method_pending == METHOD_PENDING_WAIT) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/EAP: Phase2 method is in "
	...
	if (!m->isSuccess(sm, priv)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/EAP: Phase2 method failed");
		eap_ttls_state(data, FAILURE);
		return;
	}
	...
	case PHASE2_START:
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: try EAP type %d", next_type);
		if (eap_ttls_phase2_eap_init(sm, data, next_type)) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: Failed to initialize "
					...
					const u8 *eap, size_t eap_len)
{
	...
	if (data->state == PHASE2_START) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/EAP: initializing Phase 2");
		if (eap_ttls_phase2_eap_init(sm, data, EAP_TYPE_IDENTITY) < 0)
		{
			wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/EAP: failed to "
	...
	if (eap_len < sizeof(*hdr)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/EAP: too short Phase 2 EAP "
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/EAP: received Phase 2 EAP: code=%d "
				    ...
				    struct wpabuf *in_buf)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: received %lu bytes encrypted data for"
		   " Phase 2", (unsigned long) wpabuf_len(in_buf));

	if (data->pending_phase2_eap_resp) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: Pending Phase 2 EAP response "
	...
	if (eap_ttls_avp_parse(in_decrypted, &parse) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: Failed to parse AVPs");
	...
	if (parse.user_name) {
		...
		if (eap_user_get(sm, parse.user_name, parse.user_name_len, 1)
		    != 0) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: Phase2 Identity not "
	...
	if (data->tnc_started && parse.eap == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: TNC started but no EAP "
			   "response from peer");
		eap_ttls_state(data, FAILURE);
		goto done;
	}
...
static void eap_ttls_start_tnc(struct eap_sm *sm, struct eap_ttls_data *data)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: Initialize TNC");
	if (eap_ttls_phase2_eap_init(sm, data, EAP_TYPE_TNC)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: Failed to initialize TNC");
		eap_ttls_state(data, FAILURE);
		return;
	}
				    ...
				    int peer_version)
{
	...
	if (peer_version < data->ttls_version) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: peer ver=%d, own ver=%d; "
			   "use version %d",
				 ...
				 const struct wpabuf *respData)
{
	...
	case PHASE2_MSCHAPV2_RESP:
		if (data->mschapv2_resp_ok && wpabuf_len(data->ssl.tls_in) ==
		    0) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/MSCHAPV2: Peer "
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS/MSCHAPV2: Unexpected "
		...
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: Unexpected state %d in %s",
			   data->state, __func__);
		break;
	}
}
...
static u8 * eap_ttls_getKey(struct eap_sm *sm, void *priv, size_t *len)
{
	...
	if (eapKeyData) {
		*len = EAP_TLS_KEY_LEN;
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-TTLS: Failed to derive key");
	}
