					   ...
					   struct eap_pax_data *data, u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PAX: PAX_STD-1 (sending)");
					   ...
					   struct eap_pax_data *data, u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PAX: PAX_STD-3 (sending)");
...
static struct wpabuf * eap_pax_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-PAX: Unknown state %d in buildReq",
			   data->state);
		break;
	}
	return NULL;
			     ...
			     struct wpabuf *respData)
{
	...
	if (data->state == PAX_STD_1 &&
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-PAX: Expected PAX_STD-2 - "
	...
	if (data->state == PAX_STD_3 &&
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-PAX: Expected PAX-ACK - "
			   "ignore op %d", resp->op_code);
		return TRUE;
	}
	...
	if (resp->op_code != EAP_PAX_OP_STD_2 &&
	    resp->op_code != EAP_PAX_OP_ACK) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PAX: Unknown op_code 0x%x",
			   resp->op_code);
	}

	if (data->mac_id != resp->mac_id) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PAX: Expected MAC ID 0x%x, "
				  ...
				  struct wpabuf *respData)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PAX: Received PAX_STD-2");
				...
				struct wpabuf *respData)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PAX: Received PAX-ACK - authentication "
