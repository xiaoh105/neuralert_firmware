					     ...
					     u8 id)
{
	...
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-MSCHAPV2: Unknown state %d in "
			   "buildReq", data->state);
		break;
	}
	return NULL;
				  ...
				  struct wpabuf *respData)
{
	...
	if (data->state == CHALLENGE &&
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-MSCHAPV2: Expected Response - "
	...
	if (data->state == SUCCESS_REQ &&
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-MSCHAPV2: Expected Success or "
	...
	if (data->state == FAILURE_REQ &&
	    resp->op_code != MSCHAPV2_OP_FAILURE) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-MSCHAPV2: Expected Failure "
			   "- ignore op %d", resp->op_code);
		return TRUE;
	}
					  ...
					  struct wpabuf *respData)
{
	...
	if (data->peer_challenge) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-MSCHAPV2: Using pre-configured "
			   "Peer-Challenge");
	...
	if (username_len != user_len ||
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-MSCHAPV2: Mismatch in user names");
	...
	if (os_memcmp_const(nt_response, expected, 24) == 0) {
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-MSCHAPV2: Correct NT-Response");
		...
		wpa_hexdump(MSG_MSGDUMP, "EAP-MSCHAPV2: Expected NT-Response",
			    expected, 24);
		wpa_printf_dbg(MSG_DEBUG, "EAP-MSCHAPV2: Invalid NT-Response");
					      ...
					      struct wpabuf *respData)
{
	...
	if (resp->op_code == MSCHAPV2_OP_SUCCESS) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-MSCHAPV2: Received Success Response"
			   " - authentication completed successfully");
		data->state = SUCCESS;
	} else {
		wpa_printf_dbg(MSG_DEBUG, "EAP-MSCHAPV2: Did not receive Success "
					      ...
					      struct wpabuf *respData)
{
	...
	if (resp->op_code == MSCHAPV2_OP_FAILURE) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-MSCHAPV2: Received Failure Response"
			   " - authentication failed");
	} else {
		wpa_printf_dbg(MSG_DEBUG, "EAP-MSCHAPV2: Did not receive Failure "
