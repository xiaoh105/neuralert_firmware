					 ...
					 int eap_type, int version, u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "SSL: Generating Request");
	...
	if (data->tls_out_pos == wpabuf_len(data->tls_out)) {
		wpa_printf_dbg(MSG_DEBUG, "SSL: Sending out %lu bytes "
...
struct wpabuf * eap_server_tls_build_ack(u8 id, int eap_type, int version)
{
	...
	if (req == NULL)
		return NULL;
	wpa_printf_dbg(MSG_DEBUG, "SSL: Building ACK");
				       ...
				       const u8 *buf, size_t len)
{
	...
	if (len > wpabuf_tailroom(data->tls_in)) {
		wpa_printf_dbg(MSG_DEBUG, "SSL: Fragment overflow");
		return -1;
	}

	wpabuf_put_data(data->tls_in, buf, len);
	wpa_printf_dbg(MSG_DEBUG, "SSL: Received %lu bytes, waiting for %lu "
					   ...
					   const u8 *buf, size_t len)
{
	...
	if (data->tls_in == NULL && !(flags & EAP_TLS_FLAGS_LENGTH_INCLUDED)) {
		wpa_printf_dbg(MSG_DEBUG, "SSL: No Message Length field in a "
			   "fragmented packet");
		return -1;
	}
	...
	if (data->tls_in == NULL) {
		...
		if (data->tls_in == NULL) {
			wpa_printf_dbg(MSG_DEBUG, "SSL: No memory for message");
			return -1;
		}
		wpabuf_put_data(data->tls_in, buf, len);
		wpa_printf_dbg(MSG_DEBUG, "SSL: Received %lu bytes in first "
...
int eap_server_tls_phase1(struct eap_sm *sm, struct eap_ssl_data *data)
{
	...
	if (tls_connection_get_failed(sm->ssl_ctx, data->conn)) {
		...
		wpa_printf_dbg(MSG_DEBUG, "SSL: Failed - tls_out available to "
			   "report error");
		return -1;
	}
				     ...
				     const u8 **pos, size_t *left)
{
	...
	if (flags & EAP_TLS_FLAGS_LENGTH_INCLUDED) {
		...
		wpa_printf_dbg(MSG_DEBUG, "SSL: TLS Message Length: %d",
			   tls_msg_len);
		*pos += 4;
		*left -= 4;
	...
	wpa_printf_dbg(MSG_DEBUG, "SSL: Received packet: Flags 0x%x "
		   "Message Length %u", flags, tls_msg_len);

	if (data->state == WAIT_FRAG_ACK) {
		if (*left != 0) {
			wpa_printf_dbg(MSG_DEBUG, "SSL: Unexpected payload in "
				   "WAIT_FRAG_ACK state");
			return -1;
		}
		wpa_printf_dbg(MSG_DEBUG, "SSL: Fragment acknowledged");
		return 1;
	}
	...
	if (data->state == FRAG_ACK) {
		wpa_printf_dbg(MSG_DEBUG, "SSL: All fragments received");
		data->state = MSG;
	}
					    ...
					    const struct wpabuf *respData))
{
	...
	flags = *pos++;
	left--;
	wpa_printf_dbg(MSG_DEBUG, "SSL: Received packet(len=%lu) - Flags 0x%02x",
