...
static int wps_build_e_hash(struct wps_data *wps, struct wpabuf *msg)
{
	...
	da16x_dump("WPS: E-S1", wps->snonce, WPS_SECRET_NONCE_LEN);
	da16x_dump("WPS: E-S2",
...
#ifdef ENABLE_WPS_DBG
	da16x_dump("WPS: E-Hash1", hash, SHA256_MAC_LEN);
#endif

	da16x_wps_prt("[%s] WPS:  * E-Hash2\n", __func__);
	...
	hmac_sha256_vector(wps->authkey, WPS_AUTHKEY_LEN, 4, addr, len, hash);
#ifdef ENABLE_WPS_DBG
	da16x_dump("WPS: E-Hash2", hash, SHA256_MAC_LEN);
#endif
	return 0;
}
...
static struct wpabuf * wps_build_m1(struct wps_data *wps)
{
	...
	da16x_wps_prt("[%s] WPS: Building Message M1\n", __func__);
...
static struct wpabuf * wps_build_m3(struct wps_data *wps)
{
	...
	da16x_wps_prt("[%s] WPS: Building Message M3\n", __func__);

	if (wps->dev_password == NULL) {
		da16x_wps_prt("[%s] WPS: No Device Password available\n", __func__);
		return NULL;
	}
...
static struct wpabuf * wps_build_m5(struct wps_data *wps)
{
	...
	da16x_wps_prt("[%s] WPS: Building Message M5\n", __func__);
...
static int wps_build_cred_network_key(struct wps_data *wps, struct wpabuf *msg)
{
	if ((wps->wps->ap_auth_type & (WPS_AUTH_WPAPSK | WPS_AUTH_WPA2PSK)) &&
		...
		da16x_dump("WPS: Generated per-device PSK",
				psk, sizeof(psk));
#endif
		da16x_wps_prt("[%s] WPS:  * Network Key (len=%u)\n", __func__,
...
{
	da16x_wps_prt("[%s] WPS:  * MAC Address (AP BSSID)\n", __func__);
...
static int wps_build_ap_settings(struct wps_data *wps, struct wpabuf *plain)
{
	...
	if (wps->wps->ap_settings) {
		da16x_wps_prt("[%s] WPS:  * AP Settings (pre-configured)\n",
			 __func__);
	...
	da16x_wps_prt("[%s] WPS:  * AP Settings based on current "
			"configuration\n", __func__);
...
#ifdef ENABLE_WPS_DBG
	da16x_dump("WPS: Plaintext AP Settings", start, end - start);
#endif
...
static struct wpabuf * wps_build_m7(struct wps_data *wps)
{
	...
	da16x_wps_prt("[%s] WPS: Building Message M7\n", __func__);
...
static struct wpabuf * wps_build_wsc_done(struct wps_data *wps)
{
	...
	da16x_wps_prt("[%s] WPS: Building Message WSC_Done\n", __func__);
...
static int wps_process_enrollee_nonce(struct wps_data *wps, const u8 *e_nonce)
{
	...
	if (os_memcmp(wps->nonce_e, e_nonce, WPS_NONCE_LEN) != 0) {
		da16x_wps_prt("[%s] WPS: Invalid Enrollee Nonce received\n", __func__);
		return -1;
	}
...
{
	if (uuid_r == NULL) {
		da16x_wps_prt("[%s] WPS: No UUID-R received\n", __func__);
		return -1;
	}
			      ...
			      size_t pk_len)
{
	...
	if (wps->peer_pubkey_hash_set) {
		...
		if (os_memcmp(hash, wps->peer_pubkey_hash,
			...
			da16x_wps_prt("[%s] WPS: Public Key hash mismatch\n", __func__);
#ifdef ENABLE_WPS_DBG
			da16x_dump("WPS: Received public key",
				    pk, pk_len);
			da16x_dump("WPS: Calculated public key "
				    "hash", hash, WPS_OOB_PUBKEY_HASH_LEN);
			da16x_dump("WPS: Expected public key hash",
...
{
	if (r_hash1 == NULL) {
		da16x_wps_prt("[%s] WPS: No R-Hash1 received\n", __func__);
		return -1;
	}

	os_memcpy(wps->peer_hash1, r_hash1, WPS_HASH_LEN);
#ifdef ENABLE_WPS_DBG
	da16x_dump("WPS: R-Hash1", wps->peer_hash1, WPS_HASH_LEN);
#endif
...
{
	if (r_hash2 == NULL) {
		da16x_wps_prt("[%s] WPS: No R-Hash2 received\n", __func__);
		return -1;
	}

	os_memcpy(wps->peer_hash2, r_hash2, WPS_HASH_LEN);
#ifdef ENABLE_WPS_DBG
	da16x_dump("WPS: R-Hash2", wps->peer_hash2, WPS_HASH_LEN);
#endif
...
static int wps_process_r_snonce1(struct wps_data *wps, const u8 *r_snonce1)
{
	...
	if (r_snonce1 == NULL) {
		da16x_wps_prt("[%s] WPS: No R-SNonce1 received\n", __func__);
		return -1;
	}

#ifdef ENABLE_WPS_DBG
	da16x_dump("WPS: R-SNonce1", r_snonce1,
			WPS_SECRET_NONCE_LEN);
#endif
	...
	if (os_memcmp(wps->peer_hash1, hash, WPS_HASH_LEN) != 0) {
		da16x_wps_prt("[%s] WPS: R-Hash1 derived from R-S1 does "
	...
	da16x_wps_prt("[%s] WPS: Registrar proved knowledge of the first "
...
static int wps_process_r_snonce2(struct wps_data *wps, const u8 *r_snonce2)
{
	...
	if (r_snonce2 == NULL) {
		da16x_wps_prt("[%s] WPS: No R-SNonce2 received\n", __func__);
		return -1;
	}

#ifdef ENABLE_WPS_DBG
	da16x_dump("WPS: R-SNonce2", r_snonce2,
			WPS_SECRET_NONCE_LEN);
#endif
	...
	if (os_memcmp(wps->peer_hash2, hash, WPS_HASH_LEN) != 0) {
		da16x_wps_prt("[%s] WPS: R-Hash2 derived from R-S2 does "
			   "not match with the pre-committed value\n", __func__);
		wps->config_error = WPS_CFG_DEV_PASSWORD_AUTH_FAILURE;
	...
	da16x_wps_prt("[%s] WPS: Registrar proved knowledge of the second "
		   "half of the device password\n", __func__);
			      ...
			      size_t cred_len, int wps2)
{
	...
	da16x_wps_prt("[%s] WPS: Received Credential\n", __func__);
	...
	if (os_memcmp(wps->cred.mac_addr, wps->wps->dev.mac_addr, ETH_ALEN) !=
	    0) {
		da16x_wps_prt("[%s] WPS: MAC Address in the Credential ("
			   MACSTR ") does not match with own address (" MACSTR
			   ")\n", __func__, MAC2STR(wps->cred.mac_addr),
		...
		if (wps2) {
			da16x_wps_prt("[%s] WPS: Do not accept incorrect "
	...
	if (!(wps->cred.encr_type &
		...
		if (wps->cred.encr_type & WPS_ENCR_WEP) {
			da16x_wps_prt("[%s] WPS: Reject Credential "
				   "due to WEP configuration\n", __func__);
			wps->error_indication = WPS_EI_SECURITY_WEP_PROHIBITED;
			return -2;
		}

		da16x_wps_prt("[%s] WPS: Reject Credential due to "
			   "invalid encr_type 0x%x\n", __func__, wps->cred.encr_type);
		return -1;
	}
			     ...
			     size_t cred_len[], size_t num_cred, int wps2)
{
	...
	if (num_cred == 0) {
		da16x_wps_prt("[%s] WPS: No Credential attributes "
			   "received\n", __func__);
		return -1;
	}
	...
	for (i = 0; i < num_cred; i++) {
		...
		if (res == 0)
			...
			da16x_wps_prt("[%s] WPS: WEP credential skipped\n", __func__);
		} else {
			return -1;
		}
	}
	...
	if (ok == 0) {
		da16x_wps_prt("[%s] WPS: No valid Credential attribute "
			   "received\n", __func__);
		return -1;
	}
				     ...
				     struct wpabuf *attrs, int wps2)
{
	...
	da16x_wps_prt("[%s] WPS: Received new AP configuration from "
		   "Registrar\n", __func__);

	if (os_memcmp(cred.mac_addr, wps->wps->dev.mac_addr, ETH_ALEN) !=
	    0) {
		da16x_wps_prt("[%s] WPS: MAC Address in the AP Settings ("
			   MACSTR ") does not match with own address (" MACSTR
			   ")\n", __func__, MAC2STR(cred.mac_addr),
			   MAC2STR(wps->wps->dev.mac_addr));
		/*
		 * In theory, this could be consider fatal error, but there are
		 * number of deployed implementations using other address here
	...
	if (wps2) {
		if ((cred.encr_type & (WPS_ENCR_TKIP | WPS_ENCR_AES)) ==
		    ...
		    WPS_AUTH_WPAPSK) {
			da16x_wps_prt("[%s] WPS-STRICT: Invalid WSC 2.0 "
...
static int wps_process_dev_pw_id(struct wps_data *wps, const u8 *dev_pw_id)
{
	...
	if (dev_pw_id == NULL) {
		da16x_wps_prt("[%s] WPS: Device Password ID\n", __func__);
		return -1;
	}

	id = WPA_GET_BE16(dev_pw_id);
	if (wps->dev_pw_id == id) {
		da16x_wps_prt("[%s] WPS: Device Password ID %u\n", __func__, id);
		return 0;
	}
	...
	if ((id == DEV_PW_DEFAULT &&
		 ...
		 */
		da16x_wps_prt("[%s] WPS: Allow PIN Device Password ID "
			   "change\n", __func__);
		return 0;
	}
#endif /* CONFIG_P2P */

	da16x_wps_prt("[%s] WPS: Registrar trying to change Device Password "
		   "ID from %u to %u\n", __func__, wps->dev_pw_id, id);

	if (wps->dev_pw_id == DEV_PW_PUSHBUTTON && id == DEV_PW_DEFAULT) {
		da16x_wps_prt("[%s] WPS: Workaround - ignore PBC-to-PIN change\n", __func__);
		return 0;
	}

	if (wps->alt_dev_password && wps->alt_dev_pw_id == id) {
		da16x_wps_prt("[%s] WPS: Found a matching Device Password\n", __func__);
...
static enum wps_process_res wps_process_m2(struct wps_data *wps,
					   ...
					   struct wps_parse_attr *attr)
{
	da16x_wps_prt("[%s] WPS: Received M2\n", __func__);
	...
	if (wps->wps->ap &&
		...
		da16x_wps_prt("[%s] WPS: AP Setup is locked - refuse "
...
static enum wps_process_res wps_process_m2d(struct wps_data *wps,
					    struct wps_parse_attr *attr)
{
	da16x_wps_prt("[%s] WPS: Received M2D\n", __func__);
					   ...
					   struct wps_parse_attr *attr)
{
	...
	da16x_wps_prt("[%s] WPS: Received M4\n", __func__);
					   ...
					   struct wps_parse_attr *attr)
{
	...
	da16x_wps_prt("[%s] WPS: Received M6\n", __func__);
					   ...
					   struct wps_parse_attr *attr)
{
	...
	da16x_wps_prt("[%s] WPS: Received M8\n", __func__);
						...
						const struct wpabuf *msg)
{
	...
	if (wps->state == RECV_ACK && wps->wps->ap) {
		da16x_wps_prt("[%s] WPS: External Registrar registration "
						 ...
						 const struct wpabuf *msg)
{
	...
	{
		da16x_wps_prt("[%s] WPS: Mismatch in registrar nonce\n", __func__);
		da16x_dump("WPS: Received Registrar Nonce",
			    attr.registrar_nonce, WPS_NONCE_LEN);
		da16x_dump("WPS: Expected Registrar Nonce",
			    wps->nonce_r, WPS_NONCE_LEN);
		return WPS_FAILURE;
	}
	...
	if (attr.enrollee_nonce == NULL ||
	    os_memcmp(wps->nonce_e, attr.enrollee_nonce, WPS_NONCE_LEN) != 0) {
		da16x_wps_prt("[%s] WPS: Mismatch in enrollee nonce\n", __func__);
		da16x_dump("WPS: Received Enrollee Nonce",
			    attr.enrollee_nonce, WPS_NONCE_LEN);
		da16x_dump("WPS: Expected Enrollee Nonce",
			    wps->nonce_e, WPS_NONCE_LEN);
		return WPS_FAILURE;
	}
	...
	da16x_wps_prt("[%s] WPS: Registrar terminated negotiation with "
					      ...
					      const struct wpabuf *msg)
{
	...
	da16x_wps_prt("[%s] WPS: Processing received message (len=%lu "
		   "op_code=%d)\n", __func__,
