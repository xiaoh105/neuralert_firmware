...
struct wps_data * wps_init(const struct wps_config *cfg)
{
	...
	if (cfg->pin) {
		...
		da16x_dump("WPS: AP PIN dev_password",
...
#ifdef ENABLE_WPS_DBG
		da16x_buf_dump("WPS: WPS IE from (Re)AssocReq",
				cfg->assoc_wps_ie);
#endif

		if (wps_parse_msg(cfg->assoc_wps_ie, &attr) < 0) {
			da16x_wps_prt("     [%s] WPS: Failed to parse WPS IE "
				   "from (Re)AssocReq\n", __func__);
		} else if (attr.request_type == NULL) {
			da16x_wps_prt("     [%s] WPS: No Request Type attribute "
				   "in (Re)AssocReq WPS IE\n", __func__);
		} else {
			da16x_wps_prt("     [%s] WPS: Request Type (from WPS IE "
	...
	if (cfg->new_ap_settings) {
		data->new_ap_settings =
			os_memdup(cfg->new_ap_settings, sizeof(*data->new_ap_settings));
...
void wps_deinit(struct wps_data *data)
{
	if (data->wps_pin_revealed) {
		da16x_wps_prt("     [%s] WPS: Full PIN information revealed and "
			   "negotiation failed\n", __func__);
		if (data->registrar) {
...
#if 0	/* by Shingu 20160921 (WPS Optimize) */
       	rsadeinit();
#endif	/* 0 */
}
...
struct wpabuf * wps_build_assoc_req_ie(enum wps_request_type req_type)
{
	...
	da16x_wps_prt("     [%s] WPS: Building WPS IE for (Re)Association "
		   "Request\n", __func__);
	ie = wpabuf_alloc(100);
	if (ie == NULL)
		return NULL;
...
struct wpabuf * wps_build_assoc_resp_ie(void)
{
	...
	da16x_wps_prt("[%s] Building WPS IE for (Re)Association Response\n", __func__);
	ie = wpabuf_alloc(100);
	if (ie == NULL) {
		da16x_wps_prt("[%s] Failed to alloc buffer (100 bytes)\n", __func__);
		return NULL;
	}
				       ...
				       const u8 *req_dev_types)
{
	...
	da16x_wps_prt("     [%s] WPS: Building WPS IE for Probe Request\n", __func__);
