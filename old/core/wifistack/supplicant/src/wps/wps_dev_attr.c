...
{
	size_t len;
	da16x_wps_prt("     [%s] WPS:  * Manufacturer\n", __func__);
...
{
	size_t len;
	da16x_wps_prt("     [%s] WPS:  * Model Name\n", __func__);
...
{
	size_t len;
	da16x_wps_prt("     [%s] WPS:  * Model Number\n", __func__);
...
{
	size_t len;
	da16x_wps_prt("     [%s] WPS:  * Serial Number\n", __func__);
...
{
	da16x_wps_prt("     [%s] WPS:  * Primary Device Type\n", __func__);
				  ...
				  struct wpabuf *msg)
{
	...
	da16x_wps_prt("     [%s] WPS:  * Secondary Device Type\n", __func__);
			   ...
			   const u8 *req_dev_types)
{
	...
	for (i = 0; i < num_req_dev_types; i++) {
		...
		da16x_dump("WPS: * Requested Device Type",
...
{
	size_t len;
	da16x_wps_prt("     [%s] WPS:  * Device Name\n", __func__);
...
{
	da16x_wps_prt("     [%s] WPS:  * OS Version\n", __func__);
...
#ifdef ENABLE_WPS_DBG
		da16x_dump("WPS:  * Vendor Extension M1",
...
int wps_build_vendor_ext(struct wps_device_data *dev, struct wpabuf *msg)
{
	...
	for (i = 0; i < MAX_WPS_VENDOR_EXTENSIONS; i++) {
		if (dev->vendor_ext[i] == NULL)
			continue;
#ifdef ENABLE_WPS_DBG
		da16x_dump("WPS:  * Vendor Extension",
...
{
	if (str == NULL) {
		da16x_wps_prt("     [%s] WPS: No Manufacturer received\n", __func__);
		return -1;
	}

#ifdef ENABLE_WPS_DBG
	da16x_dump("WPS: Manufacturer", str, str_len);
#endif
...
{
	if (str == NULL) {
		da16x_wps_prt("     [%s] WPS: No Model Name received\n", __func__);
		return -1;
	}

#ifdef ENABLE_WPS_DBG
	da16x_dump("WPS: Model Name", str, str_len);
#endif
...
{
	if (str == NULL) {
		da16x_wps_prt("     [%s] WPS: No Model Number received\n", __func__);
		return -1;
	}

#ifdef ENABLE_WPS_DBG
	da16x_dump("WPS: Model Number", str, str_len);
#endif
...
{
	if (str == NULL) {
		da16x_wps_prt("     [%s] WPS: No Serial Number received\n", __func__);
		return -1;
	}

#ifdef ENABLE_WPS_DBG
	da16x_dump("WPS: Serial Number", str, str_len);
#endif
...
{
	if (str == NULL) {
		da16x_wps_prt("     [%s] WPS: No Device Name received\n", __func__);
		return -1;
	}

#ifdef ENABLE_WPS_DBG
	da16x_dump("WPS: Device Name", str, str_len);
#endif
...
{
	if (dev_type == NULL) {
		da16x_wps_prt("     [%s] WPS: No Primary Device Type received\n", __func__);
		return -1;
	}
...
{
	if (ver == NULL) {
		da16x_wps_prt("     [%s] WPS: No OS Version received\n", __func__);
		return -1;
	}

	dev->os_version = WPA_GET_BE32(ver);
	da16x_wps_prt("     [%s] WPS: OS Version %08x\n", __func__, dev->os_version);
...
{
	if (bands == NULL) {
		da16x_wps_prt("     [%s] WPS: No RF Bands received\n", __func__);
		return -1;
	}

	dev->rf_bands = *bands;
	da16x_wps_prt("     [%s] WPS: Enrollee RF Bands 0x%x\n", __func__, dev->rf_bands);
