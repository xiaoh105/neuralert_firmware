...
static void radius_client_timer(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (radius->msgs) {
		if (first < now.sec)
			first = now.sec;
		da16x_eloop_register_timeout(first - now.sec, 0,
...
static void radius_client_update_timeout(struct radius_client_data *radius)
{
	...
	da16x_eloop_cancel_timeout(radius_client_timer, radius, NULL);
	...
	if (first < now.sec)
		first = now.sec;
	da16x_eloop_register_timeout(first - now.sec, 0, radius_client_timer, radius,
			       NULL);
#ifdef	CONFIG_AP_SYSLOG	/* F_F_S */
	hostapd_logger(radius->ctx, NULL, HOSTAPD_MODULE_RADIUS,
		     ...
		     int sock, int sock6, int auth)
{
	...
	if (radius->msgs) {
		da16x_eloop_cancel_timeout(radius_client_timer, radius, NULL);
		da16x_eloop_register_timeout(RADIUS_CLIENT_FIRST_WAIT, 0,
				       radius_client_timer, radius, NULL);
	}
	...
	case AF_INET:
		...
		wpa_printf_dbg(MSG_DEBUG, "RADIUS local address: %s:%u",
			   da16x_inet_ntoa(claddr.sin_addr), ntohs(claddr.sin_port));
		break;
#ifdef CONFIG_IPV6
	case AF_INET6: {
		claddrlen = sizeof(claddr6);
		getsockname(sel_sock, (struct sockaddr *) &claddr6,
			    &claddrlen);
		wpa_printf_dbg(MSG_DEBUG, "RADIUS local address: %s:%u",
			   da16x_inet_ntop(AF_INET6, &claddr6.sin6_addr,
				     abuf, sizeof(abuf)),
...
static void radius_retry_primary_timer(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (conf->retry_primary_interval)
		da16x_eloop_register_timeout(conf->retry_primary_interval, 0,
...
void radius_client_deinit(struct radius_client_data *radius)
{
	...
	da16x_eloop_cancel_timeout(radius_retry_primary_timer, radius, NULL);
