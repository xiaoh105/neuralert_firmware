				...
				const u8 *buf, size_t len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "RSN: receive pre-auth packet "
		   "from interface '%s'", piface->ifname);
	if (len < sizeof(*ethhdr) + sizeof(*hdr)) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: too short pre-auth packet "
	...
	if (os_memcmp(ethhdr->h_dest, hapd->own_addr, ETH_ALEN) != 0) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: pre-auth for foreign address "
	...
	if (sta && (sta->flags & WLAN_STA_ASSOC)) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: pre-auth for already association "
...
static int rsn_preauth_iface_add(struct hostapd_data *hapd, const char *ifname)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "RSN pre-auth interface '%s'", ifname);
...
static void rsn_preauth_finished_cb(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "RSN: Removing pre-authentication STA entry for "
			  ...
			  int success)
{
	 ...
	 */
	da16x_eloop_register_timeout(0, 0, rsn_preauth_finished_cb, hapd, sta);
}
		      ...
		      u8 *buf, size_t len)
{
	...
	if (piface == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: Could not find pre-authentication "
...
{
	da16x_eloop_cancel_timeout(rsn_preauth_finished_cb, hapd, sta);
}
