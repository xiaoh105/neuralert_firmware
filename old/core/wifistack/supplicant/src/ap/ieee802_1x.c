				   ...
				   struct sta_info *sta, int authorized)
{
	...
	if (res) {
		wpa_printf_dbg(MSG_DEBUG, "Could not set station " MACSTR
				  ...
				  u8 *key_data, size_t key_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IEEE 802.1X: Sending EAPOL-Key to " MACSTR
...
static void ieee802_1x_tx_key(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IEEE 802.1X: Sending EAPOL-Key(s) to " MACSTR,
		   MAC2STR(sta->addr));
				   ...
				   const u8 *eap, size_t len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Encapsulating EAP message into a RADIUS "
		   "packet");
	...
	if (sm->last_recv_radius &&
		...
		if (res > 0) {
			wpa_printf_dbg(MSG_DEBUG, "Copied RADIUS State Attribute");
		}
	}
		       ...
		       u8 *buf, size_t len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP: code=%d identifier=%d length=%d",
		   eap->code, eap->identifier, eap_len);
	if (eap_len < sizeof(*eap)) {
		wpa_printf_dbg(MSG_DEBUG, "   Invalid EAP length");
		return;
	} else if (eap_len > len) {
		wpa_printf_dbg(MSG_DEBUG, "   Too short frame to contain this EAP "
			   "packet");
		return;
	} else if (eap_len < len) {
		wpa_printf_dbg(MSG_DEBUG, "   Ignoring %lu extra bytes after EAP "
			   "packet", (unsigned long) len - eap_len);
	}

	switch (eap->code) {
	case EAP_CODE_REQUEST:
		wpa_printf_dbg(MSG_DEBUG, " (request)");
		return;
	case EAP_CODE_RESPONSE:
		wpa_printf_dbg(MSG_DEBUG, " (response)");
		handle_eap_response(hapd, sta, eap, eap_len);
		break;
	case EAP_CODE_SUCCESS:
		wpa_printf_dbg(MSG_DEBUG, " (success)");
		return;
	case EAP_CODE_FAILURE:
		wpa_printf_dbg(MSG_DEBUG, " (failure)");
		return;
	case EAP_CODE_INITIATE:
		wpa_printf_dbg(MSG_DEBUG, " (initiate)");
		handle_eap_initiate(hapd, sta, eap, eap_len);
		break;
	case EAP_CODE_FINISH:
		wpa_printf_dbg(MSG_DEBUG, " (finish)");
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, " (unknown code)");
		return;
	}
}
			...
			size_t len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IEEE 802.1X: %lu bytes from " MACSTR,
		   (unsigned long) len, MAC2STR(sa));
	sta = ap_get_sta(hapd, sa);
	if (!sta || (!(sta->flags & (WLAN_STA_ASSOC | WLAN_STA_PREAUTH)) &&
		     !(hapd->iface->drv_flags & WPA_DRIVER_FLAGS_WIRED))) {
		wpa_printf_dbg(MSG_DEBUG, "IEEE 802.1X data frame from not "
			   "associated/Pre-authenticating STA");

		if (sta && (sta->flags & WLAN_STA_AUTH)) {
			wpa_printf_dbg(MSG_DEBUG, "Saving EAPOL frame from " MACSTR
	...
	wpa_printf_dbg(MSG_DEBUG, "   IEEE 802.1X: version=%d type=%d length=%d",
	...
	if (len - sizeof(*hdr) > datalen) {
		wpa_printf_dbg(MSG_DEBUG, "   ignoring %lu extra octets after "
			   "IEEE 802.1X packet",
	...
	if (!hapd->conf->ieee802_1x && !hapd->conf->osen &&
		...
		wpa_printf_dbg(MSG_DEBUG, "IEEE 802.1X: Ignore EAPOL message - "
	...
	if (!sta->eapol_sm) {
		...
		if (!hapd->conf->ieee802_1x && hapd->conf->wps_state) {
			...
			if (wflags == WLAN_STA_MAYBE_WPS ||
				 ...
				 * 2.0.
				 */
				wpa_printf_dbg(MSG_DEBUG, "WPS: Do not start "
...
#else
		da16x_ap_prt("[%s] What do I do for accounting_sta_stop() ???\n", __func__);
#endif	/* 0 */
	...
	case IEEE802_1X_TYPE_EAPOL_KEY:
		wpa_printf_dbg(MSG_DEBUG, "   EAPOL-Key");
		if (!ap_sta_is_authorized(sta)) {
			wpa_printf_dbg(MSG_DEBUG, "   Dropped key data from "
	...
	case IEEE802_1X_TYPE_EAPOL_ENCAPSULATED_ASF_ALERT:
		wpa_printf_dbg(MSG_DEBUG, "   EAPOL-Encapsulated-ASF-Alert");
		/* TODO: implement support for this; show data */
		break;

	default:
		wpa_printf_dbg(MSG_DEBUG, "   unknown IEEE 802.1X packet type");
...
void ieee802_1x_new_station(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	if (!force_1x && !hapd->conf->ieee802_1x && !hapd->conf->osen) {
		wpa_printf_dbg(MSG_DEBUG, "IEEE 802.1X: Ignore STA - "
	...
	if (key_mgmt != -1 &&
	    (wpa_key_mgmt_wpa_psk(key_mgmt) || key_mgmt == WPA_KEY_MGMT_OWE ||
	     key_mgmt == WPA_KEY_MGMT_DPP)) {
		wpa_printf_dbg(MSG_DEBUG, "IEEE 802.1X: Ignore STA - using PSK");
		/*
		 * Clear any possible EAPOL authenticator state to support
	...
	if (!hapd->conf->ieee802_1x && hapd->conf->wps_state &&
		 ...
		 */
		wpa_printf_dbg(MSG_DEBUG, "WPS: Do not start EAPOL until "
...
#ifdef CONFIG_HS20
	da16x_eloop_cancel_timeout(ieee802_1x_wnm_notif_send, hapd, sta);
#endif /* CONFIG_HS20 */
					  ...
					  struct radius_msg *msg)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IEEE 802.1X: Stored %lu RADIUS Class "
...
static void ieee802_1x_hs20_sub_rem(struct sta_info *sta, u8 *pos, size_t len)
{
	...
	if (len > 2) {
		...
		wpa_printf_dbg(MSG_DEBUG, "HS 2.0: Subscription remediation needed "
				       ...
				       size_t len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "HS 2.0: Deauthentication request - Code %u  "
		   "Re-auth Delay %u",
					 ...
					 size_t len, int session_timeout)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "HS 2.0: Session Information URL='%s' SWT=%u "
		   "(session_timeout=%d)",
		   sta->hs20_session_info_url, swt, session_timeout);
	if (session_timeout < 0) {
		wpa_printf_dbg(MSG_DEBUG, "HS 2.0: No Session-Timeout set - ignore session info URL");
		return;
	}
	if (swt == 255)
			...
			void *data)
{
	...
	if (sm == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "IEEE 802.1X: Could not find matching "
	...
	if (hdr->code == RADIUS_CODE_ACCESS_REJECT &&
		...
		wpa_printf_dbg(MSG_DEBUG, "Allowing RADIUS Access-Reject without "
	...
	wpa_printf_dbg(MSG_DEBUG, "RADIUS packet matching with station " MACSTR,
		   MAC2STR(sta->addr));
...
static void ieee802_1x_rekey(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IEEE 802.1X: New default WEP key index %d",
		   eapol->default_wep_key_idx);
	...
	if (hapd->conf->wep_rekeying_period > 0) {
		da16x_eloop_register_timeout(hapd->conf->wep_rekeying_period, 0,
				  ...
				  const u8 *data, size_t datalen)
{
	...
	if ((sta->flags & (WLAN_STA_WPS | WLAN_STA_MAYBE_WPS)) ==
		...
		if (identity &&
			...
			wpa_printf_dbg(MSG_DEBUG, "WPS: WLAN_STA_MAYBE_WPS -> "
				   "WLAN_STA_WPS");
			sta->flags |= WLAN_STA_WPS;
		}
	}
...
out:
	if (rv)
		wpa_printf_dbg(MSG_DEBUG, "%s: Failed to find user", __func__);
...
{
#ifdef	CONFIG_AP_WEP_KEY
	da16x_eloop_cancel_timeout(ieee802_1x_rekey, hapd, NULL);
#else
da16x_ap_prt("[%s] What do I do for timer register ...\n", __func__);
#endif	/* CONFIG_AP_WEP_KEY */
			       ...
			       const u8 *buf, int len, int ack)
{
	...
	if (len < (int) sizeof(*xhdr))
		return 0;
	wpa_printf_dbg(MSG_DEBUG, "IEEE 802.1X: " MACSTR " TX status - version=%d "
...
static void ieee802_1x_wnm_notif_send(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (sta->remediation) {
		wpa_printf_dbg(MSG_DEBUG, "HS 2.0: Send WNM-Notification to "
				...
				int remediation)
{
...
#else
	unsigned int session_timeout = 43200;
	...
	if (success && (sta->remediation || sta->hs20_deauth_req ||
		...
		wpa_printf_dbg(MSG_DEBUG, "HS 2.0: Schedule WNM-Notification to "
			   MACSTR " in 100 ms", MAC2STR(sta->addr));
		da16x_eloop_cancel_timeout(ieee802_1x_wnm_notif_send, hapd, sta);
		da16x_eloop_register_timeout(0, 100000, ieee802_1x_wnm_notif_send,
				       hapd, sta);
	}
#endif /* CONFIG_HS20 */
