...
{
	struct hostapd_data *hapd = ctx;
	wpa_printf_dbg(MSG_DEBUG, "%s: WPA authenticator requests disconnect: "
		   "STA " MACSTR " reason %d",
...
#else
da16x_ap_prt("[%s] WPA_EAPOL_portEnabled: What do I do for thie ???\n",
				__func__, var);
#endif	/* CONFIG_8021X */
		break;
...
#else
da16x_ap_prt("[%s] WPA_EAPOL_portValid: What do I do for thie ???\n",
				__func__, var);
#endif	/* CONFIG_8021X */
		break;
...
#else
da16x_ap_prt("[%s] WPA_EAPOL_authorized: What do I do for thie ???\n",
				__func__, var);
#endif	/* CONFIG_8021X */
		break;
...
#else
da16x_ap_prt("[%s] WPA_EAPOL_inc_EapolFramesTx:: What do I do for thie ???\n",
				__func__, var);
#endif	/* CONFIG_8021X */
		break;
	}
	...
	if (sta && sta->psk && !psk) {
		...
		for (pos = sta->psk; pos; pos = pos->next) {
			if (pos->is_passphrase) {
				...
				optimized_pbkdf2_sha1(pos->passphrase,
					    hapd->conf->ssid.ssid,
				    ...
				    size_t *len)
{
	...
	if (sta == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "AUTH_GET_MSK: Cannot find STA");
		return -1;
	}
	...
	if (key == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "AUTH_GET_MSK: Key is null, eapol_sm: %p",
			   sta->eapol_sm);
		return -1;
	}
	...
	*len = keylen;
	...
	return 0;
#else
da16x_ap_prt("[%s] What do I do for ieee802_1x_get_key() ???\n", __func__);
	return -1;
#endif	/* CONFIG_8021X */
}
...
static int hostapd_wpa_auth_ft_iter(struct hostapd_iface *iface, void *ctx)
{
	...
	for (j = 0; j < iface->num_bss; j++) {
		...
		if (!da16x_eloop_is_timeout_registered(hostapd_wpa_ft_rrb_rx_later,
						 hapd, NULL))
...
static int hostapd_wpa_auth_oui_iter(struct hostapd_iface *iface, void *ctx)
{
	...
	for (j = 0; j < iface->num_bss; j++) {
		...
		if (!da16x_eloop_is_timeout_registered(hostapd_oui_deliver_later,
						 hapd, NULL))
			da16x_eloop_register_timeout(0, 0,
				...
				size_t len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: RRB received packet " MACSTR " -> "
...
void hostapd_deinit_wpa(struct hostapd_data *hapd)
{
	...
	if (hapd->wpa_auth) {
		...
		if (hapd->drv_priv && hostapd_set_privacy(hapd, 0)) {
			wpa_printf_dbg(MSG_DEBUG, "Could not disable "
		...
		if (hapd->drv_priv &&
			...
			wpa_printf_dbg(MSG_DEBUG, "Could not remove generic "
...
#ifdef CONFIG_IEEE80211R_AP
	da16x_eloop_cancel_timeout(hostapd_wpa_ft_rrb_rx_later, hapd, ELOOP_ALL_CTX);
	hostapd_wpa_ft_rrb_rx_later(hapd, NULL); /* flush without delivering */
	da16x_eloop_cancel_timeout(hostapd_oui_deliver_later, hapd, ELOOP_ALL_CTX);
