...
u8 * hostapd_eid_ht_capabilities(struct hostapd_data *hapd, u8 *eid)
{
	...
	u8 *pos = eid;
	//[[ trinity_0150618_BEGIN -- tmp
	le16 tmp;
	//]] trinity_0150618_END -- tmp
...
#if 1
	//[[ trinity_0150618_BEGIN -- tmp
	tmp = host_to_le16(hapd->iconf->ht_capab);
	os_memcpy(&cap->ht_capabilities_info, &tmp , 2);
	//]] trinity_0150618_END -- tmp
#else
...
u8 * hostapd_eid_ht_operation(struct hostapd_data *hapd, u8 *eid)
{
	...
	*pos++ = sizeof(*oper);
	...
	oper = (struct _ieee80211_ht_operation *) pos;
	os_memset(oper, 0, sizeof(*oper));

	oper->primary_chan = hapd->iconf->channel;
#if 1
	//[[ trinity_0150618_BEGIN -- tmp
	tmp = host_to_le16(hapd->iface->ht_op_mode);
	os_memcpy(&oper->operation_mode, &tmp , 2);
	//]] trinity_0150618_END -- tmp
#else
...
int hostapd_ht_operation_update(struct hostapd_iface *iface)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "%s current operation mode=0x%X",
	...
	wpa_printf_dbg(MSG_DEBUG, "%s new operation mode=0x%X changes=%d",
			      ...
			      const struct _ieee80211_mgmt *mgmt, size_t len)
{
	...
	if (bc_ie->element_id != WLAN_EID_20_40_BSS_COEXISTENCE ||
	    bc_ie->length < 1) {
		wpa_printf_dbg(MSG_DEBUG, "Unexpected IE (%u,%u) in coex report",
	...
	wpa_printf_dbg(MSG_DEBUG, "is_ht40_allowed=%d num_sta_ht40_intolerant=%d",
	...
	if (!is_ht40_allowed &&
		...
		if (!iface->num_sta_ht40_intolerant &&
			...
			delay_time = OVERLAPPING_BSS_TRANS_DELAY_FACTOR *
				iface->conf->obss_interval;
			da16x_eloop_cancel_timeout(ap_ht2040_timeout, hapd->iface,
					     NULL);
			da16x_eloop_register_timeout(delay_time, 0, ap_ht2040_timeout,
					       hapd->iface, NULL);
			wpa_printf_dbg(MSG_DEBUG,
...
void ht40_intolerant_remove(struct hostapd_iface *iface, struct sta_info *sta)
{
	...
	if (iface->num_sta_ht40_intolerant == 0 &&
		...
		da16x_eloop_register_timeout(delay_time, 0, ap_ht2040_timeout,
				       iface, NULL);
#else
da16x_ap_prt("[%s] What do I do for timer operation ???\n", __func__);
#endif	/* 0 */
	}
}
...
static void update_sta_ht(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "HT: STA " MACSTR " HT Capabilities Info: "
	...
	if ((ht_capab & HT_CAP_INFO_GREEN_FIELD) == 0) {
		...
		wpa_printf_dbg(MSG_DEBUG, "%s STA " MACSTR " - no greenfield, num "
	...
	if ((ht_capab & HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET) == 0) {
		...
		wpa_printf_dbg(MSG_DEBUG, "%s STA " MACSTR " - 20 MHz HT, num of "
			   "20MHz HT STAs %d",
			   __func__, MAC2STR(sta->addr),
			   hapd->iface->num_sta_ht_20mhz);
	}
...
static void update_sta_no_ht(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	if (hapd->iconf->ieee80211n) {
		wpa_printf_dbg(MSG_DEBUG, "%s STA " MACSTR " - no HT, num of "
			   "non-HT stations %d",
			   __func__, MAC2STR(sta->addr),
			   hapd->iface->num_sta_no_ht);
	}
