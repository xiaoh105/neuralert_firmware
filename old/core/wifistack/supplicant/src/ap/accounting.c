					  ...
					  int status_type)
{
	...
	if (msg == NULL) {
		da16x_ap_prt("[%s] Could not create new RADIUS packet\n",
					__func__);
		return NULL;
	}
	...
	if (sta) {
		...
		if (!radius_msg_add_attr(msg, RADIUS_ATTR_ACCT_SESSION_ID,
			...
			da16x_ap_prt("[%s] Could not add Acct-Session-Id\n",
						__func__);
			goto fail;
		}
	} else {
	...
	if (!radius_msg_add_attr_int32(msg, RADIUS_ATTR_ACCT_STATUS_TYPE,
				       status_type)) {
		da16x_ap_prt("[%s] Could not add Acct-Status-Type\n",
					__func__);
		goto fail;
	}
	...
	if (!hostapd_config_get_radius_attr(hapd->conf->radius_acct_req_attr,
		...
		da16x_ap_prt("[%s] Could not add Acct-Authentic\n",
					__func__);
		goto fail;
	}
	...
	if (sta) {
		...
		if (!radius_msg_add_attr(msg, RADIUS_ATTR_USER_NAME, val,
					 len)) {
			da16x_ap_prt("[%s] Could not add User-Name\n",
						__func__);
			goto fail;
		}
	}
	...
	if (sta) {
		for (i = 0; ; i++) {
			...
			if (!radius_msg_add_attr(msg, RADIUS_ATTR_CLASS,
						 val, len)) {
				da16x_ap_prt("[%s] Could not add Class\n",
						__func__);
				goto fail;
			}
		}
		...
		if (b &&
			...
			da16x_ap_prt("[%s] Could not add CUI\n", __func__);
			goto fail;
		}
		...
		if (!b && sta->radius_cui &&
			...
			da16x_ap_prt("[%s] Could not add CUI from ACL\n",
					__func__);
			goto fail;
		}
	}
...
static void accounting_interim_update(void *eloop_ctx, void *timeout_ctx)
{
	...
	da16x_eloop_register_timeout(interval, 0, accounting_interim_update,
			       hapd, sta);
}
				  ...
				  struct sta_info *sta, int stop)
{
	...
	if (!msg) {
		da16x_ap_prt("[%s] Could not create RADIUS Accounting message\n",
				__func__);
		return;
	}
	...
	if (!radius_msg_add_attr_int32(msg, RADIUS_ATTR_ACCT_SESSION_TIME,
				       diff.sec)) {
		da16x_ap_prt("[%s] Could not add Acct-Session-Time\n", __func__);
		goto fail;
	}
	...
	if (accounting_sta_update_stats(hapd, sta, &data) == 0) {
		if (!radius_msg_add_attr_int32(msg,
					       ...
					       data.rx_packets)) {
			da16x_ap_prt("[%s] Could not add Acct-Input-Packets\n",
					__func__);
			goto fail;
		}
#ifdef	CONFIG_RADIUS
		if (!radius_msg_add_attr_int32(msg,
					       ...
					       data.tx_packets)) {
			da16x_ap_prt("[%s] Could not add Acct-Output-Packets\n",
					__func__);
			goto fail;
		}
		if (!radius_msg_add_attr_int32(msg,
					       ...
					       data.rx_bytes)) {
			da16x_ap_prt("[%s] Could not add Acct-Input-Octets\n",
					__func__);
			goto fail;
		}
#endif	/* CONFIG_RADIUS */
		...
		if (gigawords &&
		    !radius_msg_add_attr_int32(
			    ...
			    gigawords)) {
			da16x_ap_prt("[%s] Could not add Acct-Input-Gigawords\n",
					__func__);
			goto fail;
		}
		if (!radius_msg_add_attr_int32(msg,
					       ...
					       data.tx_bytes)) {
			da16x_ap_prt("[%s] Could not add Acct-Output-Octets\n",
					__func__);
			goto fail;
		}
		...
		if (gigawords &&
		    !radius_msg_add_attr_int32(
			    ...
			    gigawords)) {
			da16x_ap_prt("[%s] Could not add Acct-Output-Gigawords\n",
					__func__);
			goto fail;
		}
	}
	...
	if (!radius_msg_add_attr_int32(msg, RADIUS_ATTR_EVENT_TIMESTAMP,
				       now.sec)) {
		da16x_ap_prt("[%s] Could not add Event-Timestamp\n",
					__func__);
		goto fail;
	}
	...
	if (stop && cause &&
	    !radius_msg_add_attr_int32(msg, RADIUS_ATTR_ACCT_TERMINATE_CAUSE,
				       cause)) {
		da16x_ap_prt("[%s] Could not add Acct-Terminate-Cause\n",
					__func__);
		goto fail;
	}
...
void accounting_sta_stop(struct hostapd_data *hapd, struct sta_info *sta)
{
	if (sta->acct_session_started) {
		...
		da16x_eloop_cancel_timeout(accounting_interim_update, hapd, sta);
#if 0
		hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_RADIUS,
			       HOSTAPD_LEVEL_INFO,
		   ...
		   void *data)
{
	if (radius_msg_get_hdr(msg)->code != RADIUS_CODE_ACCOUNTING_RESPONSE) {
		da16x_ap_prt("[%s] Unknown RADIUS message code\n", __func__);
		return RADIUS_RX_UNKNOWN;
	}

	if (radius_msg_verify(msg, shared_secret, shared_secret_len, req, 0)) {
		da16x_ap_prt("[%s] Incoming RADIUS packet did not have correct"
