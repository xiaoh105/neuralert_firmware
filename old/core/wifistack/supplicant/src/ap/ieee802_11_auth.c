...
#ifdef	CONFIG_ACL
extern int hostapd_maclist_found_none_sorted(struct mac_acl_entry *list,
				    ...
				    struct hostapd_acl_query_data *query)
{
	...
	if (!radius_msg_add_attr(msg, RADIUS_ATTR_USER_NAME, (u8 *) buf,
				 os_strlen(buf))) {
		wpa_printf_dbg(MSG_DEBUG, "Could not add User-Name");
		goto fail;
	}
	...
	if (!radius_msg_add_attr_user_password(
		...
		wpa_printf_dbg(MSG_DEBUG, "Could not add User-Password");
		goto fail;
	}
	...
	if (!radius_msg_add_attr(msg, RADIUS_ATTR_CALLING_STATION_ID,
		...
		wpa_printf_dbg(MSG_DEBUG, "Could not add Calling-Station-Id");
		goto fail;
	}
	...
	if (!radius_msg_add_attr(msg, RADIUS_ATTR_CONNECT_INFO,
				 (u8 *) buf, os_strlen(buf))) {
		wpa_printf_dbg(MSG_DEBUG, "Could not add Connect-Info");
		goto fail;
	}
			    ...
			    int is_probe_req)
{
	...
	da16x_ap_prt("[%s] ACL Mac is set %s\n",__func__,
			hapd->conf->num_deny_mac ? "Deny" :
			hapd->conf->num_accept_mac ? "Allow" : "Clear");

	if (hapd->conf->num_accept_mac) {
		if (hostapd_maclist_found_none_sorted(hapd->conf->accept_mac,
					  hapd->conf->num_accept_mac, addr
#ifdef CONFIG_AP_VLAN
					, vlan_id
#endif /* CONFIG_AP_VLAN */
		...
		if (hostapd_maclist_found_none_sorted(hapd->conf->deny_mac,
					  hapd->conf->num_deny_mac, addr
#ifdef CONFIG_AP_VLAN
					, vlan_id
#endif /* CONFIG_AP_VLAN */
...
#else
	res = hostapd_check_acl(hapd, addr
#ifdef CONFIG_AP_VLAN
				, vlan_id
#endif /* CONFIG_AP_VLAN */
				);
	...
	if (hapd->conf->macaddr_acl == USE_EXTERNAL_RADIUS_AUTH) {
		...
		if (hostapd_radius_acl_query(hapd, addr, query)) {
			wpa_printf_dbg(MSG_DEBUG, "Failed to send Access-Request "
				   "for ACL query.");
				     ...
				     struct os_reltime *now)
{
	...
	while (entry) {
		if (os_reltime_expired(now, &entry->timestamp,
				       RADIUS_ACL_TIMEOUT)) {
			wpa_printf_dbg(MSG_DEBUG, "Cached ACL entry for " MACSTR
				       ...
				       struct os_reltime *now)
{
	...
	while (entry) {
		if (os_reltime_expired(now, &entry->timestamp,
				       RADIUS_ACL_TIMEOUT)) {
			wpa_printf_dbg(MSG_DEBUG, "ACL query for " MACSTR
				   " has expired.", MAC2STR(entry->addr));
			if (prev)
				prev->next = entry->next;
			else
			...
			void *data)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Found matching Access-Request for RADIUS "
	...
	if (hdr->code != RADIUS_CODE_ACCESS_ACCEPT &&
		...
		wpa_printf_dbg(MSG_DEBUG, "Unknown RADIUS message code %d to ACL "
			   "query", hdr->code);
		return RADIUS_RX_UNKNOWN;
	}
	...
	if (cache == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "Failed to add ACL cache entry");
		goto done;
	}
	...
	if (hdr->code == RADIUS_CODE_ACCESS_ACCEPT) {
		...
		if (radius_msg_get_attr_int32(
			...
			wpa_printf_dbg(MSG_DEBUG, "Ignored too small "
	...
	wpa_printf_dbg(MSG_DEBUG, "Re-sending authentication frame after "
