...
static int wpa_write_wpa_ie(struct wpa_auth_config *conf, u8 *buf, size_t len)
{
	...
	if (suite == 0) {
		wpa_printf_dbg(MSG_DEBUG, "Invalid group cipher (%d).",
			   conf->wpa_group);
		return -1;
	}
	...
	if (num_suites == 0) {
		wpa_printf_dbg(MSG_DEBUG, "[%s] Invalid pairwise cipher (%d).", __func__,
			   conf->wpa_pairwise);
		return -1;
	}
	...
	if (num_suites == 0) {
		wpa_printf_dbg(MSG_DEBUG, "[%s] Invalid key management type (%d).", __func__,
			   conf->wpa_key_mgmt);
		return -1;
	}
	WPA_PUT_LE16(count, num_suites);
			...
			const u8 *owe_dh, size_t owe_dh_len)
{
	...
	if (!(wpa_auth->conf.wpa & version)) {
		wpa_printf_dbg(MSG_DEBUG, "Invalid WPA proto (%d) from " MACSTR,
	...
	if (res) {
		wpa_printf_dbg(MSG_DEBUG, "Failed to parse WPA/RSN IE from "
	...
	if (data.group_cipher != wpa_auth->conf.wpa_group) {
		wpa_printf_dbg(MSG_DEBUG, "Invalid WPA group cipher (0x%x) from "
	...
	if (!key_mgmt) {
		wpa_printf_dbg(MSG_DEBUG, "Invalid WPA key mgmt (0x%x) from "
	...
	if (!ciphers) {
		wpa_printf_dbg(MSG_DEBUG, "Invalid %s pairwise cipher (0x%x) "
			   "from " MACSTR,
	...
	if (wpa_auth->conf.ieee80211w == MGMT_FRAME_PROTECTION_REQUIRED) {
		if (!(data.capabilities & WPA_CAPABILITY_MFPC)) {
			wpa_printf_dbg(MSG_DEBUG, "Management frame protection "
		...
		{
			wpa_printf_dbg(MSG_DEBUG, "Unsupported management group "
	...
	if (wpa_key_mgmt_ft(sm->wpa_key_mgmt)) {
		if (mdie == NULL || mdie_len < MOBILITY_DOMAIN_ID_LEN + 1) {
			wpa_printf_dbg(MSG_DEBUG, "RSN: Trying to use FT, but "
				   "MDIE not included");
