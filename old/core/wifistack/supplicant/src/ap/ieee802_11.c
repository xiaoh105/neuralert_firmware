...
extern enum hostapd_hw_mode ieee80211_freq_to_channel_ext(unsigned int freq,
...
const char* auth_alg2string(int auth_alg)
{
	switch(auth_alg)
	{
...
#else
	if (hapd->iconf->require_vht && count < 8) {
#endif	/* CONFIG_IEEE80211AC */
		count++;
			   ...
			   const u8 *ies, size_t ies_len, const char *dbg)
{
	...
	buf = os_zalloc(rlen);
	if (buf == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "[%s:%d] NULL\n", __func__, __LINE__);
		return -1;
	}

	reply = (struct _ieee80211_mgmt *) buf;
	...
	wpa_printf_dbg(MSG_DEBUG, "[%s] authentication reply: STA=" MACSTR
		   " auth_alg=%s(%d) auth_transaction=%d resp=%d (IE len=%lu) (dbg=%s)", __func__,
		   MAC2STR(dst), auth_alg2string(auth_alg), auth_alg, auth_transaction,
		   resp, (unsigned long) ies_len, dbg);

	ret = hostapd_drv_send_mlme(hapd, reply, rlen, 0);

	if (ret < 0) {
		wpa_printf(MSG_INFO, "send_auth_reply: send failed(0x%x)" , ret);
	} else {
...
static void sae_set_state(struct sta_info *sta, enum sae_state state,
			  const char *reason)
{
	wpa_printf_dbg(MSG_DEBUG, "[%s] SAE: State %s(%d) -> %s(%d) for peer " MACSTR " (%s)", __func__,
		   sae_state_txt(sta->sae->state), sta->sae->state, sae_state_txt(state), state,
...
#if 0 /* used expected sae commit len. */
	buf = wpabuf_alloc(SAE_COMMIT_MAX_LEN +
			   (rx_id ? 3 + os_strlen(rx_id) : 0));
#else
	size_t sae_commit_len = SAE_COMMIT_MAX_LEN;
	if (sta->sae && sta->sae->tmp) {
		sae_commit_len = (2 + 4 * sta->sae->tmp->prime_len);
	}

	buf = wpabuf_alloc(sae_commit_len + (rx_id ? 3 + os_strlen(rx_id) : 0));
#endif
	if (buf == NULL)
		return NULL;
...
#if 0 /* used expected sae comfirm len. */
	buf = wpabuf_alloc(SAE_CONFIRM_MAX_LEN);
#else
	size_t sae_confirm_len = SAE_CONFIRM_MAX_LEN;
	if (sta->sae && sta->sae->tmp) {
		sae_confirm_len = 2 + sta->sae->tmp->prime_len;
	}

	buf = wpabuf_alloc(sae_confirm_len);
#endif
	if (buf == NULL) {
		wpa_printf(MSG_USER_DEBUG, "[%s] wpabuf_alloc error(%ld)",
			__func__, sae_confirm_len);
		return NULL;
	}
				...
				const u8 *bssid, int update)
{
	...
	if (!data && sta->sae->tmp && sta->sae->tmp->pw_id) {
		wpa_printf(MSG_USER_DEBUG, "[%s] UNKNOWN_PASSWORD_IDENTIFIER", __func__);
		return WLAN_STATUS_UNKNOWN_PASSWORD_IDENTIFIER;
	}
	if (data == NULL) {
		wpa_printf(MSG_USER_DEBUG, "[%s] WLAN_STATUS_UNSPECIFIED_FAILURE", __func__);
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
				 ...
				 const u8 *bssid)
{
	...
	if (data == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "[%s] WLAN_STATUS_UNSPECIFIED_FAILURE", __func__);
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
...
static int use_sae_anti_clogging(struct hostapd_data *hapd)
{
	...
	for (sta = hapd->sta_list; sta; sta = sta->next) {
		...
		wpa_printf_dbg(MSG_DEBUG, "[%s] sae_anti_clogging_threshold=%d/%d " MACSTR " sta->sae->state=%s\n ", __func__,
			open, hapd->conf->sae_anti_clogging_threshold, MAC2STR(sta->addr), sae_state_txt(sta->sae->state));
					    ...
					    int group, const u8 *addr)
{
	...
	if (buf == NULL) {
		wpa_printf(MSG_ERROR, "[%s] Memory Allocation Error !!!", __func__);
		return NULL;
	}	
...
static int sae_check_big_sync(struct hostapd_data *hapd, struct sta_info *sta)
{
	if (sta->sae->sync > hapd->conf->sae_sync) {
		wpa_printf_dbg(MSG_DEBUG, "[%s] sta->sae->sync(%d) > hapd->conf->sae_sync(%d)", __func__,
			sta->sae->sync, hapd->conf->sae_sync);
...
static void auth_sae_retransmit_timer(void *eloop_ctx, void *eloop_data)
{
	...
	if (sae_check_big_sync(hapd, sta))
		return;
	sta->sae->sync++;
	wpa_printf_dbg(MSG_DEBUG, "[%s] SAE: Auth SAE retransmit timer for " MACSTR
		   " (sync=%d state=%s)", __func__,
	...
	case SAE_COMMITTED:
		...
		da16x_eloop_register_timeout(0,
	...
	if (ret != WLAN_STATUS_SUCCESS)
		wpa_printf(MSG_INFO, "SAE: Failed to retransmit: ret=%d sae->state=%s(%d)",
					ret, sae_state_txt(sta->sae->state), sta->sae->state);
}


void sae_clear_retransmit_timer(struct hostapd_data *hapd, struct sta_info *sta)
{
	da16x_eloop_cancel_timeout(auth_sae_retransmit_timer, hapd, sta);
}
				     ...
				     struct sta_info *sta)
{
	...
	if (hapd->dot11RSNASAERetransPeriod == 0)
	{
		hapd->dot11RSNASAERetransPeriod = DEFAULT_DOT11_RSNA_SAE_RETRANS_PERIOD;
	}

	da16x_eloop_cancel_timeout(auth_sae_retransmit_timer, hapd, sta);
	da16x_eloop_register_timeout(0, hapd->dot11RSNASAERetransPeriod * 1000,
			       auth_sae_retransmit_timer, hapd, sta);
}
		       ...
		       const u8 *bssid, u8 auth_transaction)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "[%s] SAE: Peer " MACSTR " state=%s(%d) auth_trans=%u", __func__,
		   MAC2STR(sta->addr), sae_state_txt(sta->sae->state), sta->sae->state,
		   auth_transaction);
	...
	case SAE_ACCEPTED:
		...
		if (auth_transaction == 1 &&
			...
			wpa_printf_dbg(MSG_DEBUG, "SAE: remove the STA (" MACSTR
...
#endif /* CONFIG_MESH */
 		if (auth_transaction == 1) {
			wpa_printf_dbg(MSG_DEBUG, "SAE: Start reauthentication");
			ret = auth_sae_send_commit(hapd, sta, bssid, 1);
			if (ret)
				return ret;
			sae_set_state(sta, SAE_COMMITTED, "Sent Commit");
...
static void sae_pick_next_group(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "SAE: Previously selected group: %d", sae->group);
	...
	wpa_printf_dbg(MSG_DEBUG, "SAE: Selected new group: %d", groups[i]);
}
#endif /* CONFIG_MESH */


#define	SAE_IGNORE_DUPLICATED_CONFIRM_MSG
			    ...
			    u16 auth_transaction, u16 status_code)
{
	...
	extern int sae_is_duplicate_confirm(struct sae_data *sae, const u8 *data, size_t len);

	RX_FUNC_START("");
	wpa_printf(MSG_USER_DEBUG, "[%s] "MACSTR2" auth_trans=%d group=%d status_code=%02d sae_state=%s(%d)",
		__func__,
		MAC2STR(sta->addr),
		auth_transaction, sta->sae->group, status_code,
		sae_state_txt(sta->sae->state), sta->sae->state);

#ifdef CONFIG_TESTING_OPTIONS
	if (hapd->conf->sae_reflection_attack && auth_transaction == 1) {
		const u8 *pos, *end;

		wpa_printf_dbg(MSG_DEBUG, "SAE: TESTING - reflection attack");
	...
	if (auth_transaction == 1) {
		...
		hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211,
			       HOSTAPD_LEVEL_DEBUG,
			       "start SAE authentication auth_transaction=%u (RX commit, status=%u)", auth_transaction,
			       status_code);
		...
		if (
...
#ifdef	SAE_IGNORE_DUPLICATED_CONFIRM_MSG
			if (!sae_is_duplicate_confirm(sta->sae,
				...
				wpa_printf_dbg(MSG_DEBUG, "[%s:%d]recv same confirm",
					__func__, __LINE__);
			...
			sae_set_state(sta, SAE_NOTHING,
				      "Clear existing state to allow restart");
			sae_clear_data(sta->sae);
#endif	/* SAE_IGNORE_DUPLICATED_CONFIRM_MSG */
		}
		...
		if (token && check_sae_token(hapd, sta->addr, token, token_len)
		    < 0) {
			wpa_printf_dbg(MSG_DEBUG, "SAE: Drop commit message with "
		...
		hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211,
			       HOSTAPD_LEVEL_DEBUG,
			       "SAE authentication auth_transaction=%u (RX confirm, status=%u)", auth_transaction,
			       status_code);
		if (status_code != WLAN_STATUS_SUCCESS)
			goto remove_sta;
		...
		if (sta->sae->state >= SAE_CONFIRMED
			...
			if (sae_check_confirm(sta->sae, var, var_len) < 0) {
				wpa_printf(MSG_WARNING,
					   "[%s] SAE: WLAN_STATUS_UNSPECIFIED_FAILURE from "
					   MACSTR, __func__, MAC2STR(sta->addr));
...
#ifdef __APPLE_DEVICE_PATCH__
		if (resp == WLAN_STATUS_UNSPECIFIED_FAILURE)
		{
			//PRINTF(ANSI_COLOR_YELLOW "[%s]%d resp=%d sta->added_unassoc = %d\n" ANSI_COLOR_DEFULT, __func__, __LINE__, resp, sta->added_unassoc);
			sae_set_state(sta, SAE_COMMITTED, "ReInit");

		}
#endif // __APPLE_DEVICE_PATCH__
	}
				 ...
				 struct wpabuf *data, int pub))
{
	...
	if (auth_alg == WLAN_AUTH_FILS_SK_PFS) {
		...
		if ((size_t) (end - pos) < elem_len) {
			wpa_printf_dbg(MSG_DEBUG, "FILS: No room for Element");
			resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
			goto fail;
		}
		...
		sta->fils_dh_ss = crypto_ecdh_set_peerkey(sta->fils_ecdh, 1,
							  pos, elem_len);
		if (!sta->fils_dh_ss) {
			wpa_printf_dbg(MSG_DEBUG, "FILS: ECDH operation failed");
			resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
			goto fail;
		}
	...
	if (ieee802_11_parse_elements(pos, end - pos, &elems, 1) == ParseFailed) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: Could not parse elements");
		resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
		goto fail;
	}
	...
	if (!elems.rsn_ie ||
	    wpa_parse_wpa_ie_rsn(elems.rsn_ie - 2, elems.rsn_ie_len + 2,
				 &rsn) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: No valid RSN element");
		resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
		goto fail;
	}
	...
	if (!elems.fils_nonce) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: No FILS Nonce field");
		resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
		goto fail;
	}
	...
	if (pmksa)
		wpa_printf_dbg(MSG_DEBUG, "FILS: Found matching PMKSA cache entry");

	/* FILS Session */
	if (!elems.fils_session) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: No FILS Session element");
		resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
		goto fail;
	}
		       ...
		       int *is_pub)
{
	...
	if (!pmksa && erp_resp) {
		...
		if (fils_rmsk_to_pmk(wpa_auth_sta_key_mgmt(sta->wpa_sm),
				     ...
				     pmk_buf, &pmk_len)) {
			wpa_printf_dbg(MSG_DEBUG, "FILS: Failed to derive PMK");
			*resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
			wpabuf_free(data);
			data = NULL;
			goto fail;
	...
	if (!pmk) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: No PMK available");
		*resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
		wpabuf_free(data);
		data = NULL;
		goto fail;
			   ...
			   char **identity, char **radius_cui, int is_probe_req)
{
	...
	if (res == HOSTAPD_ACL_PENDING) {
		wpa_printf_dbg(MSG_DEBUG, "Authentication frame from " MACSTR
			...
			const struct _ieee80211_mgmt *mgmt, size_t len)
{
...
#if defined CONFIG_SAE || defined CONFIG_WPA_PRINTF_DBG
	u16 status_code;
#endif /* CONFIG_SAE || CONFIG_WPA_PRINTF_DBG */
	u16 resp = WLAN_STATUS_SUCCESS;
...
#if defined	CONFIG_LAST_SEQ_CTRL || defined CONFIG_WPA_PRINTF_DBG
	u16 seq_ctrl;
#endif	/* CONFIG_LAST_SEQ_CTRL ||CONFIG_WPA_PRINTF_DBG */

#ifdef CONFIG_ACS
	if (hapd->iconf->channel == 0)
	{
		da16x_ap_prt("[%s] Deny: ACS is not complete. " MACSTR "\n", __func__, MAC2STR(mgmt->sa));
		return;
	}
#endif // CONFIG_ACS
	...
	wpa_printf_dbg(MSG_DEBUG, "authentication: STA=" MACSTR " auth_alg=%s(%d) "
		   "auth_transaction=%d status_code=%d wep=%d%s "
		   "seq_ctrl=0x%x%s",
		   MAC2STR(mgmt->sa), auth_alg2string(auth_alg), auth_alg,
		   auth_transaction, status_code, !!(fc & WLAN_FC_ISWEP),
		   challenge ? " challenge" : "",
	...
	if (hapd->conf->no_auth_if_seen_on) {
		...
		if (other) {
			...
			wpa_printf_dbg(MSG_DEBUG, "%s: Reject authentication from "
	...
	if (sta) {
		...
		if ((hapd->conf->mesh & MESH_ENABLED) &&
			...
			wpa_printf_dbg(MSG_DEBUG, "Mesh peer " MACSTR
	...
	} else
	{
		...
		if (!sta) {
			wpa_printf_dbg(MSG_DEBUG, "ap_sta_add() failed");
	...
	if (res) {
		wpa_printf_dbg(MSG_DEBUG, "ieee802_11_set_radius_info() failed");
		resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
		goto fail;
	}
#endif /* CONFIG_RADIUS */
...
#if 1
/*
commit: b5ebe5dd5b03f4b499b88a6ce14dc67f13a0d212
2018.08.13 pm2:45
	...
	case WLAN_AUTH_FT:
		...
		if (sta->wpa_sm == NULL)
			sta->wpa_sm = wpa_auth_sta_init(hapd->wpa_auth,
							sta->addr, NULL);
		if (sta->wpa_sm == NULL) {
			wpa_printf_dbg(MSG_DEBUG, "FT: Failed to initialize WPA "
				   "state machine");
			resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
			goto fail;
		}
...
#endif /* CONFIG_FILS */
		default:
			PRINTF(ANSI_COLOR_RED "[%s] Unsupport Auth=%d \n" ANSI_COLOR_DEFULT, __func__, auth_alg);
	}
...
int hostapd_get_aid(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	/* get a unique AID */
	if (sta->aid > 0) {
		wpa_printf_dbg(MSG_DEBUG, "  old AID %d", sta->aid);
		return 0;
	}
	...
	wpa_printf_dbg(MSG_DEBUG, "  new AID %d", sta->aid);
	return 0;
}
				 ...
				 u8 owe_dh_len)
{
	...
	if (wpa_auth_sta_get_pmksa(sta->wpa_sm)) {
		wpa_printf_dbg(MSG_DEBUG, "OWE: Using PMKSA caching");
		return WLAN_STATUS_SUCCESS;
	}

	group = WPA_GET_LE16(owe_dh);
	if (!owe_group_supported(hapd, group)) {
		wpa_printf_dbg(MSG_DEBUG, "OWE: Unsupported DH group %u", group);
	...
	if (!secret) {
		wpa_printf_dbg(MSG_DEBUG, "OWE: Invalid peer DH public key");
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
			   ...
			   const u8 *ies, size_t ies_len, int reassoc)
{
...
#ifdef CONFIG_AP_WIFI_MODE // FC9000 Only
	if(hapd->iconf->wifi_mode == DA16X_WIFI_MODE_G_ONLY ||
		hapd->iconf->wifi_mode == DA16X_WIFI_MODE_GN) {
...
#else
	if (hapd->iconf->require_vht && !(sta->flags & WLAN_STA_VHT)) {
#endif	/* CONFIG_IEEE80211AC */
		hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211,
			       HOSTAPD_LEVEL_INFO, "Station does not support "
	...
	if (hapd->conf->wps_state && elems.wps_ie) {
		wpa_printf_dbg(MSG_DEBUG, "STA included WPS IE in (Re)Association "
		...
		if (sta->wps_ie && wps_is_20(sta->wps_ie)) {
			wpa_printf_dbg(MSG_DEBUG, "WPS: STA supports WPS 2.0");
			sta->flags |= WLAN_STA_WPS2;
		}
		wpa_ie = NULL;
		wpa_ie_len = 0;
		if (sta->wps_ie && wps_validate_assoc_req(sta->wps_ie) < 0) {
			wpa_printf_dbg(MSG_DEBUG, "WPS: Invalid WPS IE in "
				   "(Re)Association Request - reject");
			return WLAN_STATUS_INVALID_IE;
		}
	} else if (hapd->conf->wps_state && wpa_ie == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "STA did not include WPA/RSN IE in "
	...
	if (hapd->conf->wpa && wpa_ie) {
		...
		{
#ifdef __APPLE_DEVICE_PATCH__
				if (resp == WLAN_STATUS_INVALID_PMKID)
				{
					if (ieee802_11_vendor_ie_concat(ies, ies_len, OUI_APPLE) != NULL) /* Apple Device */
					{
						//PRINTF(ANSI_COLOR_LIGHT_MAGENTA "[%s] Apple Device\n" ANSI_COLOR_DEFULT, __func__);
						return WLAN_STATUS_ASSOC_REJECTED_TEMPORARILY;
					}
					else
					{
						; //PRINTF(ANSI_COLOR_LIGHT_MAGENTA "[%s] Normal Device\n" ANSI_COLOR_DEFULT, __func__);
					}
				}
#endif // __APPLE_DEVICE_PATCH__
			return resp;
		...
		if (sta->auth_alg == WLAN_AUTH_FT) {
			if (!reassoc) {
				wpa_printf_dbg(MSG_DEBUG, "FT: " MACSTR " tried "
		...
		if (wpa_auth_uses_sae(sta->wpa_sm) &&
			...
			if (!sa || sa->akmp != WPA_KEY_MGMT_SAE) {
				...
				if (ieee802_11_vendor_ie_concat(ies, ies_len, OUI_APPLE) != NULL) /* Apple Device */
				{
					//PRINTF(ANSI_COLOR_LIGHT_MAGENTA "[%s] #2 Apple Device\n" ANSI_COLOR_DEFULT, __func__);
					return WLAN_STATUS_ASSOC_REJECTED_TEMPORARILY;
				}
				else
#endif // __APPLE_DEVICE_PATCH__
				{
					; //PRINTF(ANSI_COLOR_LIGHT_MAGENTA "[%s] #2 Normal Device\n" ANSI_COLOR_DEFULT, __func__);
					return WLAN_STATUS_INVALID_PMKID;
				}
			}
			wpa_printf_dbg(MSG_DEBUG, "SAE: " MACSTR
				   " using PMKSA caching", MAC2STR(sta->addr));
		} else if (wpa_auth_uses_sae(sta->wpa_sm) &&
			   sta->auth_alg != WLAN_AUTH_SAE &&
			   !(sta->auth_alg == WLAN_AUTH_FT &&
			     wpa_auth_uses_ft_sae(sta->wpa_sm))) {
			wpa_printf_dbg(MSG_DEBUG, "SAE: " MACSTR " tried to use "
		...
		wpa_printf_dbg(MSG_DEBUG, "HS 2.0: OSEN association");
		if (sta->wpa_sm == NULL)
			sta->wpa_sm = wpa_auth_sta_init(hapd->wpa_auth,
							sta->addr, NULL);
		if (sta->wpa_sm == NULL) {
			      ...
			      struct sta_info *sta)
{
	...
	if (hostapd_sta_add(hapd, sta->addr, sta->aid, sta->capability,
			    sta->supported_rates, sta->supported_rates_len,
			    sta->listen_interval,
			    sta->flags & WLAN_STA_HT ? &ht_cap : NULL,	/* ht_capab */
#ifdef CONFIG_AP_VHT
			    sta->flags & WLAN_STA_VHT ? &vht_cap : NULL,	/* vht_capab */
#endif /* CONFIG_AP_VHT */
			    sta->flags | WLAN_STA_ASSOC,	/* flags */
			    sta->qosinfo,	/* qosinfo */
#ifdef CONFIG_AP_VHT
			    sta->vht_opmode,	/* vht_opmode */
#endif /* CONFIG_AP_VHT */
			    sta->p2p_ie ? 1 : 0,	/* supp_p2p_ps */
			   ...
			   const u8 *ies, size_t ies_len)
{
	...
	buflen = sizeof(struct _ieee80211_mgmt) + 1024;
#ifdef CONFIG_FILS
	...
	if (sta &&
		...
		if (ieee802_11_parse_elements(ies, ies_len, &elems, 0) ==
			   ...
			   u8 *owe_buf, size_t owe_buf_len, u16 *reason)
{
	...
	if (hapd->conf->own_ie_override) {
		wpa_printf_dbg(MSG_DEBUG, "OWE: Using IE override");
...
void fils_hlp_finish_assoc(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FILS: Finish association with " MACSTR,
		   MAC2STR(sta->addr));
			 ...
			 int reassoc)
{
	...
	if (reassoc) {
		...
		wpa_printf_dbg(MSG_DEBUG, "reassociation request: STA=" MACSTR
		...
		wpa_printf_dbg(MSG_DEBUG, "association request: STA=" MACSTR
	...
	if (sta && sta->auth_alg == WLAN_AUTH_FT &&
		...
		wpa_printf_dbg(MSG_DEBUG, "FT: Allow STA " MACSTR " to associate "
...
#ifdef CONFIG_IEEE80211W
	if ((sta->flags & WLAN_STA_MFP) && sta->sa_query_timed_out) {
		wpa_printf_dbg(MSG_DEBUG, "Allowing %sassociation after timed out "
 ...
 fail:
	...
	if (sta && delay_assoc && resp == WLAN_STATUS_SUCCESS) {
		...
		wpa_printf_dbg(MSG_DEBUG,
			   ...
			   MACSTR, MAC2STR(sta->addr));
		da16x_eloop_cancel_timeout(fils_hlp_timeout, hapd, sta);
		da16x_eloop_register_timeout(0, hapd->conf->fils_hlp_wait_time * 1024,
			    ...
			    const struct _ieee80211_mgmt *mgmt, size_t len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "disassocation: STA=" MACSTR " reason_code=%d",
		   MAC2STR(mgmt->sa),
...
#ifdef	CONFIG_IEEE80211W
	if (mgmt->u.disassoc.reason_code == WLAN_REASON_UNSPECIFIED)
		hostapd_drv_sta_disassoc(hapd, sta->addr, WLAN_REASON_CLASS3_FRAME_FROM_NONASSOC_STA);
#endif	/* CONFIG_IEEE80211W */
			  ...
			  const struct _ieee80211_mgmt *mgmt, size_t len)
{
	...
	if (len < IEEE80211_HDRLEN + sizeof(mgmt->u.deauth)) {
		wpa_dbg(hapd->msg_ctx, MSG_DEBUG, "handle_deauth - too short "
			"payload (len=%lu)", (unsigned long) len);
		return;
	}

	wpa_dbg(hapd->msg_ctx, MSG_DEBUG, "deauthentication: STA=" MACSTR
		" reason_code=%d",
		MAC2STR(mgmt->sa), le_to_host16(mgmt->u.deauth.reason_code));

	sta = ap_get_sta(hapd, mgmt->sa);
	if (sta == NULL) {
		wpa_dbg(hapd->msg_ctx, MSG_DEBUG, "Station " MACSTR " trying "
			  ...
			  struct hostapd_frame_info *fi)
{
	...
	(void) ieee802_11_parse_elements(mgmt->u.beacon.variable,
				      len - (IEEE80211_HDRLEN +
				   ...
				   size_t len)
{
	...
	if (((u8 *) mgmt) + len < end) {
		wpa_printf_dbg(MSG_DEBUG, "IEEE 802.11: Too short SA Query Action "
			 ...
			 unsigned int freq)
{
	...
	if (mgmt->u.action.category != WLAN_ACTION_PUBLIC &&
		...
		wpa_printf_dbg(MSG_DEBUG, "IEEE 802.11: Ignored Action "
	...
	if (!is_multicast_ether_addr(mgmt->da) &&
		 ...
		 */
		wpa_printf_dbg(MSG_DEBUG, "IEEE 802.11: Return unknown Action "
		    ...
		    struct hostapd_frame_info *fi)
{
	...
	case WLAN_FC_STYPE_AUTH:
		wpa_printf_dbg(MSG_DEBUG, "[%s] mgmt::auth", __func__);
		handle_auth(hapd, mgmt, len);
		ret = 1;
		break;
	case WLAN_FC_STYPE_ASSOC_REQ:
		wpa_printf_dbg(MSG_DEBUG, "mgmt::assoc_req");
		handle_assoc(hapd, mgmt, len, 0);
		ret = 1;
		break;
	case WLAN_FC_STYPE_REASSOC_REQ:
		wpa_printf_dbg(MSG_DEBUG, "mgmt::reassoc_req");
		handle_assoc(hapd, mgmt, len, 1);
		ret = 1;
		break;
	case WLAN_FC_STYPE_DISASSOC:
		wpa_printf_dbg(MSG_DEBUG, "mgmt::disassoc");
	...
	case WLAN_FC_STYPE_DEAUTH:
		wpa_dbg(hapd->msg_ctx, MSG_DEBUG, "mgmt::deauth");
	...
	case WLAN_FC_STYPE_ACTION:
		wpa_printf_dbg(MSG_DEBUG, "mgmt::action");
			   ...
			   size_t len, int ok)
{
	...
	if (!sta) {
		wpa_printf_dbg(MSG_DEBUG, "handle_auth_cb: STA " MACSTR " not found",
			   MAC2STR(mgmt->da));
		return;
	}
			    ...
			    size_t len, int reassoc, int ok)
{
	...
	if (!(sta->flags & WLAN_STA_WDS) && sta->pending_wds_enable) {
		wpa_printf_dbg(MSG_DEBUG, "Enable 4-address WDS mode for STA "
	...
	if (sta->flags & WLAN_STA_WDS) {
		...
		wpa_printf_dbg(MSG_DEBUG, "Reenable 4-address WDS mode for STA "
			   MACSTR " (aid %u)",
	...
	if ((sta->auth_alg == WLAN_AUTH_FILS_SK ||
		...
		wpa_printf_dbg(MSG_DEBUG, "FILS: TK configuration failed");
			     ...
			     size_t len, int ok)
{
	...
	if (!sta) {
		wpa_printf_dbg(MSG_DEBUG, "handle_deauth_cb: STA " MACSTR
			   " not found", MAC2STR(mgmt->da));
		return;
	}

	if (ok) {
		wpa_printf_dbg(MSG_DEBUG, "STA " MACSTR " acknowledged deauth",
			   MAC2STR(sta->addr));
	} else {
		wpa_printf_dbg(MSG_DEBUG, "STA " MACSTR " did not acknowledge "
			       ...
			       size_t len, int ok)
{
	...
	if (!sta) {
		wpa_printf_dbg(MSG_DEBUG, "handle_disassoc_cb: STA " MACSTR
			   " not found", MAC2STR(mgmt->da));
		return;
	}

	if (ok) {
		wpa_printf_dbg(MSG_DEBUG, "STA " MACSTR " acknowledged disassoc",
			   MAC2STR(sta->addr));
	} else {
		wpa_printf_dbg(MSG_DEBUG, "STA " MACSTR " did not acknowledge "
			     ...
			     size_t len, int ok)
{
	...
	sta = ap_get_sta(hapd, mgmt->da);
	if (!sta) {
		wpa_printf_dbg(MSG_DEBUG, "handle_action_cb: STA " MACSTR
			   " not found", MAC2STR(mgmt->da));
		return;
	}
			...
			u16 stype, int ok)
{
	...
	switch (stype) {
	case WLAN_FC_STYPE_AUTH:
		wpa_printf_dbg(MSG_DEBUG, "mgmt::auth cb");
		handle_auth_cb(hapd, mgmt, len, ok);
		break;
	case WLAN_FC_STYPE_ASSOC_RESP:
		wpa_printf_dbg(MSG_DEBUG, "mgmt::assoc_resp cb");
		handle_assoc_cb(hapd, mgmt, len, 0, ok);
		break;
	case WLAN_FC_STYPE_REASSOC_RESP:
		wpa_printf_dbg(MSG_DEBUG, "mgmt::reassoc_resp cb");
	...
	case WLAN_FC_STYPE_PROBE_RESP:
		...
		break;
	case WLAN_FC_STYPE_DEAUTH:
		wpa_printf_dbg(MSG_DEBUG, "mgmt::deauth cb");
		handle_deauth_cb(hapd, mgmt, len, ok);
		break;
	case WLAN_FC_STYPE_DISASSOC:
		wpa_printf_dbg(MSG_DEBUG, "mgmt::disassoc cb");
		handle_disassoc_cb(hapd, mgmt, len, ok);
		break;
	case WLAN_FC_STYPE_ACTION:
		wpa_printf_dbg(MSG_DEBUG, "mgmt::action cb ok=%d", ok);
		       ...
		       const u8 *buf, size_t len, int ack)
{
	...
	if (sta->flags & WLAN_STA_PENDING_POLL) {
		wpa_printf_dbg(MSG_DEBUG, "STA " MACSTR " %s pending "
			     ...
			     const u8 *data, size_t len, int ack)
{
	...
	if (sta == NULL || !(sta->flags & WLAN_STA_ASSOC)) {
		wpa_printf_dbg(MSG_DEBUG, "Ignore TX status for Data frame to STA "
...
void hostapd_client_poll_ok(struct hostapd_data *hapd, const u8 *addr)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "STA " MACSTR " ACKed pending "
		   "activity poll", MAC2STR(sta->addr));
	sta->flags &= ~WLAN_STA_PENDING_POLL;
}
				...
				int wds)
{
	...
	if (sta &&
		...
		if (wds && !(sta->flags & WLAN_STA_WDS)) {
			...
			wpa_printf_dbg(MSG_DEBUG, "Enable 4-address WDS mode for "
				   "STA " MACSTR " (aid %u)",
				   MAC2STR(sta->addr), sta->aid);
			sta->flags |= WLAN_STA_WDS;
	...
	wpa_printf_dbg(MSG_DEBUG, "Data/PS-poll frame from not associated STA "
		   MACSTR, MAC2STR(src));
	...
	if (sta && (sta->flags & WLAN_STA_ASSOC_REQ_OK)) {
		wpa_printf_dbg(MSG_DEBUG, "Association Response to the STA has "
