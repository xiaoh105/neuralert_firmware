...
static int dfs_is_chan_allowed(struct hostapd_channel_data *chan, int n_chans)
{
	...
	case 8:
		...
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "Unknown width for %d channels", n_chans);
		break;
	}
			    ...
			    int idx, int skip_radar)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "DFS new chan checking %d channels", n_chans);
	for (i = 0; i < mode->num_channels; i++) {
		...
		if (ret_chan && idx == channel_idx) {
			wpa_printf_dbg(MSG_DEBUG, "Selected ch. #%d", chan->chan);
			*ret_chan = chan;
			return idx;
		}
		wpa_printf_dbg(MSG_DEBUG, "Adding channel: %d", chan->chan);
		channel_idx++;
	}
	return channel_idx;
}
				       ...
				       u8 *vht_oper_centr_freq_seg1_idx)
{
	...
	*vht_oper_centr_freq_seg1_idx = 0;
	...
	wpa_printf_dbg(MSG_DEBUG, "DFS adjusting VHT center frequency: %d, %d",
...
static int dfs_get_start_chan_idx(struct hostapd_iface *iface)
{
	...
	if (res == -1) {
		...
		for (i = 0; i < mode->num_channels; i++) {
			wpa_printf_dbg(MSG_DEBUG, "Available channel: %d",
		      ...
		      int skip_radar)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "DFS: Selecting random channel");
	*secondary_channel = 0;
	*vht_oper_centr_freq_seg0_idx = 0;
	*vht_oper_centr_freq_seg1_idx = 0;
...
static int set_dfs_state_freq(struct hostapd_iface *iface, int freq, u32 state)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "set_dfs_state 0x%X for %d MHz", state, freq);
			 ...
			 int cf2, u32 state)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "DFS freq: %dMHz, n_chans: %d", frequency,
		   n_chans);
	for (i = 0; i < n_chans; i++) {
				       ...
				       int chan_width, int cf1, int cf2)
{
	...
	for (i = 0; i < n_chans; i++) {
		...
		for (j = 0; j < radar_n_chans; j++) {
			wpa_printf_dbg(MSG_DEBUG, "checking our: %d, radar: %d",
	...
	wpa_printf_dbg(MSG_DEBUG, "overlapped: %d", res);
...
int hostapd_handle_dfs(struct hostapd_iface *iface)
{
	...
	do {
		...
		res = dfs_check_chans_unavailable(iface, start_chan_idx,
						  n_chans);
		wpa_printf_dbg(MSG_DEBUG, "DFS %d chans unavailable - choose other channel: %s",
			   res, res ? "yes": "no");
		if (res) {
			int sec = 0;
			u8 cf1 = 0, cf2 = 0;
	...
	wpa_printf_dbg(MSG_DEBUG, "DFS start CAC on %d MHz", iface->freq);
...
static int hostapd_dfs_start_channel_switch_cac(struct hostapd_iface *iface)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "DFS will switch to a new channel %d",
		   channel->chan);
...
static int hostapd_dfs_start_channel_switch(struct hostapd_iface *iface)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "%s called (CAC active: %s, CSA active: %s)",
	...
	wpa_printf_dbg(MSG_DEBUG, "DFS waiting channel switch event");
	return 0;
}
