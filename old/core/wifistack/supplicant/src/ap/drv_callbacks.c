				      ...
				      struct sta_info *sta)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "%s FILS: Finish association with " MACSTR,
		   __func__, MAC2STR(sta->addr));
	da16x_eloop_cancel_timeout(fils_hlp_timeout, hapd, sta);
	if (!sta->fils_pending_assoc_req)
		return;

	ieee802_11_parse_elements(sta->fils_pending_assoc_req,
			       sta->fils_pending_assoc_req_len, &elems, 0);
	if (!elems.fils_session) {
		wpa_printf_dbg(MSG_DEBUG, "%s failed to find FILS Session element",
			   __func__);
		return;
	}
...
#ifdef	CONFIG_SUPP27_AP_NOTIF_ASSOC
			...
			const u8 *req_ies, size_t req_ies_len, int reassoc)
{
	...
	const u8 *ie;
	size_t ielen;
#if defined(CONFIG_IEEE80211R_AP) || defined(CONFIG_IEEE80211W) || defined(CONFIG_OWE) || defined(CONFIG_FILS)
	u8 buf[sizeof(struct _ieee80211_mgmt) + 1024];
	u8 *p = buf;
#endif /* CONFIG_IEEE80211R_AP || CONFIG_IEEE80211W || CONFIG_OWE || CONFIG_FILS */
	...
	ieee802_11_parse_elements(req_ies, req_ies_len, &elems, 0);
	if (elems.wps_ie) {
		ie = elems.wps_ie - 2;
		ielen = elems.wps_ie_len + 2;
		wpa_printf_dbg(MSG_DEBUG, "STA included WPS IE in (Re)AssocReq");
	} else if (elems.rsn_ie) {
		ie = elems.rsn_ie - 2;
		ielen = elems.rsn_ie_len + 2;
		wpa_printf_dbg(MSG_DEBUG, "STA included RSN IE in (Re)AssocReq");
	} else if (elems.wpa_ie) {
		ie = elems.wpa_ie - 2;
		ielen = elems.wpa_ie_len + 2;
		wpa_printf_dbg(MSG_DEBUG, "STA included WPA IE in (Re)AssocReq");
#ifdef CONFIG_HS20
	} else if (elems.osen) {
		ie = elems.osen - 2;
		ielen = elems.osen_len + 2;
		wpa_printf_dbg(MSG_DEBUG, "STA included OSEN IE in (Re)AssocReq");
#endif /* CONFIG_HS20 */
	} else {
		ie = NULL;
		ielen = 0;
	...
	if (hapd->conf->wpa) {
		if (ie == NULL || ielen == 0) {
			...
			wpa_printf_dbg(MSG_DEBUG, "No WPA/RSN IE from STA");
...
skip_wpa_check:
	...
	if (!p) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Failed to write AssocResp IEs");
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
#endif /* CONFIG_IEEE80211R_AP */
	...
	if (sta->auth_alg == WLAN_AUTH_FILS_SK ||
		...
		if (sta) {
			wpa_printf_dbg(MSG_DEBUG, "FILS: HLP callback cleanup");
			da16x_eloop_cancel_timeout(fils_hlp_timeout, hapd, sta);
			os_free(sta->fils_pending_assoc_req);
			sta->fils_pending_assoc_req = NULL;
			sta->fils_pending_assoc_req_len = 0;
...
#endif	/* CONFIG_SUPP27_AP_NOTIF_ASSOC */
			      ...
			      struct rx_mgmt *drv_mgmt)
{
	...
	mgmt = (struct _ieee80211_mgmt *) drv_mgmt->frame;
	fc = le_to_host16(mgmt->frame_control);
	if (WLAN_FC_GET_STYPE(fc) != WLAN_FC_STYPE_ACTION)
		return; /* handled by the driver */

	wpa_printf_dbg(MSG_DEBUG, "RX_ACTION cat %d action plen %d",
		   mgmt->u.action.category, (int) plen);

	sta = ap_get_sta(hapd, mgmt->sa);
	if (sta == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "%s: station not found", __func__);
		return;
	}
#ifdef CONFIG_IEEE80211R_AP
...
static int hostapd_event_new_sta(struct hostapd_data *hapd, const u8 *addr)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Data frame from unknown STA " MACSTR
		   " - adding a new STA", MAC2STR(addr));
	sta = ap_sta_add(hapd, addr);
	if (sta) {
		hostapd_new_assoc_sta(hapd, sta, 0);
	} else {
		wpa_printf_dbg(MSG_DEBUG, "Failed to add STA entry for " MACSTR,
			   MAC2STR(addr));
		return -1;
	}
					      ...
					      struct survey_results *survey_res)
{
	...
	if (survey->channel_time > iface->last_channel_time &&
		...
		wpa_printf_dbg(MSG_DEBUG, "Channel Utilization: %d",
			      ...
			      struct survey_results *survey_results)
{
	...
	if (dl_list_empty(&survey_results->survey_list)) {
		wpa_printf_dbg(MSG_DEBUG, "No survey data received");
		return;
	}
...
{
	wpa_printf_dbg(MSG_DEBUG, "Interface %s is unavailable -- stopped",
		   hapd->conf->iface);
...
static void hostapd_event_dfs_pre_cac_expired(struct hostapd_data *hapd,
					      struct dfs_event *radar)
{
	wpa_printf_dbg(MSG_DEBUG, "DFS Pre-CAC expired on %d MHz", radar->freq);
...
static void hostapd_event_dfs_cac_started(struct hostapd_data *hapd,
					  struct dfs_event *radar)
{
	wpa_printf_dbg(MSG_DEBUG, "DFS offload CAC started on %d MHz", radar->freq);
			  ...
			  union wpa_event_data *data)
{
...
#if 1
	wpa_dbg(hapd->msg_ctx, level, ANSI_COLOR_BLUE "[%s] Event %s (%d) received" ANSI_COLOR_DEFULT, __func__,
	...
	switch (event) {
		...
		case EVENT_SURVEY:
			...
			break;
#ifdef NEED_AP_MLME
#ifdef	CONFIG_SUPP27_AP_DRV_CB
		case EVENT_INTERFACE_UNAVAILABLE:
			hostapd_event_iface_unavailable(hapd);
			break;
#endif	/* CONFIG_SUPP27_AP_DRV_CB */
...
#endif	/* CONFIG_SUPP27_AP_DRV_CB */
		default:
			wpa_printf_dbg(MSG_DEBUG, "Unknown event %d", event);
			break;
	}
}
