...
#if defined (__TCP_CLIENT_DPM_SAMPLE__) && defined (__SUPPORT_DPM_MANAGER__)
...
#define TCP_CLIENT_DPM_SAMPLE_DEF_CLIENT_PORT				TCP_CLI_TEST_PORT
...
//Function prototype
void tcp_client_dpm_sample_init_callback(void);
void tcp_client_dpm_sample_wakeup_callback(void);
void tcp_client_dpm_sample_external_callback(void);
...
void tcp_client_dpm_sample_recv_callback(void *sock, UCHAR *rx_buf, UINT rx_len, ULONG rx_ip, ULONG rx_port);
void tcp_client_dpm_sample_init_user_config(dpm_user_config_t *user_config);
unsigned char tcp_client_dpm_sample_load_server_info(void);
void tcp_client_dpm_sample(void *param);
...
void tcp_client_dpm_sample_init_user_config(dpm_user_config_t *user_config)
{
    ...
    //Set local port
    user_config->sessionConfig[session_idx].sessionMyPort = TCP_CLIENT_DPM_SAMPLE_DEF_CLIENT_PORT;
    ...
    user_config->sessionConfig[session_idx].sessionConnectCallback = tcp_client_dpm_sample_connect_callback;

    //Set Recv callback
    user_config->sessionConfig[session_idx].sessionRecvCallback = tcp_client_dpm_sample_recv_callback;

    //Set connection retry count
    user_config->sessionConfig[session_idx].sessionConnRetryCnt = TCP_CLIENT_DPM_SAMPLE_DEF_MAX_CONNECTION_RETRY;

    //Set connection timeout
    user_config->sessionConfig[session_idx].sessionConnWaitTime = TCP_CLIENT_DPM_SAMPLE_DEF_MAX_CONNECTION_TIMEOUT;
...
#endif // (__TCP_CLIENT_DPM_SAMPLE__) && (__SUPPORT_DPM_MANAGER__)
