...
void app_user_data_dpm_mem_alloc(void)
{
    ...
    if (dpm_boot_type > DPM_BOOT_TYPE_NO_DPM && wf_svc_app_data_in_rtm == NULL) {
        ...
        if (status) {
            // user data exists in rtm but corrupted
            PRINTF("[%s] RTM alloc failed! (%d), reboot ....\n", __func__, status);
            combo_ble_sw_reset();
            reboot_func(SYS_REBOOT);
        }
    } else {
...
{
    if (dpm_mode_get_wakeup_source() == WAKEUP_SENSOR_WITH_RETENTION) {     // wake-up by ble
        return TRUE;
    } else {
        return FALSE;
    }
...
void app_set_mode(void)
{
    ...
    // Maximal Tx octets
    message->max_txoctets = 251;// 0 for not enabling DLE at first

    // Maximal Tx time
    message->max_txtime = 2120; // 0 for not enabling DLE at first
...
void app_diss_create_db(void)
{
    ...
    req->app_task = TASK_ID_GTL; // to check why it is different from cas prj
    req->start_hdl = 0;
    ...
    switch (type)
    {
        ...
        case CONSOLE_DISCONNECT_CMD:
            if (app_env.state == APP_CONNECTED) {
                ...
                DBG_ERR("BLE is not in connected state. \n");
            }
