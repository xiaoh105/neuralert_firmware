...
void HandleBleMsg(ble_msg *blemsg)
{
	...
	switch (blemsg->bType)
	{
		...
		case APP_GAS_LEAK_SENSOR_START_CFM:
		{
			DBG_INFO("\n<<< APP_GAS_LEAK_SENSOR_START_CFM \n");
			app_sensor_start_cfm_hnd(blemsg->bType, 
									(app_gas_leak_sensor_start_cfm_t *)blemsg->bData,
									 blemsg->bDstid, blemsg->bSrcid);
		} break;

		case APP_GAS_LEAK_SENSOR_STOP_CFM:
		{
			DBG_INFO("\n<<< APP_GAS_LEAK_SENSOR_STOP_CFM \n");
			app_sensor_stop_cfm_hnd(blemsg->bType, 
									(app_gas_leak_sensor_stop_cfm_t *)blemsg->bData,
									blemsg->bDstid, blemsg->bSrcid);
		} break;

		case APP_GAS_LEAK_EVT_IND:
		{
			DBG_INFO(YELLOW_COLOR"\n<<< APP_GAS_LEAK_EVT_IND \n"CLEAR_COLOR);
			app_sensor_event_ind_hnd(blemsg->bType, 
									(app_gas_leak_evt_ind_t *)blemsg->bData,
									 blemsg->bDstid, blemsg->bSrcid);
		} break;
