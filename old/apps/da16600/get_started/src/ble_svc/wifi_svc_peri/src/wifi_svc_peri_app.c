...
void app_user_data_save(void)
{
    if (wf_svc_app_data_in_rtm != NULL) {
        ...
        memcpy(&(wf_svc_app_data_in_rtm->user_custom_profile_wfsvc_env),
               &user_custom_profile_wfsvc_env,
               sizeof(user_custom_profile_wfsvc_env_t));
...
void combo_init(void)
{
    ...
    // additional handling on dpm_boot_type if any
...
void app_adv_start(void)
{
    ...
    // Scan Response Data
#if (NVDS_SUPPORT)
    if (nvds_get(NVDS_TAG_APP_BLE_SCAN_RESP_DATA, &cmd->info.host.scan_rsp_data_len, &cmd->info.host.scan_rsp_data[0]) != NVDS_OK)
#endif
    {
...
// blinky ------------------------------------------------------------------------------
void app_peri_blinky_start_send(uint8_t gpio_port, uint8_t gpio_pin, uint8_t blink_count)
{
    app_peri_blinky_start_t *req = (app_peri_blinky_start_t*)BleMsgAlloc(APP_PERI_BLINKY_START,
                                                                         TASK_ID_EXT_HOST_BLE_AUX,
                                                                         TASK_ID_GTL,
                                                                         sizeof(app_peri_blinky_start_t));
    
    req->gpio_port = gpio_port;
    req->gpio_pin  = gpio_pin;
    req->blink_cnt = blink_count;

    BleSendMsg(req);
}

// systick ------------------------------------------------------------------------------
void app_peri_systick_start_send(uint8_t gpio_port, uint8_t gpio_pin, uint32_t period_us)
{
    app_peri_systick_start_t *req = (app_peri_systick_start_t*)BleMsgAlloc(APP_PERI_SYSTICK_START,
                                                                           TASK_ID_EXT_HOST_BLE_AUX,
                                                                           TASK_ID_GTL,
                                                                           sizeof(app_peri_systick_start_t));
    
    req->gpio_port = gpio_port;
    req->gpio_pin  = gpio_pin;
    req->period_us = period_us;

    BleSendMsg(req);
}

void app_peri_systick_stop_send()
{
    void *cmd = BleMsgAlloc(APP_PERI_SYSTICK_STOP, 
                            TASK_ID_EXT_HOST_BLE_AUX, 
                            TASK_ID_GTL, 
                            0);
...
void app_peri_timer0_gen_start_send(uint8_t gpio_port,
                                    ...
                                    int reload_val)
{
    app_peri_timer0_gen_start_t *req = (app_peri_timer0_gen_start_t*)BleMsgAlloc(APP_PERI_TIMER0_GEN_START, 
                                                                                 TASK_ID_EXT_HOST_BLE_AUX, 
                                                                                 TASK_ID_GTL, 
                                                                                 sizeof(app_peri_timer0_gen_start_t));

    req->gpio_port         = gpio_port;
    req->gpio_pin          = gpio_pin;
    req->test_dur          = test_dur;
    req->in_clk_div_factor     = in_clk_div_factor;
    req->timer0_clk_src     = timer0_clk_src;
    req->pwm_mode         = pwm_mode;
    req->timer0_clk_div     = timer0_clk_div;
    req->reload_val     = reload_val;
...
{
    app_peri_timer0_buz_start_t *req = (app_peri_timer0_buz_start_t*)BleMsgAlloc(APP_PERI_TIMER0_BUZ_START, 
                                                                                 TASK_ID_EXT_HOST_BLE_AUX, 
                                                                                 TASK_ID_GTL, 
...
{
    app_peri_timer2_pwm_start_t *req = (app_peri_timer2_pwm_start_t*)BleMsgAlloc(APP_PERI_TIMER2_PWM_START, 
                                                                                 TASK_ID_EXT_HOST_BLE_AUX, 
                                                                                 TASK_ID_GTL, 
                                                                                 sizeof(app_peri_timer2_pwm_start_t));
    
    memcpy((void*)req, (void*)config_param, sizeof(app_peri_timer2_pwm_start_t));
...
{
    // Allocate a message for BLE_AUX
    void *cmd = BleMsgAlloc(APP_PERI_GET_BATT_LVL,
                            TASK_ID_EXT_HOST_BLE_AUX, 
                            TASK_ID_GTL,
                            0);
...
{
    // Allocate a message for BLE_AUX
    void *cmd = BleMsgAlloc(APP_PERI_I2C_EEPROM_START,
                            TASK_ID_EXT_HOST_BLE_AUX, 
                            TASK_ID_GTL,
                            0);
...
{
    // Allocate a message for BLE_AUX
    void *cmd = BleMsgAlloc(APP_PERI_SPI_FLASH_START,
                            TASK_ID_EXT_HOST_BLE_AUX, 
                            TASK_ID_GTL,
                            0);
...
{
    app_peri_quad_dec_start_t *req = (app_peri_quad_dec_start_t*)BleMsgAlloc(APP_PERI_QUAD_DEC_START, 
                                                                             TASK_ID_EXT_HOST_BLE_AUX, 
                                                                             TASK_ID_GTL, 
                                                                             sizeof(app_peri_quad_dec_start_t));
    
    memcpy((void*)req, (void*)config_param, sizeof(app_peri_quad_dec_start_t));
...
// gpio set/get ---------------------------------------------------------------------------
void app_peri_gpio_set_cmd_send(uint8_t gpio_port, uint8_t gpio_pin, uint8_t active, GPIO_PUPD mode)
{
    app_peri_gpio_cmd_t *req = (app_peri_gpio_cmd_t*)BleMsgAlloc(APP_PERI_GPIO_SET_CMD, 
                                                                 TASK_ID_EXT_HOST_BLE_AUX, 
                                                                 TASK_ID_GTL, 
                                                                 sizeof(app_peri_gpio_cmd_t));
...
void app_peri_gpio_get_cmd_send(uint8_t gpio_port, uint8_t gpio_pin)
{
    app_peri_gpio_cmd_t *req = (app_peri_gpio_cmd_t*)BleMsgAlloc(APP_PERI_GPIO_GET_CMD, 
                                                                 TASK_ID_EXT_HOST_BLE_AUX, 
                                                                 TASK_ID_GTL, 
                                                                 sizeof(app_peri_gpio_cmd_t));
...
void app_peri_quad_dec_stop_send(void)
{
    // Allocate a message for BLE_AUX
    void *cmd = BleMsgAlloc(APP_PERI_QUAD_DEC_STOP, 
                            TASK_ID_EXT_HOST_BLE_AUX, 
                            TASK_ID_GTL, 0);
...
void app_from_ext_host_security_start(void)
{
    ...
    struct gapc_security_cmd *cmd = BleMsgAlloc(GAPC_SECURITY_CMD,
