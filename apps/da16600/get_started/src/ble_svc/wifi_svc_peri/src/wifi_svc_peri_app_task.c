...
//extern TX_EVENT_FLAGS_GROUP  iot_sensor_event_grp;
...
void handle_cmp_evt_for_gapm_resolv_addr_cmd(uint8_t error)
{
	if (error == true)
	{
		...
		else
		{
			...
			// do nothing as the single bond data is already saved
#endif
		}
	}
}
				...
				ke_task_id_t src_id)
{
	...
	if (app_env.state == APP_IDLE || app_env.state == APP_CONNECTABLE)
	{
		...
		{
			dpm_app_sleep_ready_clear(APP_GTL_MAIN);

			// hold DPM Abnormal Checker (to prevent system from enter sleep2)
			DBG_TMP(CYAN_COLOR"DPM Abnormal Checker HOLD (%s:%d) \n"CLEAR_COLOR, 
								__func__, __LINE__);
...
#if defined (__WFSVC_ENABLE__)
		// enable wfsvc
		user_custom_profile_wfsvc_enable(0); // param does nothing
#endif
								...
								ke_task_id_t src_id)
{
	...
	if (param->conhdl == app_env.proxr_device.device.conhdl)
	{
		...
		if (dpm_mode_is_enabled() == TRUE)
		{
			...
			DBG_TMP(CYAN_COLOR"DPM Abnormal Checker RESUME (%s:%d) \n"CLEAR_COLOR, __func__, __LINE__);
                                    ...
                                    ke_task_id_t src_id)
{
	...
	if (param->result_code != 0)
	{
		DBG_ERR(" Not successful, result code = %d\n", param->result_code);
	}
	else
	{
		DBG_INFO(" ... Started\n");
...
{    
	...
	DBG_INFO(" battery type = %s\n", (param->batt_type)?"ALKALINE":"CR2032");
	DBG_INFO(" battery level = %d\n", param->batt_lvl);
				  ...
				  ke_task_id_t src_id)
{
	...
	if (msgid == APP_PERI_GPIO_SET_IND) {
		switch (param->result_code) {
		case APP_GTL_GPIO_STATUS_INVALID:
			DBG_ERR("Fail to set the gpio(%d)\n", param->result_code);
			break;
		case APP_GTL_GPIO_STATUS_HANDLED:
			DBG_INFO("Success\n");
			break;
		};
	} else {
		switch (param->result_code) {
		case APP_GTL_GPIO_STATUS_NOT_IN_USED:
			DBG_ERR("Can't get a status of the GPIO. Need to set the GPIO status first.\n");
			break;
		case APP_GTL_GPIO_STATUS_INVALID:
			DBG_ERR("Fail to get the gpio status(%d)\n", param->result_code);
			break;
		case APP_GTL_GPIO_STATUS_HIGH:
			/* Pass through */
		case APP_GTL_GPIO_STATUS_LOW:
			DBG_INFO("GPIO Status: %s\n",
			       param->result_code == APP_GTL_GPIO_STATUS_HIGH ? "HIGH" : "LOW");
			break;
		};
	}
