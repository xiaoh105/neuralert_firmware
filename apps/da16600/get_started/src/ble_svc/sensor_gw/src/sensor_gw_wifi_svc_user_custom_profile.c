 ...
 ****************************************************************************************
 *
 * @file sensor_gw_wifi_svc_user_custom_profile.c
 *
...
#include "defs.h"
#if defined (__DA14531_BOOT_FROM_UART__) || defined ( __SUPPORT_WIFI_CONN_CB__ )
	...
	*cmd = COMBO_WIFI_CMD_UNKNOWN;
	...
	else if (strcmp(obj->valuestring, "network_info") == 0)
	{
		...
		*cmd = COMBO_WIFI_CMD_SET_NETWORK_INFO;	
		...
		memset(wifi_conf.udp_srv_ip, 0, sizeof(wifi_conf.udp_srv_ip));
		strcpy(wifi_conf.udp_srv_ip, obj->valuestring);
		...
		wifi_conf.udp_srv_port = obj->valueint;
...
int32_t wifi_sta_select_ap(void)
{
	...
	if (strlen(wifi_conf.ssid) > 0)
	{
		...
		write_nvram_string((const char *)"UDP_SERVER_IP", wifi_conf.udp_srv_ip);
		write_nvram_int((const char *)"UDP_SERVER_PORT", wifi_conf.udp_srv_port);
