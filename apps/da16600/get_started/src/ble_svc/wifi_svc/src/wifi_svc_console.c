...
static void cmd_iot_sensor(int argc, char *argv[]);
...
extern void cmd_sleep2_monitor(int argc, char *argv[]);	
#endif
...
#if defined (__LOW_POWER_IOT_SENSOR__)
	CMD_OPTION_S0(  { "iot_sensor",     CMD_FUNC_NODE,  NULL,       &cmd_iot_sensor,    "BLE iot_sensor demo command"}, )
#endif /* __LOW_POWER_IOT_SENSOR__ */
#if defined (__EXCEPTION_RST_EMUL__)	
...
#if defined (__APP_SLEEP2_MONITOR__)
	CMD_OPTION_S0(  { "sleep2mon",     CMD_FUNC_NODE,  NULL,       &cmd_sleep2_monitor,    "the status of sleep2 monitor"}, )
#endif
	{ NULL, 		CMD_NULL_NODE,	NULL,		NULL,		NULL }			// Tail
};

#if defined (__APP_SLEEP2_MONITOR__)
void cmd_sleep2_monitor(int argc, char *argv[])
{
	DA16X_UNUSED_ARG(argc);
	DA16X_UNUSED_ARG(argv);

	extern void sleep2_monitor_get_status_all(void);
	sleep2_monitor_get_status_all();
}
#endif
...
#if defined (__LOW_POWER_IOT_SENSOR__)
void cmd_iot_sensor(int argc, char *argv[])
{
	UINT	status = ERR_OK;
	int	index = 0;
	char	**cur_argv = ++argv;
	...
	else
	{
		for (index = 1 ; index < argc ; index++, cur_argv++)
		{
			...
			{
				ConsoleSendSensorStop();
				break;
			}
			else
			{
				DBG_ERR("[%s]invalid paramter(%s)\n",
						__func__, *cur_argv);
				status = ERR_ARG;
				break;
			}
	...
	if (status)
	{
		PRINTF(" Usage: iot_sensor demo command \n");
		PRINTF(" \x1b[93mName\x1b[0m\n");
		PRINTF("\tiot_sensor - iot_sensor demo application command \n");
		PRINTF(" \x1b[93mSYNOPSIS\x1b[0m\n");
		PRINTF("\tproxr [OPTION]\n");
		PRINTF(" \x1b[93mDESCRIPTION\x1b[0m\n");

		PRINTF("\t\x1b[93m start \x1b[0m\n");
		PRINTF("\t\t start periodic iot_sensor reading task on ble (gtl peer) \n");
		PRINTF("\t\x1b[93m stop \x1b[0m\n");
		PRINTF("\t\t stop iot_sensor on ble (gtl peer) \n");
	}
