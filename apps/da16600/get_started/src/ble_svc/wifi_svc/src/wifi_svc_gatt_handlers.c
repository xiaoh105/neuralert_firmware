							  ...
							  ke_task_id_t const src_id)
{
	...
	if (param->status == ATT_ERR_NO_ERROR)
	{
		...
		DBG_TMP("[%s] GATTM_ADD_SVC_RSP: ok, wfsvc->start_hdl = %d, wfsvc->end_handle = %d, \n",
		__func__, user_custom_profile_wfsvc_env.start_handle,
			  user_custom_profile_wfsvc_env.end_handle);
#if defined (__IOT_SENSOR_SVC_ENABLE__)
                        iot_sensor_svc_create_custom_db(user_custom_profile_wfsvc_env.end_handle + 1);
...
#if defined (__IOT_SENSOR_SVC_ENABLE__)
                iot_sensor_svc_set_handle(param->start_hdl);
								...
								ke_task_id_t const src_id)
{
	...
	DBG_TMP("[%s] GATTC_READ_REQ_IND, char_enum_idx = %d \n", __func__, handle_index);
	...
	switch (handle_index)
	{
		...
		case USER_CUSTOM_CHARACTERISTIC_WFSVC_APSCAN_RES_CHAR_VALUE_IDX:
		{
			...
			DBG_TMP("total_scan_result_len = %d, offset = %d \n", total_len, idx);
			...
			if (idx >= total_len)
			{
				idx = 0;
#if defined (__BLE_PEER_INACTIVITY_CHK_TIMER__)				
				// the data that we are about to send is the last block of AP Scan result.
				if (dpm_boot_type > DPM_BOOT_TYPE_NO_DPM)
				{
					app_set_ble_peer_idle_timer(IDLE_DURATION_AFTER_SCAN_RESULT_READ);
				}
...
#if defined(__IOT_SENSOR_SVC_ENABLE__)
	if (cfm_msg_status == ATT_ERR_APP_ERROR) {
	        if (KE_MSG_CONSUMED == iot_sensor_svc_gattc_read_req_ind(msgid, (void *)param, dest_id, src_id)) {
	                return KE_MSG_CONSUMED;
	        }
	}
#endif
...
void user_gapc_set_le_pkt_size_cmd(void)
{
	struct gapc_set_le_pkt_size_cmd *cmd =  BleMsgAlloc(GAPC_SET_LE_PKT_SIZE_CMD,
											...
											TASK_ID_GTL, sizeof(struct gapc_set_le_pkt_size_cmd));
...
#endif
#if defined (__IOT_SENSOR_SVC_ENABLE__)
	iot_sensor_svc_gattc_write_req_ind(msgid, (void *)param, dest_id, src_id);
#endif
	return KE_MSG_CONSUMED;
}
									...
									ke_task_id_t const src_id)
{
	...
	DBG_TMP("[%s]	GATTM_ATT_SET_VALUE_RSP, char_enum_idx = %d, status = %d \n",
		...
		ke_task_id_t const src_id)
{
	...
	{
		user_custom_profile_wfsvc_gattm_svc_set_permission_rsp_hnd(msgid, 
																  param, dest_id, src_id);
	}
						  ...
						  ke_task_id_t const src_id)
{
	...
	switch (param->operation)
	{
		case GATTC_MTU_EXCH:
		{
			if (param->status == ATT_ERR_NO_ERROR)
			{
				///app_adv_start(Adv_No, ADV_TYPE_NONCON);	//start advertising
