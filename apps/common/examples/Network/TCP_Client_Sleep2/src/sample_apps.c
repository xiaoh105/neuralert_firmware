...
// Template Samples
#if defined (__TCP_CLIENT_SLEEP2_SAMPLE__)
	{ SAMPLE_TCP_SLP2_CLI,			tcp_client_sleep2_sample,		2048, (tskIDLE_PRIORITY + 3), FALSE, FALSE,	TCP_CLI_TEST_PORT,		RUN_ALL_MODE },
#endif // (__TEMPLATE_SAMPLE__)
...
//	PRINTF("\n**Neuralert: In create_sample_apps()\n"); // FRSDEBUG
	...
	// Note that the task info blocks (cur_task) are linked to each other by this
	for (i = 0 ; sample_apps_table[i].name != NULL ; i++) {
