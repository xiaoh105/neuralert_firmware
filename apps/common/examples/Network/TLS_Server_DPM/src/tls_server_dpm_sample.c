 ...
 ****************************************************************************************
 *
 * @file tls_server_dpm_sample.c
 *
 * @brief Sample app of TLS Server in DPM mode.
 *
...
#if defined (__TLS_SERVER_DPM_SAMPLE__)				\
...
#define TLS_SERVER_DPM_SAMPLE_DEF_SERVER_PORT			TLS_SVR_TEST_PORT

//NVRAM
#define TLS_SERVER_DPM_SAMPLE_NVRAM_SERVER_PORT_NAME	"TLS_SERVER_PORT"

//Hex Dump
#undef	TLS_SERVER_DPM_SAMPLE_ENABLED_HEXDUMP

const unsigned char  tls_server_dpm_sample_cert[] =
    "-----BEGIN CERTIFICATE-----\n"
    ...
    "NOds4a5SIHfFZw==\n"
    "-----END CERTIFICATE-----\n";
const size_t tls_server_dpm_sample_cert_len = sizeof(tls_server_dpm_sample_cert);

const unsigned char tls_server_dpm_sample_key[] =
    ...
    "-----END RSA PRIVATE KEY-----\n";
const size_t tls_server_dpm_sample_key_len = sizeof(tls_server_dpm_sample_key);

//TLS Server inforamtion
typedef struct {
    int port;
} tls_server_dpm_sample_svr_info_t;

//Function prototype
void tls_server_dpm_sample_init_callback();
void tls_server_dpm_sample_error_callback(UINT error_code, char *comment);
void tls_server_dpm_sample_connect_callback(void *sock, UINT conn_status);
void tls_server_dpm_sample_recv_callback(void *sock, UCHAR *rx_buf, UINT rx_len,
...
unsigned char tls_server_dpm_sample_load_server_info();
void tls_server_dpm_sample(void *param);

//Global variable
tls_server_dpm_sample_svr_info_t srv_info = {0x00,};

//Function
static void tls_server_dpm_sample_hex_dump(const char *title, UCHAR *buf, size_t len)
{
#if defined (TLS_SERVER_DPM_SAMPLE_ENABLED_HEXDUMP)
...
#endif // (TLS_SERVER_DPM_SAMPLE_ENABLED_HEXDUMP)

    return ;
}

static int tls_server_dpm_sample_rsa_decrypt_func(void *ctx, int mode, size_t *olen,
...
static int tls_server_dpm_sample_rsa_sign_func( void *ctx,
...
static size_t tls_server_dpm_sample_rsa_key_len_func(void *ctx)
{
...
void tls_server_dpm_sample_wakeup_callback()
{
        ...
        ULONG rx_ip, ULONG rx_port)
{
    ...
    tls_server_dpm_sample_hex_dump("Recevied Packet", rx_buf, rx_len);
    ...
    if (status) {
        ...
        tls_server_dpm_sample_hex_dump("Sent Packet", rx_buf, rx_len);
    }
...
unsigned char tls_server_dpm_sample_load_server_info()
{
    ...
    tls_server_dpm_sample_svr_info_t *srv_info_ptr = &srv_info;

    memset(srv_info_ptr, 0x00, sizeof(tls_server_dpm_sample_svr_info_t));

    //Read server port in nvram
    if (read_nvram_int(TLS_SERVER_DPM_SAMPLE_NVRAM_SERVER_PORT_NAME, &srv_port)) {
        srv_port = TLS_SERVER_DPM_SAMPLE_DEF_SERVER_PORT;
    }

    //Set server port
    srv_info_ptr->port = srv_port;

    PRINTF(" [%s] TLS Server Information(%d)\n", __func__, srv_info_ptr->port);
...
void tls_server_dpm_sample(void *param)
{
    ...
    PRINTF(" [%s] Start TLS Server Sample\n", __func__);

    //Load server information
    status = tls_server_dpm_sample_load_server_info();
    if (status == pdFAIL) {
    ...
    dpm_mng_regist_config_cb(tls_server_dpm_sample_init_user_config);

    //Start TLS Server Sample
    dpm_mng_start();
...
#endif // (__TLS_SERVER_DPM_SAMPLE__) && (__SUPPORT_DPM_MANAGER__) && !(__LIGHT_DPM_MANAGER) 
