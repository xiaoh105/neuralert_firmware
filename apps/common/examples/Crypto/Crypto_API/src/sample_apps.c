 ...
 *****************************************************************************/
void run_crypto_samples(void *param);
...
static const app_task_info_t	sample_apps_table[] = {
    ...
    // Crypto Sample
    { SAMPLE_CRYPTO_APIS,			run_crypto_samples,					(256 * 5), (OS_TASK_PRIORITY_USER + 6), FALSE, FALSE,	UNDEF_PORT,				RUN_ALL_MODE },
...
void run_crypto_samples(void *param)
{
#if defined ( __CRYPTO_SAMPLE_AES__ )
    crypto_sample_aes(param);
#endif // (__CRYPTO_SAMPLE_AES__)

#if defined ( __CRYPTO_SAMPLE_CIPHER__ )
    crypto_sample_cipher(param);
#endif // (__CRYPTO_SAMPLE_CIPHER__)

#if defined ( __CRYPTO_SAMPLE_DES__ )
    crypto_sample_des(param);
#endif // (__CRYPTO_SAMPLE_DES__)

#if defined ( __CRYPTO_SAMPLE_DHM__ )
    crypto_sample_dhm(param);
#endif // (__CRYPTO_SAMPLE_DHM__)

#if defined ( __CRYPTO_SAMPLE_DRBG__ )
    crypto_sample_drbg(param);
#endif // (__CRYPTO_SAMPLE_DRBG__)

#if defined ( __CRYPTO_SAMPLE_ECDH__ )
    crypto_sample_ecdh(param);
#endif // (__CRYPTO_SAMPLE_ECDH__)

#if defined ( __CRYPTO_SAMPLE_ECDSA__ )
    crypto_sample_ecdsa(param);
#endif // (__CRYPTO_SAMPLE_ECDSA__)

#if defined ( __CRYPTO_SAMPLE_HASH__ )
    crypto_sample_hash(param);
#endif // (__CRYPTO_SAMPLE_HASH__)

#if defined ( __CRYPTO_SAMPLE_KDF__ )
    crypto_sample_kdf(param);
#endif // (__CRYPTO_SAMPLE_KDF__)

#if defined ( __CRYPTO_SAMPLE_PK__ )
    crypto_sample_pk(param);
#endif // (__CRYPTO_SAMPLE_PK__)

#if defined ( __CRYPTO_SAMPLE_RSA__ )
    crypto_sample_rsa(param);
#endif // (__CRYPTO_SAMPLE_RSA__)
