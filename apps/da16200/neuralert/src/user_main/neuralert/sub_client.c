...
// See Neuralert code for why this is needed
#if defined (__TCP_CLIENT_SLEEP2_SAMPLE__)
void user_set_mid_sent(int mid){
	//mid_sent = mid;
	mosq_sub->last_mid = mid;
}
...
/*
 * mqtt_client_start: Start the MQTT client task
 *
 * returns -1 if the task is already running
 * returns 0 if the task is started successfully
 * returns -1 if unable to start the task
 */
///////////////////////////////////////////////////////////////////////
...
int mqtt_client_send_message(char *top, char *publish)
{
	...
	if (dpm_mode_is_enabled()) {
		PRINTF ("+++++ Calling my_publish from dpm_mode_is_enabled");
...
//JW: Added code for check_mqtt_client_state(void)
/*
 * check_mqtt_client_state: returns the internal client state enum
 * Author: F. Strathmann 7.8.22
 *
 * returns:  -1 if client is not running (status struct not allocated)
 *           enum of the internal state
 *           (see mosquitto_internal.h)
 */
int check_mqtt_client_state(void)
{
	if (!mosq_sub)
	{
		return -1;
