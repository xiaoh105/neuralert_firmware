...
#define USER_PROCESS_MQTT_TRANSMIT_REQ			(1 << 4)
#define USER_PROCESS_MQTT_TRANSMIT				(1 << 5)
...
#define MQTT_TRANSMIT_TRIGGER_FIFO_BUFFERS 32
...
#define FIFO_BLOCKS_PER_PACKET 20 // 24 used in 1.9 -- lowering for now.
...
#define MQTT_QOS_TIMEOUT_MS 50 //5000 -- this was in the 1.9 release
...
static void user_process_send_MQTT_data(void* arg)
{
	...
	SET_PROCESS_BIT(processLists, USER_PROCESS_MQTT_TRANSMIT); // signal we are now in transmit mode
	vTaskDelay(1);
	CLR_PROCESS_BIT(processLists, USER_PROCESS_MQTT_TRANSMIT_REQ); // clear transmit request
	vTaskDelay(1);
...
static void user_process_send_MQTT_data_old(void* arg)
{
	int status = 0;
	int clear_to_transmit = FALSE;
	int samples_to_send;
 ...
 *******************************************************************************
 */
void user_create_MQTT_task()
{

	// This function is assumed to be called on a wifi connection callback.  It is possible
	// that the wifi could generate multiple callbacks due to intermittent wifi.  We only want
	// to process the first request.
	if (!PROCESS_BIT_SET(processLists, USER_PROCESS_MQTT_TRANSMIT_REQ)) {
		PRINTF(">>>>>>>> Skipping MQTT transmit task creation -- not requested <<<<<<<<<\n");
		return;
	} else if (PROCESS_BIT_SET(processLists, USER_PROCESS_MQTT_TRANSMIT_REQ)
			&& PROCESS_BIT_SET(processLists, USER_PROCESS_MQTT_TRANSMIT) ) {
		PRINTF(">>>>>>>> Skipping MQTT transmit task creation -- already in progress <<<<<<<<<\n");
		CLR_PROCESS_BIT(processLists, USER_PROCESS_MQTT_TRANSMIT_REQ); // clear request
		return;
	}
...
static UCHAR user_process_event(UINT32 event)
{
	...
	int	ret = 0;
	char reply[32] = {0, }; // JW: was 128 not 32
...
//		user_process_create_rtc_timer(USER_RTC_TIMER_ID, USER_DATA_TX_TIMER_SEC);
	...
	if (event & USER_ATTEMPT_TRANSMIT_EVENT) {
		...
		// Let the other tasks know that we're active so we don't sleep
		SET_PROCESS_BIT(processLists, USER_PROCESS_MQTT_TRANSMIT_REQ);
		//SET_PROCESS_BIT(processLists, USER_PROCESS_MQTT_TRANSMIT);

		// Start the RF section power up
		wifi_cs_rf_cntrl(FALSE);

		// Select the network to connect to (and wait for callback)
		ret = da16x_cli_reply("select_network 0", NULL, reply);

//#if defined(__RUNTIME_CALCULATION__) && defined(XIP_CACHE_BOOT)
//	printf_with_run_time("======= ABOUT TO START MQTT TASK");
//#endif

		//user_create_MQTT_task(); // JW: This is the other thing I took out.
