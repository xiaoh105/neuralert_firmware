...
#define TIMP_BACKUP_ADDR   (DA16200_RETMEM_BASE | 0x05cc) ///< Deprecated
...
struct timp_performance_tag {
	...
	union {
		...
		struct {
			...
			unsigned int       invalid_timestamp_warning: 1;
		};
	};
	...
	unsigned int       bcn_rx_done: 1;      ///< bcn received successfully
	...
	unsigned long      rx_on_start_c; ///< RX on time by CLK
	unsigned long      rx_on_end_c;   ///< RX off time by CLK
