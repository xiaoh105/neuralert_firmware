...
#define MESH_AUTH_TIMEOUT 5 //  10
#define MESH_AUTH_RETRY  3 // 1// 3
...
void mesh_auth_timer(void *eloop_ctx, void *user_data)
{
	...
	if (sta->sae->state != SAE_ACCEPTED) {
		wpa_printf_dbg(MSG_DEBUG, "AUTH: Re-authenticate with " MACSTR
			   " (attempt %d) ",
...
{
	if (addr) {
		wpa_printf_dbg(MSG_DEBUG, "AUTH: " MACSTR " - %s",
			   MAC2STR(addr), txt);
	} else {
...
static int auth_start_ampe(void *ctx, const u8 *addr)
{
	...
	if (sta)
		da16x_eloop_cancel_timeout(mesh_auth_timer, mesh_rsn->wpa_s, sta);
				     ...
				     struct sta_info *sta)
{
	...
	if (!password) {
		wpa_dbg(wpa_s, MSG_DEBUG, "SAE: No password available");
		return -1;
	}

	if (mesh_rsn_sae_group(wpa_s, sta->sae) < 0) {
		wpa_dbg(wpa_s, MSG_DEBUG, "SAE: Failed to select group");
		return -1;
	}
			  ...
			  struct sta_info *sta)
{
	...
	da16x_eloop_cancel_timeout(mesh_auth_timer, wpa_s, sta);
	rnd = rand() % MESH_AUTH_TIMEOUT;
	da16x_eloop_register_timeout(MESH_AUTH_TIMEOUT + rnd, 0, mesh_auth_timer,
			       wpa_s, sta);
	return 0;
}
			  ...
			  const u8 *start, size_t elems_len)
{
	...
	if (!elems->mic || elems->mic_len < AES_BLOCK_SIZE) {
		wpa_dbg(wpa_s, MSG_DEBUG, "Mesh RSN: missing mic ie");
		return -1;
	}
	...
	if (crypt_len < 2 + AES_BLOCK_SIZE) {
		wpa_dbg(wpa_s, MSG_DEBUG, "Mesh RSN: missing ampe ie");
		return -1;
	}
	...
	if (ampe_eid != WLAN_EID_AMPE ||
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "Mesh RSN: invalid ampe ie");
		ret = -1;
		goto free;
	}
	...
	if (os_memcmp(ampe->peer_nonce, null_nonce, WPA_NONCE_LEN) != 0 &&
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "Mesh RSN: invalid peer nonce");
		ret = -1;
		goto free;
	}
	...
	if (end - pos >= (int) (2 + 6 + key_len)) {
		...
		wpa_printf_dbg(MSG_DEBUG, "mesh: IGTKdata - Key ID %u",
			   sta->igtk_key_id);
		pos += 2;
