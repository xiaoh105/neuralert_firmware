...
static int wifi_display_update_wfd_ie(struct wpa_global *global)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "WFD: Update WFD IE");

	if (!global->wifi_display) {
		wpa_printf_dbg(MSG_DEBUG, "WFD: Wi-Fi Display disabled - do not "
			   "include WFD IE");
...
{
	wpa_printf_dbg(MSG_DEBUG, "WFD: Wi-Fi Display %s",
...
int wifi_display_subelem_set(struct wpa_global *global, char *cmd)
{
	...
	*pos++ = '\0';
	...
	if (len == 0) {
		/* Clear subelement */
		e = NULL;
		wpa_printf_dbg(MSG_DEBUG, "WFD: Clear subelement %d", subelem);
	} else {
		...
		wpa_printf_dbg(MSG_DEBUG, "WFD: Set subelement %d", subelem);
	}
