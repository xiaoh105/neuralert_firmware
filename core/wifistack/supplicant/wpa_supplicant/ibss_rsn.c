...
{
	da16x_eloop_cancel_timeout(ibss_rsn_auth_timeout, peer, NULL);
			   ...
			   size_t len, int is_hs_ack)
{
	...
	if (wpa_s->l2)
		return l2_packet_send(wpa_s->l2, dest, proto, buf, len, is_hs_ack);
			     ...
			     u16 data_len, size_t *msg_len, void **data_pos)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "SUPP: %s(type=%d data_len=%d)",
...
static int supp_get_beacon_ie(void *ctx)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "SUPP: %s", __func__);
			...
			const u8 *key, size_t key_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "SUPP: %s(alg=%d addr=" MACSTR " key_idx=%d "
		   "set_tx=%d)",
	...
	if (key_idx == 0) {
		...
		if (os_memcmp(peer->ibss_rsn->wpa_s->own_addr, peer->addr,
			      ETH_ALEN) > 0) {
			wpa_printf_dbg(MSG_DEBUG, "SUPP: Do not use this PTK");
			return 0;
		}
	}
...
{
	wpa_printf_dbg(MSG_DEBUG, "SUPP: %s(addr=" MACSTR " protection_type=%d "
		   "key_type=%d)",
...
{
	wpa_printf_dbg(MSG_DEBUG, "SUPP: %s (TODO)", __func__);
}
			      ...
			      const u8 *psk)
{
	...
	if (peer->supp == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "SUPP: wpa_sm_init() failed");
		return -1;
	}
	...
	if (wpa_sm_set_assoc_wpa_ie_default(peer->supp, peer->supp_ie,
					    &peer->supp_ie_len) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "SUPP: wpa_sm_set_assoc_wpa_ie_default()"
			   " failed");
		return -1;
	}
...
{
	if(addr){
		wpa_printf_dbg(MSG_DEBUG, "AUTH: " MACSTR " - %s",MAC2STR(addr), txt);
	}else{
		wpa_printf_dbg(MSG_DEBUG, "AUTH: %s", txt);
	}
}
			       ...
			       const u8 *p2p_dev_addr, const u8 *prev_psk)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "AUTH: %s (addr=" MACSTR " prev_psk=%p)",
			   ...
			   size_t data_len, int encrypt)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "AUTH: %s(addr=" MACSTR " data_len=%lu "
		   "encrypt=%d)",
			...
			const u8 *addr, int idx, u8 *key, size_t key_len)
{
	...
	if (addr) {
		wpa_printf_dbg(MSG_DEBUG, "AUTH: %s(alg=%d addr=" MACSTR
			   " key_idx=%d)",
			   __func__, alg, MAC2STR(addr), idx);
	} else {
		wpa_printf_dbg(MSG_DEBUG, "AUTH: %s(alg=%d key_idx=%d)",
			   __func__, alg, idx);
	}
	...
	if (idx == 0) {
		...
		if (addr == NULL ||
			...
			wpa_printf_dbg(MSG_DEBUG, "AUTH: Do not use this PTK");
			return 0;
		}
	}
			     ...
			     void *cb_ctx)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "AUTH: for_each_sta");
				    ...
				    struct ibss_rsn_peer *peer, int authorized)
{
	...
	if (authorized) {
		...
		wpa_printf_dbg(MSG_DEBUG, "AUTH: " MACSTR " authorizing port",
			   MAC2STR(peer->addr));
	} else {
		res = wpa_drv_sta_set_flags(ibss_rsn->wpa_s, peer->addr,
					    0, 0, ~WPA_STA_AUTHORIZED);
		wpa_printf_dbg(MSG_DEBUG, "AUTH: " MACSTR " unauthorizing port",
			   MAC2STR(peer->addr));
	}

#ifdef CHECK_SUPPLICANT_ERR
	if (res && errno != ENOENT) {
		wpa_printf_dbg(MSG_DEBUG, "Could not set station " MACSTR " flags "
				       ...
				       wpa_eapol_variable var, int value)
{
	...
	default:
		...
		wpa_printf_dbg(MSG_DEBUG, "AUTH: eapol event not handled %d", var);
		break;
	}
}
				    ...
				    const u8 *own_addr)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "AUTH: Initializing group state machine");
	...
	if (ibss_rsn->auth_group == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "AUTH: wpa_init() failed");
		return -1;
	}
			      ...
			      struct ibss_rsn_peer *peer)
{
	...
	if (peer->auth == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "AUTH: wpa_auth_sta_init() failed");
		return -1;
	}
	...
	if (wpa_validate_wpa_ie(ibss_rsn->auth_group, peer->auth,
				...
				"\x00\x00", 22, NULL, 0) !=
	    WPA_IE_OK) {
		wpa_printf_dbg(MSG_DEBUG, "AUTH: wpa_validate_wpa_ie() failed");
		return -1;
	}
#endif	/*CHECK_SUPPLICANT_ERR*/
...
static int ibss_rsn_send_auth(struct ibss_rsn *ibss_rsn, const u8 *da, int seq)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "RSN: IBSS TX Auth frame (SEQ %d) to " MACSTR,
		   seq, MAC2STR(da));
...
ibss_rsn_peer_init(struct ibss_rsn *ibss_rsn, const u8 *addr)
{
	...
	peer = ibss_rsn_get_peer(ibss_rsn, addr);
	if (peer) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: IBSS Supplicant for peer "MACSTR
			   " already running", MAC2STR(addr));
		return peer;
	}

	wpa_printf_dbg(MSG_DEBUG, "RSN: Starting IBSS Supplicant for peer "MACSTR,
		   MAC2STR(addr));

	peer = os_zalloc(sizeof(*peer));
	if (peer == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: Could not allocate memory.");
		return NULL;
	}
...
int ibss_rsn_start(struct ibss_rsn *ibss_rsn, const u8 *addr)
{
	...
	if (res) {
		...
		da16x_eloop_register_timeout(1, 0, ibss_rsn_auth_timeout, peer, NULL);
	}
				       ...
				       struct ibss_rsn_peer *peer, int reason)
{
	...
	if (already_started) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: IBSS Authenticator already "
			   "started for peer " MACSTR, MAC2STR(peer->addr));
		return 0;
	}

	wpa_printf_dbg(MSG_DEBUG, "RSN: Starting IBSS Authenticator "
...
void ibss_rsn_stop(struct ibss_rsn *ibss_rsn, const u8 *peermac)
{
	...
	if (peermac == NULL) {
		/* remove all peers */
		wpa_printf_dbg(MSG_DEBUG, "%s: Remove all peers", __func__);
		peer = ibss_rsn->peers;
		while (peer) {
			prev = peer;
			peer = peer->next;
		...
		wpa_printf_dbg(MSG_DEBUG, "%s: Remove specific peer " MACSTR,
		...
		for (prev = NULL, peer = ibss_rsn->peers; peer != NULL;
			...
			if (os_memcmp(peermac, peer->addr, ETH_ALEN) == 0) {
				if (prev == NULL)
					...
					prev->next = peer->next;
				ibss_rsn_free(peer);
				wpa_printf_dbg(MSG_DEBUG, "%s: Successfully "
...
static int ibss_rsn_eapol_dst_supp(const u8 *buf, size_t len)
{
	...
	if (hdr->type != IEEE802_1X_TYPE_EAPOL_KEY) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: EAPOL frame (type %u) discarded, "
			"not a Key frame", hdr->type);
		return -1;
	}
	if (plen > len - sizeof(*hdr) || plen < sizeof(*key)) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: EAPOL frame payload size %lu "
	...
	if (key->type != EAPOL_KEY_TYPE_RSN) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: EAPOL-Key type (%d) unknown, "
			   "discarded", key->type);
		return -1;
	}
				     ...
				     const u8 *buf, size_t len)
{
	...
	if (supp) {
		...
		wpa_printf_dbg(MSG_DEBUG, "RSN: IBSS RX EAPOL for Supplicant from "
			   MACSTR, MAC2STR(peer->addr));
		wpa_sm_rx_eapol(peer->supp, peer->addr, tmp, len);
	} else {
		if (ibss_rsn_is_auth_started(peer) == 0) {
			wpa_printf_dbg(MSG_DEBUG, "RSN: IBSS EAPOL for "
		...
		wpa_printf_dbg(MSG_DEBUG, "RSN: IBSS RX EAPOL for Authenticator "
		      ...
		      const u8 *buf, size_t len)
{
	...
	if (ibss_rsn_eapol_dst_supp(buf, len) > 0) {
		...
		wpa_printf_dbg(MSG_DEBUG, "RSN: IBSS Not using IBSS Auth for peer "
...
static void ibss_rsn_handle_auth_1_of_2(struct ibss_rsn *ibss_rsn,
					...
					const u8* addr)
{
	wpa_printf_dbg(MSG_DEBUG, "RSN: IBSS RX Auth frame (SEQ 1) from " MACSTR,
		   MAC2STR(addr));
	...
	if (peer &&
		...
		if (peer->own_auth_tx.sec) {
			...
			if (diff.sec == 0 && diff.usec < 500000) {
				wpa_printf_dbg(MSG_DEBUG, "RSN: Skip IBSS reinit since only %u usec from own Auth frame TX",
					   (int) diff.usec);
				goto skip_reinit;
			}
		}
		 ...
		 */
		wpa_printf_dbg(MSG_DEBUG, "RSN: IBSS Reinitializing station "
			   MACSTR, MAC2STR(addr));
	...
	if (!peer) {
		...
		wpa_printf_dbg(MSG_DEBUG, "RSN: IBSS Auth started by peer " MACSTR,
			   MAC2STR(addr));
	}
			  ...
			  size_t len)
{
	...
	case 1:
		...
		break;
	case 2:
		wpa_printf_dbg(MSG_DEBUG, "RSN: IBSS RX Auth frame (SEQ 2) from "
			   MACSTR, MAC2STR(header->sa));
		if (!peer) {
			wpa_printf_dbg(MSG_DEBUG, "RSN: Received Auth seq 2 from "
		...
		da16x_eloop_cancel_timeout(ibss_rsn_auth_timeout, peer, NULL);
		wpa_printf_dbg(MSG_DEBUG, "RSN: IBSS Auth completed with " MACSTR,
			   MAC2STR(header->sa));
		ibss_rsn_peer_authenticated(ibss_rsn, peer,
					    IBSS_RSN_AUTH_BY_US);
		break;
