...
static void bgscan_simple_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "bgscan simple: Request a background scan");
	if (wpa_supplicant_trigger_scan(wpa_s, &params)) {
		wpa_printf_dbg(MSG_DEBUG, "bgscan simple: Failed to trigger scan");
		...
		if (data->scan_interval == data->short_interval) {
			...
			if (data->short_scan_count > data->max_short_scans) {
				...
				wpa_printf_dbg(MSG_DEBUG, "bgscan simple: Backing "
				 ...
				 const struct wpa_ssid *ssid)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "bgscan simple: Signal strength threshold %d  "
	...
	wpa_printf_dbg(MSG_DEBUG, "bgscan simple: Init scan interval: %d",
		   data->scan_interval);
	da16x_eloop_register_timeout(data->scan_interval, 0, bgscan_simple_timeout,
			       data, NULL);
...
static void bgscan_simple_deinit(void *priv)
{
	...
	da16x_eloop_cancel_timeout(bgscan_simple_timeout, data, NULL);
	if (data->signal_threshold)
				     ...
				     struct wpa_scan_results *scan_res)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "bgscan simple: scan result notification");
...
{
	wpa_printf_dbg(MSG_DEBUG, "bgscan simple: beacon loss");
					       ...
					       int current_txrate)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "bgscan simple: signal level changed "
		   "(above=%d current_signal=%d current_noise=%d "
		   "current_txrate=%d))", above, current_signal,
		   current_noise, current_txrate);
	if (data->scan_interval == data->long_interval && !above) {
		wpa_printf_dbg(MSG_DEBUG, "bgscan simple: Start using short "
			   "bgscan interval");
		data->scan_interval = data->short_interval;
		os_get_reltime(&now);
		if (now.sec > data->last_bgscan.sec + 1 &&
			 ...
			 */
			da16x_eloop_cancel_timeout(bgscan_simple_timeout, data,
					     NULL);
			da16x_eloop_register_timeout(data->scan_interval, 0,
		...
		wpa_printf_dbg(MSG_DEBUG, "bgscan simple: Start using long bgscan "
			   "interval");
		data->scan_interval = data->long_interval;
		da16x_eloop_cancel_timeout(bgscan_simple_timeout, data, NULL);
		da16x_eloop_register_timeout(data->scan_interval, 0,
	...
	if (scan) {
		wpa_printf_dbg(MSG_DEBUG, "bgscan simple: Trigger immediate scan");
		da16x_eloop_cancel_timeout(bgscan_simple_timeout, data, NULL);
		da16x_eloop_register_timeout(0, 0, bgscan_simple_timeout, data,
				       NULL);
	}
}
