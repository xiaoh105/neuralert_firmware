...
int wpa_blacklist_add(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
	...
	if (e) {
		e->count++;
		da16x_assoc_prt("[%s] BSSID " MACSTR " blacklist count "
			   "incremented to %d\n",
			   __func__, MAC2STR(bssid), e->count);
		return e->count;
	}
	...
	da16x_assoc_prt("!!! Add BSSID=[" MACSTR "] into blacklist.\n",
		       MAC2STR(bssid));
...
int wpa_blacklist_del(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
	...
	while (e) {
		if (os_memcmp(e->bssid, bssid, ETH_ALEN) == 0) {
			...
			da16x_assoc_prt("[%s] Removed BSSID " MACSTR " from "
...
void wpa_blacklist_clear(struct wpa_supplicant *wpa_s)
{
	...
	while (e) {
		...
		prev = e;
		e = e->next;
		da16x_assoc_prt("[%s] Removed BSSID " MACSTR " from blacklist "
			       "(clear)\n", __func__, MAC2STR(prev->bssid));
		os_free(prev);
	}
