...
void wpa_supplicant_mesh_all_sta_remove(struct wpa_supplicant *wpa_s)
{
	if (!wpa_s->ifmsh)
		return;

	if (wpa_s->ifmsh->mconf) {
		mesh_mpm_deinit(wpa_s, wpa_s->ifmsh);
	}
}


void wpa_supplicant_mesh_sta_remove(struct wpa_supplicant *wpa_s, const u8 *addr)
{
	if (!wpa_s->ifmsh)
		return;

	if (wpa_s->ifmsh->mconf) {
		mesh_mpm_sta_remove(wpa_s, addr);
	}
}
				    ...
				    struct hostapd_freq_params *freq)
{
	...
	static int default_groups[] = { 19, 20, -1 }; // { 19, 20, 21, 25, 26, -1 };
	const char *password;
	size_t len;
	int rate_len;
	int frequency;
	...
	if (frequency != freq->freq &&
		...
		wpa_printf_dbg(MSG_DEBUG, "mesh: pri/sec channels switched");
		frequency = freq->freq;
	}
#if 1 
	struct i802_bss *bss_temp;
	bss_temp = (struct i802_bss *)bss->drv_priv;
	if(bss_temp->freq == 0)
		bss_temp->freq = frequency;
#endif
...
#if 0	/* Fix me lator we already add mesh interface, Chang */
...
static int mesh_attr_text(const u8 *ies, size_t ies_len, char *buf, char *end)
{
	...
	if (ieee802_11_parse_elements(ies, ies_len, &elems, 0) == ParseFailed)
		return -1;
...
#if 1
struct wpa_supplicant * wpas_mesh_add_interface(struct wpa_supplicant *wpa_s)
{
	...
	os_snprintf(ifname, sizeof(ifname), MESH_POINT_DEVICE_NAME);

	if (ifname[0] == '\0' && wpas_mesh_get_ifname(wpa_s, ifname, sizeof(ifname)) < 0)
		//return -1;
		return mesh_wpa_s;
...
//	mesh_wpa_s = wpa_supplicant_add_iface(wpa_s->global, &iface, wpa_s);
	mesh_wpa_s = wpa_supplicant_add_iface(wpa_s->global, &iface, NULL);
	...
	for (ssid_p = mesh_wpa_s->conf->ssid ; ssid_p ; ssid_p = ssid_p->next) {
		if (ssid_p->ssid == NULL || ssid_p->ssid_len == 0) {
			da16x_ap_prt("[%s] No SSID configured for AP mode\n", __func__);
			continue;
		} else if (ssid_p->mode != WPAS_MODE_MESH) {
	...
	wpa_supplicant_select_network(mesh_wpa_s, ssid_p);
	return mesh_wpa_s;
}
