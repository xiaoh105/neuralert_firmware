...
static void da16x_sme_stop_sa_query(struct wpa_supplicant *wpa_s);
				...
				struct wpa_bss *bss)
{
	...
	if (!rrm_ie || !(bss->caps & IEEE80211_CAP_RRM)) {
		wpa_printf_dbg(MSG_DEBUG, "RRM: No RRM in network");
		return;
	}
	...
	wpa_printf_dbg(MSG_DEBUG, "RRM: Adding RRM IE to Association Request");
				    ...
				    int start)
{
	...
	if (bss == NULL) {
		da16x_assoc_prt("[%s] No scan result available for "
				"the network\n", __func__);
	...
	da16x_assoc_prt("        [%s] freq=%d, bssid=[" MACSTR "], ssid=[%s], "
		"ssid_len=%d\n",
			__func__,
			params.freq,
	...
	if ((wpa_bss_get_vendor_ie(bss, WPA_IE_VENDOR_TYPE) ||
		...
		if (wpa_supplicant_set_suites(wpa_s, bss, ssid,
			...
			da16x_assoc_prt("[%s] Failed to set WPA "
...
no_fils:
...
#else
	 da16x_assoc_prt("[%s] SME: Trying to authenticate with " MACSTR
        " (SSID='%s' freq=%d MHz)\n",
	...
	da16x_eloop_register_timeout(SME_AUTH_TIMEOUT, 0, sme_auth_timer, wpa_s,
			       NULL);
...
#else
static void da16x_sme_send_auth(struct wpa_supplicant *wpa_s,
		    struct wpa_bss *bss, struct wpa_ssid *ssid, int start)
{
	struct wpa_driver_auth_params params;
#ifdef CONFIG_IEEE80211R
	const u8 *ie;
...
#endif /* IEEE8021X_EAPOL */
	da16x_assoc_prt("        [%s] Automatic auth_alg selection: 0x%x\n",
			__func__, params.auth_alg);
	if (ssid->auth_alg) {
		params.auth_alg = ssid->auth_alg;
		da16x_assoc_prt("        [%s] Overriding auth_alg selection:0x%x\n",
				__func__, params.auth_alg);
	}
	...
	if (md && wpa_key_mgmt_ft(ssid->key_mgmt)) {
		...
		if (wpa_s->sme.ft_used &&
		    os_memcmp(md, wpa_s->sme.mobility_domain, 2) == 0 &&
		    wpa_sm_has_ptk(wpa_s->wpa)) {
			da16x_assoc_prt("        [%s] Trying to use FT over-the-air\n",
							 __func__);
	...
	if (wpa_s->sme.mfp != NO_MGMT_FRAME_PROTECTION) {
		...
		if (rsn && wpa_parse_wpa_ie(rsn, 2 + rsn[1], &_ie) == 0 &&
		    _ie.capabilities &
		    (WPA_CAPABILITY_MFPC | WPA_CAPABILITY_MFPR)) {
			da16x_assoc_prt("[%s]SME: Selected AP supports "
				"MFP: require MFP\n", __func__);
	...
	da16x_assoc_prt("[%s] Trying to authenticate with " MACSTR
		" (SSID='%s' freq=%d MHz)\n",
			__func__, MAC2STR(params.bssid),
	...
	wpa_s->current_ssid = ssid;
       ieee80211_freq_to_chan(bss->freq, (u8 *)&wpa_s->current_ssid->channel);
	wpa_supplicant_rsn_supp_set_config(wpa_s, wpa_s->current_ssid);
#ifdef	IEEE8021X_EAPOL
	wpa_supplicant_initiate_eapol(wpa_s);
#endif	/* IEEE8021X_EAPOL */
	...
	if (wpa_s->num_multichan_concurrent < 2) {
		...
		num = get_shared_radio_freqs(wpa_s, &freq, 1);
		if (num > 0 && freq > 0 && freq != params.freq) {
			da16x_p2p_prt("        [%s] Conflicting frequency "
					"found (%d != %d)\n",
				   __func__, freq, params.freq);
	...
	wpa_s->sme.auth_alg = params.auth_alg;
	if (wpa_drv_authenticate(wpa_s, &params) < 0) {
		da16x_err_prt("        [%s] Authentication request to the "
			"driver failed\n", __func__);
		wpas_connection_failed(wpa_s, bss->bssid);
		wpa_supplicant_mark_disassoc(wpa_s);
		wpabuf_free(resp);

da16x_assoc_prt("        [%s] END #2\n", __func__);
		return;
	}

	da16x_eloop_register_timeout(SME_AUTH_TIMEOUT, 0,
				da16x_sme_timeout_cb, wpa_s,
				NULL);
...
#if 1 // FC9000
void da16x_sme_authenticate(struct wpa_supplicant *wpa_s,
		      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
	TX_FUNC_START("    ");
				       ...
				       u16 auth_transaction, u16 seq_num)
{
	...
	resp = wpabuf_put(buf, offsetof(struct _ieee80211_mgmt,
					u.auth.variable));
					       ...
					       const u8 *da)
{
	...
	if (!resp) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Confirm message buf alloc failure");
		return;
	}
	...
	if (!buf) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Auth Confirm buf alloc failure");
		wpabuf_free(resp);
		return;
	}
	wpa_s->sme.seq_num++;
...
#if 1 /* Station WPA3 SAE ������ �õ� ���� �ʴ� ���� Debug */
			wpa_dbg(wpa_s, MSG_USER_DEBUG, "[%s] SAE_SILENTLY_DISCARD", __func__);
			return -1;
#else
			return 0;
#endif /* Station WPA3 SAE ������ �õ� ���� �ʴ� ���� Debug */
		}
		if (res != WLAN_STATUS_SUCCESS)
			return -1;

		if (sae_process_commit(&wpa_s->sme.sae) < 0) {
			wpa_printf_dbg(MSG_DEBUG, "SAE: Failed to process peer "
				   "commit");
			return -1;
		}
...
{
	const struct _ieee80211_mgmt *header;
	size_t auth_length;

	header = (const struct _ieee80211_mgmt *) auth_frame;
...
void da16x_sme_event_auth(struct wpa_supplicant *wpa_s, union wpa_event_data *data)
{
	...
	da16x_eloop_cancel_timeout(sme_auth_timer, wpa_s, NULL);
	...
	if (data->auth.auth_type == WLAN_AUTH_SAE) {
		...
		wpa_printf_dbg(MSG_DEBUG, "SME: SAE completed - setting PMK for "
			   "4-way handshake");
		wpa_sm_set_pmk(wpa_s->wpa, wpa_s->sme.sae.pmk, PMK_LEN,
			       wpa_s->sme.sae.pmkid, wpa_s->pending_bssid);
	}
...
void da16x_sme_event_auth_timed_out(struct wpa_supplicant *wpa_s,
			      union wpa_event_data *data)
{
	da16x_assoc_prt("[%s] SME: Authentication timed out\n", __func__);
...
void da16x_sme_event_assoc_timed_out(struct wpa_supplicant *wpa_s,
			       union wpa_event_data *data)
{
	da16x_assoc_prt("[%s] SME: Association timed out\n", __func__);
		   ...
		   const u8 *bssid, u16 auth_type)
{
	...
	wpa_msg(wpa_s, MSG_INFO, "Trying to associate with " MACSTR
		...
		params.freq.freq);
#else
	da16x_assoc_prt("[%s] Trying to associate with " MACSTR
        " (SSID='%s' freq=%d MHz)\n",
        __func__, MAC2STR(params.bssid),
        params.ssid ? wpa_ssid_txt(params.ssid, params.ssid_len) : "",
        params.freq);
	...
	if (params.wpa_ie == NULL ||
	    ieee802_11_parse_elements(params.wpa_ie, params.wpa_ie_len, &elems, 0) < 0) {
	...
	da16x_eloop_register_timeout(SME_ASSOC_TIMEOUT, 0, sme_assoc_timer, wpa_s,
			       NULL);
...
void da16x_sme_associate(struct wpa_supplicant *wpa_s, enum wpas_mode mode,
			const u8 *bssid, u16 auth_type)
{
	struct wpa_driver_associate_params params;
	struct ieee802_11_elems elems;
	...
	if (params.wpa_ie == NULL ||
	    ieee802_11_parse_elements(params.wpa_ie, params.wpa_ie_len, &elems, 0)
	    < 0) {
		da16x_assoc_prt("    [%s] Could not parse own IEs?!\n",
							__func__);
		os_memset(&elems, 0, sizeof(elems));
	}
	...
	if (wpa_drv_associate(wpa_s, &params) < 0) {
		da16x_err_prt("    [%s] Association request to the "
					"driver failed\n", __func__);
	...
	da16x_eloop_register_timeout(SME_ASSOC_TIMEOUT, 0,
				da16x_sme_timeout_cb, wpa_s, NULL);
			    ...
			    union wpa_event_data *data)
{
	...
	da16x_eloop_cancel_timeout(sme_assoc_timer, wpa_s, NULL);
	 ...
	 * theory, this should not be needed, but macd11 gets quite confused
			...
			struct disassoc_info *info)
{
	...
	if (wpa_s->sme.prev_bssid_set) {
		/*
		 * cfgd11/macd11 can get into somewhat confused state if
...
static void sme_auth_timer(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (wpa_s->wpa_state == WPA_AUTHENTICATING) {
		wpa_dbg(wpa_s, MSG_DEBUG, "SME: Authentication timeout");
		sme_deauth(wpa_s);
	}
}
...
#if 0 /* FC9000 */
	da16x_assoc_prt("    [%s] wpa_state = %s (%d)\n", __func__,
			wpa_supplicant_state_txt(wpa_s->wpa_state),
			wpa_s->wpa_state);

	if (wpa_s->wpa_state == WPA_AUTHENTICATING ||
			wpa_s->wpa_state == WPA_ASSOCIATING) {
		da16x_assoc_prt("    [%s] SME timeout\n", __func__ );
		sme_deauth(wpa_s);
	}
#else
	/* Supplciant 2.7 */
...
{
	/*
	 * macd11-workaround to force deauth on failed auth cmd,
...
#ifdef CONFIG_IEEE80211W
void da16x_sme_deinit(struct wpa_supplicant *wpa_s)
{
#ifdef CONFIG_IEEE80211R
	os_free(wpa_s->sme.ft_ies);
	wpa_s->sme.ft_ies = NULL;
	wpa_s->sme.ft_ies_len = 0;
#endif /* CONFIG_IEEE80211R */
	da16x_sme_stop_sa_query(wpa_s);
	sme_clear_on_disassoc(wpa_s);

	da16x_eloop_cancel_timeout(sme_assoc_timer, wpa_s, NULL);
	da16x_eloop_cancel_timeout(sme_auth_timer, wpa_s, NULL);
	da16x_eloop_cancel_timeout(sme_obss_scan_timeout, wpa_s, NULL);
}
				   ...
				   u8 num_intol)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "SME: Send 20/40 BSS Coexistence to " MACSTR
...
int sme_proc_obss_scan(struct wpa_supplicant *wpa_s)
{
	...
	dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) {
		...
		wpa_printf_dbg(MSG_DEBUG, "SME OBSS scan BSS " MACSTR
...
static void sme_obss_scan_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (!wpa_s->current_bss) {
		wpa_printf_dbg(MSG_DEBUG, "SME OBSS: Ignore scan request");
		return;
	}
...
#endif /* UNUSED_CODE */
	wpa_printf_dbg(MSG_DEBUG, "SME OBSS: Request an OBSS scan");

	if (wpa_supplicant_trigger_scan(wpa_s, &params)){
		wpa_printf_dbg(MSG_DEBUG, "SME OBSS: Failed to trigger scan");
	} else {
		wpa_s->sme.sched_obss_scan = 1;
	}
	os_free(params.freqs);

	da16x_eloop_register_timeout(wpa_s->sme.obss_scan_int, 0,
...
void sme_sched_obss_scan(struct wpa_supplicant *wpa_s, int enable)
{
	...
	if (wpa_s->sme.obss_scan_int < 10) {
		wpa_printf_dbg(MSG_DEBUG, "SME: Invalid OBSS Scan Interval %u "
			   "replaced with the minimum 10 sec",
			   wpa_s->sme.obss_scan_int);
		wpa_s->sme.obss_scan_int = 10;
	}
	wpa_printf_dbg(MSG_DEBUG, "SME: OBSS Scan Interval %u sec",
		   wpa_s->sme.obss_scan_int);
	da16x_eloop_register_timeout(wpa_s->sme.obss_scan_int, 0,
			       sme_obss_scan_timeout, wpa_s, NULL);
}
...
static int da16x_sme_check_sa_query_timeout(struct wpa_supplicant *wpa_s)
{
	u32 tu;
...
static void da16x_sme_send_sa_query_req(struct wpa_supplicant *wpa_s,
				  const u8 *trans_id)
{
...
static void da16x_sme_sa_query_timer(void *eloop_ctx, void *timeout_ctx)
{
	struct wpa_supplicant *wpa_s = eloop_ctx;
	...
	if (wpa_s->sme.sa_query_count > 0 &&
	    da16x_sme_check_sa_query_timeout(wpa_s))
		return;
	...
	if (os_get_random(trans_id, WLAN_SA_QUERY_TR_ID_LEN) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "Could not generate SA Query ID");
		da16x_sme_stop_sa_query(wpa_s);
		return;
	}
	...
	da16x_eloop_register_timeout(sec, usec, da16x_sme_sa_query_timer, wpa_s, NULL);

	wpa_dbg(wpa_s, MSG_DEBUG, "SME: Association SA Query attempt %d",
		wpa_s->sme.sa_query_count);

	da16x_sme_send_sa_query_req(wpa_s, trans_id);
}


static void da16x_sme_start_sa_query(struct wpa_supplicant *wpa_s)
{
	da16x_sme_sa_query_timer(wpa_s, NULL);
}


static void da16x_sme_stop_sa_query(struct wpa_supplicant *wpa_s)
{
	da16x_eloop_cancel_timeout(da16x_sme_sa_query_timer, wpa_s, NULL);
...
void da16x_sme_event_unprot_disconnect(struct wpa_supplicant *wpa_s, const u8 *sa,
				 const u8 *da, u16 reason_code)
{
	...
	da16x_sme_start_sa_query(wpa_s);
}


void da16x_sme_sa_query_rx(struct wpa_supplicant *wpa_s, const u8 *sa,
