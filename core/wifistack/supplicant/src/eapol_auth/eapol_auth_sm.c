...
static void eapol_auth_tx_req(struct eapol_state_machine *sm)
{
	...
	if (sm->flags & EAPOL_SM_WAIT_START) {
		da16x_eapol_prt("     [%s] EAPOL: Drop EAPOL TX to " MACSTR
...
static void eapol_port_timers_tick(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (state->aWhile > 0) {
		state->aWhile--;
		if (state->aWhile == 0) {
			da16x_eapol_prt("     [%s] IEEE 802.1X: " MACSTR
				   " - aWhile --> 0\n", __func__ ,
				   MAC2STR(state->addr));
		}
	}
	...
	da16x_eloop_register_timeout(1, 0, eapol_port_timers_tick, eloop_ctx, state);
}
		 ...
		 const char *identity, const char *radius_cui)
{
	...
	if (sm == NULL) {
		da16x_eapol_prt("     [%s] IEEE 802.1X state machine allocation "
			   "failed\n", __func__);
		return NULL;
	}
#ifdef	CONFIG_RADIUS
...
void eapol_auth_free(struct eapol_state_machine *sm)
{
	...
	da16x_eloop_cancel_timeout(eapol_sm_step_cb, sm, NULL);
	...
	if (eapol_sm_sta_entry_alive(eapol, addr) && sm->eap) {
		...
		if (sm->eap_if->aaaEapResp) {
			...
			if (sm->eap_if->aaaEapRespData == NULL) {
				da16x_eapol_prt("     [%s] EAPOL: aaaEapResp "
					"set, but no aaaEapRespData "
					"available\n", __func__);
				return;
			}
...
void eapol_auth_step(struct eapol_state_machine *sm)
{
	...
	da16x_eloop_register_timeout(0, 0, eapol_sm_step_cb, sm, NULL);
}
