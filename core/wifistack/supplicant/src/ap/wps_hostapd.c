				  ...
				  size_t psk_len)
{
	...
	if (psk_len != PMK_LEN) {
		wpa_printf_dbg(MSG_DEBUG, "Unexpected PSK length %lu",
...
#else
		hapd->new_psk_cb(hapd->new_psk_cb_ctx, mac_addr, NULL, psk, psk_len);
#endif /* CONFIG_P2P */
	}
	...
	if (ssid->wpa_psk_file) {
		...
		if (f == NULL) {
			wpa_printf_dbg(MSG_DEBUG, "Failed to add the PSK to "
				   "'%s'", ssid->wpa_psk_file);
			return -1;
		}
				      ...
				      const struct wps_device_data *dev)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "WPS: PIN needed for E-UUID %s", uuid);
#if 0	/* by Bright */
...
#else
da16x_ap_prt("[%s] What do I do for this ??? No filesystem ...\n", __func__);
#endif	/* 0 */
}
					 ...
					 const char *dev_name)
{
	...
	char uuid[40];
#if defined(ENABLE_EVENT_DBG) && defined (CONFIG_WPA_MSG_CTRL)
	char devtype[WPS_DEV_TYPE_BUFSIZE];
#endif /* ENABLE_EVENT_DBG && CONFIG_WPA_MSG_CTRL*/
...
static void wps_reload_config(void *eloop_data, void *user_ctx)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "WPS: Reload configuration data");
	if (iface->interfaces == NULL ||
...
void hostapd_wps_eap_completed(struct hostapd_data *hapd)
{
	 ...
	 */
	if (da16x_eloop_deplete_timeout(0, 0, wps_reload_config, hapd->iface, NULL) ==
	    1)
		wpa_printf_dbg(MSG_DEBUG, "WPS: Reschedule immediate configuration reload");
}
				       ...
				       const struct wps_credential *cred)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "WPS: Updating in-memory configuration");
	 ...
	 */
	da16x_eloop_register_timeout(0, 100000, wps_reload_config, hapd->iface,
			       NULL);
...
static int hapd_wps_cred_cb(struct hostapd_data *hapd, void *ctx)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "WPS: Received new AP Settings");
	wpa_hexdump_ascii(MSG_DEBUG, "WPS: SSID", cred->ssid, cred->ssid_len);
	wpa_printf_dbg(MSG_DEBUG, "WPS: Authentication Type 0x%x",
		   cred->auth_type);
	wpa_printf_dbg(MSG_DEBUG, "WPS: Encryption Type 0x%x", cred->encr_type);
	wpa_printf_dbg(MSG_DEBUG, "WPS: Network Key Index %d", cred->key_idx);
	wpa_hexdump_key(MSG_DEBUG, "WPS: Network Key",
			cred->key, cred->key_len);
	wpa_printf_dbg(MSG_DEBUG, "WPS: MAC Address " MACSTR,
	...
	os_free(tmp_fname);
#else
da16x_ap_prt("[%s] !!! What do I do for no filesystem...\n", __func__);
#endif	/* 0 */
	...
	wpa_printf_dbg(MSG_DEBUG, "WPS: AP configuration updated");
...
static void hostapd_wps_reenable_ap_pin(void *eloop_data, void *user_ctx)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "WPS: Re-enable AP PIN");
	wpa_msg(hapd->msg_ctx, MSG_INFO, WPS_EVENT_AP_SETUP_UNLOCKED);
#ifdef	CONFIG_SUPP27_WPS_NFC
...
static int wps_pwd_auth_fail(struct hostapd_data *hapd, void *ctx)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "WPS: AP PIN authentication failure number %u "
		   "(%u consecutive)",
	...
	if (!hapd->conf->ap_setup_locked &&
		 ...
		 * reenablement.
		 */
		da16x_eloop_cancel_timeout(hostapd_wps_reenable_ap_pin, hapd, NULL);
		wpa_printf_dbg(MSG_DEBUG, "WPS: AP PIN disabled indefinitely");
		...
		wpa_printf_dbg(MSG_DEBUG, "WPS: Disable AP PIN for %u seconds",
			   hapd->ap_pin_lockout_time);
		da16x_eloop_cancel_timeout(hostapd_wps_reenable_ap_pin, hapd, NULL);
		da16x_eloop_register_timeout(hapd->ap_pin_lockout_time, 0,
				  ...
				  struct wps_event_pwd_auth_fail *data)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "WPS: Authentication failure update");
...
static int wps_ap_pin_success(struct hostapd_data *hapd, void *ctx)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "WPS: Clear consecutive AP PIN failure counter "
				   ...
				   struct wps_event_fail *fail)
{
	...
	if (fail->error_indication > 0 &&
		...
		wpa_msg(hapd->msg_ctx, MSG_INFO,
			WPS_EVENT_FAIL "msg=%d config_error=%d reason=%d (%d)",
...
#ifdef CONFIG_AP_WPS_NFC
	hostapd_wps_nfc_clear(wps);
#endif /* CONFIG_AP_WPS_NFC */
		     ...
		     struct hostapd_bss_config *conf)
{
	...
	if (cfg.dualband)
		wpa_printf_dbg(MSG_DEBUG, "WPS: Dualband AP");
#else
	cfg.dualband = 0;
#endif	/* CONFIG_AP_SUPPORT_5GHZ */
#endif	/* CONFIG_SUPP27_WPS_DUALBAND */
...
{
	wpa_printf_dbg(MSG_DEBUG, "WPS: Clear NFC Tag context %p", wps);
...
{
	da16x_eloop_cancel_timeout(hostapd_wps_reenable_ap_pin, hapd, NULL);
	da16x_eloop_cancel_timeout(hostapd_wps_ap_pin_timeout, hapd, NULL);
	da16x_eloop_cancel_timeout(wps_reload_config, hapd->iface, NULL);
	if (hapd->wps == NULL) {
				    ...
				    int ssi_signal)
{
	...
	if (ieee802_11_parse_elements(ie, ie_len, &elems, 0) == ParseFailed) {
		wpa_printf_dbg(MSG_DEBUG, "WPS: Could not parse ProbeReq from "
			   MACSTR, MAC2STR(addr));
		return 0;
	}
...
#else
		wps_registrar_probe_req_rx(hapd->wps->registrar, addr, wps_ie, 0);
#endif /* CONFIG_P2P */
	...
	enum wps_msg_type msg_type)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "WPS UPnP: PutWLANResponse ev_type=%d mac_addr="
		   MACSTR, ev_type, MAC2STR(mac_addr));
	wpa_hexdump(MSG_MSGDUMP, "WPS UPnP: PutWLANResponse NewMessage",
		    wpabuf_head(msg), wpabuf_len(msg));
	if (ev_type != UPNP_WPS_WLANEVENT_TYPE_EAP) {
		wpa_printf_dbg(MSG_DEBUG, "WPS UPnP: Ignored unexpected "
	...
	if (!sta) {
		 ...
		 */
		wpa_printf_dbg(MSG_DEBUG, "WPS UPnP: No matching STA found based "
	...
	if (!sta || !(sta->flags & WLAN_STA_WPS)) {
		wpa_printf_dbg(MSG_DEBUG, "WPS UPnP: No matching STA found");
		return 0;
	}
	...
	if (!sta->eapol_sm) {
		 ...
		 * deauthenticated).
		 */
		wpa_printf_dbg(MSG_DEBUG, "WPS UPnP: Matching STA did not have EAPOL state machine initialized");
		return 0;
	}
...
{
	struct hostapd_data *hapd = eloop_data;
	wpa_printf_dbg(MSG_DEBUG, "WPS: AP PIN timed out");
...
{
	wpa_printf_dbg(MSG_DEBUG, "WPS: Enabling AP PIN (timeout=%d)", timeout);
...
{
	wpa_printf_dbg(MSG_DEBUG, "WPS: Disabling AP PIN");
				       ...
				       const struct wpabuf *wps)
{
	...
	if (wps_parse_msg(wps, &attr)) {
		wpa_printf_dbg(MSG_DEBUG, "WPS: Ignore invalid data from NFC tag");
		return -1;
	}
	...
	wpa_printf_dbg(MSG_DEBUG, "WPS: Ignore unrecognized NFC tag");
	return -1;
}
			     ...
			     const struct wpabuf *data)
{
	...
	if (*wpabuf_head_u8(data) != 0x10) {
		...
		if (tmp == NULL) {
			wpa_printf_dbg(MSG_DEBUG, "WPS: Could not parse NDEF");
			return -1;
		}
		wps = tmp;
	}
				    ...
				    const struct wpabuf *sel)
{
	...
	if (wps == NULL)
		return -1;
	wpa_printf_dbg(MSG_DEBUG, "WPS: Received application/vnd.wfa.wsc "
		   "payload from NFC connection handover");
	wpa_hexdump_buf(MSG_DEBUG, "WPS: NFC payload", wps);
	if (wpabuf_len(wps) < 2) {
		wpa_printf_dbg(MSG_DEBUG, "WPS: Too short Wi-Fi Handover Request "
			   "Message");
		goto out;
	}
	pos = wpabuf_head(wps);
	...
	if (wsc_len > wpabuf_len(wps) - 2) {
		wpa_printf_dbg(MSG_DEBUG, "WPS: Invalid WSC attribute length (%u) "
	...
	if (ret < 0) {
		wpa_printf_dbg(MSG_DEBUG, "WPS: Could not parse WSC attributes in "
	...
	if (attr.oob_dev_password == NULL ||
		...
		wpa_printf_dbg(MSG_DEBUG, "WPS: No Out-of-Band Device Password "
	...
	if (attr.uuid_e == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "WPS: No UUID-E included in Wi-Fi "
	...
	if (dev_pw_id != DEV_PW_NFC_CONNECTION_HANDOVER) {
		wpa_printf_dbg(MSG_DEBUG, "WPS: Unexpected OOB Device Password ID "
...
{
	wpa_printf_dbg(MSG_DEBUG, "WPS: Disable NFC token for AP interface %s",
		   hapd->conf->iface);
