...
#if 0	// for IAR compiler
			  "rx_bytes=%llu\ntx_bytes=%llu\n"
#else	// for GCC cross-compiler
			  "rx_bytes=%lu\ntx_bytes=%lu\n"
#endif	//
#ifdef UNUSED
			  "inactive_msec=%lu\n"
				      ...
				      char *buf, size_t buflen)
{
	...
	for (i = 0; i < sta->supported_rates_len; i++) {
		if((sta->supported_rates[i] & 0x7f)/2 > 11){
			gmode = 1;
			break;
		}
	}

	if (gmode == 1 && sta->no_ht_set == 1) {
		ret = os_snprintf(buf + len, buflen - len, "802.11g\n");
	} else if(gmode == 1 && sta->no_ht_set == 0) {
	...
	if (hapd->conf->mesh & MESH_ENABLED) {
		res = os_snprintf(buf + len, buflen - len, "plink_state=%d\n",
				  sta->plink_state);
		if (!os_snprintf_error(buflen - len, res))
			len += res;
	}
...
#define SET_CSA_SETTING(str) \
	...
	SET_CSA_SETTING(cent_freq1);
	SET_CSA_SETTING(cent_freq2);
...
int hostapd_ctrl_iface_save_config(struct hostapd_data *hapd)
{
	...
	if (hostapd_iconf_write(hapd->iconf))
		err++;

	if (err) {
		da16x_err_prt("[%s] SAVE_CONFIG - Failed to update hostapd configuration \n", __func__);
		return -1;
	}
	da16x_ap_prt("[%s] SAVE_CONFIG - Configuration hostapd updated\n", __func__);
	return 0;
}
...
int hostapd_ctrl_iface_read_config(struct hostapd_config *iconf)
{
	...
	if (hostapd_iconf_read(iconf))
		err++;

	if (err) {
		da16x_err_prt("[%s] READ_CONFIG - Failed to update hostapd configuration \n", __func__);
		return -1;
	}
	da16x_ap_prt("[%s] READ_CONFIG - Configuration hostapd updated\n", __func__);
	return 0;
}
