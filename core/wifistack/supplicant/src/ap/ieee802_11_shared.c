				  ...
				  const u8 *addr, const u8 *trans_id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IEEE 802.11: Sending SA Query Request to "
		   MACSTR, MAC2STR(addr));
					  ...
					  const u8 *sa, const u8 *trans_id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IEEE 802.11: Received SA Query Request from "
		   MACSTR, MAC2STR(sa));
	wpa_hexdump(MSG_DEBUG, "IEEE 802.11: SA Query Transaction ID",
		    trans_id, WLAN_SA_QUERY_TR_ID_LEN);
	...
	if (sta == NULL || !(sta->flags & WLAN_STA_ASSOC)) {
		wpa_printf_dbg(MSG_DEBUG, "IEEE 802.11: Ignore SA Query Request "
			   "from unassociated STA " MACSTR, MAC2STR(sa));
		return;
	}

	wpa_printf_dbg(MSG_DEBUG, "IEEE 802.11: Sending SA Query Response to "
		   MACSTR, MAC2STR(sa));
	...
	if (hostapd_drv_send_mlme(hapd, &resp, end - (u8 *) &resp, 0) < 0)
		wpa_printf(MSG_INFO, "_ieee80211_mgmt_sa_query_request: send failed");
}
				...
				const u8 action_type, const u8 *trans_id)
{
	...
	if (action_type != WLAN_SA_QUERY_RESPONSE) {
		wpa_printf_dbg(MSG_DEBUG, "IEEE 802.11: Unexpected SA Query "
			   "Action %d", action_type);
		return;
	}

	wpa_printf_dbg(MSG_DEBUG, "IEEE 802.11: Received SA Query Response from "
		   MACSTR, MAC2STR(sa));
	wpa_hexdump(MSG_DEBUG, "IEEE 802.11: SA Query Transaction ID",
		    trans_id, WLAN_SA_QUERY_TR_ID_LEN);
	...
	if (sta == NULL || sta->sa_query_trans_id == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "IEEE 802.11: No matching STA with "
	...
	if (i >= sta->sa_query_count) {
		wpa_printf_dbg(MSG_DEBUG, "IEEE 802.11: No matching SA Query "
			   "transaction identifier found");
		return;
	}

#if 0 //[[ trinity_0170920_BEGIN
