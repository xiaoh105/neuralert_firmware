			       ...
			       struct eapol_state_machine *eapol)
{
	...
	if (entry->identity) {
		...
		da16x_ascii_dump("STA identity from PMKSA",
	...
	if (eapol->radius_class.attr) {
		da16x_ap_prt("[%s] Copied %lu Class attribute(s) from PMKSA\n",
			__func__, (unsigned long) eapol->radius_class.count);
	}
#endif /* CONFIG_RADIUS */
				   ...
				   struct rsn_pmksa_cache_entry *entry)
{
	...
	da16x_ap_prt("[%s] RSN: added PMKSA cache entry for " MACSTR "\n",
		   __func__, MAC2STR(entry->spa));

	da16x_dump("RSN: added PMKID", entry->pmkid, PMKID_LEN);
}
#endif /* 1 */
			      ...
			      struct eapol_state_machine *eapol, int akmp)
{
	...
	if (session_timeout > 0)
        ...
        if (wpa_config_read_nvram_int(ENV_PMK_LIFE_TIME, &val) != -1) {
            ...
            entry->expiration += DEFAULT_PMK_LIFE_TIME;
        }
    }
	entry->akmp = akmp;
			       ...
			       struct rsn_pmksa_cache_entry *entry)
{
	...
	if (pmksa->pmksa_count >= pmksa_cache_max_entries && pmksa->pmksa) {
		...
		wpa_printf_dbg(MSG_DEBUG, "RSN: removed the oldest PMKSA cache "
		...
		struct eapol_state_machine *eapol, int akmp)
{
	...
	if (pmksa->pmksa_count >= pmksa_cache_max_entries && pmksa->pmksa) {
		/* Remove the oldest entry to make room for the new entry */
		da16x_ap_prt("[%s] RSN: removed the oldest PMKSA cache "
			   "entry (for " MACSTR ") to make room for new one\n",
			   __func__, MAC2STR(pmksa->pmksa->spa));
...
void pmksa_cache_auth_deinit(struct rsn_pmksa_cache *pmksa)
{
	...
	da16x_eloop_cancel_timeout(pmksa_cache_expire, pmksa, NULL);
#else
da16x_ap_prt("[%s] What do I do for timer cancel\n", __func__);
#endif	/* 0 */
