...
{
	wpa_printf_dbg(MSG_DEBUG, "VLAN: Set interface %s up", if_name);
...
{
	wpa_printf_dbg(MSG_DEBUG, "VLAN: Set interface %s down", if_name);
...
static int br_delif(const char *br_name, const char *if_name)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "VLAN: br_delif(%s, %s)", br_name, if_name);
	if ((fd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
		wpa_printf(MSG_ERROR, "VLAN: %s: socket(AF_INET,SOCK_STREAM) "
			   "failed: %s", __func__, strerror(errno));
		return -1;
...
static int br_addif(const char *br_name, const char *if_name)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "VLAN: br_addif(%s, %s)", br_name, if_name);
	if ((fd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
		wpa_printf(MSG_ERROR, "VLAN: %s: socket(AF_INET,SOCK_STREAM) "
			   "failed: %s", __func__, strerror(errno));
		return -1;
...
static int br_delbr(const char *br_name)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "VLAN: br_delbr(%s)", br_name);
	if ((fd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
		wpa_printf(MSG_ERROR, "VLAN: %s: socket(AF_INET,SOCK_STREAM) "
			   "failed: %s", __func__, strerror(errno));
		return -1;
...
static int br_addbr(const char *br_name)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "VLAN: br_addbr(%s)", br_name);
	if ((fd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
		wpa_printf(MSG_ERROR, "VLAN: %s: socket(AF_INET,SOCK_STREAM) "
			   "failed: %s", __func__, strerror(errno));
		return -1;
...
int vlan_rem(const char *if_name)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "VLAN: vlan_rem(%s)", if_name);
...
int vlan_add(const char *if_name, int vid, const char *vlan_if_name)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "VLAN: vlan_add(if_name=%s, vid=%d)",
		   if_name, vid);
	ifconfig_up(if_name);
	...
	if (ioctl(fd, SIOCSIFVLAN, &if_request) == 0) {
		...
		if (if_request.u.VID == vid) {
			...
			if (ioctl(fd, SIOCSIFVLAN, &if_request) == 0 &&
				...
				close(fd);
				wpa_printf_dbg(MSG_DEBUG, "VLAN: vlan_add: "
...
static int vlan_set_name_type(unsigned int name_type)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "VLAN: vlan_set_name_type(name_type=%u)",
		   name_type);
	if ((fd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
		wpa_printf(MSG_ERROR, "VLAN: %s: socket(AF_INET,SOCK_STREAM) "
			   "failed: %s", __func__, strerror(errno));
...
static void vlan_newlink(char *ifname, struct hostapd_data *hapd)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "VLAN: vlan_newlink(%s)", ifname);
...
static void vlan_dellink(char *ifname, struct hostapd_data *hapd)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "VLAN: vlan_dellink(%s)", ifname);
...
static void vlan_event_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	while (left >= (int) sizeof(*h)) {
		...
		plen = len - sizeof(*h);
		if (len > left || plen < 0) {
			wpa_printf_dbg(MSG_DEBUG, "VLAN: Malformed netlink "
	...
	if (left > 0) {
		wpa_printf_dbg(MSG_DEBUG, "VLAN: %s: %d extra bytes in the end of "
				       ...
				       int vlan_id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "VLAN: %s(vlan_id=%d ifname=%s)",
...
int vlan_remove_dynamic(struct hostapd_data *hapd, int vlan_id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "VLAN: %s(vlan_id=%d)", __func__, vlan_id);
