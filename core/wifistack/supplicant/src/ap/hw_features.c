...
extern int allowed_ht40_channel_pair(struct hostapd_hw_modes *mode, int pri_chan, int sec_chan);

extern int check_40mhz_5g(struct hostapd_hw_modes *mode, struct wpa_scan_results *scan_res, int pri_chan, int sec_chan);
extern int check_40mhz_2g4(struct hostapd_hw_modes *mode, struct wpa_scan_results *scan_res, int pri_chan, int sec_chan);
#endif	/* CONFIG_IEEE80211N */

#ifdef CONFIG_AP_HW_FEATURE
extern int hw_get_freq(struct hostapd_hw_modes *mode, int chan);
extern int hw_get_chan(struct hostapd_hw_modes *mode, int freq);
			  ...
			  struct hostapd_hw_modes *mode)
{
	...
	for (i = 0; i < mode->num_rates; i++) {
		...
		wpa_printf_dbg(MSG_DEBUG, "RATE[%d] rate=%d flags=0x%x",
					 ...
					 struct wpa_driver_scan_params *params)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "40 MHz affected channel range: [%d,%d] MHz",
...
#define HT2040_COEX_SCAN_RETRY 15
	...
	if (ret == -EBUSY &&
		...
		da16x_eloop_register_timeout(1, 0, ap_ht40_scan_retry, iface, NULL);
		return;
	}
...
{
	da16x_eloop_cancel_timeout(ap_ht40_scan_retry, iface, NULL);
}
...
static int ieee80211n_check_40mhz(struct hostapd_iface *iface)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Scan for neighboring BSSes prior to enabling "
		   "40 MHz channel");
	os_memset(&params, 0, sizeof(params));
	if (iface->current_mode->mode == HOSTAPD_MODE_IEEE80211G)
		ieee80211n_scan_channels_2g4(iface, &params);
...
static int ieee80211ac_supported_vht_capab(struct hostapd_iface *iface)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "hw vht capab: 0x%x, conf vht capab: 0x%x",
		   hw, conf);
...
int hostapd_acs_completed(struct hostapd_iface *iface, int err)
{
	...
	case HOSTAPD_CHAN_VALID:
		...
		wpa_msg(iface->bss[0]->msg_ctx, MSG_INFO,
			"[%s] " ACS_EVENT_COMPLETED "freq=%d channel=%d", __func__,
	...
	if (ret < 0)
		goto out;
	if (ret == 1) {
		wpa_printf_dbg(MSG_DEBUG, "Interface initialization will be completed in a callback");
		return 0;
	}
