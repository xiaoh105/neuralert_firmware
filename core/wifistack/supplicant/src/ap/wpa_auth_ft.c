			   ...
			   const u8 *data, size_t data_len)
{
	if (wpa_auth->cb.send_ether == NULL)
		return -1;
	wpa_printf_dbg(MSG_DEBUG, "FT: RRB send to " MACSTR, MAC2STR(dst));
			    ...
			    u8 *tspec_ie, size_t tspec_ielen)
{
	if (wpa_auth->cb.add_tspec == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "FT: add_tspec is not initialized");
		return -1;
	}
			      ...
			      const u8 *pmk_r0_name)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: Send PMK-R1 pull request to remote R0KH "
	...
	if (random_get_bytes(f.nonce, FT_R0KH_R1KH_PULL_NONCE_LEN)) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Failed to get random data for "
			   "nonce");
		return -1;
	}
			   ...
			   struct wpa_ptk *ptk, size_t ptk_len)
{
	...
	if (sm->xxkey_len == 0) {
		wpa_printf_dbg(MSG_DEBUG, "FT: XXKey not available for key "
			   "derivation");
		return -1;
	}
				...
				const u8 *ies, size_t ies_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: Resource Request: id=%d descr_count=%d",
		   id, descr_count);
	...
	*pos++ = sizeof(*rdie);
	...
	if (ieee802_11_parse_elements((u8 *) ies, ies_len, &parse, 1) ==
	    ParseFailed) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Failed to parse request IEs");
		rdie->status_code =
	...
	if (parse.wmm_tspec && sm->wpa_auth->conf.ap_mlme) {
		...
		if (parse.wmm_tspec_len + 2 < (int) sizeof(*tspec)) {
			wpa_printf_dbg(MSG_DEBUG, "FT: Too short WMM TSPEC IE "
		...
		wpa_printf_dbg(MSG_DEBUG, "FT: ADDTS processing result: %d", res);
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: No supported resource requested");
				 ...
				 const u8 *req_ies, size_t req_ies_len)
{
	...
	if (auth_alg == WLAN_AUTH_FT &&
	    wpa_ft_mic(sm->PTK.kck, sm->addr, sm->wpa_auth->addr, 6,
		       ...
		       _ftie->mic) < 0)
		wpa_printf_dbg(MSG_DEBUG, "FT: Failed to calculate MIC");
...
void wpa_ft_install_ptk(struct wpa_state_machine *sm)
{
	...
	if (!wpa_cipher_valid_pairwise(sm->pairwise)) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Unknown pairwise alg 0x%x - skip "
				   ...
				   u8 **resp_ies, size_t *resp_ies_len)
{
	...
	*resp_ies_len = 0;
	...
	if (wpa_ft_parse_ies(ies, ies_len, &parse) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Failed to parse FT IEs");
	...
	if (mdie == NULL || parse.mdie_len < sizeof(*mdie) ||
		...
		wpa_printf_dbg(MSG_DEBUG, "FT: Invalid MDIE");
		return WLAN_STATUS_INVALID_MDIE;
	}

	ftie = (struct rsn_ftie *) parse.ftie;
	if (ftie == NULL || parse.ftie_len < sizeof(*ftie)) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Invalid FTIE");
	...
	if (parse.r0kh_id == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Invalid FTIE - no R0KH-ID");
		return WLAN_STATUS_INVALID_FTIE;
	}
	...
	if (parse.rsn_pmkid == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "FT: No PMKID in RSNIE");
	...
	if (wpa_ft_fetch_pmk_r1(sm->wpa_auth, sm->addr, pmk_r1_name, pmk_r1,
		...
		if (wpa_ft_pull_pmk_r1(sm, ies, ies_len, parse.rsn_pmkid) < 0) {
			wpa_printf_dbg(MSG_DEBUG, "FT: Did not have matching "
			 ...
			 void *ctx)
{
	...
	if (sm == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Received authentication frame, but "
			   "WPA SM not available");
		return;
	}

	wpa_printf_dbg(MSG_DEBUG, "FT: Received authentication frame: STA=" MACSTR
	...
	res = wpa_ft_process_auth_req(sm, ies, ies_len, &resp_ies,
				      &resp_ies_len);
	if (res < 0) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Callback postponed until response is available");
		return;
	}
	status = res;

	wpa_printf_dbg(MSG_DEBUG, "FT: FT authentication response: dst=" MACSTR
			    ...
			    size_t ies_len)
{
	...
	if (parse.rsn == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "FT: No RSNIE in Reassoc Req");
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
	...
	{
		wpa_printf_dbg(MSG_DEBUG, "FT: PMKID in Reassoc Req did not match "
	...
	if (os_memcmp(ftie->snonce, sm->SNonce, WPA_NONCE_LEN) != 0) {
		wpa_printf_dbg(MSG_DEBUG, "FT: SNonce mismatch in FTIE");
		wpa_hexdump(MSG_DEBUG, "FT: Received SNonce",
	...
	if (os_memcmp(ftie->anonce, sm->ANonce, WPA_NONCE_LEN) != 0) {
		wpa_printf_dbg(MSG_DEBUG, "FT: ANonce mismatch in FTIE");
	...
	if (parse.r0kh_id == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "FT: No R0KH-ID subelem in FTIE");
		return -1;
	}

	if (parse.r0kh_id_len != sm->r0kh_id_len ||
	    os_memcmp(parse.r0kh_id, sm->r0kh_id, parse.r0kh_id_len) != 0) {
		wpa_printf_dbg(MSG_DEBUG, "FT: R0KH-ID in FTIE did not match with "
			   "the current R0KH-ID");
	...
	if (parse.r1kh_id == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "FT: No R1KH-ID subelem in FTIE");
		return -1;
	}

	if (os_memcmp(parse.r1kh_id, sm->wpa_auth->conf.r1_key_holder,
		      FT_R1KH_ID_LEN) != 0) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Unknown R1KH-ID used in "
			   "ReassocReq");
	...
	if (parse.rsn_pmkid == NULL ||
		...
		wpa_printf_dbg(MSG_DEBUG, "FT: No matching PMKR1Name (PMKID) in "
	...
	if (ftie->mic_control[1] != count) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Unexpected IE count in MIC "
	...
	if (os_memcmp(mic, ftie->mic, 16) != 0) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Invalid MIC in FTIE");
		wpa_printf_dbg(MSG_DEBUG, "FT: addr=" MACSTR " auth_addr=" MACSTR,
...
int wpa_ft_action_rx(struct wpa_state_machine *sm, const u8 *data, size_t len)
{
	...
	if (len < 14) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Too short FT Action frame "
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: Received FT Action frame (STA=" MACSTR
		   " Target AP=" MACSTR " Action=%d)",
		   MAC2STR(sta_addr), MAC2STR(target_ap), action);

	if (os_memcmp(sta_addr, sm->addr, ETH_ALEN) != 0) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Mismatch in FT Action STA address: "
	...
	if ((target_ap[0] & 0x01) ||
		...
		wpa_printf_dbg(MSG_DEBUG, "FT: Invalid Target AP in FT Action "
			   "frame");
		return -1;
	}
				     ...
				     const u8 *ies, size_t ies_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: Over-the-DS RX request cb for " MACSTR,
		   MAC2STR(sm->addr));
				 ...
				 const u8 *body, size_t len)
{
	...
	if (sm == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Failed to add new STA based on "
			   "RRB Request");
		return -1;
	}
	...
	res = wpa_ft_process_auth_req(sm, body, len, &resp_ies,
				      &resp_ies_len);
	if (res < 0) {
		wpa_printf_dbg(MSG_DEBUG, "FT: No immediate response available - wait for pull response");
		return 0;
	}
	status = res;
				     ...
				     size_t resp_ies_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: RRB authentication response: STA=" MACSTR
			      ...
			      const u8 *data, size_t data_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: Received PMK-R1 pull");
	...
	if (r1kh == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "FT: No matching R1KH address found for "
	...
	if (aes_unwrap(r1kh->key, (FT_R0KH_R1KH_PULL_DATA_LEN + 7) / 8,
		...
		wpa_printf_dbg(MSG_DEBUG, "FT: Failed to decrypt PMK-R1 pull "
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: PMK-R1 pull - R1KH-ID=" MACSTR " S1KH-ID="
	...
	if (wpa_ft_fetch_pmk_r0(wpa_auth, f.s1kh_id, f.pmk_r0_name, pmk_r0,
				&pairwise) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "FT: No matching PMKR0Name found for "
			   "PMK-R1 pull");
		return -1;
	}
...
static void ft_pull_resp_cb_finish(void *eloop_ctx, void *timeout_ctx)
{
	...
	status = res;
	wpa_printf_dbg(MSG_DEBUG, "FT: Postponed auth callback result for " MACSTR
...
static int ft_pull_resp_cb(struct wpa_state_machine *sm, void *ctx)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: Response to a pending pull request for "
		   MACSTR " - process from timeout", MAC2STR(sm->addr));
	da16x_eloop_register_timeout(0, 0, ft_pull_resp_cb_finish, sm, NULL);
	return 1;
}
			      ...
			      const u8 *data, size_t data_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: Received PMK-R1 pull response");
	...
	if (r0kh == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "FT: No matching R0KH address found for "
	...
	if (os_memcmp(f.r1kh_id, wpa_auth->conf.r1_key_holder, FT_R1KH_ID_LEN)
	    != 0) {
		wpa_printf_dbg(MSG_DEBUG, "FT: PMK-R1 pull response did not use a "
			   "matching R1KH-ID");
		return -1;
	}
	...
	res = wpa_ft_store_pmk_r1(wpa_auth, f.s1kh_id, f.pmk_r1, f.pmk_r1_name,
				  pairwise);
	wpa_printf_dbg(MSG_DEBUG, "FT: Look for pending pull request");
			      ...
			      const u8 *data, size_t data_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: Received PMK-R1 push");
	...
	if (aes_unwrap(r0kh->key, (FT_R0KH_R1KH_PUSH_DATA_LEN + 7) / 8,
		...
		wpa_printf_dbg(MSG_DEBUG, "FT: Failed to decrypt PMK-R1 push from "
	...
	if ((now.sec > tsend && now.sec - tsend > 60) ||
		...
		wpa_printf_dbg(MSG_DEBUG, "FT: PMK-R1 push did not have a valid "
	...
	if (os_memcmp(f.r1kh_id, wpa_auth->conf.r1_key_holder, FT_R1KH_ID_LEN)
	    != 0) {
		wpa_printf_dbg(MSG_DEBUG, "FT: PMK-R1 push did not use a matching "
	...
	pairwise = le_to_host16(f.pairwise);
	wpa_printf_dbg(MSG_DEBUG, "FT: PMK-R1 push - R1KH-ID=" MACSTR " S1KH-ID="
		   MACSTR " pairwise=0x%x",
		   MAC2STR(f.r1kh_id), MAC2STR(f.s1kh_id), pairwise);
		  ...
		  const u8 *data, size_t data_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: RRB received frame from remote AP " MACSTR,
		   MAC2STR(src_addr));

	if (data_len < sizeof(*frame)) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Too short RRB frame (data_len=%lu)",
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: RRB frame - frame_type=%d packet_type=%d "
	...
	if (frame->frame_type != RSN_REMOTE_FRAME_TYPE_FT_RRB) {
		...
		wpa_printf_dbg(MSG_DEBUG, "FT: RRB discarded frame with "
			   "unrecognized type %d", frame->frame_type);
		return -1;
	}

	if (alen > data_len - sizeof(*frame)) {
		wpa_printf_dbg(MSG_DEBUG, "FT: RRB frame too short for action "
			   "frame");
		return -1;
	}
	...
	if (alen < 1 + 1 + 2 * ETH_ALEN) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Too short RRB frame (not enough "
	...
	if (*pos != WLAN_ACTION_FT) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Unexpected Action frame category "
			   "%d", *pos);
		return -1;
	}
	...
	target_ap_addr = pos;
	pos += ETH_ALEN;
	wpa_printf_dbg(MSG_DEBUG, "FT: RRB Action Frame: action=%d sta_addr="
		   MACSTR " target_ap_addr=" MACSTR,
		   action, MAC2STR(sta_addr), MAC2STR(target_ap_addr));

	if (frame->packet_type == FT_PACKET_REQUEST) {
		wpa_printf_dbg(MSG_DEBUG, "FT: FT Packet Type - Request");

		if (action != 1) {
			wpa_printf_dbg(MSG_DEBUG, "FT: Unexpected Action %d in "
				   "RRB Request", action);
			return -1;
		}

		if (os_memcmp(target_ap_addr, wpa_auth->addr, ETH_ALEN) != 0) {
			wpa_printf_dbg(MSG_DEBUG, "FT: Target AP address in the "
		...
		if (end - pos < 2) {
			wpa_printf_dbg(MSG_DEBUG, "FT: Not enough room for status "
		...
		wpa_printf_dbg(MSG_DEBUG, "FT: FT Packet Type - Response "
			   "(status_code=%d)", status_code);

		if (wpa_ft_action_send(wpa_auth, sta_addr, start, alen) < 0)
			return -1;
	} else {
		wpa_printf_dbg(MSG_DEBUG, "FT: RRB discarded frame with unknown "
				   ...
				   const u8 *s1kh_id, int pairwise)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: R1KH-ID " MACSTR, MAC2STR(r1kh->id));
...
void wpa_ft_push_pmk_r1(struct wpa_authenticator *wpa_auth, const u8 *addr)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: Deriving and pushing PMK-R1 keys to R1KHs "
