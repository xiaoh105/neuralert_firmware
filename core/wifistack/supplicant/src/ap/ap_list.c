...
static struct ap_info * ap_ap_add(struct hostapd_iface *iface, const u8 *addr)
{
	...
	if (iface->num_ap > iface->conf->ap_table_max_size && ap != ap->prev) {
		wpa_printf_dbg(MSG_DEBUG, "Removing the least recently used AP "
			    ...
			    struct hostapd_frame_info *fi)
{
	...
	if (!iface->olbc &&
		...
		iface->olbc = 1;
		wpa_printf_dbg(MSG_DEBUG, "OLBC AP detected: " MACSTR
	...
	if (!iface->olbc_ht && !ap->ht_support &&
		...
		wpa_printf_dbg(MSG_DEBUG, "OLBC HT AP detected: " MACSTR
			   " (channel %d) - enable protection",
			   MAC2STR(ap->addr), ap->channel);
		set_beacon++;
	}
...
void ap_list_timer(struct hostapd_iface *iface)
{
...
#if 0 // Supplicant 2.7
	da16x_eloop_register_timeout(10, 0, ap_list_timer, iface, NULL);
#endif /* 0 */
	...
	if (iface->olbc || iface->olbc_ht) {
		...
		if (!olbc && iface->olbc) {
			wpa_printf_dbg(MSG_DEBUG, "OLBC not detected anymore");
			iface->olbc = 0;
			set_beacon++;
		}
#ifdef CONFIG_IEEE80211N
		...
		if (!olbc_ht && iface->olbc_ht) {
			wpa_printf_dbg(MSG_DEBUG, "OLBC HT not detected anymore");
			iface->olbc_ht = 0;
			hostapd_ht_operation_update(iface);
			set_beacon++;
		}
...
{
#if 0 // Supplicant 2.7
	da16x_eloop_cancel_timeout(ap_list_timer, iface, NULL);
#endif /* 0 */
