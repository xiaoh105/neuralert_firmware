			       ...
			       const u8 *addr, u16 reason)
{
	if (wpa_auth->cb->disconnect == NULL)
		return;
	wpa_printf_dbg(MSG_DEBUG, "wpa_sta_disconnect STA " MACSTR " (reason %u)",
		   MAC2STR(addr), reason);
...
static void wpa_rekey_gmk(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (wpa_auth->conf.wpa_gmk_rekey) {
		da16x_eloop_register_timeout(wpa_auth->conf.wpa_gmk_rekey, 0,
...
static void wpa_rekey_gtk(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (wpa_auth->conf.wpa_group_rekey) {
		da16x_eloop_register_timeout(wpa_auth->conf.wpa_group_rekey,
					 ...
					 int vlan_id, int delay_init)
{
	...
	group->GInit = TRUE;
	if (delay_init) {
		wpa_printf_dbg(MSG_DEBUG, "WPA: Delay group state machine start "
				    ...
				    void *cb_ctx)
{
	...
	if (wpa_auth->conf.wpa_group_rekey) {
		da16x_eloop_register_timeout(wpa_auth->conf.wpa_group_rekey, 0,
...
int wpa_init_keys(struct wpa_authenticator *wpa_auth)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "WPA: Start group state machine to set initial "
		   "keys");
	wpa_group_sm_step(wpa_auth, group);
	group->GInit = FALSE;
	wpa_group_sm_step(wpa_auth, group);
...
void wpa_deinit(struct wpa_authenticator *wpa_auth)
{
	...
	da16x_eloop_cancel_timeout(wpa_rekey_gmk, wpa_auth, NULL);
	da16x_eloop_cancel_timeout(wpa_rekey_gtk, wpa_auth, NULL);
...
static void wpa_free_sta_sm(struct wpa_state_machine *sm)
{
	...
	if (WPA_GET_BE32(sm->ip_addr)) {
		u32 start;
		wpa_printf_dbg(MSG_DEBUG, "P2P: Free assigned IP "
	...
	os_free(sm->wpa_ie);
#ifdef	CONFIG_SUPP27_STA_SM
	wpa_group_put(sm->wpa_auth, sm->group);
#endif	/* CONFIG_SUPP27_STA_SM */
	os_free(sm);
}
...
void wpa_auth_sta_deinit(struct wpa_state_machine *sm)
{
	...
	if (sm->wpa_auth->conf.wpa_strict_rekey && sm->has_GTK) {
		wpa_auth_logger(sm->wpa_auth, sm->addr, LOGGER_DEBUG,
				...
				"is leaving");
		if (da16x_eloop_deplete_timeout(0, 500000, wpa_rekey_gtk,
					  sm->wpa_auth, NULL) == -1)
			da16x_eloop_register_timeout(0, 500000, wpa_rekey_gtk, sm->wpa_auth,
					       NULL);
	}

	da16x_eloop_cancel_timeout(wpa_send_eapol_timeout, sm->wpa_auth, sm);
	sm->pending_1_of_4_timeout = 0;
	da16x_eloop_cancel_timeout(wpa_sm_call_step, sm, NULL);
	da16x_eloop_cancel_timeout(wpa_rekey_ptk, sm->wpa_auth, sm);
#ifdef CONFIG_IEEE80211R_AP
	...
	if (sm->in_step_loop) {
		...
		wpa_printf_dbg(MSG_DEBUG, "WPA: Registering pending STA state "
			       ...
			       struct wpa_eapol_ie_parse *kde)
{
	...
	if (wpa_parse_wpa_ie_rsn(kde->rsn_ie, kde->rsn_ie_len, &ie) < 0 ||
		...
		wpa_printf_dbg(MSG_DEBUG, "FT: No PMKR1Name in "
	...
	if (!kde->mdie || !kde->ftie) {
		wpa_printf_dbg(MSG_DEBUG, "FT: No %s in FT 4-way handshake "
	...
	if (kde->mdie[1] < sizeof(struct rsn_mdie) ||
		...
		wpa_printf_dbg(MSG_DEBUG, "FT: MDIE mismatch");
		return -1;
	}
	...
	if (sm->assoc_resp_ftie &&
		...
		wpa_printf_dbg(MSG_DEBUG, "FT: FTIE mismatch");
		 ...
		 u8 *data, size_t data_len)
{
	...
	if (data_len < sizeof(*hdr) + keyhdrlen) {
		wpa_printf_dbg(MSG_DEBUG, "WPA: Ignore too short EAPOL-Key frame");
		return;
	}
...
#ifdef	CONFIG_SUPP27_AUTH
	wpa_printf_dbg(MSG_DEBUG, "WPA: Received EAPOL-Key from " MACSTR
	...
	if (sm->wpa == WPA_VERSION_WPA2) {
		if (key->type == EAPOL_KEY_TYPE_WPA) {
			 ...
			 */
			wpa_printf_dbg(MSG_DEBUG, "Workaround: Allow EAPOL-Key "
				   "with unexpected WPA type in RSN mode");
		} else if (key->type != EAPOL_KEY_TYPE_RSN) {
			wpa_printf_dbg(MSG_DEBUG, "Ignore EAPOL-Key with "
	...
	if (key_info & WPA_KEY_INFO_SMK_MESSAGE) {
		wpa_printf_dbg(MSG_DEBUG, "WPA: Ignore SMK message");
		return;
	}
...
continue_processing:
	...
	case PAIRWISE_2:
		...
		if (sm->group->reject_4way_hs_for_entropy) {
			 ...
			 * continue.
			 */
			wpa_printf_dbg(MSG_DEBUG, "WPA: Reject 4-way handshake to "
...
#endif /* CONFIG_FILS */
		sm->MICVerified = TRUE;
		da16x_eloop_cancel_timeout(wpa_send_eapol_timeout, wpa_auth, sm);
		sm->pending_1_of_4_timeout = 0;
	}
		      ...
		      int keyidx, int encr, int force_version)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "WPA: Send EAPOL(version=%d secure=%d mic=%d "
	...
	if (kde && !encr) {
		...
		if (aes_siv_encrypt(sm->PTK.kek, sm->PTK.kek_len, kde, kde_len,
			...
			wpa_printf_dbg(MSG_DEBUG, "WPA: AES-SIV encryption failed");
			return;
		}
			   ...
			   int keyidx, int encr)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "WPA: Use EAPOL-Key timeout of %u ms (retry "
		   "counter %u)", timeout_ms, ctr);

	da16x_eloop_register_timeout(timeout_ms / 1000, (timeout_ms % 1000) * 1000,
...
void wpa_remove_ptk(struct wpa_state_machine *sm)
{
	...
	if (wpa_auth_set_key(sm->wpa_auth, 0, WPA_ALG_NONE, sm->addr, 0, NULL, 0))
		wpa_printf_dbg(MSG_DEBUG, "RSN: PTK removal from the driver failed");
...
int wpa_auth_sm_event(struct wpa_state_machine *sm, enum wpa_event event)
{
	...
	case WPA_REAUTH_EAPOL:
		if (!sm->started) {
			 ...
			 */
			wpa_printf_dbg(MSG_DEBUG, "WPA state machine had not been "
	...
	case WPA_ASSOC_FT:
#ifdef CONFIG_IEEE80211R_AP
		wpa_printf_dbg(MSG_DEBUG, "FT: Retry PTK configuration "
			   "after association");
				  ...
				  struct wpa_group *group)
{
	 ...
	 */
	wpa_printf_dbg(MSG_DEBUG, "WPA: Re-initialize GMK/Counter on first "
		   "station");
	if (random_pool_ready() != 1) {
		wpa_printf(MSG_INFO, "WPA: Not enough entropy in random pool "
...
#endif /* CONFIG_IEEE80211R_AP */
	if (sm->pmksa) {
		wpa_printf_dbg(MSG_DEBUG, "WPA: PMK from PMKSA cache");
		...
		if (wpa_key_mgmt_sha384(sm->wpa_key_mgmt))
			...
			pmk_len = PMK_LEN;
		wpa_printf_dbg(MSG_DEBUG, "WPA: PMK from EAPOL state machine "
		...
		wpa_printf_dbg(MSG_DEBUG, "WPA: Could not get PMK, get_msk: %p",
...
SM_STATE(WPA_PTK, INITPSK)
{
	...
	if (wpa_auth_uses_sae(sm) && sm->pmksa) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: PMK from PMKSA cache");
		os_memcpy(sm->PMK, sm->pmksa->pmk, sm->pmksa->pmk_len);
		sm->pmk_len = sm->pmksa->pmk_len;
	}
#endif /* CONFIG_SAE */
...
int fils_decrypt_assoc(struct wpa_state_machine *sm, const u8 *fils_session,
		       const struct i3ed11_mgmt *mgmt, size_t frame_len,
		       u8 *pos, size_t left)
{
	u16 fc, stype;
	...
	session = wpa_fils_validate_fils_session(sm, ie, end - ie,
						 fils_session);
	if (!session) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: Session validation failed");
		return -1;
	}
	...
	if (wpa_fils_validate_key_confirm(sm, pos, left - AES_BLOCK_SIZE) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: Key Confirm validation failed");
		return -1;
	}
		       ...
		       const struct wpabuf *hlp)
{
	...
	if (!plain) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: Plain buffer prep failed");
		return -1;
	}
...
int fils_set_tk(struct wpa_state_machine *sm)
{
	...
	if (!sm || !sm->PTK_valid) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: No valid PTK available to set TK");
		return -1;
	}
	if (sm->tk_already_set) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: TK already set to the driver");
		return -1;
	}
	...
	wpa_printf_dbg(MSG_DEBUG, "FILS: Configure TK to the driver");
	if (wpa_auth_set_key(sm->wpa_auth, 0, alg, sm->addr, 0, sm->PTK.tk, klen)) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: Failed to set TK to the driver");
		return -1;
	}
				    ...
				    const u8 *fils_session, struct wpabuf *hlp)
{
	...
	*pos++ = WLAN_EID_EXT_FILS_SESSION; /* Element ID Extension */
	...
	wpa_printf_dbg(MSG_DEBUG, "%s: plain buf_len: %u", __func__,
...
SM_STATE(WPA_PTK, PTKINITDONE)
{
	...
	if (sm->Pair) {
		...
		if (sm->wpa_auth->conf.wpa_ptk_rekey) {
			da16x_eloop_cancel_timeout(wpa_rekey_ptk, sm->wpa_auth, sm);
			da16x_eloop_register_timeout(sm->wpa_auth->conf.
...
static void wpa_group_gtk_init(struct wpa_authenticator *wpa_auth,
			       struct wpa_group *group)
{
	wpa_printf_dbg(MSG_DEBUG, "WPA: group state machine entering state "
...
int wpa_wnmsleep_gtk_subelem(struct wpa_state_machine *sm, u8 *pos)
{
	...
	*pos++ = gsm->GTK_len;
	...
	wpa_printf_dbg(MSG_DEBUG, "WNM: GTK Key ID %u in WNM-Sleep Mode exit",
		   gsm->GN);
...
int wpa_wnmsleep_igtk_subelem(struct wpa_state_machine *sm, u8 *pos)
{
	...
	*pos++ = 2 + 6 + len;
	...
	wpa_printf_dbg(MSG_DEBUG, "WNM: IGTK Key ID %u in WNM-Sleep Mode exit",
		   gsm->GN_igtk);
			      ...
			      struct wpa_group *group)
{
	...
	if (group->GKeyDoneStations) {
		wpa_printf_dbg(MSG_DEBUG, "wpa_group_setkeys: Unexpected "
			   "GKeyDoneStations=%d when starting new GTK rekey",
			   group->GKeyDoneStations);
		group->GKeyDoneStations = 0;
	}
	wpa_auth_for_each_sta(wpa_auth, wpa_group_update_sta, group);
	wpa_printf_dbg(MSG_DEBUG, "wpa_group_setkeys: GKeyDoneStations=%d",
		   group->GKeyDoneStations);
}
...
{
	if (sm->group == ctx) {
		wpa_printf_dbg(MSG_DEBUG, "WPA: Mark STA " MACSTR
...
static void wpa_group_fatal_failure(struct wpa_authenticator *wpa_auth,
				    struct wpa_group *group)
{
	wpa_printf_dbg(MSG_DEBUG, "WPA: group state machine entering state FATAL_FAILURE");
	group->changed = TRUE;
...
static int wpa_sm_step(struct wpa_state_machine *sm)
{
	...
	if (sm->pending_deinit) {
		wpa_printf_dbg(MSG_DEBUG, "WPA: Completing pending STA state "
			   "machine deinit for " MACSTR, MAC2STR(sm->addr));
		wpa_free_sta_sm(sm);
		return 1;
	}
...
void wpa_auth_sm_notify(struct wpa_state_machine *sm)
{
	if (sm == NULL)
		return;
	da16x_eloop_register_timeout(0, 0, wpa_sm_call_step, sm, NULL);
}
			   ...
			   const u8 *sta_addr)
{
	...
	if (pmksa) {
		wpa_printf_dbg(MSG_DEBUG, "WPA: Remove PMKSA cache entry for "
			   ...
			   struct wpa_group *group)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "WPA: Remove group state machine for VLAN-ID %d",
		   group->vlan_id);
...
wpa_auth_add_group(struct wpa_authenticator *wpa_auth, int vlan_id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "WPA: Add group state machine for VLAN-ID %d",
		   vlan_id);
...
int wpa_auth_sta_set_vlan(struct wpa_state_machine *sm, int vlan_id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "WPA: Moving STA " MACSTR " to use group state "
				  ...
				  struct wpa_state_machine *sm, int ack)
{
	if (wpa_auth == NULL || sm == NULL)
		return;
	wpa_printf_dbg(MSG_DEBUG, "WPA: EAPOL-Key TX status for STA " MACSTR
	...
	if (sm->pending_1_of_4_timeout && ack) {
		...
		wpa_printf_dbg(MSG_DEBUG, "WPA: Increase initial EAPOL-Key 1/4 "
			   "timeout by %u ms because of acknowledged frame",
			   timeout_ms);
		da16x_eloop_cancel_timeout(wpa_send_eapol_timeout, wpa_auth, sm);
		da16x_eloop_register_timeout(timeout_ms / 1000,
...
int wpa_auth_rekey_gtk(struct wpa_authenticator *wpa_auth)
{
	if (!wpa_auth)
		return -1;
	da16x_eloop_cancel_timeout(wpa_rekey_gtk, wpa_auth, NULL);
	return da16x_eloop_register_timeout(0, 0, wpa_rekey_gtk, wpa_auth, NULL);
}
