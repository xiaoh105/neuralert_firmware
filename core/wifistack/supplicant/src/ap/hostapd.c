...
static void hostapd_reload_bss(struct hostapd_data *hapd)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Reconfigured interface %s", hapd->conf->iface);
}
					      ...
					      const char *ifname)
{
	...
	for (i = 0; i < NUM_WEP_KEYS; i++) {
		if (hostapd_drv_set_key(ifname, hapd, WPA_ALG_NONE, NULL, i,
					0, NULL, 0, NULL, 0)) {
			wpa_printf_dbg(MSG_DEBUG, "Failed to clear default "
	...
	if (hapd->conf->ieee80211w) {
		for (i = NUM_WEP_KEYS; i < NUM_WEP_KEYS + 2; i++) {
			if (hostapd_drv_set_key(ifname, hapd, WPA_ALG_NONE,
						NULL, i, 0, NULL,
						0, NULL, 0)) {
				wpa_printf_dbg(MSG_DEBUG, "Failed to clear "
...
static void hostapd_free_hapd_data(struct hostapd_data *hapd)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "%s(%s)", __func__, hapd->conf->iface);
#ifdef	CONFIG_IEEE80211F
...
{
	wpa_printf_dbg(MSG_DEBUG, "%s(hapd=%p (%s))", __func__, hapd,
		   hapd->conf->iface);
...
#endif	/* CONFIG_SUPP27_STA_TRACK */


static void hostapd_cleanup_iface_partial(struct hostapd_iface *iface)
{
	wpa_printf_dbg(MSG_DEBUG, "%s(%p)", __func__, iface);
...
static void hostapd_cleanup_iface(struct hostapd_iface *iface)
{
	wpa_printf_dbg(MSG_DEBUG, "%s(%p)", __func__, iface);
#ifdef CONFIG_AP_DFS
	da16x_eloop_cancel_timeout(channel_list_update_timeout, iface, NULL);
	da16x_eloop_cancel_timeout(hostapd_interface_setup_failure_handler, iface,
			     NULL);
#endif /* CONFIG_AP_DFS */
	...
	os_free(iface->bss);
	wpa_printf_dbg(MSG_DEBUG, "%s: free iface=%p", __func__, iface);
	os_free(iface);
}
...
static int hostapd_flush_old_stations(struct hostapd_data *hapd, u16 reason)
{
	...
	if (!hapd->iface->driver_ap_teardown) {
		...
		if (hostapd_flush(hapd)) {
			wpa_dbg(hapd->msg_ctx, MSG_WARNING,
	...
	if (hapd->conf && hapd->conf->broadcast_deauth) {
		...
		if (hapd->p2p == NULL) {
			...
			if (hapd->num_sta &&
				...
				da16x_ap_prt("[%s] Sending DEAUTH FAIL!\n",
					    __func__);
			}
		}
#endif	/* CONFIG_P2P */
...
skip_mask_ext:
	wpa_printf_dbg(MSG_DEBUG, "BSS count %lu, BSSID mask " MACSTR " (%d bits)",
				    ...
				    struct radius_das_attrs *attr)
{
	if (attr->nas_identifier &&
		...
		wpa_printf_dbg(MSG_DEBUG, "RADIUS DAS: NAS-Identifier mismatch");
		return 1;
	}
	...
	if (attr->nas_ip_addr &&
	    (hapd->conf->own_ip_addr.af != AF_INET ||
	     ...
	     0)) {
		wpa_printf_dbg(MSG_DEBUG, "RADIUS DAS: NAS-IP-Address mismatch");
		return 1;
	}
	...
	if (attr->nas_ipv6_addr &&
	    (hapd->conf->own_ip_addr.af != AF_INET6 ||
	     ...
	     != 0)) {
		wpa_printf_dbg(MSG_DEBUG, "RADIUS DAS: NAS-IPv6-Address mismatch");
		return 1;
	}
#endif /* CONFIG_IPV6 */
					      ...
					      int *multi)
{
	...
	*multi = 0;
	...
	if (attr->sta_addr) {
		...
		wpa_printf_dbg(MSG_DEBUG, "RADIUS DAS: Calling-Station-Id match");
	}
	...
	if (attr->acct_session_id) {
		...
		wpa_printf_dbg(MSG_DEBUG, "RADIUS DAS: Acct-Session-Id match");
	}
...
hostapd_das_disconnect(void *ctx, struct radius_das_attrs *attr)
{
	...
	if (sta == NULL) {
		...
		wpa_printf_dbg(MSG_DEBUG, "RADIUS DAS: No matching session found");
		return RADIUS_DAS_SESSION_NOT_FOUND;
	}

	wpa_printf_dbg(MSG_DEBUG, "RADIUS DAS: Found a matching session " MACSTR
...
static int hostapd_setup_bss(struct hostapd_data *hapd, int first)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "[%s] (hapd=%p (%s), first=%d)",
	...
	if (!first || first == -1) {
		...
		memset(conf->ssid.ssid, NULL, SSID_MAX_LEN);
		...
		memset(conf_temp->ssid.ssid, NULL, SSID_MAX_LEN);
...
void hostapd_tx_queue_params(struct hostapd_iface *iface)
{
	...
	for (i = 0; i < NUM_TX_QUEUES; i++) {
		...
		if (hostapd_set_tx_queue_params(hapd, i, p->aifs, p->cwmin,
						p->cwmax, p->burst)) {
			wpa_printf_dbg(MSG_DEBUG, "Failed to set TX queue "
...
static void hostapd_set_acl(struct hostapd_data *hapd)
{
	...
	if (conf->bss[0]->macaddr_acl == DENY_UNLESS_ACCEPTED) {
		...
		err = hostapd_set_acl_list(hapd, conf->bss[0]->accept_mac,
					   ...
					   accept_acl);
		if (err) {
			wpa_printf_dbg(MSG_DEBUG, "Failed to set accept acl");
			return;
		}
		...
		err = hostapd_set_acl_list(hapd, conf->bss[0]->deny_mac,
					   ...
					   accept_acl);
		if (err) {
			wpa_printf_dbg(MSG_DEBUG, "Failed to set deny acl");
			return;
		}
	}
}
...
static void channel_list_update_timeout(void *eloop_ctx, void *timeout_ctx)
{
	 ...
	 * to proceed.
	 */
	wpa_printf_dbg(MSG_DEBUG, "Channel list update timeout - try to continue anyway");
...
void hostapd_channel_list_updated(struct hostapd_iface *iface, int initiator)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Channel list updated - continue setup");
#if 0	/* by Bright */
	da16x_eloop_cancel_timeout(channel_list_update_timeout, iface, NULL);
#else
da16x_ap_prt("[%s] What do I do for cancel timer\n", __func__);
#endif	/* 0 */
	setup_interface2(iface);
}
#endif /* CONFIG_AP_DFS */
...
static int setup_interface(struct hostapd_iface *iface)
{
	...
	if (!iface->phy[0]) {
		...
		if (phy) {
			wpa_printf_dbg(MSG_DEBUG, "[%s] phy: %s", __func__, phy);
...
PRINTF("=== [%s] AAA\n", __func__);
	...
	if (hapd->iconf->country[0] && hapd->iconf->country[1]) {
		...
		wpa_printf_dbg(MSG_DEBUG, "Previous country code %s, new country code %s",
			   previous_country, country);

		if (os_strncmp(previous_country, country, 2) != 0) {
			wpa_printf_dbg(MSG_DEBUG, "Continue interface setup after channel list update");
...
#else
	da16x_ap_prt("[%s] What do I do for set country ???\n", __func__);
#endif	/* 0 */
...
static int setup_interface2(struct hostapd_iface *iface)
{
	...
	if (hostapd_get_hw_features(iface)) {
		...
		if (ret == 1) {
			wpa_printf_dbg(MSG_DEBUG, "Interface initialization will be completed in a callback (ACS)");
			return 0;
		}
		...
		if (ret == 1) {
			wpa_printf_dbg(MSG_DEBUG, "Interface initialization will "
				   "be completed in a callback");
			return 0;
		}
#ifdef	CONFIG_IEEE80211H
		if (iface->conf->ieee80211h)
			wpa_printf_dbg(MSG_DEBUG, "DFS support is enabled");
...
fail:
	...
	if (iface->interfaces && iface->interfaces->terminate_on_error)
		...
		eloop_terminate();
#else
da16x_ap_prt("[%s] What do I do for timer terminate\n", __func__);
#endif	/* 0 */
...
#else
	width = hostapd_get_nr_chan_width(hapd, ht, 0);
#endif /* CONFIG_IEEE80211AC */
	if (ht) {
						 ...
						 int err)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "[%s] Completing interface initialization", __func__);
	...
	if (iface->conf->channel) {
		...
		wpa_printf_dbg(MSG_DEBUG, "Mode: %s  Channel: %d  "
			   "Frequency: %d MHz",
	...
	if ((iface->drv_flags & WPA_DRIVER_FLAGS_DFS_OFFLOAD) &&
		 ...
		 * for CAC to complete.
		 */
		wpa_printf_dbg(MSG_DEBUG, "%s: Wait for CAC to complete", __func__);
...
dfs_offload:
	...
	wpa_printf_dbg(MSG_DEBUG, "%s: Setup of interface done.",
		   iface->bss[0]->conf->iface);
...
static void hostapd_bss_deinit(struct hostapd_data *hapd)
{
	if (!hapd)
		return;
	wpa_printf_dbg(MSG_DEBUG, "%s: deinit bss %s", __func__,
		   hapd->conf->iface);
	hostapd_bss_deinit_no_free(hapd);
	wpa_msg(hapd->msg_ctx, MSG_INFO, AP_EVENT_DISABLED);
...
void hostapd_interface_deinit(struct hostapd_iface *iface)
{
	...
	hostapd_stop_setup_timers(iface);
#ifdef	CONFIG_AP_HT
	da16x_eloop_cancel_timeout(ap_ht2040_timeout, iface, NULL);
#endif /* CONFIG_AP_HT */
#endif /* NEED_AP_MLME */
#endif /* CONFIG_IEEE80211N */
}
...
void hostapd_interface_free(struct hostapd_iface *iface)
{
	...
	for (j = 0; j < iface->num_bss; j++) {
		if (!iface->bss)
			break;
		wpa_printf_dbg(MSG_DEBUG, "%s: free hapd %p",
...
#ifdef	CONFIG_SUPP27_STA_SEEN
	dl_list_init(&hapd_iface->sta_seen);
#endif	/* CONFIG_SUPP27_STA_SEEN */
...
fail:
	...
	if (hapd_iface) {
		...
		wpa_printf_dbg(MSG_DEBUG, "%s: free iface %p",
			   __func__, hapd_iface);
		os_free(hapd_iface);
	}
	return NULL;
...
void hostapd_interface_deinit_free(struct hostapd_iface *iface)
{
	...
	if (iface == NULL)
		return;
	wpa_printf_dbg(MSG_DEBUG, "%s: num_bss=%u conf->num_bss=%u",
		   __func__, (unsigned int) iface->num_bss,
		   (unsigned int) iface->conf->num_bss);
	driver = iface->bss[0]->driver;
	drv_priv = iface->bss[0]->drv_priv;
	hostapd_interface_deinit(iface);
	wpa_printf_dbg(MSG_DEBUG, "%s: driver=%p drv_priv=%p -> hapd_deinit",
				  ...
				  struct hostapd_iface *hapd_iface)
{
	...
	if (driver && driver->hapd_deinit && drv_priv) {
		...
		for (j = 0; j < hapd_iface->num_bss; j++) {
			wpa_printf_dbg(MSG_DEBUG, "%s:bss[%d]->drv_priv=%p",
				   __func__, (int) j,
...
int hostapd_enable_iface(struct hostapd_iface *hapd_iface)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Enable interface %s",
		   hapd_iface->conf->bss[0]->iface);
...
int hostapd_reload_iface(struct hostapd_iface *hapd_iface)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Reload interface %s",
		   hapd_iface->conf->bss[0]->iface);
	for (j = 0; j < hapd_iface->num_bss; j++)
		hostapd_set_security_params(hapd_iface->conf->bss[j], 1);
	if (hostapd_config_check(hapd_iface->conf, 1) < 0) {
...
int hostapd_disable_iface(struct hostapd_iface *hapd_iface)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Interface %s disabled",
		...
		*pos++ = '\0';
		...
		if (new_iface) {
			...
			if (start_ctrl_iface_bss(hapd) < 0 ||
				...
				hapd_iface->num_bss--;
				wpa_printf_dbg(MSG_DEBUG, "%s: free hapd %p %s",
			   ...
			   int reassoc)
{
	...
	if (hapd->iface->drv_flags & WPA_DRIVER_FLAGS_WIRED) {
		if (da16x_eloop_cancel_timeout(ap_handle_timer, hapd, sta) > 0) {
			wpa_printf_dbg(MSG_DEBUG,
...
#if 0	/* by Bright : Code sync with Supp 2.4 */
		da16x_eloop_register_timeout(hapd->conf->ap_max_inactivity, 0,
				       ap_handle_timer, hapd, sta);
#else
		da16x_eloop_register_timeout(INACTIVE_CHECK_INTERVAL, 0, ap_handle_timer, hapd, sta);
#endif	/* 0 */
	}
}
...
static void free_beacon_data(struct hostapd_beacon_data *beacon)
{
	os_free(beacon->head);
	beacon->head = NULL;
	os_free(beacon->tail);
...
static int hostapd_build_beacon_data(struct hostapd_data *hapd,
				     struct hostapd_beacon_data *beacon)
{
				      ...
				      struct hostapd_freq_params *old_params)
{
	...
	ieee80211_freq_to_chan(params->cent_freq1,
#ifdef CONFIG_IEEE80211AC
			       &conf->vht_oper_centr_freq_seg0_idx
#else
			       NULL
	...
	ieee80211_freq_to_chan(params->cent_freq2,
#ifdef CONFIG_IEEE80211AC
			       &conf->vht_oper_centr_freq_seg1_idx
#else
			       NULL
...
#ifdef CONFIG_IEEE80211AC
	case 80:
		if (settings->freq_params.cent_freq2)
				...
				const struct hostapd_freq_params *freq_params)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Restarting all CSA-related BSSes");

	if (freq_params->cent_freq1)
		vht_seg0_idx = 36 + (freq_params->cent_freq1 - 5180) / 5;
	if (freq_params->cent_freq2)
		vht_seg1_idx = 36 + (freq_params->cent_freq2 - 5180) / 5;
