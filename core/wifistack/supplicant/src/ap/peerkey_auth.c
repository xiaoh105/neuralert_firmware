		...
		struct wpa_state_machine *sm, struct wpa_eapol_key *key)
{
	...
	if (wpa_auth_for_each_sta(wpa_auth, wpa_stsl_select_sta, &search) ==
	    0 || search.sm == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: SMK handshake with " MACSTR
		...
		struct wpa_state_machine *sm, struct wpa_eapol_key *key)
{
	...
	if (random_get_bytes(smk, PMK_LEN)) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: Failed to generate SMK");
		return;
	}
		   ...
		   struct wpa_state_machine *sm, struct wpa_eapol_key *key)
{
	...
	if (wpa_auth_for_each_sta(wpa_auth, wpa_stsl_select_sta, &search) ==
	    0 || search.sm == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: Peer STA " MACSTR " not "
		    ...
		    struct wpa_stsl_negotiation *neg)
{
	...
	while (pos) {
		if (pos == neg) {
			...
			da16x_eloop_cancel_timeout(wpa_stsl_step, wpa_auth, pos);
			os_free(pos);
			return 0;
		}
		prev = pos;
