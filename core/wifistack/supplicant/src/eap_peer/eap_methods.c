...
int eap_peer_method_load(const char *so)
{
	...
	if (handle == NULL) {
		da16x_eap_prt("     [%s] EAP: Failed to open dynamic EAP method "
			   "'%s': %s\n", __func__, so, dlerror());
		return -1;
	}
	...
	if (dyn_init == NULL) {
		dlclose(handle);
		da16x_eap_prt("     [%s] EAP: Invalid EAP method '%s' - no "
	...
	if (ret) {
		dlclose(handle);
		da16x_eap_prt("     [%s] EAP: Failed to add EAP method '%s' - "
			   "ret %d\n", __func__, so, ret);
		return ret;
	}
	...
	da16x_eap_prt("     [%s] EAP: Loaded dynamic EAP method: '%s'\n", __func__, so);
