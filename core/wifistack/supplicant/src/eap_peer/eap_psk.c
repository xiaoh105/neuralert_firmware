...
static void * eap_psk_init(struct eap_sm *sm)
{
	...
	if (!password || password_len != 16) {
		wpa_printf_dbg(MSG_INFO, "EAP-PSK: 16-octet pre-shared key not "
			   "configured");
		return NULL;
	}
	...
	if (data->id_p == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-PSK: could not get own identity");
		os_free(data);
		return NULL;
	}
					 ...
					 const struct wpabuf *reqData)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: in INIT state");
	...
	if (cpos == NULL || len < sizeof(*hdr1)) {
		wpa_printf_dbg(MSG_INFO, "EAP-PSK: Invalid first message "
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: Flags=0x%x", hdr1->flags);
	if (EAP_PSK_FLAGS_GET_T(hdr1->flags) != 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-PSK: Unexpected T=%d (expected 0)",
	...
	if (data->id_s == NULL) {
		wpa_printf_dbg(MSG_ERROR, "EAP-PSK: Failed to allocate memory for "
	...
	if (random_get_bytes(data->rand_p, EAP_PSK_RAND_LEN)) {
		wpa_printf_dbg(MSG_ERROR, "EAP-PSK: Failed to get random data");
		ret->ignore = TRUE;
		return NULL;
	}
					 ...
					 const struct wpabuf *reqData)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: Flags=0x%x", hdr3->flags);
	if (EAP_PSK_FLAGS_GET_T(hdr3->flags) != 2) {
		wpa_printf_dbg(MSG_INFO, "EAP-PSK: Unexpected T=%d (expected 2)",
	...
	if (left < 4 + 16 + 1) {
		wpa_printf_dbg(MSG_INFO, "EAP-PSK: Too short PCHANNEL data in "
	...
	if (os_memcmp_const(mac, hdr3->mac_s, EAP_PSK_MAC_LEN) != 0) {
		wpa_printf_dbg(MSG_WARNING, "EAP-PSK: Invalid MAC_S in third "
			   "message");
		ret->methodState = METHOD_DONE;
		ret->decision = DECISION_FAIL;
		return NULL;
	}
	wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: MAC_S verified successfully");
	...
	if (aes_128_eax_decrypt(data->tek, nonce, sizeof(nonce),
				...
				left, tag)) {
		wpa_printf_dbg(MSG_WARNING, "EAP-PSK: PCHANNEL decryption failed");
		os_free(decrypted);
		return NULL;
	}
	...
	case EAP_PSK_R_FLAG_CONT:
		wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: R flag - CONT - unsupported");
		failed = 1;
		break;
	case EAP_PSK_R_FLAG_DONE_SUCCESS:
		wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: R flag - DONE_SUCCESS");
		break;
	case EAP_PSK_R_FLAG_DONE_FAILURE:
		wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: R flag - DONE_FAILURE");
		wpa_printf_dbg(MSG_INFO, "EAP-PSK: Authentication server rejected "
			   "authentication");
		failed = 1;
		break;
	}
	...
	if (decrypted[0] & EAP_PSK_E_FLAG) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: Unsupported E (Ext) flag");
		failed = 1;
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: Completed %ssuccessfully",
		   failed ? "un" : "");
	data->state = PSK_DONE;
	ret->methodState = METHOD_DONE;
				       ...
				       const struct wpabuf *reqData)
{
	...
	case PSK_MAC_SENT:
		...
		break;
	case PSK_DONE:
		wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: in DONE state - ignore "
			   "unexpected message");
		ret->ignore = TRUE;
		return NULL;
	}
