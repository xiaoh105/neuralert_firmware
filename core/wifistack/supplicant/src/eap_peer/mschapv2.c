			     ...
			     u8 *master_key)
{
	...
	da16x_dump("MSCHAPV2: Identity", identity, identity_len);
	username_len = identity_len;
	username = mschapv2_remove_domain(identity, &username_len);
	da16x_dump("MSCHAPV2: Username", username, username_len);

	da16x_dump("MSCHAPV2: auth_challenge",
		  auth_challenge, MSCHAPV2_CHAL_LEN);
	da16x_dump("MSCHAPV2: peer_challenge",
		  peer_challenge, MSCHAPV2_CHAL_LEN);
	da16x_dump("MSCHAPV2: username", username, username_len);
	/* Authenticator response is not really needed yet, but calculate it
	 * here so that challenges need not be saved. */
	if (pwhash) {
		da16x_dump("MSCHAPV2: password hash", password, password_len);
		...
		da16x_dump("MSCHAPV2: password", password, password_len);
	...
	da16x_dump("MSCHAPV2: NT Response",
		  nt_response, MSCHAPV2_NT_RESPONSE_LEN);
	da16x_dump("MSCHAPV2: Auth Response",
	...
	if (get_master_key(password_hash_hash, nt_response, master_key))
		return -1;
	da16x_dump("MSCHAPV2: Master Key", master_key, MSCHAPV2_MASTER_KEY_LEN);
