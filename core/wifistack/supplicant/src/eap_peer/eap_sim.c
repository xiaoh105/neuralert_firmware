...
{
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: %s -> %s",
...
static void * eap_sim_init(struct eap_sm *sm)
{
	...
	if (random_get_bytes(data->nonce_mt, EAP_SIM_NONCE_MT_LEN)) {
		wpa_printf_dbg(MSG_WARNING, "EAP-SIM: Failed to get random data "
			   "for NONCE_MT");
		os_free(data);
		return NULL;
	}
	...
	if (config && config->phase1) {
		...
		if (pos) {
			...
			if (data->min_num_chal < 2 || data->min_num_chal > 3) {
				wpa_printf_dbg(MSG_WARNING, "EAP-SIM: Invalid "
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: Set minimum number of "
				   "challenges to %lu",
				   (unsigned long) data->min_num_chal);
		}
...
static int eap_sim_ext_sim_req(struct eap_sm *sm, struct eap_sim_data *data)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: Use external SIM processing");
	pos = req;
	end = pos + sizeof(req);
				  ...
				  struct eap_peer_config *conf)
{
	...
	if (os_strncmp(resp, "GSM-AUTH:", 9) != 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: Unrecognized external SIM processing response");
		os_free(resp);
		return -1;
	}
...
invalid:
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: Invalid external SIM processing GSM-AUTH response");
	os_free(resp);
	return -1;
}
...
static int eap_sim_gsm_auth(struct eap_sm *sm, struct eap_sim_data *data)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: GSM authentication algorithm");
	...
	if (conf->pcsc) {
		if (scard_gsm_auth(sm->scard_ctx, data->rand[0],
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: GSM SIM "
	...
	if (conf->password) {
		...
		const char *pos;
		size_t i;
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: Use internal GSM-Milenage "
			   "implementation for authentication");
		if (conf->password_len < 65) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: invalid GSM-Milenage "
				   "password");
			return -1;
		}
		...
		for (i = 0; i < data->num_chal; i++) {
			if (gsm_milenage(opc, k, data->rand[i],
				...
				wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: "
	 ...
	 * the order of pre-configured values. */
	{
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: Use hardcoded Kc and SRES "
			   "values for testing");
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: No GSM authentication algorithm "
		   "enabled");
	return -1;
				     ...
				     struct eap_sim_data *data, int id)
{
	if ((id & CLEAR_PSEUDONYM) && data->pseudonym) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: forgetting old pseudonym");
		os_free(data->pseudonym);
		data->pseudonym = NULL;
	...
	if ((id & CLEAR_REAUTH_ID) && data->reauth_id) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: forgetting old reauth_id");
		os_free(data->reauth_id);
		data->reauth_id = NULL;
		data->reauth_id_len = 0;
	}
	if ((id & CLEAR_EAP_ID) && data->last_eap_identity) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: forgetting old eap_id");
		os_free(data->last_eap_identity);
			     ...
			     struct eap_sim_attrs *attr)
{
	if (attr->next_pseudonym) {
		...
		if (data->pseudonym == NULL) {
			wpa_printf_dbg(MSG_INFO, "EAP-SIM: (encr) No memory for "
				   "next pseudonym");
			data->pseudonym_len = 0;
			return -1;
		}
					    ...
					    int err)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: Send Client-Error (error code %d)",
		   err);
					      ...
					      enum eap_sim_id_req id_req)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Generating EAP-SIM Start (id=%d)", id);
	...
	if (!data->reauth) {
		...
		eap_sim_msg_add(msg, EAP_SIM_AT_NONCE_MT, 0,
				data->nonce_mt, EAP_SIM_NONCE_MT_LEN);
		wpa_printf_dbg(MSG_DEBUG, "   AT_SELECTED_VERSION %d",
						  ...
						  u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Generating EAP-SIM Challenge (id=%d)", id);
	msg = eap_sim_msg_init(EAP_CODE_RESPONSE, id, EAP_TYPE_SIM,
					       ...
					       const u8 *nonce_s)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Generating EAP-SIM Reauthentication (id=%d)",
		   id);
	msg = eap_sim_msg_init(EAP_CODE_RESPONSE, id, EAP_TYPE_SIM,
	...
	if (eap_sim_msg_add_encr_end(msg, data->k_encr, EAP_SIM_AT_PADDING)) {
		wpa_printf_dbg(MSG_WARNING, "EAP-SIM: Failed to encrypt "
			   "AT_ENCR_DATA");
						     ...
						     u8 id, u16 notification)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "Generating EAP-SIM Notification (id=%d)", id);
	msg = eap_sim_msg_init(EAP_CODE_RESPONSE, id,
					     ...
					     struct eap_sim_attrs *attr)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: subtype Start");
	if (attr->version_list == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-SIM: No AT_VERSION_LIST in "
			   "SIM/Start");
	...
	if (data->ver_list == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: Failed to allocate "
	...
	if (selected_version < 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-SIM: Could not find a supported "
			   "version");
		return eap_sim_client_error(data, id,
					    EAP_SIM_UNSUPPORTED_VERSION);
	}
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: Selected Version %d",
		   selected_version);
	...
	if (id_error) {
		wpa_printf_dbg(MSG_INFO, "EAP-SIM: Too many ID requests "
						 ...
						 struct eap_sim_attrs *attr)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: subtype Challenge");
	data->reauth = 0;
	if (!attr->mac || !attr->rand) {
		wpa_printf_dbg(MSG_WARNING, "EAP-SIM: Challenge message "
			   "did not include%s%s",
			   !attr->mac ? " AT_MAC" : "",
			   !attr->rand ? " AT_RAND" : "");
		return eap_sim_client_error(data, id,
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: %lu challenges",
		   (unsigned long) attr->num_chal);
	if (attr->num_chal < data->min_num_chal) {
		wpa_printf_dbg(MSG_INFO, "EAP-SIM: Insufficient number of "
			   "challenges (%lu)", (unsigned long) attr->num_chal);
		return eap_sim_client_error(data, id,
					    EAP_SIM_INSUFFICIENT_NUM_OF_CHAL);
	}
	if (attr->num_chal > 3) {
		wpa_printf_dbg(MSG_INFO, "EAP-SIM: Too many challenges "
	    ...
	    (attr->num_chal > 2 &&
	     (os_memcmp(attr->rand, attr->rand + 2 * GSM_RAND_LEN,
	      ...
	      os_memcmp(attr->rand + GSM_RAND_LEN,
			...
			GSM_RAND_LEN) == 0))) {
		wpa_printf_dbg(MSG_INFO, "EAP-SIM: Same RAND used multiple times");
		return eap_sim_client_error(data, id,
	...
	if (res > 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: Wait for external SIM processing");
		return NULL;
	}
	if (res) {
		wpa_printf_dbg(MSG_WARNING, "EAP-SIM: GSM authentication failed");
		return eap_sim_client_error(data, id,
					    EAP_SIM_UNABLE_TO_PROCESS_PACKET);
	}
					       ...
					       struct eap_sim_attrs *attr)
{
	...
	if (attr->encr_data == NULL || attr->iv == NULL) {
		wpa_printf_dbg(MSG_WARNING, "EAP-SIM: Notification message after "
	...
	decrypted = eap_sim_parse_encr(data->k_encr, attr->encr_data,
				       ...
				       0);
	if (decrypted == NULL) {
		wpa_printf_dbg(MSG_WARNING, "EAP-SIM: Failed to parse encrypted "
			   "data from notification message");
		return -1;
	}

	if (eattr.counter < 0 || (size_t) eattr.counter != data->counter) {
		wpa_printf_dbg(MSG_WARNING, "EAP-SIM: Counter in notification "
...
{
	if (attr->mac == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-SIM: no AT_MAC in after_auth "
	...
	{
		wpa_printf_dbg(MSG_WARNING, "EAP-SIM: Notification message "
			   "used invalid AT_MAC");
		return -1;
	}
	...
	if (data->reauth &&
		...
		wpa_printf_dbg(MSG_WARNING, "EAP-SIM: Invalid notification "
...
{
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: subtype Notification");
	if (data->num_notification > 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-SIM: too many notification "
	...
	if (attr->notification == -1) {
		wpa_printf_dbg(MSG_INFO, "EAP-SIM: no AT_NOTIFICATION in "
			   "Notification message");
		return eap_sim_client_error(data, id,
					    EAP_SIM_UNABLE_TO_PROCESS_PACKET);
	}
	...
	const struct wpabuf *reqData, struct eap_sim_attrs *attr)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: subtype Reauthentication");

	if (data->reauth_id == NULL) {
		wpa_printf_dbg(MSG_WARNING, "EAP-SIM: Server is trying "
	...
	if (eap_sim_verify_mac(data->k_aut, reqData, attr->mac, (u8 *) "", 0))
	{
		wpa_printf_dbg(MSG_WARNING, "EAP-SIM: Reauthentication "
	...
	if (eattr.nonce_s == NULL || eattr.counter < 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-SIM: (encr) No%s%s in reauth packet",
	...
	if (eattr.counter < 0 || (size_t) eattr.counter <= data->counter) {
		struct wpabuf *res;
		wpa_printf_dbg(MSG_INFO, "EAP-SIM: (encr) Invalid counter "
	...
	if (data->counter > EAP_SIM_MAX_FAST_REAUTHS) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: Maximum number of "
				       ...
				       const struct wpabuf *reqData)
{
	...
	if (eap_get_config_identity(sm, &len) == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-SIM: Identity not configured");
	...
	subtype = *pos++;
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: Subtype=%d", subtype);
	pos += 2; /* Reserved */
	...
	case EAP_SIM_SUBTYPE_CLIENT_ERROR:
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: subtype Client-Error");
		res = eap_sim_client_error(data, id,
					   EAP_SIM_UNABLE_TO_PROCESS_PACKET);
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: Unknown subtype=%d", subtype);
		res = eap_sim_client_error(data, id,
					   EAP_SIM_UNABLE_TO_PROCESS_PACKET);
		break;
	}
