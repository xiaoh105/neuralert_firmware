...
static void eap_vendor_ready(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-VENDOR-TEST: Ready to re-process pending "
		   "request");
					       ...
					       const struct wpabuf *reqData)
{
	...
	if (data->state == INIT && *pos != 1) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-VENDOR-TEST: Unexpected message "
			   "%d in INIT state", *pos);
		ret->ignore = TRUE;
		return NULL;
	}
	...
	if (data->state == CONFIRM) {
		...
		if (data->first_try) {
			data->first_try = 0;
			wpa_printf_dbg(MSG_DEBUG, "EAP-VENDOR-TEST: Testing "
				   "pending request");
			ret->ignore = TRUE;
			da16x_eloop_register_timeout(1, 0, eap_vendor_ready, sm,
					       NULL);
			return NULL;
		}
#endif /* TEST_PENDING_REQUEST */
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-VENDOR-TEST: Generating Response");
