...
static void * eap_eke_init(struct eap_sm *sm)
{
	...
	if (!password) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: No password configured");
		return NULL;
	}
	...
	if (phase1) {
		...
		if (pos) {
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Forced dhgroup %u",
				   data->dhgroup);
		}
		...
		if (pos) {
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Forced encr %u",
				   data->encr);
		}
		...
		if (pos) {
			data->prf = atoi(pos + 4);
			wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Forced prf %u",
				   data->prf);
		}
		...
		if (pos) {
			data->mac = atoi(pos + 4);
			wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Forced mac %u",
				   data->mac);
		}
	}
					  ...
					  u8 id, u32 failure_code)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Sending EAP-EKE-Failure/Response - code=0x%x",
		   failure_code);
					  ...
					  size_t payload_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Received EAP-EKE-ID/Request");

	if (payload_len < 2 + 4) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Too short ID/Request Data");
		return eap_eke_build_fail(data, ret, id,
					  EAP_EKE_FAIL_PROTO_ERROR);
	}
	...
	if (pos + num_prop * 4 > end) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Too short ID/Request Data (num_prop=%u)",
			   num_prop);
		return eap_eke_build_fail(data, ret, id,
					  EAP_EKE_FAIL_PROTO_ERROR);
	}
	...
	for (i = 0; i < num_prop; i++) {
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Proposal #%u: dh=%u encr=%u prf=%u mac=%u",
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Selected proposal");
		break;
	}

	if (prop == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: No acceptable proposal found");
		return eap_eke_build_fail(data, ret, id,
	...
	if (pos == end) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Too short ID/Request Data to include IDType/Identity");
		return eap_eke_build_fail(data, ret, id,
					  EAP_EKE_FAIL_PROTO_ERROR);
	}

	idtype = *pos++;
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Server IDType %u", idtype);
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Sending EAP-EKE-ID/Response");
					      ...
					      size_t payload_len)
{
	...
	if (data->state != COMMIT) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: EAP-EKE-Commit/Request received in unexpected state (%d)", data->state);
		return eap_eke_build_fail(data, ret, id,
					  EAP_EKE_FAIL_PROTO_ERROR);
	}

	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Received EAP-EKE-Commit/Request");
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Sending EAP-EKE-Commit/Response");
	...
	if (eap_eke_dhcomp(&data->sess, key, pub, rpos) < 0) {
		wpabuf_free(resp);
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Failed to build DHComponent_P");
		os_memset(key, 0, sizeof(key));
		return eap_eke_build_fail(data, ret, id,
					  EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
	}
					       ...
					       size_t payload_len)
{
	...
	if (data->state != CONFIRM) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: EAP-EKE-Confirm/Request received in unexpected state (%d)",
			   data->state);
		return eap_eke_build_fail(data, ret, id,
					  EAP_EKE_FAIL_PROTO_ERROR);
	}

	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Received EAP-EKE-Confirm/Request");
	...
	if (eap_eke_decrypt_prot(&data->sess, pos, data->sess.pnonce_ps_len,
		...
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Failed to decrypt PNonce_PS");
		return eap_eke_build_fail(data, ret, id,
					  EAP_EKE_FAIL_AUTHENTICATION_FAIL);
	}
	if (decrypt_len != (size_t) 2 * data->sess.nonce_len) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: PNonce_PS protected data length does not match length of Nonce_P and Nonce_S");
		return eap_eke_build_fail(data, ret, id,
					  EAP_EKE_FAIL_AUTHENTICATION_FAIL);
	}
	...
	if (os_memcmp(data->nonce_p, nonces, data->sess.nonce_len) != 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Received Nonce_P does not match transmitted Nonce_P");
		return eap_eke_build_fail(data, ret, id,
					  EAP_EKE_FAIL_AUTHENTICATION_FAIL);
	}
	...
	if (os_memcmp_const(auth_s, pos + data->sess.pnonce_ps_len,
			    data->sess.prf_len) != 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Auth_S does not match");
		return eap_eke_build_fail(data, ret, id,
					  EAP_EKE_FAIL_AUTHENTICATION_FAIL);
	}

	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Sending EAP-EKE-Confirm/Response");
...
static struct wpabuf * eap_eke_process_failure(struct eap_eke_data *data,
					       ...
					       size_t payload_len)
{
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Received EAP-EKE-Failure/Request");
	...
	if (payload_len < 4) {
		...
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Failure-Code 0x%x", code);
	}
				       ...
				       const struct wpabuf *reqData)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Received frame: exch %d", eke_exch);
	...
	case EAP_EKE_FAILURE:
		...
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Ignoring message with unknown EKE-Exch %d", eke_exch);
		ret->ignore = TRUE;
		return NULL;
	}
