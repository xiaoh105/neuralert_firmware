...
int da16x_peap_version = EAP_PEAP_VERSION;
				  ...
				  const char *phase1)
{
	...
	if (os_strstr(phase1, "tnc=soh2")) {
		data->soh = 2;
		da16x_eap_prt("[%s] EAP-PEAP: SoH version 2 enabled\n",
			     __func__);
	} else if (os_strstr(phase1, "tnc=soh1")) {
		data->soh = 1;
		da16x_eap_prt("[%s] EAP-PEAP: SoH version 1 enabled\n",
			     __func__);
...
static void * eap_peap_init(struct eap_sm *sm)
{
	...
	if (data == NULL)
		return NULL;
	sm->peap_done = FALSE;
	data->peap_version = da16x_peap_version;
...
#if 0	/* by Shingu 20170112 (TTA Certificate) */
	if (data->phase2_types->method == EAP_TYPE_GTC)
		data->peap_version = 1;

	da16x_peap_version = data->peap_version;
#endif	/* 1 */
	...
	if (eap_peer_tls_ssl_init(sm, &data->ssl, config, EAP_TYPE_PEAP)) {
		da16x_eap_prt("[%s] EAP-PEAP: Failed to initialize SSL.\n",
			     __func__);
		eap_peap_deinit(sm, data);
		return NULL;
	}
			    ...
			    u8 *isk, size_t isk_len)
{
	...
	if (!data->phase2_method->isKeyAvailable(sm, data->phase2_priv) ||
	    (key = data->phase2_method->getKey(sm, data->phase2_priv,
					       &key_len)) == NULL) {
		da16x_eap_prt("[%s] EAP-PEAP: Could not get key material "
			     "from Phase 2\n", __func__);
		return -1;
	}
...
static int eap_peap_derive_cmk(struct eap_sm *sm, struct eap_peap_data *data)
{
	...
	if (tk == NULL)
		return -1;
	da16x_dump("EAP-PEAP: TK", tk, 60);

	resumed = tls_connection_resumed(sm->ssl_ctx, data->ssl.conn);
	da16x_eap_prt("[%s]EAP-PEAP: CMK derivation - reauth=%d resumed=%d phase2_eap_started=%d phase2_success=%d\n",
		   __func__,
	...
	if (data->reauth && !data->phase2_eap_started && resumed) {
		...
		da16x_dump("EAP-PEAP: IPMK from TK", data->ipmk, 40);
		os_memcpy(data->cmk, tk + 40, 20);
		da16x_dump("EAP-PEAP: CMK from TK", data->cmk, 20);
		return 0;
	}

	if (eap_peap_get_isk(sm, data, isk, sizeof(isk)) < 0)
		return -1;
	da16x_dump("EAP-PEAP: ISK", isk, sizeof(isk));
	 ...
	 */
	da16x_dump("EAP-PEAP: TempKey", tk, 40);
	if (peap_prfplus(data->peap_version, tk, 40,
			 "Inner Methods Compound Keys",
			 isk, sizeof(isk), imck, sizeof(imck)) < 0)
		return -1;
	da16x_dump("EAP-PEAP: IMCK (IPMKj)", imck, sizeof(imck));

	os_memcpy(data->ipmk, imck, 40);
	da16x_dump("EAP-PEAP: IPMK (S-IPMKj)", data->ipmk, 40);
	os_memcpy(data->cmk, imck + 40, 20);
	da16x_dump("EAP-PEAP: CMK (CMKj)", data->cmk, 20);
				     ...
				     struct wpabuf *buf)
{
	...
	da16x_dump("EAP-PEAP: Compound_MAC CMK", data->cmk, 20);
	da16x_dump("EAP-PEAP: Compound_MAC data 1", addr[0], len[0]);
	da16x_dump("EAP-PEAP: Compound_MAC data 2", addr[1], len[1]);
	if (hmac_sha1_vector(data->cmk, 20, 2, addr, len, mac) < 0)
		return -1;
	da16x_dump("EAP-PEAP: Compound_MAC", mac, SHA1_MAC_LEN);
					  ...
					  size_t crypto_tlv_len)
{
	...
	if (eap_peap_derive_cmk(sm, data) < 0) {
		da16x_eap_prt("[%s] EAP-PEAP: Could not derive CMK\n", __func__);
		return -1;
	}

	if (crypto_tlv_len != 4 + 56) {
		da16x_eap_prt("[%s] EAP-PEAP: Invalid cryptobinding TLV "
			     "length %d\n", __func__, (int) crypto_tlv_len);
		return -1;
	}
	...
	if (pos[1] != data->peap_version) {
		da16x_eap_prt("[%s] EAP-PEAP: Cryptobinding TLV Version "
	...
	if (pos[3] != 0) {
		da16x_eap_prt("[%s] EAP-PEAP: Unexpected Cryptobinding TLV "
			     "SubType %d\n", __func__, pos[3]);
		return -1;
	}
	pos += 4;
	...
	da16x_dump("EAP-PEAP: Compound_MAC data", buf, sizeof(buf));
	hmac_sha1(data->cmk, 20, buf, sizeof(buf), mac);

	if (os_memcmp_const(mac, pos, SHA1_MAC_LEN) != 0) {
		da16x_eap_prt("[%s] EAP-PEAP: Invalid Compound_MAC in "
			     "cryptobinding TLV\n", __func__);
		da16x_dump("EAP-PEAP: Received MAC", pos, SHA1_MAC_LEN);
		da16x_dump("EAP-PEAP: Expected MAC", mac, SHA1_MAC_LEN);
		return -1;
	}

	da16x_eap_prt("[%s] EAP-PEAP: Valid cryptobinding TLV received\n",
		     __func__);
			   ...
			   int force_failure)
{
	...
	if (pos == NULL)
		return -1;
	da16x_dump("EAP-TLV: Received TLVs", pos, left);
	while (left >= 4) {
		...
		left -= 4;
		if (tlv_len > left) {
			da16x_eap_prt("[%s] EAP-TLV: TLV underrun "
				     "(tlv_len=%lu left=%lu)\n", __func__,
		...
		case EAP_TLV_CRYPTO_BINDING_TLV:
			...
			break;
		default:
			da16x_eap_prt("[%s] EAP-TLV: Unsupported TLV Type "
				     "%d%s\n", __func__, tlv_type,
				     mandatory ? " (mandatory)" : "");
			if (mandatory) {
	...
	if (left) {
		da16x_eap_prt("[%s] EAP-TLV: Last TLV too short in "
			     "Request (left=%lu)\n", __func__,
			     (unsigned long) left);
		return -1;
	}
	...
	if (crypto_tlv && data->crypto_binding != NO_BINDING) {
		da16x_dump("EAP-PEAP: Cryptobinding TLV",
		...
		da16x_eap_prt("[%s] EAP-PEAP: No cryptobinding TLV\n",
			     __func__);
		return -1;
	}
	...
	if (result_tlv) {
		...
		da16x_dump("EAP-TLV: Result TLV", result_tlv, result_tlv_len);
		if (result_tlv_len < 2) {
			da16x_eap_prt("[%s] EAP-TLV: Too short Result TLV "
				     "(len=%lu)\n", __func__,
		...
		if (status == EAP_TLV_RESULT_SUCCESS) {
			da16x_eap_prt("[%s] EAP-TLV: TLV Result - Success "
				     "- EAP-TLV/Phase2 Completed\n", __func__);
			if (force_failure) {
				da16x_eap_prt("[%s] EAP-TLV: Earlier failure"
			...
			da16x_eap_prt("[%s] EAP-TLV: TLV Result - Failure\n",
				     __func__);
			resp_status = EAP_TLV_RESULT_FAILURE;
			ret->decision = DECISION_FAIL;
		} else {
			da16x_eap_prt("[%s] EAP-TLV: Unknown TLV Result "
				     "Status %d\n", __func__, status);
			resp_status = EAP_TLV_RESULT_FAILURE;
			ret->decision = DECISION_FAIL;
		}
				   ...
				   struct wpabuf **resp)
{
	...
	if (len <= sizeof(struct eap_hdr)) {
		da16x_eap_prt("[%s] EAP-PEAP: too short Phase 2 request "
			     "(len=%lu)\n", __func__, (unsigned long) len);
		return -1;
	}
	pos = (u8 *) (hdr + 1);
	da16x_eap_prt("[%s] EAP-PEAP: Phase 2 Request: type=%d\n",
		     __func__, *pos);
	switch (*pos) {
	...
	case EAP_TYPE_EXPANDED:
		...
		if (data->soh) {
			...
			if (epos) {
				struct wpabuf *buf;
				da16x_eap_prt("[%s] EAP-PEAP: SoH EAP "
					     "Extensions\n", __func__);
	...
	default:
		if (data->phase2_type.vendor == EAP_VENDOR_IETF &&
			...
			for (i = 0; i < data->num_phase2_types; i++) {
				...
				da16x_eap_prt("[%s] EAP-PEAP: Selected Phase 2 "
		...
		if (data->phase2_priv == NULL || data->phase2_method == NULL) {
			da16x_eap_prt("[%s] EAP-PEAP: failed to initialize "
			    ...
			    struct wpabuf **out_data)
{
	...
	da16x_eap_prt("[%s] EAP-PEAP: received %lu bytes encrypted data for"
		     " Phase 2\n", __func__,
		     (unsigned long) wpabuf_len(in_data));

	if (data->pending_phase2_req) {
		da16x_eap_prt("[%s] EAP-PEAP: Pending Phase 2 request - "
	...
	if (wpabuf_len(in_data) == 0 && sm->workaround &&
		 ...
		 */
		da16x_eap_prt("[%s] EAP-PEAP: Received TLS ACK, but "
...
continue_req:
	...
	if (wpabuf_len(in_decrypted) < sizeof(*hdr)) {
		da16x_eap_prt("[%s] EAP-PEAP: Too short Phase 2 "
			     "EAP frame (len=%lu)\n", __func__,
	...
	if (len > wpabuf_len(in_decrypted)) {
		da16x_eap_prt("[%s] EAP-PEAP: Length mismatch in "
	...
	if (len < wpabuf_len(in_decrypted)) {
		da16x_eap_prt("[%s] EAP-PEAP: Odd.. Phase 2 EAP header has "
			     "shorter length than full decrypted data "
				     "(%lu < %lu)\n", __func__,
				     (unsigned long) len,
				     (unsigned long) wpabuf_len(in_decrypted));
	}
	da16x_eap_prt("[%s] EAP-PEAP: received Phase 2: code=%d "
	...
	case EAP_CODE_REQUEST:
		if (eap_peap_phase2_request(sm, data, ret, in_decrypted,
					    &resp)) {
			wpabuf_free(in_decrypted);
			da16x_eap_prt("[%s] EAP-PEAP: Phase2 Request "
				     "processing failed\n", __func__);
			return 0;
		}
		break;
	case EAP_CODE_SUCCESS:
		da16x_eap_prt("[%s] EAP-PEAP: Phase 2 Success\n", __func__);
		if (data->peap_version == 1) {
			...
			if (data->phase2_eap_started &&
			    !data->phase2_eap_success) {
				da16x_eap_prt("[%s] EAP-PEAP: Phase 2 "
			...
			da16x_eap_prt("[%s] EAP-PEAP: Version 1 - "
			...
			if (data->peap_outer_success == 2) {
				wpabuf_free(in_decrypted);
				da16x_eap_prt("[%s] EAP-PEAP: Use TLS ACK "
	...
	case EAP_CODE_FAILURE:
		da16x_eap_prt("[%s] EAP-PEAP: Phase 2 Failure\n", __func__);
		ret->decision = DECISION_FAIL;
		...
		break;
	default:
		da16x_eap_prt("[%s] EAP-PEAP: Unexpected code=%d in "
			     "Phase 2 EAP header\n", __func__, hdr->code);
		break;
	}
	...
	if (resp) {
		...
		if (eap_peer_tls_encrypt(sm, &data->ssl, EAP_TYPE_PEAP,
					 ...
					 rmsg, out_data)) {
			da16x_eap_prt("[%s] EAP-PEAP: Failed to encrypt "
				     "a Phase 2 frame\n", __func__);
		}
		wpabuf_free(resp);
	}
					...
					const struct wpabuf *reqData)
{
	...
	if (flags & EAP_TLS_FLAGS_START) {
		da16x_eap_prt("[%s] EAP-PEAP: Start (server ver=%d, own "
			     "ver=%d)\n", __func__,
			     flags & EAP_TLS_VERSION_MASK,
			     data->peap_version);
#if 1	/* by Shingu 20170112 (TTA Certificate) */
		...
		if (data->force_peap_version >= 0 &&
			...
			da16x_eap_prt("[%s] EAP-PEAP: Failed to select "
		...
		data->peap_version = da16x_peap_version;
#endif	/* 0 */
		da16x_eap_prt("[%s] EAP-PEAP: Using PEAP version %d\n",
			     __func__, data->peap_version);
	...
	if (tls_connection_established(sm->ssl_ctx, data->ssl.conn) &&
		...
		if (tls_connection_established(sm->ssl_ctx, data->ssl.conn)) {
			char *label;
			da16x_eap_prt("[%s] EAP-PEAP: TLS done, proceed to "
				     "Phase 2\n", __func__);
			eap_peap_free_key(data);
			...
			da16x_eap_prt("[%s] EAP-PEAP: using label '%s' in "
				     "key derivation\n", __func__, label);
			data->key_data =
				eap_peer_tls_derive_key(sm, &data->ssl, label,
							EAP_TLS_KEY_LEN);
			if (data->key_data) {
				da16x_dump("EAP-PEAP: Derived key",
					  data->key_data, EAP_TLS_KEY_LEN);
			} else {
				da16x_eap_prt("[%s] EAP-PEAP: Failed to "
					     "derive key\n", __func__);
			}
			...
			data->session_id =
				eap_peer_tls_derive_session_id(sm, &data->ssl,
							       ...
							       &data->id_len);
			if (data->session_id) {
				da16x_dump("EAP-PEAP: Derived Session-Id",
			...
			if (sm->workaround && data->resuming) {
				 ...
				 */
				da16x_eap_prt("[%s] EAP-PEAP: Workaround - "
...
static u8 * eap_peap_getKey(struct eap_sm *sm, void *priv, size_t *len)
{
	...
	*len = EAP_TLS_KEY_LEN;
	...
	if (data->crypto_binding_used) {
		...
		da16x_dump("EAP-PEAP: CSK", csk, sizeof(csk));
		os_memcpy(key, csk, EAP_TLS_KEY_LEN);
		da16x_dump("EAP-PEAP: Derived key", key, EAP_TLS_KEY_LEN);
	} else
