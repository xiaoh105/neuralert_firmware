	...
	u8 mschapv2_id, const u8 *auth_challenge)
{
	...
	da16x_eap_prt("[%s] EAP-MSCHAPV2: Generating Challenge Response\n",
		     __func__);
	...
	if (data->peer_challenge) {
		da16x_eap_prt("[%s] EAP-MSCHAPV2: peer_challenge generated "
			     "in Phase 1\n", __func__);
	...
	if (data->auth_challenge) {
		da16x_eap_prt("[%s] EAP-MSCHAPV2: auth_challenge generated "
			     "in Phase 1\n", __func__);
	...
	if (mschapv2_derive_response(identity, identity_len, password,
		...
		da16x_eap_prt("[%s] EAP-MSCHAPV2: Failed to derive "
			     "response\n", __func__);
		wpabuf_free(resp);
		return NULL;
	}
	...
	da16x_eap_prt("[%s] EAP-MSCHAPV2: TX identifier %d mschapv2_id %d "
		     "(response)\n", __func__, id, ms->mschapv2_id);
	return resp;
}
	...
	size_t req_len, u8 id)
{
	...
	da16x_eap_prt("[%s] EAP-MSCHAPV2: Received challenge\n", __func__);
	if (req_len < sizeof(*req) + 1) {
		da16x_eap_prt("[%s] EAP-MSCHAPV2: Too short challenge data "
			     "(len %lu)\n", __func__, (unsigned long) req_len);
		ret->ignore = TRUE;
		return NULL;
	}
	pos = (const u8 *) (req + 1);
	...
	if (challenge_len != MSCHAPV2_CHAL_LEN) {
		da16x_eap_prt("[%s] EAP-MSCHAPV2: Invalid challenge length "
			     "%lu\n", __func__, (unsigned long) challenge_len);
		ret->ignore = TRUE;
		return NULL;
	}

	if (len < challenge_len) {
		da16x_eap_prt("[%s] EAP-MSCHAPV2: Too short challenge"
	...
	if (data->passwd_change_challenge_valid) {
		da16x_eap_prt("[%s] EAP-MSCHAPV2: Using challenge from the "
			     "failure message\n", __func__);
	...
	pos += challenge_len;
	len -= challenge_len;
	da16x_dump("EAP-MSCHAPV2: Authentication Servername", pos, len);
					  ...
					  struct eap_mschapv2_data *data)
{
	...
	if (config && config->new_password) {
		da16x_eap_prt("[%s] EAP-MSCHAPV2: Password changed "
			     "successfully\n", __func__);
		data->prev_error = 0;
					    ...
					    size_t req_len, u8 id)
{
	...
	da16x_eap_prt("[%s] EAP-MSCHAPV2: Received success\n", __func__);
	len = req_len - sizeof(*req);
	pos = (const u8 *) (req + 1);
	if (!data->auth_response_valid ||
	    mschapv2_verify_auth_response(data->auth_response, pos, len)) {
		da16x_eap_prt("[%s] EAP-MSCHAPV2: Invalid authenticator "
	...
	da16x_dump("EAP-MSCHAPV2: Success message", pos, len);
	da16x_eap_prt("[%s] EAP-MSCHAPV2: Authentication succeeded\n", __func__);
	...
	resp = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_MSCHAPV2, 1,
			     EAP_CODE_RESPONSE, id);
	if (resp == NULL) {
		da16x_eap_prt("[%s] EAP-MSCHAPV2: Failed to allocate "
				    ...
				    struct eap_mschapv2_data *data, char *txt)
{
	...
	if (pos && os_strncmp(pos, "E=", 2) == 0) {
		...
		da16x_eap_prt("[%s] EAP-MSCHAPV2: error %d\n",
			     __func__, data->prev_error);
		pos = os_strchr(pos, ' ');
		if (pos)
			pos++;
	}
	...
	if (pos && os_strncmp(pos, "R=", 2) == 0) {
		pos += 2;
		retry = atoi(pos);
		da16x_eap_prt("[%s] EAP-MSCHAPV2: retry is %sallowed\n",
			     __func__, retry == 1 ? "" : "not ");
		pos = os_strchr(pos, ' ');
		if (pos)
			pos++;
	}
	...
	if (pos && os_strncmp(pos, "C=", 2) == 0) {
		...
		if (hex_len == PASSWD_CHANGE_CHAL_LEN * 2) {
			if (hexstr2bin(pos, data->passwd_change_challenge,
				...
				da16x_eap_prt("[%s] EAP-MSCHAPV2: invalid "
					     "failure challenge\n", __func__);
			} else {
				da16x_dump("EAP-MSCHAPV2: failure challenge\n",
					  data->passwd_change_challenge,
					  PASSWD_CHANGE_CHAL_LEN);
				data->passwd_change_challenge_valid = 1;
			}
		} else {
			da16x_eap_prt("[%s] EAP-MSCHAPV2: invalid failure "
				     "challenge len %d\n", __func__, hex_len);
		}
		pos = os_strchr(pos, ' ');
		if (pos)
		...
		da16x_eap_prt("[%s] EAP-MSCHAPV2: required challenge field "
	...
	if (pos && os_strncmp(pos, "V=", 2) == 0) {
		...
		da16x_eap_prt("[%s] EAP-MSCHAPV2: password changing "
			     "protocol version %d\n", __func__,
	...
	if (data->prev_error == ERROR_AUTHENTICATION_FAILURE && retry &&
		...
		da16x_eap_prt("[%s]EAP-MSCHAPV2: mark password retry disabled based on local configuration", __func__);
		retry = 0;
	}
	da16x_eap_prt("[%s] EAP-MSCHAPV2: failure message: '%s' (retry "
		     "%sallowed, error %d)\n", __func__,
		     pos, retry == 1 ? "" : "not ", data->prev_error);
	if (data->prev_error == ERROR_PASSWD_EXPIRED &&
	    data->passwd_change_version == 3 && config) {
		if (config->new_password == NULL) {
			da16x_eap_prt("[%s] EAP-MSCHAPV2: Password expired - "
...
{
#ifdef CONFIG_NO_RC4
	da16x_eap_prt("[%s]EAP-MSCHAPV2: RC4 not support in the build - cannot change password",
		__func__);
	return NULL;
#else /* CONFIG_NO_RC4 */
	struct wpabuf *resp;
	...
	/* NT-Response */
	da16x_dump("EAP-MSCHAPV2: auth_challenge",
		  data->passwd_change_challenge, PASSWD_CHANGE_CHAL_LEN);
	da16x_dump("EAP-MSCHAPV2: peer_challenge",
		    cp->peer_challenge, MSCHAPV2_CHAL_LEN);
	da16x_dump("EAP-MSCHAPV2: username", username, username_len);
	da16x_dump("EAP-MSCHAPV2: new password", new_password, new_password_len);
	generate_nt_response(data->passwd_change_challenge, cp->peer_challenge,
			     username, username_len,
			     new_password, new_password_len,
			     cp->nt_response);
	da16x_dump("EAP-MSCHAPV2: NT-Response",
					    ...
					    size_t req_len, u8 id)
{
	...
	da16x_eap_prt("[%s] EAP-MSCHAPV2: Received failure\n", __func__);
	da16x_dump("EAP-MSCHAPV2: Failure data", msdata, len);
	/*
...
static int eap_mschapv2_check_config(struct eap_sm *sm)
{
	...
	if (eap_get_config_identity(sm, &len) == NULL) {
		da16x_eap_prt("[%s] EAP-MSCHAPV2: Identity not configured\n",
			     __func__);
		eap_sm_request_identity(sm);
		return -1;
	}
	...
	if (eap_get_config_password(sm, &len) == NULL) {
		da16x_eap_prt("[%s] EAP-MSCHAPV2: Password not configured\n",
			     __func__);
		eap_sm_request_password(sm);
		return -1;
	}
					    ...
					    const struct wpabuf *reqData)
{
	...
	if (config->mschapv2_retry && data->prev_challenge &&
		...
		da16x_eap_prt("[%s] EAP-MSCHAPV2: Replacing pending packet "
	...
	da16x_eap_prt("[%s] EAP-MSCHAPV2: RX identifier %d mschapv2_id %d\n",
		     __func__, id, ms->mschapv2_id);
	...
	default:
		da16x_eap_prt("[%s] EAP-MSCHAPV2: Unknown op %d - ignored\n",
			     __func__, ms->op_code);
		ret->ignore = TRUE;
		return NULL;
	}
...
static u8 * eap_mschapv2_getKey(struct eap_sm *sm, void *priv, size_t *len)
{
	...
	da16x_dump("EAP-MSCHAPV2: Derived key", key, key_len);
