...
static int ikev2_derive_keys(struct ikev2_responder_data *data)
{
	...
	if (integ == NULL || prf == NULL || encr == NULL) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Unsupported proposal");
		return -1;
	}
				 ...
				 const u8 *pos, const u8 *end)
{
	...
	if (end - pos < (int) sizeof(*t)) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Too short transform");
		return -1;
	}
	...
	if (transform_len < (int) sizeof(*t) || transform_len > end - pos) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Invalid transform length %d",
			   transform_len);
		return -1;
	}
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2:   Transform:");
	wpa_printf_dbg(MSG_DEBUG, "IKEV2:     Type: %d  Transform Length: %d  "
		   "Transform Type: %d  Transform ID: %d",
		   t->type, transform_len, t->transform_type, transform_id);

	if (t->type != 0 && t->type != 3) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Unexpected Transform type");
		return -1;
	}
	...
	case IKEV2_TRANSFORM_ENCR:
		if (ikev2_get_encr(transform_id)) {
			if (transform_id == ENCR_AES_CBC) {
				if (tend - pos != 4) {
					wpa_printf_dbg(MSG_DEBUG, "IKEV2: No "
						   "Transform Attr for AES");
					break;
				}
				if (WPA_GET_BE16(pos) != 0x800e) {
					wpa_printf_dbg(MSG_DEBUG, "IKEV2: Not a "
						   "Key Size attribute for "
						   "AES");
					break;
				}
				if (WPA_GET_BE16(pos + 2) != 128) {
					wpa_printf_dbg(MSG_DEBUG, "IKEV2: "
				...
				const u8 *pos, const u8 *end)
{
	...
	if (end - pos < (int) sizeof(*p)) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Too short proposal");
		return -1;
	}
	...
	if (proposal_len < (int) sizeof(*p) || proposal_len > end - pos) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Invalid proposal length %d",
			   proposal_len);
		return -1;
	}
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: SAi1 Proposal # %d",
		   p->proposal_num);
	wpa_printf_dbg(MSG_DEBUG, "IKEV2:   Type: %d  Proposal Length: %d "
		   " Protocol ID: %d",
		   p->type, proposal_len, p->protocol_id);
	wpa_printf_dbg(MSG_DEBUG, "IKEV2:   SPI Size: %d  Transforms: %d",
		   p->spi_size, p->num_transforms);

	if (p->type != 0 && p->type != 2) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Unexpected Proposal type");
		return -1;
	}

	if (p->protocol_id != IKEV2_PROTOCOL_IKE) {
		wpa_printf_dbg(MSG_DEBUG, "IKEV2: Unexpected Protocol ID "
	...
	if (p->proposal_num != prop->proposal_num) {
		if (p->proposal_num == prop->proposal_num + 1)
			...
			wpa_printf_dbg(MSG_INFO, "IKEV2: Unexpected Proposal #");
			return -1;
		}
	}
	...
	if (p->spi_size > pend - ppos) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Not enough room for SPI "
			   "in proposal");
		return -1;
	}
	if (p->spi_size) {
	 ...
	 */
	if (p->spi_size != 0) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Unexpected SPI Size");
		return -1;
	}

	if (p->num_transforms == 0) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: At least one transform required");
		return -1;
	}
	...
	if (ppos != pend) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Unexpected data after "
			   "transforms");
		return -1;
	}
			      ...
			      const u8 *sai1, size_t sai1_len)
{
	...
	if (sai1 == NULL) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: SAi1 not received");
		return -1;
	}
	...
	if (pos != end) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Unexpected data after proposals");
		return -1;
	}

	if (!found) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: No acceptable proposal found");
		return -1;
	}

	wpa_printf_dbg(MSG_DEBUG, "IKEV2: Accepted proposal #%d: ENCR:%d PRF:%d "
			     ...
			     const u8 *kei, size_t kei_len)
{
	...
	if (kei == NULL) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: KEi not received");
		return -1;
	}

	if (kei_len < 4 + 96) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Too short Key Exchange Payload");
		return -1;
	}

	group = WPA_GET_BE16(kei);
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: KEi DH Group #%u", group);

	if (group != data->proposal.dh) {
		wpa_printf_dbg(MSG_DEBUG, "IKEV2: KEi DH Group #%u does not match "
	...
	if (data->dh == NULL) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Unsupported DH group");
		return -1;
	}
	...
	if (kei_len - 4 != data->dh->prime_len) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Invalid DH public value length "
			   "%ld (expected %ld)",
...
{
	if (ni == NULL) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Ni not received");
		return -1;
	}

	if (ni_len < IKEV2_NONCE_MIN_LEN || ni_len > IKEV2_NONCE_MAX_LEN) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Invalid Ni length %ld",
		           (long) ni_len);
		return -1;
	}
			     ...
			     const u8 *idi, size_t idi_len)
{
	...
	if (idi == NULL) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: No IDi received");
		return -1;
	}

	if (idi_len < 4) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Too short IDi payload");
		return -1;
	}
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: IDi ID Type %d", id_type);
			      ...
			      const u8 *cert, size_t cert_len)
{
	...
	if (cert == NULL) {
		if (data->peer_auth == PEER_AUTH_CERT) {
			wpa_printf_dbg(MSG_INFO, "IKEV2: No Certificate received");
			return -1;
		}
		return 0;
	}
	...
	if (cert_len < 1) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: No Cert Encoding field");
		return -1;
	}
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: Cert Encoding %d", cert_encoding);
				   ...
				   u8 method, const u8 *auth, size_t auth_len)
{
	if (method != AUTH_RSA_SIGN) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Unsupported authentication "
			   "method %d", method);
		return -1;
	}
				     ...
				     size_t auth_len)
{
	...
	if (ikev2_derive_auth_data(data->proposal.prf, data->i_sign_msg,
				   ...
				   auth_data) < 0) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Could not derive AUTH data");
		return -1;
	}
	...
	if (auth_len != prf->hash_len ||
		...
		wpa_printf_dbg(MSG_INFO, "IKEV2: Invalid Authentication Data");
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: Server authenticated successfully "
		   "using shared keys");
			      ...
			      const u8 *auth, size_t auth_len)
{
	...
	if (auth == NULL) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: No Authentication Payload");
		return -1;
	}

	if (auth_len < 4) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Too short Authentication "
			   "Payload");
		return -1;
	}
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: Auth Method %d", auth_method);
					   ...
					   u8 *payload, size_t payload_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: Processing decrypted payloads");

	if (ikev2_parse_payloads(&pl, next_payload, payload, payload +
				 payload_len) < 0) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Failed to parse decrypted "
			   "payloads");
		return -1;
	}
				   ...
				   u8 exchange_type, u32 message_id)
{
	...
	case SA_INIT:
		...
		if (exchange_type != IKE_SA_INIT) {
			wpa_printf_dbg(MSG_INFO, "IKEV2: Unexpected Exchange Type "
				   "%u in SA_INIT state", exchange_type);
			return -1;
		}
		if (message_id != 0) {
			wpa_printf_dbg(MSG_INFO, "IKEV2: Unexpected Message ID %u "
			    ...
			    const struct wpabuf *buf)
{
	...
	wpa_printf_dbg(MSG_MSGDUMP, "IKEV2: Received message (len %lu)",
		   (unsigned long) wpabuf_len(buf));

	if (wpabuf_len(buf) < sizeof(*hdr)) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Too short frame to include HDR");
		return -1;
	}
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2:   Next Payload: %u  Version: 0x%x  "
		   "Exchange Type: %u",
		   hdr->next_payload, hdr->version, hdr->exchange_type);
	wpa_printf_dbg(MSG_DEBUG, "IKEV2:   Message ID: %u  Length: %u",
		   message_id, length);

	if (hdr->version != IKEV2_VERSION) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Unsupported HDR version 0x%x "
			   "(expected 0x%x)", hdr->version, IKEV2_VERSION);
		return -1;
	}

	if (length != wpabuf_len(buf)) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Invalid length (HDR: %lu != "
	...
	if ((hdr->flags & (IKEV2_HDR_INITIATOR | IKEV2_HDR_RESPONSE)) !=
	    IKEV2_HDR_INITIATOR) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: Unexpected Flags value 0x%x",
			   hdr->flags);
		return -1;
	}
	...
	if (data->state != SA_INIT) {
		if (os_memcmp(data->i_spi, hdr->i_spi, IKEV2_SPI_LEN) != 0) {
			wpa_printf_dbg(MSG_INFO, "IKEV2: Unexpected IKE_SA "
				   "Initiator's SPI");
			return -1;
		}
			    ...
			    u8 next_payload, u32 message_id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: Adding HDR");
			    ...
			    struct wpabuf *msg, u8 next_payload)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: Adding SAr1 payload");
			   ...
			   struct wpabuf *msg, u8 next_payload)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: Adding KEr payload");

	pv = dh_init(data->dh, &data->r_dh_private);
	if (pv == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "IKEV2: Failed to initialize DH");
		return -1;
	}
			  ...
			  struct wpabuf *msg, u8 next_payload)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: Adding Nr payload");
			   ...
			   struct wpabuf *msg, u8 next_payload)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: Adding IDr payload");

	if (data->IDr == NULL) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: No IDr available");
		return -1;
	}
			    ...
			    struct wpabuf *msg, u8 next_payload)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: Adding AUTH payload");
				    ...
				    struct wpabuf *msg, u8 next_payload)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: Adding Notification payload");

	if (data->error_type == 0) {
		wpa_printf_dbg(MSG_INFO, "IKEV2: No Notify Message Type "
			   "available");
		return -1;
	}
	...
	case INVALID_KE_PAYLOAD:
		if (data->proposal.dh == -1) {
			wpa_printf_dbg(MSG_INFO, "IKEV2: No DH Group selected for "
				   "INVALID_KE_PAYLOAD notifications");
			return -1;
		}
		wpabuf_put_be16(msg, data->proposal.dh);
		wpa_printf_dbg(MSG_DEBUG, "IKEV2: INVALID_KE_PAYLOAD - request "
	...
	case AUTHENTICATION_FAILED:
		...
		break;
	default:
		wpa_printf_dbg(MSG_INFO, "IKEV2: Unsupported Notify Message Type "
			   "%d", data->error_type);
		return -1;
	}
