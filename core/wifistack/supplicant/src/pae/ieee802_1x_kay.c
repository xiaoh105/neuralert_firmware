...
ieee802_1x_mka_dump_basic_body(struct ieee802_1x_mka_basic_body *body)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "*** MKA Basic Parameter set ***");
	wpa_printf_dbg(MSG_DEBUG, "\tVersion.......: %d", body->version);
	wpa_printf_dbg(MSG_DEBUG, "\tPriority......: %d", body->priority);
	wpa_printf_dbg(MSG_DEBUG, "\tKeySvr........: %d", body->key_server);
	wpa_printf_dbg(MSG_DEBUG, "\tMACSecDesired.: %d", body->macsec_desired);
	wpa_printf_dbg(MSG_DEBUG, "\tMACSecCapable.: %d", body->macsec_capbility);
	wpa_printf_dbg(MSG_DEBUG, "\tBody Length...: %d", (int) body_len);
	wpa_printf_dbg(MSG_DEBUG, "\tSCI MAC.......: " MACSTR,
		   MAC2STR(body->actor_sci.addr));
	wpa_printf_dbg(MSG_DEBUG, "\tSCI Port .....: %d",
		   be_to_host16(body->actor_sci.port));
	wpa_hexdump(MSG_DEBUG, "\tMember Id.....:",
		    body->actor_mi, sizeof(body->actor_mi));
	wpa_printf_dbg(MSG_DEBUG, "\tMessage Number: %d",
...
ieee802_1x_mka_dump_peer_body(struct ieee802_1x_mka_peer_body *body)
{
	...
	if (body->type == MKA_LIVE_PEER_LIST) {
		wpa_printf_dbg(MSG_DEBUG, "*** Live Peer List ***");
		wpa_printf_dbg(MSG_DEBUG, "\tBody Length...: %d", (int) body_len);
	} else if (body->type == MKA_POTENTIAL_PEER_LIST) {
		wpa_printf_dbg(MSG_DEBUG, "*** Potential Live Peer List ***");
		wpa_printf_dbg(MSG_DEBUG, "\tBody Length...: %d", (int) body_len);
	}
	...
	for (i = 0; i < body_len; i += MI_LEN + sizeof(mn)) {
		...
		wpa_printf_dbg(MSG_DEBUG, "\tMessage Number: %d", be_to_host32(mn));
	}
}
...
ieee802_1x_mka_dump_sak_use_body(struct ieee802_1x_mka_sak_use_body *body)
{
	...
	body_len = get_mka_param_body_len(body);
	wpa_printf_dbg(MSG_DEBUG, "*** MACsec SAK Use ***");
	wpa_printf_dbg(MSG_DEBUG, "\tLatest Key AN....: %d", body->lan);
	wpa_printf_dbg(MSG_DEBUG, "\tLatest Key Tx....: %s", yes_no(body->ltx));
	wpa_printf_dbg(MSG_DEBUG, "\tLatest Key Rx....: %s", yes_no(body->lrx));
	wpa_printf_dbg(MSG_DEBUG, "\tOld Key AN....: %d", body->oan);
	wpa_printf_dbg(MSG_DEBUG, "\tOld Key Tx....: %s", yes_no(body->otx));
	wpa_printf_dbg(MSG_DEBUG, "\tOld Key Rx....: %s", yes_no(body->orx));
	wpa_printf_dbg(MSG_DEBUG, "\tPlain Key Tx....: %s", yes_no(body->ptx));
	wpa_printf_dbg(MSG_DEBUG, "\tPlain Key Rx....: %s", yes_no(body->prx));
	wpa_printf_dbg(MSG_DEBUG, "\tDelay Protect....: %s",
		   yes_no(body->delay_protect));
	wpa_printf_dbg(MSG_DEBUG, "\tBody Length......: %d", body_len);
	if (!body_len)
		return;

	wpa_hexdump(MSG_DEBUG, "\tKey Server MI....:",
		    body->lsrv_mi, sizeof(body->lsrv_mi));
	wpa_printf_dbg(MSG_DEBUG, "\tKey Number.......: %u",
		   be_to_host32(body->lkn));
	wpa_printf_dbg(MSG_DEBUG, "\tLowest PN........: %u",
		   be_to_host32(body->llpn));
	wpa_hexdump_ascii(MSG_DEBUG, "\tOld Key Server MI....:",
			  body->osrv_mi, sizeof(body->osrv_mi));
	wpa_printf_dbg(MSG_DEBUG, "\tOld Key Number.......: %u",
		   be_to_host32(body->okn));
	wpa_printf_dbg(MSG_DEBUG, "\tOld Lowest PN........: %u",
...
ieee802_1x_kay_get_participant(struct ieee802_1x_kay *kay, const u8 *ckn)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "KaY: participant is not found");
...
ieee802_1x_kay_get_principal_participant(struct ieee802_1x_kay *kay)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "KaY: principal participant is not founded");
	return NULL;
}
			       ...
			       int channel)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "KaY: Create receive SC(channel: %d)", channel);
	...
	struct ieee802_1x_mka_participant *participant, struct receive_sc *psc)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "KaY: Delete receive SC(channel: %d)",
		   psc->channel);
				...
				u8 *mi, u32 mn)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "KaY: Live peer created");
	wpa_hexdump(MSG_DEBUG, "\tMI: ", peer->mi, sizeof(peer->mi));
	wpa_printf_dbg(MSG_DEBUG, "\tMN: %d", peer->mn);
	wpa_hexdump(MSG_DEBUG, "\tSCI Addr: ", peer->sci.addr, ETH_ALEN);
	wpa_printf_dbg(MSG_DEBUG, "\tPort: %d", peer->sci.port);
	...
	struct ieee802_1x_mka_participant *participant, const u8 *mi, u32 mn)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "KaY: potential peer created");
	wpa_hexdump(MSG_DEBUG, "\tMI: ", peer->mi, sizeof(peer->mi));
	wpa_printf_dbg(MSG_DEBUG, "\tMN: %d", peer->mn);
	wpa_hexdump(MSG_DEBUG, "\tSCI Addr: ", peer->sci.addr, ETH_ALEN);
	wpa_printf_dbg(MSG_DEBUG, "\tPort: %d", peer->sci.port);
			      ...
			      u8 *mi, u32 mn)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "KaY: move potential peer to live peer");
	wpa_hexdump(MSG_DEBUG, "\tMI: ", peer->mi, sizeof(peer->mi));
	wpa_printf_dbg(MSG_DEBUG, "\tMN: %d", peer->mn);
	wpa_hexdump(MSG_DEBUG, "\tSCI Addr: ", peer->sci.addr, ETH_ALEN);
	wpa_printf_dbg(MSG_DEBUG, "\tPort: %d", peer->sci.port);
				 ...
				 size_t msg_len)
{
	...
	if (kay->is_obliged_key_server && body->key_server) {
		wpa_printf_dbg(MSG_DEBUG, "I must be as key server");
		return NULL;
	}

	participant = ieee802_1x_kay_get_participant(kay, body->ckn);
	if (!participant) {
		wpa_printf_dbg(MSG_DEBUG, "Peer is not included in my CA");
		return NULL;
	}
	...
	const u8 *mka_msg, size_t msg_len)
{
	...
	if (body->ltx || body->lrx) {
		...
		if (os_memcmp(participant->lki.mi, body->lsrv_mi,
		    ...
		    (u32)ntohl(body->lkn) == participant->lki.kn &&
		    body->lan == participant->lan) {
			peer->sak_used = TRUE;
		}
	...
	if (body->otx || body->orx) {
		if (os_memcmp(participant->oki.mi, body->osrv_mi,
		    ...
		    (u32)ntohl(body->okn) != participant->oki.kn ||
		    body->oan != participant->oan) {
	...
	const u8 *mka_msg, size_t msg_len)
{
	...
	if (aes_unwrap(participant->kek.key, participant->kek.len, sak_len >> 3, wrap_sak,
		       unwrap_sak)) {
...
ieee802_1x_kay_elect_key_server(struct ieee802_1x_mka_participant *participant)
{
	...
	if (i_is_key_server) {
		...
		participant->new_sak = TRUE;
		wpa_printf_dbg(MSG_DEBUG, "KaY: I is elected as key server");
		participant->to_dist_sak = FALSE;
	...
	struct ieee802_1x_mka_participant *participant)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "KaY: to enpacket and send the MKPDU");
	...
	l2_packet_send(kay->l2_mka, NULL, 0, wpabuf_head(buf), wpabuf_len(buf), 0);
	wpabuf_free(buf);
...
static void ieee802_1x_participant_timer(void *eloop_ctx, void *timeout_ctx)
{
	...
	dl_list_for_each_safe(peer, pre_peer, &participant->live_peers,
			      struct ieee802_1x_kay_peer, list) {
		if (now > peer->expire) {
			wpa_printf_dbg(MSG_DEBUG, "KaY: Live peer removed");
	...
	dl_list_for_each_safe(peer, pre_peer, &participant->potential_peers,
			      struct ieee802_1x_kay_peer, list) {
		if (now > peer->expire) {
			wpa_printf_dbg(MSG_DEBUG, "KaY: Potential peer removed");
			wpa_hexdump(MSG_DEBUG, "\tMI: ", peer->mi,
				    sizeof(peer->mi));
			wpa_printf_dbg(MSG_DEBUG, "\tMN: %d", peer->mn);
			dl_list_del(&peer->list);
	...
	da16x_eloop_register_timeout(MKA_HELLO_TIME / 1000, 0,
			       ieee802_1x_participant_timer,
			       participant, NULL);
}
				...
				int channel)
{
	...
	dl_list_init(&psc->sa_list);
	wpa_printf_dbg(MSG_DEBUG, "KaY: Create transmit SC(channel: %d)", channel);
	...
	struct ieee802_1x_mka_participant *participant, struct transmit_sc *psc)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "KaY: Delete transmit SC(channel: %d)",
		   psc->channel);
			      ...
			      struct ieee802_1x_mka_ki *ki)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "KaY: Entry into %s", __func__);
	principal = ieee802_1x_kay_get_principal_participant(kay);
	if (!principal)
		return -1;
					     ...
					     const u8 *buf, size_t len)
{
	...
	participant = ieee802_1x_kay_get_participant(kay, body->ckn);
	if (!participant) {
		wpa_printf_dbg(MSG_DEBUG, "CKN is not included in my CA");
		return -1;
	}
		    ...
		    const char *ifname, const u8 *addr)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "KaY: state machine created");
	...
	wpa_printf_dbg(MSG_DEBUG, "KaY: secy init macsec done");
...
ieee802_1x_kay_deinit(struct ieee802_1x_kay *kay)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "KaY: state machine removed");
			  ...
			  enum mka_created_mode mode, Boolean is_authenticator)
{
	...
	da16x_eloop_register_timeout(0, usecs, ieee802_1x_participant_timer,
			       participant, NULL);
...
ieee802_1x_kay_delete_mka(struct ieee802_1x_kay *kay, struct mka_key_name *ckn)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "KaY: participant removed");
...
ieee802_1x_kay_new_sak(struct ieee802_1x_kay *kay)
{
	...
	participant->new_sak = TRUE;
	wpa_printf_dbg(MSG_DEBUG, "KaY: new SAK signal");
