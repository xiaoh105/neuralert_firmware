...
SM_STATE(CP, RECEIVING)
{
	...
	da16x_eloop_cancel_timeout(ieee802_1x_cp_transmit_when_timeout, sm, NULL);
	da16x_eloop_register_timeout(sm->transmit_when / 1000, 0,
...
SM_STATE(CP, TRANSMITTING)
{
	...
	ieee802_1x_kay_set_old_sa_attr(sm->kay, sm->oki, sm->oan,
				       sm->otx, sm->orx);
	ieee802_1x_kay_enable_new_info(sm->kay);
	da16x_eloop_cancel_timeout(ieee802_1x_cp_retire_when_timeout, sm, NULL);
	da16x_eloop_register_timeout(sm->retire_when / 1000, 0,
	...
	struct ieee802_1x_cp_conf *pcp_conf)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "CP: state machine created");
...
{
	wpa_printf_dbg(MSG_DEBUG, "CP: state machine removed");
	if (!sm)
		return;

	da16x_eloop_cancel_timeout(ieee802_1x_cp_retire_when_timeout, sm, NULL);
	da16x_eloop_cancel_timeout(ieee802_1x_cp_transmit_when_timeout, sm, NULL);
	da16x_eloop_cancel_timeout(ieee802_1x_cp_step_cb, sm, NULL);
	os_free(sm->lki);
	os_free(sm->oki);
	os_free(sm->cipher_suite);
	os_free(sm->current_cipher_suite);
...
void ieee802_1x_cp_sm_step(void *cp_ctx)
{
	...
	struct ieee802_1x_cp_sm *sm = cp_ctx;
	da16x_eloop_cancel_timeout(ieee802_1x_cp_step_cb, sm, NULL);
	da16x_eloop_register_timeout(0, 0, ieee802_1x_cp_step_cb, sm, NULL);
}
