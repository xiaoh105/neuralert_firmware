		       ...
		       const u8 *plain, u8 *crypt, size_t len)
{
	...
	case ENCR_AES_CBC:
		...
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "IKEV2: Unsupported encr alg %d", alg);
		return -1;
	}
			 ...
			 u8 next_payload, const u8 *pos, const u8 *end)
{
	...
	while (next_payload != IKEV2_PAYLOAD_NO_NEXT_PAYLOAD) {
		...
		const u8 *pdata;
		wpa_printf_dbg(MSG_DEBUG, "IKEV2: Processing payload %u",
			   next_payload);
		if (end < pos)
			return -1;
		left = end - pos;
		...
		wpa_printf_dbg(MSG_DEBUG, "IKEV2:   Next Payload: %u  Flags: 0x%x"
			   "  Payload Length: %u",
		...
		default:
			if (phdr->flags & IKEV2_PAYLOAD_FLAGS_CRITICAL) {
				...
				wpa_printf_dbg(MSG_DEBUG, "IKEV2:   Skipped "
			  ...
			  struct wpabuf *plain, u8 next_payload)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "IKEV2: Adding Encrypted payload");
