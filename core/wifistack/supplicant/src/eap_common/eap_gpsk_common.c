			 ...
			 u8 *pk, size_t *pk_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Deriving keys (%d:%d)",
		   vendor, specifier);
	...
	if (seed == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Failed to allocate memory "
			   "for key derivation");
		return -1;
	}
...
#endif /* EAP_GPSK_SHA256 */
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Unknown cipher %d:%d used in "
...
#endif /* EAP_GPSK_SHA256 */
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Unknown cipher %d used in "
			       ...
			       u8 method_type, u8 *sid, size_t *sid_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Deriving Session ID(%d:%d)",
		   vendor, specifier);
...
{
	if (sk_len != 16) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GPSK: Invalid SK length %lu for "
