			       ...
			       u8 *emsk)
{
	...
	while (identity_len > 0 && identity[identity_len - 1] == 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: Workaround - drop null "
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: Deriving keying data from reauth");
				      ...
				      size_t network_name_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-AKA': Derive (CK',IK') from (CK,IK)");
	wpa_hexdump_key(MSG_DEBUG, "EAP-AKA': CK", ck, EAP_AKA_CK_LEN);
	wpa_hexdump_key(MSG_DEBUG, "EAP-AKA': IK", ik, EAP_AKA_IK_LEN);
	wpa_printf_dbg(MSG_DEBUG, "EAP-AKA': FC = 0x%x", fc);
		       ...
		       struct eap_sim_attrs *attr, int aka, int encr)
{
	...
	while (pos < end) {
		...
		case EAP_SIM_AT_RAND:
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: AT_RAND");
			apos += 2;
			alen -= 2;
		...
		case EAP_SIM_AT_AUTN:
			wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: AT_AUTN");
			if (!aka) {
		...
		case EAP_SIM_AT_PADDING:
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: (encr) AT_PADDING");
			for (i = 2; i < alen; i++) {
				if (apos[i] != 0) {
		...
		case EAP_SIM_AT_NONCE_MT:
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: AT_NONCE_MT");
		...
		case EAP_SIM_AT_PERMANENT_ID_REQ:
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: AT_PERMANENT_ID_REQ");
			attr->id_req = PERMANENT_ID;
			break;
		case EAP_SIM_AT_MAC:
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: AT_MAC");
		...
		case EAP_SIM_AT_NOTIFICATION:
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: AT_NOTIFICATION %d",
				   attr->notification);
			break;
		case EAP_SIM_AT_ANY_ID_REQ:
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: AT_ANY_ID_REQ");
			attr->id_req = ANY_ID;
			break;
		case EAP_SIM_AT_IDENTITY:
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: AT_IDENTITY");
		...
		case EAP_SIM_AT_VERSION_LIST:
			if (aka) {
				wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: "
					   "Unexpected AT_VERSION_LIST");
				return -1;
			}
			list_len = apos[0] * 256 + apos[1];
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: AT_VERSION_LIST");
		...
		case EAP_SIM_AT_SELECTED_VERSION:
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: AT_SELECTED_VERSION");
			if (alen != 2) {
				wpa_printf(MSG_INFO, "EAP-SIM: Invalid "
		...
		case EAP_SIM_AT_FULLAUTH_ID_REQ:
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: AT_FULLAUTH_ID_REQ");
		...
		case EAP_SIM_AT_COUNTER:
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: (encr) AT_COUNTER %d",
				   attr->counter);
			break;
		case EAP_SIM_AT_COUNTER_TOO_SMALL:
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: (encr) "
				   "AT_COUNTER_TOO_SMALL");
		...
		case EAP_SIM_AT_CLIENT_ERROR_CODE:
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: AT_CLIENT_ERROR_CODE "
				   "%d", attr->client_error_code);
			break;
		case EAP_SIM_AT_IV:
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: AT_IV");
			if (alen != 2 + EAP_SIM_MAC_LEN) {
		...
		case EAP_SIM_AT_ENCR_DATA:
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: AT_ENCR_DATA");
		...
		case EAP_SIM_AT_NEXT_REAUTH_ID:
			...
			break;
		case EAP_SIM_AT_RES:
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: AT_RES");
		...
		case EAP_SIM_AT_AUTS:
			wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: AT_AUTS");
			if (!aka) {
				wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: "
					   "Unexpected AT_AUTS");
				return -1;
		...
		case EAP_SIM_AT_CHECKCODE:
			wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: AT_CHECKCODE");
			if (!aka) {
				wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: "
		...
		case EAP_SIM_AT_RESULT_IND:
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: AT_RESULT_IND");
			attr->result_ind = 1;
			break;
#if defined(EAP_AKA_PRIME) || defined(EAP_SERVER_AKA_PRIME)
		case EAP_SIM_AT_KDF_INPUT:
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: AT_KDF_INPUT");
			plen = WPA_GET_BE16(apos);
			apos += 2;
			alen -= 2;
			if (plen > alen) {
		...
		case EAP_SIM_AT_KDF:
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: AT_KDF");
			if (alen != 2) {
				wpa_printf(MSG_INFO, "EAP-AKA': Invalid "
					   "AT_KDF (len %lu)",
					   (unsigned long) alen);
			...
			if (attr->kdf_count == EAP_AKA_PRIME_KDF_MAX) {
				wpa_printf_dbg(MSG_DEBUG, "EAP-AKA': Too many "
		...
		case EAP_SIM_AT_BIDDING:
			wpa_printf_dbg(MSG_DEBUG, "EAP-AKA: AT_BIDDING");
			if (alen != 2) {
				wpa_printf(MSG_INFO, "EAP-AKA: Invalid "
					   "AT_BIDDING (len %lu)",
					   (unsigned long) alen);
		...
		default:
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: Unrecognized skippable"
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SIM: Attributes parsed successfully "
...
int eap_sim_msg_add_encr_end(struct eap_sim_msg *msg, u8 *k_encr, int attr_pad)
{
	...
	if (encr_len % 16) {
		...
		wpa_printf_dbg(MSG_DEBUG, "   *AT_PADDING");
	...
	wpa_printf_dbg(MSG_DEBUG, "   (AT_ENCR_DATA data len %lu)",
