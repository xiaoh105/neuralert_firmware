...
#if 0
#include "qca-vendor.h" // Qualcomm Atheros OUI and vendor specific assignments
#endif /* 0 */
...
#else
			da16x_scan_prt("[%s] Unknown Microsoft "
					"information element ignored "
					"(type=%d len=%lu)\n",
					__func__,
					pos[3], (unsigned long) elen);
	...
	case OUI_BROADCOM:
		...
		default:
			...
			wpa_printf(MSG_EXCESSIVE, "Unknown Broadcom "
				   ...
				   pos[3], (unsigned long) elen);
#else
			da16x_scan_prt("[%s] Unknown Broadcom "
                   "information element ignored "
                   "(type=%d len=%lu)\n",
                   __func__, pos[3], (unsigned long) elen);
#endif	/* __SUPP_27_SUPPORT__ */
			return -1;
		}
		break;

#ifdef CONFIG_QCA_VENDOR
	case OUI_QCA:
		switch (pos[3]) {
		...
		break;
#endif /* CONFIG_QCA_VENDOR */
#ifdef FEATURE_MESH_STAR_LINK_CONTROL_ADDED
		case OUI_DIALOG:
			switch (pos[3]) {
			case WLAN_EID_MESH_LINK_METRIC_REPORT:
				//PRINTF("[%s]WLAN_EID_MESH_LINK_METRIC_REPORT\n", __func__);
				elems->mesh_star_link = pos;
				elems->mesh_star_link_len = elen;			
				break;
			default:
				wpa_printf(MSG_EXCESSIVE,
					   "Unknown QCA information element ignored (type=%d len=%lu)",
...
#else
		da16x_scan_prt("[%s] unknown vendor specific "
				"information element ignored (vendor OUI "
				"%02x:%02x:%02x len=%lu)\n",
				__func__,
				pos[0], pos[1], pos[2], (unsigned long) elen);
...
/**
 * ieee802_11_parse_elements - Parse information elements in management frames
 ...
 */
ParseRes ieee802_11_parse_elements(const u8 *start, size_t len,
				struct ieee802_11_elems *elems,
				int show_errors)
{
	size_t left = len;
	...
	while (left >= 2) {
		...
		if (elen > left) {
			if (show_errors) {
				wpa_printf_dbg(MSG_DEBUG, "IEEE 802.11 element "
		...
		case WLAN_EID_HT_OPERATION:
			if (elen < sizeof(struct _ieee80211_ht_operation))
				break;
		...
		default:
			...
			wpa_printf(MSG_MSGDUMP, "IEEE 802.11 element parse "
				   ...
				   id, elen);
#else
			da16x_scan_prt("[%s] IEEE 802.11 element parse "
                   "ignored unknown element (id=%d elen=%d)\n",
                __func__,
                id, elen);
#endif	/* __SUPP_27_SUPPORT__ */
		       ...
		       size_t ies_len)
{
	...
	while (ies_buf && ies_len >= 2 &&
		...
		if (ies_buf[0] == WLAN_EID_MULTI_BAND) {
			wpa_printf_dbg(MSG_DEBUG, "MB IE of %zu bytes found", len);
...
const struct oper_class_map global_op_class[] = {
	...
	{ (enum hostapd_hw_mode)-1, 0, 0, 0, 0, BW20, NO_P2P_SUPP }
};
	...
	while (pos) {
		...
		*nei_pos++ = WLAN_EID_NEIGHBOR_REPORT;
		...
		if (hwaddr_aton(pos, nei_pos)) {
			wpa_printf_dbg(MSG_DEBUG, "Invalid BSSID");
			return -1;
		}
		nei_pos += ETH_ALEN;
		pos += 17;
		if (*pos != ',') {
			wpa_printf_dbg(MSG_DEBUG, "Missing BSSID Information");
			return -1;
		}
		pos++;
		...
		nei_pos += 4;
		if (*endptr != ',') {
			wpa_printf_dbg(MSG_DEBUG, "Missing Operating Class");
			return -1;
		}
		pos = endptr + 1;
		...
		*nei_pos++ = atoi(pos); /* Operating Class */
		pos = os_strchr(pos, ',');
		if (pos == NULL) {
			wpa_printf_dbg(MSG_DEBUG, "Missing Channel Number");
			return -1;
		}
		pos++;
		...
		*nei_pos++ = atoi(pos); /* Channel Number */
		pos = os_strchr(pos, ',');
		if (pos == NULL) {
			wpa_printf_dbg(MSG_DEBUG, "Missing PHY Type");
			return -1;
		}
		pos++;
