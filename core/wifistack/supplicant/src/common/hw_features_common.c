...
void get_pri_sec_chan(struct wpa_scan_res *bss, int *pri_chan, int *sec_chan)
{
	...
	*pri_chan = *sec_chan = 0;
	...
	ieee802_11_parse_elements((u8 *) (bss->ie), bss->ie_len, &elems, 0);
#else
	ieee802_11_parse_elements((u8 *) (bss + 1), bss->ie_len, &elems, 0);
#endif
			   ...
			   int end)
{
	...
	if (!elems.ht_capabilities) {
		wpa_printf_dbg(MSG_DEBUG, "Found overlapping legacy BSS: "
	...
	if (elems.ht_operation) {
		...
		wpa_printf_dbg(MSG_DEBUG, "Found overlapping 20 MHz HT BSS: "
			   MACSTR " freq=%d", MAC2STR(bss->bssid), bss->freq);
		return 1;
	}
	return 0;
		    ...
		    int sec_chan)
{
	...
	for (i = 0; i < scan_res->num; i++) {
		...
		wpa_printf_dbg(MSG_DEBUG, "Neighboring BSS: " MACSTR
			   " freq=%d pri=%d sec=%d",
		...
		ieee802_11_parse_elements((u8 *) (bss->ie), bss->ie_len, &elems, 0);
#else
		ieee802_11_parse_elements((u8 *) (bss + 1), bss->ie_len, &elems,
				       0);
#endif
		if (elems.ht_capabilities) {
			    ...
			    int center_segment1, u32 vht_caps)
{
	...
	data->cent_freq1 = freq + sec_channel_offset * 10;
	data->cent_freq2 = 0;
	...
	if (data->vht_enabled) {
		switch (vht_oper_chwidth) {
			case VHT_CHANWIDTH_USE_HT:
				if (center_segment1 ||
				    (center_segment0 != 0 &&
				     5000 + center_segment0 * 5 != data->cent_freq1 &&
				     2407 + center_segment0 * 5 != data->cent_freq1))
					return -1;
				break;
			case VHT_CHANWIDTH_80P80MHZ:
				...
				if (center_segment1 == center_segment0 + 4 ||
					...
					return -1;
				data->cent_freq2 = 5000 + center_segment1 * 5;
				/* fall through */
			case VHT_CHANWIDTH_80MHZ:
				...
				if (!center_segment0) {
					if (channel <= 48)
						...
						center_segment0 = 155;
					data->cent_freq1 = 5000 + center_segment0 * 5;
				} else {
					/*
