 ...
 * @brief	IEEE 802.11 Frame type definitions from wpa_supplicant-2.7
 *****************************************************************************************
 */
		...
		struct {
			...
			union {
				...
				struct {
					...
					le16 status_code;
					u8 variable[0]; /* FT Request */
				}  __packed ft_action_resp;
				struct {
					u8 action;
					u8 trans_id[WLAN_SA_QUERY_TR_ID_LEN];
				}  __packed sa_query_req;
				struct {
					u8 action; /* */
					u8 trans_id[WLAN_SA_QUERY_TR_ID_LEN];
				}  __packed sa_query_resp;
				struct {
					u8 action;
					u8 dialogtoken;
					u8 variable[0];
				}  __packed wnm_sleep_req;
				struct {
					u8 action;
					u8 dialogtoken;
					le16 keydata_len;
					u8 variable[0];
				}  __packed wnm_sleep_resp;
				struct {
					u8 action;
					u8 variable[0];
				...
				struct {
					 ...
					 * Entries (optional) */
					u8 variable[0];
				}  __packed bss_tm_query;
				struct {
					u8 action; /* 15 */
					u8 variable[0];
				}  __packed slf_prot_action;
				struct {
					u8 action;
					u8 variable[0];
				}  __packed fst_action;
...
struct _ieee80211_ht_operation {
	u8 primary_chan;
...
#define OUI_APPLE	0x0017f2
#define OUI_DIALOG	0xd43d39 
