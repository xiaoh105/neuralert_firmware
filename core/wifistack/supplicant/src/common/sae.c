...
int sae_set_group(struct sae_data *sae, int group)
{
	...
	if (tmp->ec) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Selecting supported ECC group %d",
			   group);
		sae->group = group;
...
#ifndef	__DISABLE_SAE_FFC__	
	/* Not an ECC group, check FFC */
	tmp->dh = dh_groups_get(group);
	if (tmp->dh) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Selecting supported FFC group %d",
			   group);
		sae->group = group;
...
{
	wpa_printf_dbg(MSG_DEBUG, "SAE: PWE derivation - addr1=" MACSTR
...
{
#if 1 /* Use H/W engine directly. */
	struct crypto_bignum *r = NULL;
	int r_odd, res = -1;
#else
...
#if 1 /* Use H/W engine directly. */
	res = crypto_bignum_is_quadratic_residue_blind(r, sae->tmp->prime, 
						qr, qnr, y_sqr, r_odd);
				 ...
				 struct crypto_bignum *pwe)
{
	...
	{
		wpa_printf_dbg(MSG_DEBUG, "SAE: pwd-value >= p");
		return 0;
	}
	...
	if (res < 0) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Failed to calculate PWE");
		return -1;
	}

	/* if (PWE > 1) --> found */
	if (crypto_bignum_is_zero(pwe) || crypto_bignum_is_one(pwe)) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: PWE <= 1");
		return 0;
	}

	wpa_printf_dbg(MSG_DEBUG, "SAE: PWE found");
	return 1;
}
#endif	/* __DISABLE_SAE_FFC__ */
			      ...
			      size_t password_len, const char *identifier)
{
	...
	if (identifier)
		wpa_printf_dbg(MSG_DEBUG, "SAE: password identifier: %s",
			   identifier);
	...
	for (counter = 1; counter <= k || !x; counter++) {
		...
		if (counter > 200) {
			...
			wpa_printf_dbg(MSG_DEBUG, "SAE: Failed to derive PWE");
			break;
		}

//		wpa_printf_dbg(MSG_DEBUG, "SAE: counter = %u", counter);
	...
	if (!x) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Could not generate PWE");
		res = -1;
		goto fail;
	}
	...
	if (res < 0) {
		 ...
		 * is a result.
		 */
		wpa_printf_dbg(MSG_DEBUG, "SAE: Could not solve y");
	}
...
//		wpa_printf_dbg(MSG_DEBUG, "SAE: counter = %u", counter);
		...
		if (res < 0)
			break;
		if (res > 0) {
			wpa_printf_dbg(MSG_DEBUG, "SAE: Use this PWE");
			found = 1;
		}
	}
					 ...
					 struct crypto_bignum *mask)
{
	...
	if (crypto_ec_point_mul(sae->tmp->ec, sae->tmp->pwe_ecc, mask,
	    ...
	    crypto_ec_point_invert(sae->tmp->ec,
				   sae->tmp->own_commit_element_ecc) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Could not compute commit-element");
		return -1;
	}
...
static int sae_derive_commit(struct sae_data *sae)
{
	...
	do {
		...
		if (mask == NULL) {
			wpa_printf_dbg(MSG_DEBUG, "SAE: Could not get rand/mask");
			return -1;
		}
		       ...
		       const char *identifier, struct sae_data *sae)
{
	...
	if (sae->tmp == NULL ||
		...
		wpa_printf(MSG_WARNING, "[%s] error", __func__);
		return -1;
	    }
#else
	if (sae->tmp == NULL ||
...
static int sae_derive_k_ecc(struct sae_data *sae, u8 *k)
{
	...
	if (crypto_ec_point_mul(sae->tmp->ec, sae->tmp->pwe_ecc,
		...
		wpa_printf_dbg(MSG_DEBUG, "SAE: Failed to calculate K and k");
		goto fail;
	}
...
int sae_process_commit(struct sae_data *sae)
{
	...
	if (sae->tmp == NULL ||
		...
		wpa_printf(MSG_WARNING, "[%s] error\n", __func__);
		return -1;
	}
#else
	if (sae->tmp == NULL ||
		      ...
		      const struct wpabuf *token, const char *identifier)
{
	...
	if (identifier) {
		...
		wpa_printf_dbg(MSG_DEBUG, "SAE: own Password Identifier: %s",
			   identifier);
	}
...
u16 sae_group_allowed(struct sae_data *sae, int *allowed_groups, u16 group)
{
	...
	if (allowed_groups) {
		...
		if (allowed_groups[i] != group) {
			wpa_printf_dbg(MSG_DEBUG, "SAE: Proposed group %u not "
	...
	if (sae->state == SAE_COMMITTED && group != sae->group) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Do not allow group to be changed");
		return WLAN_STATUS_FINITE_CYCLIC_GROUP_NOT_SUPPORTED;
	}

	if (group != sae->group && sae_set_group(sae, group) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Unsupported Finite Cyclic Group %u",
			   group);
		return WLAN_STATUS_FINITE_CYCLIC_GROUP_NOT_SUPPORTED;
	}

	if (sae->tmp == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Group information not yet initialized");
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}

#ifndef	__DISABLE_SAE_FFC__
	if (sae->tmp->dh && !allowed_groups) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Do not allow FFC group %u without "
				   ...
				   const u8 *end)
{
	...
	if (sae->tmp->prime_len > end - *pos) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Not enough data for scalar");
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
	...
	if (sae->state == SAE_ACCEPTED && sae->peer_commit_scalar &&
		...
		wpa_printf_dbg(MSG_DEBUG, "SAE: Do not accept re-use of previous "
			   "peer-commit-scalar");
	...
	if (crypto_bignum_is_zero(peer_scalar) ||
		...
		wpa_printf_dbg(MSG_DEBUG, "SAE: Invalid peer scalar");
		crypto_bignum_deinit(peer_scalar, 0);
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
					...
					const u8 *end)
{
	...
	if (2 * sae->tmp->prime_len > end - *pos) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Not enough data for "
			   "commit-element");
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
	...
	if (os_memcmp(*pos, prime, sae->tmp->prime_len) >= 0 ||
	    os_memcmp(*pos + sae->tmp->prime_len, prime,
		      sae->tmp->prime_len) >= 0) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Invalid coordinates in peer "
			   "element");
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
	...
	if (!crypto_ec_point_is_on_curve(sae->tmp->ec,
					 sae->tmp->peer_commit_element_ecc)) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Peer element is not on curve");
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
					...
					const u8 *end)
{
	...
	if (!res || !one ||
		...
		wpa_printf_dbg(MSG_DEBUG, "SAE: Invalid peer element");
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
	crypto_bignum_deinit(one, 0);
	...
	if (crypto_bignum_exptmod(sae->tmp->peer_commit_element_ffc,
		...
		wpa_printf_dbg(MSG_DEBUG, "SAE: Invalid peer element (scalar-op)");
		crypto_bignum_deinit(res, 0);
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
	crypto_bignum_deinit(res, 0);
...
void sae_write_confirm(struct sae_data *sae, struct wpabuf *buf)
{
	...
	//wpa_printf_dbg(MSG_DEBUG, "[%s] time = %d ms\n",  __func__, tx_time_get()*10);
	...
	if (sae->tmp->ec) {
		//wpa_printf_dbg(MSG_DEBUG, "[%s] ECC\n", __func__);
...
#ifndef	__DISABLE_SAE_FFC__
		//wpa_printf_dbg(MSG_DEBUG, "[%s] FFC\n", __func__);
...
int sae_check_confirm(struct sae_data *sae, const u8 *data, size_t len)
{
	...
	if (len < 2 + SHA256_MAC_LEN) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Too short confirm message");
		return -1;
	}

	wpa_printf_dbg(MSG_DEBUG, "SAE: peer-send-confirm %u", WPA_GET_LE16(data));

	if (sae->tmp == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Temporary data not yet available");
		return -1;
	}
...
#if 0
	wpa_hexdump(MSG_DEBUG, "SAE: peer_commit_scalar", sae->peer_commit_scalar, 64);
	wpa_hexdump(MSG_DEBUG, "SAE: peer_commit_element_ecc", sae->tmp->peer_commit_element_ecc, 64);
	wpa_hexdump(MSG_DEBUG, "SAE: own_commit_scalar", sae->tmp->own_commit_scalar, 64);
	wpa_hexdump(MSG_DEBUG, "SAE: own_commit_element_ecc", sae->tmp->own_commit_element_ecc, 64);
#endif /* 0 */

	if (os_memcmp_const(verifier, data + 2, SHA256_MAC_LEN) != 0) {
		wpa_printf(MSG_WARNING, "SAE: Confirm mismatch");
...
int sae_is_duplicate_confirm(struct sae_data *sae, const u8 *data, size_t len)
{
	const u8 *pos = data, *end = data + len;
	...
	const u8 *token = NULL;
	size_t token_len = NULL;

	int is_password_id_elem = 0;

	RX_FUNC_START("");

	//check packet length
	if (end - pos < 2) {
		wpa_printf_dbg(MSG_DEBUG, "[%s:%d]invalid packet size",
			__func__, __LINE__);
		return 0;
	}

	//check group
	if (sae->group != WPA_GET_LE16(pos)) {
		wpa_printf_dbg(MSG_DEBUG, "[%s:%d]Not match group id(%d,%d)",
			__func__, __LINE__,
			sae->group, WPA_GET_LE16(pos));
		return 0;
	}
	...
	/* check peer-scalar */
	if (sae->tmp->prime_len > end - pos) {
		wpa_printf_dbg(MSG_DEBUG, "[%s:%d]Not enough data for scalar",
			__func__, __LINE__);
		return 0;
	}
	...
	peer_scalar = crypto_bignum_init_set(pos, sae->tmp->prime_len);
	if (peer_scalar == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "[%s:%d]failed to get peer scalar",
			__func__, __LINE__);
		return 0;
	}
	...
	if (!(sae->peer_commit_scalar
		&& crypto_bignum_cmp(sae->peer_commit_scalar, peer_scalar) == 0)) {
		wpa_printf_dbg(MSG_DEBUG, "[%s:%d]Not match peer commmit scalar",
			__func__, __LINE__);
		return 0;
	}
	...
	//check commit-element
#ifndef	__DISABLE_SAE_FFC__
	if (sae->tmp->dh) {
		wpa_printf_dbg(MSG_DEBUG, "[%s:%d]Not implemented for ffc",
			__func__, __LINE__);
		return 0;
	}
#endif	/* __DISABLE_SAE_FFC__ */
	...
	if (2 * sae->tmp->prime_len > end - pos) {
		wpa_printf_dbg(MSG_DEBUG, "[%s:%d]Not enough data for commit-element",
			__func__, __LINE__);
		return 0;
	}
	...
	if (crypto_bignum_to_bin(sae->tmp->prime, prime, sizeof(prime),
				sae->tmp->prime_len) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "[%s:%d]failed to get prime",
			__func__, __LINE__);
		return 0;
	}
	...
	if (os_memcmp(pos, prime, sae->tmp->prime_len) >= 0 ||
		...
		wpa_printf_dbg(MSG_DEBUG, "[%s:%d]Invalid coordinates in peer element",
			__func__, __LINE__);
		return 0;
	}

	peer_commit_element_ecc = crypto_ec_point_from_bin(sae->tmp->ec, pos);

	if (!(sae->tmp->peer_commit_element_ecc
		&& peer_commit_element_ecc
		&& crypto_ec_point_cmp(sae->tmp->ec,
			sae->tmp->peer_commit_element_ecc,
			peer_commit_element_ecc) == 0)) {
		wpa_printf_dbg(MSG_DEBUG, "[%s:%d]Not match peer commit element",
			__func__, __LINE__);
		return 0;
	}
	...
	/* Optional Password Identifier element */
	is_password_id_elem = sae_is_password_id_elem(pos, end);
	if (is_password_id_elem && sae->tmp->pw_id) {
		//compare pw element
		if (!(pos[1] - 1 == (int) os_strlen(sae->tmp->pw_id)
			|| os_memcmp(sae->tmp->pw_id, pos + 3, pos[1] - 1) == 0)) {
			wpa_printf_dbg(MSG_DEBUG, "[%s:%d]Not match password identifier",
				__func__, __LINE__);
			return 0;
		}
	} else if ((is_password_id_elem && sae->tmp->pw_id == NULL)
		|| (!is_password_id_elem && sae->tmp->pw_id)) {
		wpa_printf_dbg(MSG_DEBUG, "[%s:%d]Not match password identifier element",
			__func__, __LINE__);
		return 0;
	}
