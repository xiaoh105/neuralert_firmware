			      ...
			      int key_mgmt)
{
	...
	if (suite == 0) {
		wpa_printf(MSG_WARNING, "[%s] Invalid group cipher (%d).", __func__,
			   group_cipher);
		return -1;
	}
	...
	*pos++ = 0;
	...
	if (suite == 0 ||
		...
		wpa_printf(MSG_WARNING, "[%s] Invalid pairwise cipher (%d).", __func__,
			   pairwise_cipher);
		return -1;
	}
	RSN_SELECTOR_PUT(pos, suite);
	...
	*pos++ = 0;
	if (key_mgmt == WPA_KEY_MGMT_IEEE8021X) {
		...
		wpa_printf(MSG_WARNING, "[%s] Invalid key management type (%d).", __func__,
			   key_mgmt);
		return -1;
	}
	pos += WPA_SELECTOR_LEN;
			      ...
			      struct wpa_sm *sm)
{
	...
	if (rsn_ie_len < sizeof(*hdr) + RSN_SELECTOR_LEN +
		...
		wpa_printf_dbg(MSG_DEBUG, "RSN: Too short IE buffer (%lu bytes)",
			   (unsigned long) rsn_ie_len);
#else
		da16x_err_prt("[%s] RSN: Too short IE buffer (%lu bytes)\n",
	...
	if (suite == 0) {
		...
		wpa_printf(MSG_WARNING, "Invalid group cipher (%d).",
			   group_cipher);
#else
		da16x_err_prt("[%s] Invalid group cipher (%d)\n",
               __func__, group_cipher);
#endif	/* CONFIG_DBG_LOG_MSG */
		return -1;
	}
	RSN_SELECTOR_PUT(pos, suite);
	...
	*pos++ = 0;
	...
	if (suite == 0 ||
		...
		wpa_printf(MSG_WARNING, "[%s] Invalid pairwise cipher (%d).", __func__,
			   pairwise_cipher);
#else
		da16x_warn_prt("[%s] Invalid key management type (%d)\n",
               __func__, key_mgmt);
	...
	*pos++ = 0;
	if (key_mgmt == WPA_KEY_MGMT_IEEE8021X) {
		...
		wpa_printf(MSG_WARNING, "[%s] Invalid key management type (%d).", __func__,
			   key_mgmt);
#else
		da16x_err_prt("[%s] Invalid key management type (%d)\n",
               __func__, key_mgmt);
#endif	/* CONFIG_DBG_LOG_MSG */
		return -1;
	}
	...
	if (oui == OUI_MICROSOFT && pos[5] == WMM_OUI_TYPE && pos[1] > 4) {
		if (pos[6] == WMM_OUI_SUBTYPE_INFORMATION_ELEMENT) {
			...
			wpa_hexdump(MSG_DEBUG, "WPA: WMM IE",
				    ie->wmm, ie->wmm_len);
#else
			da16x_dump("WPA: WMM IE", ie->wmm, ie->wmm_len);
#endif	/* CONFIG_DBG_LOG_MSG */
			...
			wpa_hexdump(MSG_DEBUG, "WPA: WMM Parameter Element",
				    ie->wmm, ie->wmm_len);
#else
			da16x_dump("WPA: WMM Parameter Element",
                        ie->wmm, ie->wmm_len);
#endif	/* CONFIG_DBG_LOG_MSG */
		}
	}
			     ...
			     struct wpa_eapol_ie_parse *ie)
{
	...
	for (pos = buf, end = pos + len; end - pos > 1; pos += 2 + pos[1]) {
		...
		if (2 + pos[1] > end - pos) {
#ifdef	CONFIG_DBG_LOG_MSG
			wpa_printf_dbg(MSG_DEBUG, "WPA: EAPOL-Key Key Data "
				   "underflow (ie=%d len=%d pos=%d)",
				   pos[0], pos[1], (int) (pos - buf));
			wpa_hexdump_key(MSG_DEBUG, "WPA: Key Data",
					buf, len);
#else
			da16x_err_prt("[%s] EAPOL-Key Key Data "
                   "underflow (ie=%d len=%d pos=%d)\n",
                   __func__, pos[0], pos[1], (int) (pos - buf));
            da16x_dump("WPA: Key Data", buf, len);
#endif	/* CONFIG_DBG_LOG_MSG */
			ret = -1;
			break;
		}
...
#else
			da16x_dump("WPA: RSN IE in EAPOL-Key",
                        ie->rsn_ie, ie->rsn_ie_len);
#endif	/* CONFIG_DBG_LOG_MSG */
#ifdef CONFIG_IEEE80211R
...
#else
				da16x_dump("WPA: Reassoc Deadline in EAPOL-Key",
                        ie->reassoc_deadline, pos[1] + 2);
#endif	/* CONFIG_DBG_LOG_MSG */
...
#else
				da16x_dump("WPA: KeyLifetime in EAPOL-Key",
                        ie->key_lifetime, pos[1] + 2);
#endif	/* CONFIG_DBG_LOG_MSG */
			} else {
#ifdef	CONFIG_DBG_LOG_MSG
