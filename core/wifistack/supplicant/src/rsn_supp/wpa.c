...
#undef  DEBUG_DPM_KEY_BACKUP

#ifdef DEBUG_DPM_KEY_BACKUP
#include "common_utils.h"
#endif // DEBUG_DPM_KEY_BACKUP
		       ...
		       u8 *msg, size_t msg_len, u8 *key_mic)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "WPA: Send EAPOL-Key frame to " MACSTR
			       ...
			       struct wpa_ptk *ptk)
{
	...
	if (wpa_ie == NULL) {
		wpa_dbg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: No wpa_ie set - "
					  ...
					  size_t key_data_len)
{
	...
	if (wpa_sm_get_network_ctx(sm) == NULL) {
		wpa_dbg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: No SSID info "
			"found (msg 1 of 4)");
		return;
	}
	...
	if (sm->p2p) {
		...
		if (kde_buf) {
			u8 *pos;
			wpa_printf_dbg(MSG_DEBUG, "P2P: Add IP Address Request KDE "
				   "into EAPOL-Key 2/4");
...
#if 1   /* by Shingu 20170713 (4HS ALL FAIL) */
	sm->cnt_hs_3rd = 0;
#endif	/* 1 */
					    ...
					    const u8 *addr, int secure)
{
...
#else
	da16x_wpa_prt("[%s] Key negotiation completed with "
		MACSTR " [PTK=%s GTK=%s]\n",
	...
	if (secure) {
		 ...
		 * the target AP.
		 */
		da16x_eloop_register_timeout(1, 0, wpa_sm_start_preauth, sm, NULL);
#endif	/* CONFIG_PRE_AUTH */
	}
				      ...
				      const struct wpa_eapol_key *key)
{
	...
	if (keylen <= 0 || (unsigned int) keylen != sm->ptk.tk_len) {
		wpa_printf_dbg(MSG_DEBUG, "WPA: TK length mismatch: %d != %lu",
	...
	wpa_supp_dpm_backup_key_info(KEY_PTK, alg, 0, 1, key_rsc, rsclen, sm); // Backup for PTK
	...
	if (sm->wpa_ptk_rekey) {
		da16x_eloop_cancel_timeout(wpa_sm_rekey_ptk, sm, NULL);
		da16x_eloop_register_timeout(sm->wpa_ptk_rekey, 0, wpa_sm_rekey_ptk,
				       sm, NULL);
	}
				      ...
				      const u8 *key_rsc, int wnm_sleep)
{
	...
	wpa_supp_dpm_backup_key_info(KEY_GTK, gd->alg, gd->keyidx, gd->tx, key_rsc, gd->key_rsc_len, sm);	// Backup for GTK
	return 0;
}
			       ...
			       struct wpa_eapol_ie_parse *ie)
{
	...
	if (ie->igtk) {
		...
		da16x_wpa_prt("[%s] IGTK keyid %d pn %02x%02x%02x%02x%02x%02x\n",
			__func__, keyidx, MAC2STR(igtk->pn));

		da16x_dump("IGTK", igtk->igtk, len);

		if (keyidx > 4095) {
			da16x_wpa_prt("[%s] Invalid IGTK KeyID %d\n",
						__func__, keyidx);
			return -1;
		}
		...
		if (wpa_sm_set_key(sm, wpa_cipher_to_alg(sm->mgmt_group_cipher),
				   ...
				   keyidx, 0, igtk->pn, sizeof(igtk->pn),
				   igtk->igtk, len) < 0) {
			da16x_wpa_prt("[%s] Failed to configure IGTK to the driver\n",
				__func__);
			return -1;
		}
#else /* WPA2_PATCH_201701 */
...
{
	wpa_dbg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: %s (src=" MACSTR ")",
...
#if 1   /* by Shingu 20170713 (4HS ALL FAIL) */
#define MAX_3RD_HS_WAIT_COUNT 4
#endif	/* 1 */
...
#ifndef WPA2_PATCH_201701   /* by Shingu 20170713 (4HS ALL FAIL) */ /* 20171120 - block for Krack patch , trinity*/
	sm->cnt_hs_3rd++;
	if (sm->cnt_hs_3rd >= MAX_3RD_HS_WAIT_COUNT) {
		da16x_notice_prt("<%s> Processing 4/4 Handshake message FAIL "
				"on AP\n", __func__);
		sm->cnt_hs_3rd = 0;
		goto failed;
					   ...
					   u8 *key_data, size_t *key_data_len)
{
	...
	if (ver == WPA_KEY_INFO_TYPE_HMAC_MD5_RC4 && sm->ptk.kek_len == 16) {
		...
		wpa_printf_dbg(MSG_DEBUG, "WPA: Decrypt Key Data using RC4");
		os_memcpy(ek, key->key_iv, 16);
		os_memcpy(ek + 16, sm->ptk.kek, sm->ptk.kek_len);
...
struct wpa_sm * wpa_sm_init(struct wpa_sm_ctx *ctx)
{
    ...
    wpa_conf = (struct wpa_config*)(((struct wpa_supplicant*)(ctx->ctx))->conf);
    sm->dot11RSNAConfigPMKLifetime = wpa_conf->dot11RSNAConfigPMKLifetime;
...
#ifdef	CONFIG_PRE_AUTH
	da16x_eloop_cancel_timeout(wpa_sm_start_preauth, sm, NULL);
	da16x_eloop_cancel_timeout(wpa_sm_rekey_ptk, sm, NULL);
#endif	/* CONFIG_PRE_AUTH */
...
void wpa_sm_set_pmk_from_pmksa(struct wpa_sm *sm)
{
	...
	if (sm->cur_pmksa) {
		...
		wpa_printf_dbg(MSG_DEBUG, "WPA: No current PMKSA - clear PMK");
		sm->pmk_len = 0;
	...
	if (sm->mfp != NO_MGMT_FRAME_PROTECTION && sm->ap_rsn_ie) {
		...
		if (wpa_parse_wpa_ie_rsn(sm->ap_rsn_ie, sm->ap_rsn_ie_len, &rsn)
		    ...
		    rsn.capabilities & (WPA_CAPABILITY_MFPR |
					WPA_CAPABILITY_MFPC)) {
			ret = os_snprintf(pos, end - pos, "pmf=%s\n"
					  "mgmt_group_cipher=%s\n",
					  pmf_mode_txt((rsn.capabilities & WPA_CAPABILITY_MFPR) ? 2 : 1),
					  wpa_cipher_txt(sm->mgmt_group_cipher));
			if (os_snprintf_error(end - pos, ret))
				return pos - buf;
			pos += ret;
		}
...
int wpa_sm_set_ap_wpa_ie(struct wpa_sm *sm, const u8 *ie, size_t len)
{
	...
	os_free(sm->ap_wpa_ie);
	if (ie == NULL || len == 0) {
		da16x_wpa_prt("[%s] clearing AP WPA IE\n", __func__);
		sm->ap_wpa_ie = NULL;
		sm->ap_wpa_ie_len = 0;
	} else {
		da16x_dump("WPA: set AP WPA IE", ie, len);
...
int wpa_sm_set_ap_rsn_ie(struct wpa_sm *sm, const u8 *ie, size_t len)
{
	...
	os_free(sm->ap_rsn_ie);
	if (ie == NULL || len == 0) {
		da16x_wpa_prt("[%s] clearing AP RSN IE\n", __func__);

		sm->ap_rsn_ie = NULL;
		sm->ap_rsn_ie_len = 0;
	} else {
		da16x_dump("WPA: set AP RSN IE", ie, len);
...
int wpa_wnmsleep_install_key(struct wpa_sm *sm, u8 subelem_id, u8 *buf)
{
	...
	if (subelem_id == WNM_SLEEP_SUBELEM_GTK) {
		...
		if (gd.alg == WPA_ALG_NONE) {
			wpa_printf_dbg(MSG_DEBUG, "Unsupported group cipher suite");
			return -1;
		}
		...
		gd.gtk_len = keylen;
		if (gd.gtk_len != buf[4]) {
			wpa_printf_dbg(MSG_DEBUG, "GTK len mismatch len %d vs %d",
				   gd.gtk_len, buf[4]);
			return -1;
		}
		...
		if (wpa_supplicant_install_gtk(sm, &gd, key_rsc, 1)) {
			os_memset(&gd, 0, sizeof(gd));
			wpa_printf_dbg(MSG_DEBUG, "Failed to install the GTK in "
				   "WNM mode");
			return -1;
		}
		os_memset(&gd, 0, sizeof(gd));
		...
		wpa_printf_dbg(MSG_DEBUG, "Unknown element id");
		return -1;
	}
...
void wpa_sm_set_rx_replay_ctr(struct wpa_sm *sm, const u8 *rx_replay_counter)
{
	...
	os_memcpy(sm->rx_replay_counter, rx_replay_counter,
		  WPA_REPLAY_COUNTER_LEN);
	sm->rx_replay_counter_set = 1;
	wpa_printf_dbg(MSG_DEBUG, "Updated key replay counter");
}
...
struct wpabuf * fils_build_auth(struct wpa_sm *sm, int dh_group, const u8 *md)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FILS: Try to use FILS (erp=%d pmksa_cache=%d)",
		      ...
		      size_t len)
{
	...
	if (sm->fils_dh_group) {
		...
		if (!sm->fils_ecdh) {
			wpa_printf_dbg(MSG_DEBUG, "FILS: No ECDH state available");
			goto fail;
		}
	...
	if (!elems.rsn_ie ||
	    wpa_parse_wpa_ie_rsn(elems.rsn_ie - 2, elems.rsn_ie_len + 2,
				 &rsn) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: No RSN element");
		goto fail;
	}
	...
	if (wpa_key_mgmt_ft(sm->key_mgmt)) {
		...
		if (!elems.mdie || !elems.ftie) {
			wpa_printf_dbg(MSG_DEBUG, "FILS+FT: No MDE or FTE");
			goto fail;
		}

		if (wpa_ft_parse_ies(pos, end - pos, &parse,
				     wpa_key_mgmt_sha384(sm->key_mgmt)) < 0) {
			wpa_printf_dbg(MSG_DEBUG, "FILS+FT: Failed to parse IEs");
			goto fail;
		}
	...
	if (os_memcmp(sm->fils_session, elems.fils_session, FILS_SESSION_LEN)
	    != 0) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: Session mismatch");
	...
	if (!sm->cur_pmksa && elems.fils_wrapped_data) {
		...
		if (!sm->fils_erp_pmkid_set) {
			wpa_printf_dbg(MSG_DEBUG, "FILS: PMKID not available");
			goto fail;
		}
		wpa_hexdump(MSG_DEBUG, "FILS: PMKID", sm->fils_erp_pmkid,
			    PMKID_LEN);
		wpa_printf_dbg(MSG_DEBUG, "FILS: ERP processing succeeded - add PMKSA cache entry for the result");
	...
	if (fils_pmk_to_ptk(sm->pmk, sm->pmk_len, sm->own_addr, sm->bssid,
		...
		wpa_printf_dbg(MSG_DEBUG, "FILS: Failed to derive PTK");
		goto fail;
	}
...
static int fils_ft_build_assoc_req_rsne(struct wpa_sm *sm, struct wpabuf *buf)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FILS+FT: R1KH-ID: " MACSTR,
		   MAC2STR(sm->r1kh_id));
...
int fils_process_assoc_resp(struct wpa_sm *sm, const u8 *resp, size_t len)
{
	...
	if (!sm || !sm->ptk_set) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: No KEK available");
		return -1;
	}

	if (!wpa_key_mgmt_fils(sm->key_mgmt)) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: Not a FILS AKM");
		return -1;
	}
	...
	mgmt = (const struct i3ed11_mgmt *) resp;
	...
	if (ieee802_11_parse_elements(ie_start, end - ie_start, &elems, 1) ==
	    ParseFailed) {
		wpa_printf_dbg(MSG_DEBUG,
	...
	if (os_memcmp(elems.fils_session, sm->fils_session,
		      FILS_SESSION_LEN) != 0) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: FILS Session mismatch");
	...
	if (!elems.fils_key_confirm) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: No FILS Key Confirm element");
		goto fail;
	}
	...
	if (os_memcmp(elems.fils_key_confirm, sm->fils_key_auth_ap,
		...
		wpa_printf_dbg(MSG_DEBUG, "FILS: Key-Auth mismatch");
	...
	if (!elems.key_delivery) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: No Key Delivery element");
		goto fail;
	}
	...
	if (wpa_supplicant_parse_ies(elems.key_delivery + WPA_KEY_RSC_LEN,
				     ...
				     &kde) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: Failed to parse KDEs");
		goto fail;
	}
	if (!kde.gtk) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: No GTK KDE");
		goto fail;
	}
	...
	if (kde.gtk_len - 2 > sizeof(gd.gtk)) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: Too long GTK in GTK KDE (len=%lu)",
			   (unsigned long) kde.gtk_len - 2);
		goto fail;
	}
	os_memcpy(gd.gtk, kde.gtk + 2, kde.gtk_len - 2);

	wpa_printf_dbg(MSG_DEBUG, "FILS: Set GTK to driver");
	if (wpa_supplicant_install_gtk(sm, &gd, elems.key_delivery, 0) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: Failed to set GTK");
		goto fail;
	}

	if (ieee80211w_set_keys(sm, &kde) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: Failed to set IGTK");
		goto fail;
	}
	...
	keylen = wpa_cipher_key_len(sm->pairwise_cipher);
	if (keylen <= 0 || (unsigned int) keylen != sm->ptk.tk_len) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: TK length mismatch: %u != %lu",
			   keylen, (long unsigned int) sm->ptk.tk_len);
		goto fail;
	}
	rsclen = wpa_cipher_rsc_len(sm->pairwise_cipher);
	...
	wpa_printf_dbg(MSG_DEBUG, "FILS: Auth+Assoc completed successfully");
	sm->fils_completed = 1;
			   ...
			   const u8 *resp_ies, size_t resp_ies_len)
{
	...
	if (!resp_ies ||
	    ieee802_11_parse_elements(resp_ies, resp_ies_len, &elems, 1) ==
	    ParseFailed) {
		wpa_printf(MSG_INFO,
	...
	if (sm->cur_pmksa && elems.rsn_ie &&
		...
		wpa_printf_dbg(MSG_DEBUG, "OWE: Use PMKSA caching");
		wpa_sm_set_pmk_from_pmksa(sm);
		return 0;
	}
			...
			struct wpa_sm *sm)
{
	...
	if (type == KEY_PTK) {
...
#ifdef DEBUG_DPM_KEY_BACKUP
			da16x_prt("\tptk_kck(%02d)=", (int)sm->ptk.kck_len);
			for (int i = 0; i < (int)sm->ptk.kck_len; i++)
				da16x_prt("%02x", sm->ptk.kck[i]);	
			da16x_prt("\n");
#endif // DEBUG_DPM_KEY_BACKUP
		}
...
#ifdef DEBUG_DPM_KEY_BACKUP
			da16x_prt("\tptk_kek(%02d)=", (int)sm->ptk.kek_len);
			for (int i = 0; i < (int)sm->ptk.kek_len; i++)
				da16x_prt("%02x", sm->ptk.kek[i]);
			da16x_prt("\n");
#endif // DEBUG_DPM_KEY_BACKUP
		}
...
#ifdef DEBUG_DPM_KEY_BACKUP
			da16x_prt("\tptk_tk (%02d)=", (int)sm->ptk.tk_len);
			for (int i = 0; i < (int)sm->ptk.tk_len; i++)
				da16x_prt("%02x", sm->ptk.tk[i]);
			da16x_prt("\n");
#endif // DEBUG_DPM_KEY_BACKUP
		}

#ifdef DEBUG_DPM_KEY_BACKUP
		da16x_prt(ANSI_BCOLOR_DEFULT);
#endif // DEBUG_DPM_KEY_BACKUP
	} else if (type == KEY_GTK) {	
		/* === GTK === */
#ifdef DEBUG_DPM_KEY_BACKUP
		da16x_prt(ANSI_COLOR_GREEN "<GTK>\n");
#endif // DEBUG_DPM_KEY_BACKUP
...
#ifdef DEBUG_DPM_KEY_BACKUP
			da16x_prt("\tgtk(%02d)=", (int)sm->gtk.gtk_len);
			for (int i = 0; (unsigned int)i < sm->gtk.gtk_len; i++)
				da16x_prt("%02x", sm->gtk.gtk[i]);
			da16x_prt("\n");
#endif // DEBUG_DPM_KEY_BACKUP
		}
#ifdef DEBUG_DPM_KEY_BACKUP
		...
		da16x_dpm_prt("	  <%s> Failed backup key info!!\n", __func__);
	}
}
...
int wpa_supp_dpm_restore_key_info(struct wpa_supplicant *wpa_s)
{
	...
	int ret;
 		   ...
 		   || (dpm_supp_key_info->gtk.wpa_alg == WPA_ALG_TKIP || dpm_supp_key_info->gtk.wpa_alg == WPA_ALG_CCMP))
 	{
 		char retry = 0;
ptk_set: 		
		if (wpa_sm_set_key(sm, (enum wpa_alg)(dpm_supp_key_info->ptk.wpa_alg), dpm_supp_conn_info->bssid,
				  dpm_supp_key_info->ptk.key_idx, dpm_supp_key_info->ptk.set_tx,
				  dpm_supp_key_info->ptk.seq, (size_t)dpm_supp_key_info->ptk.seq_len,
				  dpm_supp_key_info->ptk.ptk_tk, (size_t)dpm_supp_key_info->ptk.ptk_tk_len) < 0)
		{
			da16x_err_prt("<%s> Failed to set PTK to the driver(retry=%d)\n", __func__, retry);
			err += 1;
...
gtk_set:
        _gtk = dpm_supp_key_info->gtk.gtk;
        ...
        if (sm->pairwise_cipher == WPA_CIPHER_NONE) {
            ret = wpa_sm_set_key(sm, (enum wpa_alg)(dpm_supp_key_info->gtk.wpa_alg), NULL,
                       dpm_supp_key_info->gtk.key_idx, 1, dpm_supp_key_info->gtk.seq, (size_t)dpm_supp_key_info->gtk.seq_len,
                       _gtk, (size_t)dpm_supp_key_info->gtk.gtk_len); 
        } else {
            ret = wpa_sm_set_key(sm, (enum wpa_alg)(dpm_supp_key_info->gtk.wpa_alg), broadcast_ether_addr,
                      dpm_supp_key_info->gtk.key_idx, dpm_supp_key_info->gtk.set_tx, dpm_supp_key_info->gtk.seq,
                      (size_t)dpm_supp_key_info->gtk.seq_len,_gtk, (size_t)dpm_supp_key_info->gtk.gtk_len);
        }
        
        if (ret < 0)
        {
			da16x_err_prt("<%s> Failed to set GTK to the driver(retry=%d)\n", __func__, retry);
			err += 1;
 	...
 	{
		da16x_dpm_prt("<%s> Security : Open mode\n", __func__);
		return 0;
 	}

	if (err > 0) {
 		da16x_err_prt("<%s> Unsupported Auth Algorithm\n", __func__);
		return -1;
 	}

#ifdef DEBUG_DPM_KEY_BACKUP
	if (dpm_supp_key_info->wep_key_len == 0) {
		da16x_prt(ANSI_COLOR_YELLOW "[%s]\n"
				"\twpa_alg=%d\n"
				"\tkey_idx=%d\n"
				"\tset_tx =%d\n",
				__func__,
		...
		da16x_prt("\tseq(%02d)=", (int)dpm_supp_key_info->ptk.seq_len);
		for (int i = 0; i < dpm_supp_key_info->ptk.seq_len; i++)
			da16x_prt("%02x", dpm_supp_key_info->ptk.seq[i]);
		da16x_prt("\n");

		da16x_prt("\tptk_kck(%02d)=", (int)dpm_supp_key_info->ptk.ptk_kck_len);
		for (int i = 0; i < dpm_supp_key_info->ptk.ptk_kck_len; i++)
			da16x_prt("%02x", dpm_supp_key_info->ptk.ptk_kck[i]);	
		da16x_prt("\n");
		
		da16x_prt("\tptk_kek(%02d)=", (int)dpm_supp_key_info->ptk.ptk_kek_len);
		for (int i = 0; i < dpm_supp_key_info->ptk.ptk_kek_len; i++)
			da16x_prt("%02x", dpm_supp_key_info->ptk.ptk_kek[i]);	
		da16x_prt("\n");

		da16x_prt("\tptk_tk (%02d)=", (int)dpm_supp_key_info->ptk.ptk_tk_len);
		for (int i = 0; i < dpm_supp_key_info->ptk.ptk_tk_len; i++)
			da16x_prt("%02x", dpm_supp_key_info->ptk.ptk_tk[i]);
		da16x_prt("\n");
		...
		da16x_prt("\tseq(%02d)=", (int)dpm_supp_key_info->gtk.seq_len);
		for (int i = 0; i < dpm_supp_key_info->gtk.seq_len; i++)
			da16x_prt("%02x", dpm_supp_key_info->gtk.seq[i]);
		da16x_prt("\n");
		
		da16x_prt("\tgtk(%02d)=", (int)dpm_supp_key_info->gtk.gtk_len);
		for (int i = 0; i < dpm_supp_key_info->gtk.gtk_len; i++)
			da16x_prt("%02x", dpm_supp_key_info->gtk.gtk[i]);
		da16x_prt("\n");
		da16x_prt(ANSI_COLOR_DEFULT);
		}
#endif // DEBUG_DPM_KEY_BACKUP
	return 0;
}
