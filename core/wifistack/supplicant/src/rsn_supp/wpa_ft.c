...
int wpa_sm_set_ft_params(struct wpa_sm *sm, const u8 *ies, size_t ies_len)
{
	...
	if (ft.mdie) {
		...
		wpa_printf_dbg(MSG_DEBUG, "FT: Capability and Policy: 0x%02x",
			   sm->mdie_ft_capab);
	} else
...
static int wpa_ft_install_ptk(struct wpa_sm *sm, const u8 *bssid)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: Installing PTK to the driver.");
			    ...
			    const u8 *ric_ies, size_t ric_ies_len)
{
	...
	if (ft_action) {
		if (!sm->over_the_ds_in_progress) {
			wpa_printf_dbg(MSG_DEBUG, "FT: No over-the-DS in progress "
	...
	if (!wpa_key_mgmt_ft(sm->key_mgmt)) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Reject FT IEs since FT is not "
			   "enabled for this connection");
		return -1;
	}

	if (wpa_ft_parse_ies(ies, ies_len, &parse) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Failed to parse IEs");
		return -1;
	}
	...
	if (parse.rsn_pmkid == NULL ||
		...
		wpa_printf_dbg(MSG_DEBUG, "FT: No matching PMKR0Name (PMKID) in "
			   "RSNIE");
		return -1;
	}
	...
	if (ret) {
		 ...
		 * completed.
		 */
		wpa_printf_dbg(MSG_DEBUG, "FT: Failed to set PTK prior to "
				      ...
				      size_t gtk_elem_len)
{
	...
	if (gtk_elem == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "FT: No GTK included in FTIE");
		return 0;
	}
	...
	if (gtk_elem_len < 11 + 24 || (gtk_elem_len - 11) % 8 ||
		...
		wpa_printf_dbg(MSG_DEBUG, "FT: Invalid GTK sub-elem "
	...
	if (gtk_len < keylen) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Too short GTK in FTIE");
		return -1;
	}
	...
	if (gtk_elem[2] != keylen) {
		wpa_printf_dbg(MSG_DEBUG, "FT: GTK length mismatch: received %d "
			   "negotiated %lu",
				       ...
				       size_t igtk_elem_len)
{
	...
	if (igtk_elem == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "FT: No IGTK included in FTIE");
		return 0;
	}

	da16x_dump("FT: Received IGTK in Reassoc Resp",
			igtk_elem, igtk_elem_len);

	if (igtk_elem_len != 2 + 6 + 1 + WPA_IGTK_LEN + 8) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Invalid IGTK sub-elem "
			   "length %lu", (unsigned long) igtk_elem_len);
		return -1;
	}
	if (igtk_elem[8] != WPA_IGTK_LEN) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Invalid IGTK sub-elem Key Length "
			   "%d", igtk_elem[8]);
		return -1;
	}
	...
	da16x_dump("FT: IGTK from Reassoc Resp", igtk,
			WPA_IGTK_LEN);
				 ...
				 size_t ies_len, const u8 *src_addr)
{
	...
	if (sm->set_ptk_after_assoc) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Try to set PTK again now that we "
			   "are associated");
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: Completed successfully");
			 ...
			 const u8 *mdie)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: Request over-the-DS with " MACSTR,
		   MAC2STR(target_ap));
