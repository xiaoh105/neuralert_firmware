...
static void eapol_port_timers_tick(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (sm->authWhile > 0) {
		sm->authWhile--;
		if (sm->authWhile == 0)
			wpa_printf_dbg(MSG_DEBUG, "EAPOL: authWhile --> 0");
	}
	if (sm->heldWhile > 0) {
		sm->heldWhile--;
		if (sm->heldWhile == 0)
			wpa_printf_dbg(MSG_DEBUG, "EAPOL: heldWhile --> 0");
	}
	if (sm->startWhen > 0) {
		sm->startWhen--;
		if (sm->startWhen == 0)
			wpa_printf_dbg(MSG_DEBUG, "EAPOL: startWhen --> 0");
	}
	if (sm->idleWhile > 0) {
		sm->idleWhile--;
		if (sm->idleWhile == 0)
			wpa_printf_dbg(MSG_DEBUG, "EAPOL: idleWhile --> 0");
	}

	if (sm->authWhile | sm->heldWhile | sm->startWhen | sm->idleWhile) {
		da16x_eloop_register_timeout(1, 0, eapol_port_timers_tick, eloop_ctx,
				       sm);
	} else {
		wpa_printf_dbg(MSG_DEBUG, "EAPOL: disable timer tick");
...
static void eapol_enable_timer_tick(struct eapol_sm *sm)
{
	if (sm->timer_tick_enabled)
		return;
	wpa_printf_dbg(MSG_DEBUG, "EAPOL: enable timer tick");
	sm->timer_tick_enabled = 1;

	da16x_eloop_cancel_timeout(eapol_port_timers_tick, NULL, sm);
	da16x_eloop_register_timeout(1, 0, eapol_port_timers_tick, NULL, sm);
}


SM_STATE(SUPP_PAE, LOGOFF)
{
	SM_ENTRY_EAPOL(SUPP_PAE, LOGOFF);
...
{
	SM_ENTRY_EAPOL(SUPP_PAE, DISCONNECTED);
...
SM_STATE(SUPP_PAE, CONNECTING)
{
	...
	SM_ENTRY_EAPOL(SUPP_PAE, CONNECTING);
...
{
	SM_ENTRY_EAPOL(SUPP_PAE, AUTHENTICATING);
	sm->startCount = 0;
...
{
	SM_ENTRY_EAPOL(SUPP_PAE, AUTHENTICATED);
...
{
	SM_ENTRY_EAPOL(SUPP_PAE, RESTART);
	sm->eapRestart = TRUE;
	if (sm->altAccept) {
		/*
		 ...
		 */
		wpa_printf_dbg(MSG_DEBUG, "EAPOL: Clearing prior altAccept TRUE");
		sm->eapSuccess = FALSE;
		sm->altAccept = FALSE;
	}
}
...
{
	SM_ENTRY_EAPOL(SUPP_PAE, S_FORCE_AUTH);
	eapol_sm_set_port_authorized(sm);
...
{
	SM_ENTRY_EAPOL(SUPP_PAE, S_FORCE_UNAUTH);
	eapol_sm_set_port_unauthorized(sm);
...
SM_STEP(SUPP_PAE)
{
	...
	case SUPP_PAE_AUTHENTICATING:
		if (sm->eapSuccess && !sm->portValid &&
			...
			wpa_printf_dbg(MSG_DEBUG, "EAPOL: IEEE 802.1X for "
...
{
	SM_ENTRY_EAPOL(KEY_RX, NO_KEY_RECEIVE);
}


SM_STATE(KEY_RX, KEY_RECEIVE)
{
	SM_ENTRY_EAPOL(KEY_RX, KEY_RECEIVE);
...
{
	SM_ENTRY_EAPOL(SUPP_BE, REQUEST);
	sm->authWhile = 0;
	sm->eapReq = TRUE;
...
{
	SM_ENTRY_EAPOL(SUPP_BE, RESPONSE);
...
{
	SM_ENTRY_EAPOL(SUPP_BE, SUCCESS);
	sm->keyRun = TRUE;
	sm->suppSuccess = TRUE;
...
{
	SM_ENTRY_EAPOL(SUPP_BE, TIMEOUT);
	sm->suppTimeout = TRUE;
}
...
{
	SM_ENTRY_EAPOL(SUPP_BE, INITIALIZE);
...
{
	SM_ENTRY_EAPOL(SUPP_BE, RECEIVE);
...
SM_STEP(SUPP_BE)
{
	...
	case SUPP_BE_REQUEST:
		...
		if (sm->eapResp && sm->eapNoResp) {
			wpa_printf_dbg(MSG_DEBUG, "EAPOL: SUPP_BE REQUEST: both "
...
static void eapol_sm_txLogoff(struct eapol_sm *sm)
{
	wpa_printf_dbg(MSG_DEBUG, "EAPOL: txLogoff");
...
static void eapol_sm_txStart(struct eapol_sm *sm)
{
	wpa_printf_dbg(MSG_DEBUG, "EAPOL: txStart");
	sm->ctx->eapol_send(sm->ctx->eapol_send_ctx,
...
static void eapol_sm_processKey(struct eapol_sm *sm)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAPOL: processKey");
	...
	wpa_printf_dbg(MSG_DEBUG, "EAPOL: RX IEEE 802.1X ver=%d type=%d len=%d "
	...
	if (res < 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAPOL: Could not get master key for "
	...
	if (res == 16) {
		...
		if (res) {
			wpa_printf_dbg(MSG_DEBUG, "EAPOL: Could not get LEAP "
		...
		wpa_printf_dbg(MSG_DEBUG, "EAPOL: Could not get enough master key "
	...
	if (os_memcmp_const(orig_key_sign, key->key_signature,
		...
		wpa_printf_dbg(MSG_DEBUG, "EAPOL: Invalid key signature in "
			   "EAPOL-Key packet");
		os_memcpy(key->key_signature, orig_key_sign,
			  IEEE8021X_KEY_SIGN_LEN);
		return;
	}
	wpa_printf_dbg(MSG_DEBUG, "EAPOL: EAPOL-Key key signature verified");
	...
	if (key_len == rx_key_length) {
		...
		wpa_printf_dbg(MSG_DEBUG, "EAPOL: Invalid key data length %d "
	...
	wpa_printf_dbg(MSG_DEBUG, "EAPOL: Setting dynamic WEP key: %s keyidx %d "
		   "len %d",
	...
	if (sm->ctx->set_wep_key &&
		...
		{
			wpa_printf_dbg(MSG_DEBUG, "EAPOL: all required EAPOL-Key "
				   "frames received");
			sm->portValid = TRUE;
			if (sm->ctx->eapol_done_cb)
				sm->ctx->eapol_done_cb(sm->ctx->ctx);
...
static void eapol_sm_getSuppRsp(struct eapol_sm *sm)
{
	wpa_printf_dbg(MSG_DEBUG, "EAPOL: getSuppRsp");
...
static void eapol_sm_txSuppRsp(struct eapol_sm *sm)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAPOL: txSuppRsp");
...
void eapol_sm_step(struct eapol_sm *sm)
{
	...
	if (sm->changed) {
		...
		da16x_eloop_cancel_timeout(eapol_sm_step_timeout, NULL, sm);
		da16x_eloop_register_timeout(0, 0, eapol_sm_step_timeout, NULL, sm);
	}
		      ...
		      size_t len)
{
	...
	if (sm->conf.wps && sm->conf.workaround &&
		...
		if (elen > plen && elen <= len - sizeof(*hdr)) {
			 ...
			 */
			wpa_printf_dbg(MSG_DEBUG, "EAPOL: Workaround - fix EAPOL "
	...
	case IEEE802_1X_TYPE_EAP_PACKET:
		if (sm->conf.workaround) {
			...
			if (plen >= sizeof(*ehdr) && ehdr->code == 10) {
				wpa_printf_dbg(MSG_DEBUG, "EAPOL: Ignore EAP packet with unknown code 10");
				break;
			}
		}
		...
		if (sm->eapReqData) {
			wpa_printf_dbg(MSG_DEBUG, "EAPOL: Received EAP-Packet "
				   "frame");
			sm->eapolEap = TRUE;
#ifdef CONFIG_EAP_PROXY
			if (sm->use_eap_proxy) {
				...
				wpa_printf_dbg(MSG_DEBUG, "EAPOL: eap_proxy "
					   "EAP Req updated");
			}
#endif /* CONFIG_EAP_PROXY */
			eapol_sm_step(sm);
	...
	case IEEE802_1X_TYPE_EAPOL_KEY:
		if (plen < sizeof(*key)) {
			wpa_printf_dbg(MSG_DEBUG, "EAPOL: Too short EAPOL-Key "
				   "frame received");
			break;
		}
		...
		if (key->type == EAPOL_KEY_TYPE_WPA ||
			...
			wpa_printf_dbg(MSG_DEBUG, "EAPOL: Ignoring WPA EAPOL-Key "
				   "frame in EAPOL state machines");
			res = 0;
			break;
		}
		if (key->type != EAPOL_KEY_TYPE_RC4) {
			wpa_printf_dbg(MSG_DEBUG, "EAPOL: Ignored unknown "
		...
		if (sm->last_rx_key) {
			wpa_printf_dbg(MSG_DEBUG, "EAPOL: Received EAPOL-Key "
				   "frame");
...
#endif /* CONFIG_MACSEC */
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAPOL: Received unknown EAPOL type %d",
			   hdr->type);
		sm->dot1xSuppInvalidEapolFramesRx++;
		break;
	}
...
void eapol_sm_notify_portEnabled(struct eapol_sm *sm, Boolean enabled)
{
	if (sm == NULL)
		return;
	wpa_printf_dbg(MSG_DEBUG, "EAPOL: External notification - "
...
int eapol_sm_get_key(struct eapol_sm *sm, u8 *key, size_t len)
{
	...
	if (sm && sm->use_eap_proxy) {
		...
		if (sm == NULL || !eap_proxy_key_available(sm->eap_proxy)) {
			wpa_printf_dbg(MSG_DEBUG, "EAPOL: EAP key not available");
			return -1;
		}
		eap_key = eap_proxy_get_eapKeyData(sm->eap_proxy, &eap_len);
		if (eap_key == NULL) {
			wpa_printf_dbg(MSG_DEBUG, "EAPOL: Failed to get "
				   "eapKeyData");
			return -1;
		}
		goto key_fetched;
	...
	if (eap_key == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAPOL: Failed to get eapKeyData");
		return -1;
	}
#else
da16x_eapol_prt("[[%s]] What do I do for this one ???\n", __func__);
	return -1;
#endif	/* CONFIG_EAP_PEER */
...
#ifdef	CONFIG_EAP_PEER
	if (len > eap_len) {
		wpa_printf_dbg(MSG_DEBUG, "EAPOL: Requested key length (%lu) not "
			   "available (len=%lu)",
			   (unsigned long) len, (unsigned long) eap_len);
		return eap_len;
	}
	os_memcpy(key, eap_key, len);
	wpa_printf_dbg(MSG_DEBUG, "EAPOL: Successfully fetched key (len=%lu)",
		   (unsigned long) len);
	return 0;
#endif	/* CONFIG_EAP_PEER */
}
...
#endif	/* CONFIG_EAP_PEER */
	{
		wpa_printf_dbg(MSG_DEBUG, "EAPOL: EAP Session-Id not available");
		return NULL;
	}
...
void eapol_sm_notify_cached(struct eapol_sm *sm)
{
	if (sm == NULL)
		return;
	wpa_printf_dbg(MSG_DEBUG, "EAPOL: PMKSA caching was used - skip EAPOL");
	sm->eapSuccess = TRUE;
#ifdef	CONFIG_EAP_PEER
	eap_notify_success(sm->eap);
#endif	/* CONFIG_EAP_PEER */
...
void eapol_sm_notify_pmkid_attempt(struct eapol_sm *sm)
{
	if (sm == NULL)
		return;
	wpa_printf_dbg(MSG_DEBUG, "RSN: Trying to use cached PMKSA");
	sm->cached_pmk = TRUE;
}
#else
void eapol_sm_notify_pmkid_attempt(struct eapol_sm *sm)
{
	...
	da16x_eapol_prt("[%s] RSN: Trying to use cached PMKSA\n", __func__);
...
static void eapol_sm_abort_cached(struct eapol_sm *sm)
{
	wpa_printf_dbg(MSG_DEBUG, "RSN: Authenticator did not accept PMKID, "
...
void eapol_sm_notify_ctrl_response(struct eapol_sm *sm)
{
	...
	if (sm->eapReqData && !sm->eapReq) {
		wpa_printf_dbg(MSG_DEBUG, "EAPOL: received control response (user "
...
static void eapol_sm_notify_pending(void *ctx)
{
	...
	if (sm == NULL)
		return;
	if (sm->eapReqData && !sm->eapReq) {
		wpa_printf_dbg(MSG_DEBUG, "EAPOL: received notification from EAP "
...
{
	struct eapol_sm *sm = ctx;
	wpa_printf_dbg(MSG_DEBUG, "EAPOL: EAP parameter needed");
