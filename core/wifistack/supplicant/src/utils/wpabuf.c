...
#endif	/* 0 */
#if 0 //[[ trinity_0160928_BEGIN
	abort();
#endif //]] trinity_0160928_END
}

#ifdef DEBUG_SUPP_MALLOC
int _wpabuf_resize(struct wpabuf **_buf, size_t add_len, const char* caller)
#else
...
#endif /* DEBUG_SUPP_MALLOC */
{
	...
	if (buf == NULL) {
#ifdef DEBUG_SUPP_MALLOC
		*_buf = _wpabuf_alloc(add_len, caller);
#else
 ...
 */
#ifdef DEBUG_SUPP_MALLOC
struct wpabuf * _wpabuf_alloc(size_t len, const char* caller)
#else
...
#ifdef WPA_TRACE
#ifdef DEBUG_SUPP_MALLOC
	struct wpabuf_trace *trace = _os_zalloc(sizeof(struct wpabuf_trace) +
					       sizeof(struct wpabuf) + len, caller);
#else
...
#else /* WPA_TRACE */
#ifdef DEBUG_SUPP_MALLOC
	struct wpabuf *buf = _os_zalloc(sizeof(struct wpabuf) + len, caller);
#else
	struct wpabuf *buf = os_zalloc(sizeof(struct wpabuf) + len);
#endif /* DEBUG_SUPP_MALLOC */

	if (buf == NULL) {
		wpa_printf(MSG_ERROR, "[%s] alloc Failed(len=%d)", __func__, len);
		return NULL;
	}
#endif /* WPA_TRACE */
...
#ifdef DEBUG_SUPP_MALLOC
struct wpabuf * _wpabuf_alloc_ext_data(u8 *data, size_t len, const char* caller)
#else
...
#else /* WPA_TRACE */
#ifdef DEBUG_SUPP_MALLOC
	struct wpabuf *buf = _os_zalloc(sizeof(struct wpabuf), caller);
#else
...
#ifdef DEBUG_SUPP_MALLOC
struct wpabuf * _wpabuf_alloc_copy(const void *data, size_t len, const char* caller)
#else
struct wpabuf * wpabuf_alloc_copy(const void *data, size_t len)
#endif /* DEBUG_SUPP_MALLOC */
{
#ifdef DEBUG_SUPP_MALLOC
	struct wpabuf *buf = _wpabuf_alloc(len, caller);
#else
...
#ifdef DEBUG_SUPP_MALLOC
struct wpabuf * _wpabuf_dup(const struct wpabuf *src, const char* caller)
#else
struct wpabuf * wpabuf_dup(const struct wpabuf *src)
#endif /* DEBUG_SUPP_MALLOC */
{
#ifdef DEBUG_SUPP_MALLOC
	struct wpabuf *buf = _wpabuf_alloc(wpabuf_len(src), caller);
#else
