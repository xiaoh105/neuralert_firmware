...
static void http_req(void *ctx, struct http_request *req)
{
	...
	if (done) {
		da16x_eloop_cancel_timeout(browser_timeout, NULL, NULL);
		da16x_eloop_register_timeout(0, 500000, browser_timeout, &data, NULL);
	}
...
int hs20_web_browser(const char *url)
{
	...
	da16x_eloop_register_timeout(120, 0, browser_timeout, &data, NULL);
	da16x_eloop_run();
	da16x_eloop_cancel_timeout(browser_timeout, &data, NULL);
	http_server_deinit(http);
	eloop_destroy();
