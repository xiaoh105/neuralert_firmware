...
{
	wpa_printf_dbg(MSG_DEBUG, "BROWSER:%s", __func__);
	gtk_main_quit();
}
				    ...
				    struct browser_context *ctx)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "BROWSER:%s progress=%d", __func__,
		   ctx->progress);
				       ...
				       struct browser_context *ctx)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "BROWSER:%s load-status=%d uri=%s",
					      ...
					      struct browser_context *ctx)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "BROWSER:%s uri=%s", __func__, uri);
...
static gboolean view_cb_mime_type_policy_decision(
	...
	struct browser_context *ctx)
{
	wpa_printf_dbg(MSG_DEBUG, "BROWSER:%s mime=%s", __func__, mime);
...
{
	wpa_printf_dbg(MSG_DEBUG, "BROWSER:%s title=%s uri=%s", __func__, title,
		   uri);
...
static void view_cb_title_changed(WebKitWebView *view, WebKitWebFrame *frame,
				  ...
				  struct browser_context *ctx)
{
	wpa_printf_dbg(MSG_DEBUG, "BROWSER:%s title=%s", __func__, title);
	os_free(ctx->title);
