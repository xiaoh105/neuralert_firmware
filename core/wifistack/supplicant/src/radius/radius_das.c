...
static void radius_das_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "DAS: Received %d bytes from %s:%d",
		   len, abuf, from_port);
	if (das->client_addr.u.v4.s_addr != from.sin.sin_addr.s_addr) {
		wpa_printf_dbg(MSG_DEBUG, "DAS: Drop message from unknown client");
		return;
	}

	msg = radius_msg_parse(buf, len);
	if (msg == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "DAS: Parsing incoming RADIUS packet "
	...
	if (radius_msg_verify_das_req(msg, das->shared_secret,
		...
		wpa_printf_dbg(MSG_DEBUG, "DAS: Invalid authenticator in packet "
	...
	if (res == 4) {
		...
		if ((unsigned int) abs(now.sec - timestamp) >
		    das->time_window) {
			wpa_printf_dbg(MSG_DEBUG, "DAS: Unacceptable "
		...
		wpa_printf_dbg(MSG_DEBUG, "DAS: Missing Event-Timestamp in packet "
			   "from %s:%d - drop", abuf, from_port);
		goto fail;
	}
	...
	case RADIUS_CODE_COA_REQUEST:
		...
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "DAS: Unexpected RADIUS code %u in "
			   "packet from %s:%d",
			   hdr->code, abuf, from_port);
	}

	if (reply) {
		wpa_printf_dbg(MSG_DEBUG, "DAS: Reply to %s:%d", abuf, from_port);

		if (!radius_msg_add_attr_int32(reply,
					       RADIUS_ATTR_EVENT_TIMESTAMP,
					       now.sec)) {
			wpa_printf_dbg(MSG_DEBUG, "DAS: Failed to add "
