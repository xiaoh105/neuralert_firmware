...
wpa_printf_dbg(MSG_DEBUG, "RADIUS SRV: " args)
...
extern int da16x_eloop_register_timeout(unsigned int secs, unsigned int usecs,
										da16x_eloop_timeout_handler handler, void *eloop_data, void *user_data);
...
{
	da16x_eloop_cancel_timeout(radius_server_session_timeout, data, sess);
	da16x_eloop_cancel_timeout(radius_server_session_remove_timeout, data, sess);
			  ...
			  struct radius_client *client)
{
	...
	da16x_eloop_register_timeout(RADIUS_SESSION_TIMEOUT, 0,
...
send_reply:
	...
	if (is_complete) {
		RADIUS_DEBUG("Removing completed session 0x%x after timeout",
			     sess->sess_id);
		da16x_eloop_cancel_timeout(radius_server_session_remove_timeout,
				     data, sess);
		da16x_eloop_register_timeout(10, 0,
...
#ifdef CONFIG_IPV6
	if (data->ipv6) {
		if (da16x_inet_ntop(AF_INET6, &from.sin6.sin6_addr, abuf,
			      sizeof(abuf)) == NULL)
			abuf[0] = '\0';
...
radius_server_read_clients(const char *client_file, int ipv6)
{
	...
	while (fgets(buf, buf_size, f)) {
		...
		if (!ipv6 && da16x_inet_aton(buf, &addr) == 0) {
			failed = 1;
			break;
		}
#ifdef CONFIG_IPV6
		if (ipv6 && da16x_inet_pton(AF_INET6, buf, &addr6) <= 0) {
			if (da16x_inet_pton(AF_INET, buf, &addr) <= 0) {
				failed = 1;
				break;
			}
...
#ifdef CONFIG_IPV6
		if (data->ipv6) {
			if (da16x_inet_ntop(AF_INET6, &cli->addr6, abuf,
				      sizeof(abuf)) == NULL)
				abuf[0] = '\0';
			if (da16x_inet_ntop(AF_INET6, &cli->mask6, abuf,
				      sizeof(mbuf)) == NULL)
				mbuf[0] = '\0';
		}
#endif /* CONFIG_IPV6 */
		if (!data->ipv6) {
			os_strlcpy(abuf, da16x_inet_ntoa(cli->addr), sizeof(abuf));
			os_strlcpy(mbuf, da16x_inet_ntoa(cli->mask), sizeof(mbuf));
		}
