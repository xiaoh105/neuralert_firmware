...
{
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: %s -> %s",
...
{
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Failure - code 0x%x", code);
					 ...
					 u8 id, size_t length, u8 eke_exch)
{
	...
	msg = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_EKE, plen,
			    EAP_CODE_REQUEST, id);
	if (msg == NULL) {
		wpa_printf_dbg(MSG_ERROR, "EAP-EKE: Failed to allocate memory");
		return NULL;
	}
...
static struct wpabuf * eap_eke_build_failure(struct eap_eke_data *data, u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Request/Failure: Failure-Code=0x%x",
		   data->failure_code);
					      ...
					      u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Request/Identity");
					    ...
					    struct eap_eke_data *data, u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Request/Commit");

	if (sm->user == NULL || sm->user->password == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Password with not configured");
	...
	if (eap_eke_derive_key(&data->sess, sm->user->password,
		...
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Failed to derive key");
	...
	if (eap_eke_dh_init(data->sess.dhgroup, data->dh_priv, pub) < 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Failed to initialize DH");
		eap_eke_fail(data, EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
		return eap_eke_build_failure(data, id);
	}
	...
	if (eap_eke_dhcomp(&data->sess, data->key, pub,
	    ...
	    < 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Failed to build DHComponent_S");
		wpabuf_free(msg);
		eap_eke_fail(data, EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
		return eap_eke_build_failure(data, id);
	}
					     ...
					     struct eap_eke_data *data, u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Request/Confirm");
...
static struct wpabuf * eap_eke_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	case FAILURE_REPORT:
		return eap_eke_build_failure(data, id);
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Unknown state %d in buildReq",
			   data->state);
		break;
	}
	return NULL;
			     ...
			     struct wpabuf *respData)
{
	...
	if (pos == NULL || len < 1) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Invalid frame");
		return TRUE;
	}

	eke_exch = *pos;
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Received frame: EKE-Exch=%d", eke_exch);
	...
	wpa_printf_dbg(MSG_INFO, "EAP-EKE: Unexpected EKE-Exch=%d in state=%d",
		   eke_exch, data->state);
				     ...
				     const u8 *payload, size_t payloadlen)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Received Response/Identity");
	...
	if (pos + 2 + 4 + 1 > end) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Too short EAP-EKE-ID payload");
		eap_eke_fail(data, EAP_EKE_FAIL_PROTO_ERROR);
		return;
	}

	if (*pos != 1) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Unexpected NumProposals %d (expected 1)",
			   *pos);
		eap_eke_fail(data, EAP_EKE_FAIL_PROTO_ERROR);
		return;
	}
	...
	if (!supported_proposal(pos)) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Unexpected Proposal (%u:%u:%u:%u)",
			   pos[0], pos[1], pos[2], pos[3]);
		eap_eke_fail(data, EAP_EKE_FAIL_PROTO_ERROR);
		return;
	}

	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Selected Proposal (%u:%u:%u:%u)",
		   pos[0], pos[1], pos[2], pos[3]);
	...
	if (data->peerid == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Failed to allocate memory for peerid");
		eap_eke_fail(data, EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
		return;
	}
	data->peerid_len = end - pos;
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Peer IDType %u", data->peerid_type);
	wpa_hexdump_ascii(MSG_DEBUG, "EAP-EKE: Peer Identity",
			  data->peerid, data->peerid_len);

	if (eap_user_get(sm, data->peerid, data->peerid_len, data->phase2)) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Peer Identity not found from user database");
		eap_eke_fail(data, EAP_EKE_FAIL_PASSWD_NOT_FOUND);
		return;
	}
	...
	if (i == EAP_MAX_METHODS) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Matching user entry does not allow EAP-EKE");
		eap_eke_fail(data, EAP_EKE_FAIL_PASSWD_NOT_FOUND);
		return;
	}

	if (sm->user->password == NULL || sm->user->password_len == 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: No password configured for peer");
		eap_eke_fail(data, EAP_EKE_FAIL_PASSWD_NOT_FOUND);
		return;
	}
				   ...
				   const u8 *payload, size_t payloadlen)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Received Response/Commit");
	...
	if (pos + data->sess.dhcomp_len + data->sess.pnonce_len > end) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Too short EAP-EKE-Commit");
		eap_eke_fail(data, EAP_EKE_FAIL_PROTO_ERROR);
		return;
	}
	...
	if (eap_eke_shared_secret(&data->sess, data->key, data->dh_priv, dhcomp)
	    < 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Failed to derive shared secret");
		eap_eke_fail(data, EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
		return;
	}
	...
	if (eap_eke_derive_ke_ki(&data->sess,
		...
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Failed to derive Ke/Ki");
		eap_eke_fail(data, EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
		return;
	}
	...
	if (eap_eke_decrypt_prot(&data->sess, pnonce, data->sess.pnonce_len,
		...
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Failed to decrypt PNonce_P");
		eap_eke_fail(data, EAP_EKE_FAIL_AUTHENTICATION_FAIL);
		return;
	}
	if (decrypt_len < (size_t) data->sess.nonce_len) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: PNonce_P protected data too short to include Nonce_P");
		eap_eke_fail(data, EAP_EKE_FAIL_AUTHENTICATION_FAIL);
		return;
	}
				    ...
				    const u8 *payload, size_t payloadlen)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Received Response/Confirm");
	...
	if (payloadlen < (size_t) data->sess.pnonce_len + data->sess.prf_len) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Too short EAP-EKE-Confirm");
		eap_eke_fail(data, EAP_EKE_FAIL_PROTO_ERROR);
		return;
	}
	...
	if (eap_eke_decrypt_prot(&data->sess, payload, data->sess.pnonce_len,
				 nonce, &decrypt_len) < 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Failed to decrypt PNonce_S");
		eap_eke_fail(data, EAP_EKE_FAIL_AUTHENTICATION_FAIL);
		return;
	}
	if (decrypt_len < (size_t) data->sess.nonce_len) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: PNonce_S protected data too short to include Nonce_S");
		eap_eke_fail(data, EAP_EKE_FAIL_AUTHENTICATION_FAIL);
		return;
	}
	...
	if (os_memcmp(nonce, data->nonce_s, data->sess.nonce_len) != 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Received Nonce_S does not match previously sent Nonce_S");
		eap_eke_fail(data, EAP_EKE_FAIL_AUTHENTICATION_FAIL);
		return;
	}

	if (eap_eke_auth(&data->sess, "EAP-EKE peer", data->msgs, auth_p) < 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Could not derive Auth_P");
		eap_eke_fail(data, EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
		return;
	}
	...
	if (os_memcmp_const(auth_p, payload + data->sess.pnonce_len,
			    data->sess.prf_len) != 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Auth_P does not match");
		eap_eke_fail(data, EAP_EKE_FAIL_AUTHENTICATION_FAIL);
		return;
	}
	...
	if (eap_eke_derive_msk(&data->sess, sm->server_id, sm->server_id_len,
		...
		wpa_printf_dbg(MSG_INFO, "EAP-EKE: Failed to derive MSK/EMSK");
		eap_eke_fail(data, EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
		return;
	}
				    ...
				    const u8 *payload, size_t payloadlen)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Received Response/Failure");

	if (payloadlen < 4) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Too short EAP-EKE-Failure");
		eap_eke_state(data, FAILURE);
		return;
	}

	code = WPA_GET_BE32(payload);
	wpa_printf_dbg(MSG_DEBUG, "EAP-EKE: Peer reported failure code 0x%x", code);
