...
static void * eap_gtc_init(struct eap_sm *sm)
{
	...
	if (sm->m && sm->m->vendor == EAP_VENDOR_IETF &&
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-GTC: EAP-FAST tunnel - use prefix "
			    ...
			    struct wpabuf *respData)
{
	...
	if (data->prefix) {
		...
		if (rlen < 10) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-GTC: Too short response "
				   "for EAP-FAST prefix");
			data->state = FAILURE;
			return;
		}
		...
		while (pos2 < end && *pos2)
			pos2++;
		if (pos2 == end) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-GTC: No password in "
		...
		if (sm->identity && sm->require_identity_match &&
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-GTC: Phase 2 Identity did "
	...
	if (rlen != sm->user->password_len ||
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-GTC: Done - Failure");
		data->state = FAILURE;
	} else {
		wpa_printf_dbg(MSG_DEBUG, "EAP-GTC: Done - Success");
		data->state = SUCCESS;
	}
}
