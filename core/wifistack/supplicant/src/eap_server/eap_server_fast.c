...
{
	wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: %s -> %s",
				      ...
				      u8 *master_secret)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: SessionTicket callback");
	wpa_hexdump(MSG_DEBUG, "EAP-FAST: SessionTicket (PAC-Opaque)",
		    ticket, len);

	if (len < 4 || WPA_GET_BE16(ticket) != PAC_TYPE_PAC_OPAQUE) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Ignore invalid "
			   "SessionTicket");
		return 0;
	}
	...
	if (pac_opaque_len < 8 || pac_opaque_len % 8 ||
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Ignore invalid PAC-Opaque "
			   "(len=%lu left=%lu)",
	...
	if (buf == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Failed to allocate memory "
	...
	if (aes_unwrap(data->pac_opaque_encr, sizeof(data->pac_opaque_encr),
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Failed to decrypt "
			   "PAC-Opaque");
		os_free(buf);
		/*
	...
	while (end - pos > 1) {
		...
		case PAC_OPAQUE_TYPE_LIFETIME:
			if (elen != 4) {
				wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Invalid "
	...
	if (pac_key == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: No PAC-Key included in "
			   "PAC-Opaque");
		os_free(buf);
		return -1;
	}
	...
	if (os_get_time(&now) < 0 || lifetime <= 0 || now.sec > lifetime) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: PAC-Key not valid anymore "
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: PAC-Key soft timeout; send "
				     ...
				     struct eap_fast_data *data)
{
	...
	sks = eap_fast_derive_key(sm->ssl_ctx, data->ssl.conn,
				  EAP_FAST_SKS_LEN);
	if (sks == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Failed to derive "
			   "session_key_seed");
		return;
	}
					     ...
					     struct eap_fast_data *data)
{
	...
	if (data->key_block_p == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Failed to derive key block");
		return;
	}
	/*
	 * RFC 4851, Section 5.2:
				   ...
				   u8 *isk, size_t isk_len)
{
	...
	if (data->phase2_method == NULL || data->phase2_priv == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Phase 2 method not "
			   "available");
		return -1;
	}
	...
	if ((key = data->phase2_method->getKey(sm, data->phase2_priv,
					       &key_len)) == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Could not get key material "
			   "from Phase 2");
		return -1;
	}
...
static int eap_fast_update_icmk(struct eap_sm *sm, struct eap_fast_data *data)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Deriving ICMK[%d] (S-IMCK and CMK)",
		   data->simck_idx + 1);
...
static void * eap_fast_init(struct eap_sm *sm)
{
	...
	if (sm->user && sm->user->force_version >= 0) {
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: forcing version %d",
			   data->force_version);
	...
	if (eap_server_tls_ssl_init(sm, &data->ssl, 0, EAP_TYPE_FAST)) {
		wpa_printf_dbg(MSG_INFO, "EAP-FAST: Failed to initialize SSL.");
	...
	if (tls_connection_set_cipher_list(sm->ssl_ctx, data->ssl.conn,
					   ciphers) < 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-FAST: Failed to set TLS cipher "
			   "suites");
		eap_fast_reset(sm, data);
		return NULL;
	}
	...
	if (tls_connection_set_session_ticket_cb(sm->ssl_ctx, data->ssl.conn,
						 ...
						 data) < 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-FAST: Failed to set SessionTicket "
			   "callback");
		eap_fast_reset(sm, data);
		return NULL;
	}
	...
	if (sm->pac_opaque_encr_key == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-FAST: No PAC-Opaque encryption key "
			   "configured");
		eap_fast_reset(sm, data);
		return NULL;
	}
	...
	if (sm->eap_fast_a_id == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-FAST: No A-ID configured");
		eap_fast_reset(sm, data);
		return NULL;
	}
	...
	if (sm->eap_fast_a_id_info == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-FAST: No A-ID-Info configured");
		eap_fast_reset(sm, data);
		return NULL;
	}
					    ...
					    struct eap_fast_data *data, u8 id)
{
	...
	req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_FAST,
			    ...
			    EAP_CODE_REQUEST, id);
	if (req == NULL) {
		wpa_printf_dbg(MSG_ERROR, "EAP-FAST: Failed to allocate memory for"
			   " request");
		eap_fast_state(data, FAILURE);
		return NULL;
	}
...
static int eap_fast_phase1_done(struct eap_sm *sm, struct eap_fast_data *data)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Phase1 done, starting Phase2");

	if (tls_get_cipher(sm->ssl_ctx, data->ssl.conn, cipher, sizeof(cipher))
	    < 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Failed to get cipher "
			   "information");
		eap_fast_state(data, FAILURE);
		return -1;
	}
	...
	if (data->anon_provisioning) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Anonymous provisioning");
	...
	struct eap_sm *sm, struct eap_fast_data *data)
{
	...
	if (!data->final_result || data->eap_seq > 1) {
		/* Intermediate-Result */
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Add Intermediate-Result TLV "
			   "(status=SUCCESS)");
	...
	if (data->final_result) {
		/* Result TLV */
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Add Result TLV "
			   "(status=SUCCESS)");
		result = wpabuf_put(buf, sizeof(*result));
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Add Crypto-Binding TLV: Version %d "
					  ...
					  struct eap_fast_data *data)
{
	...
	*pos++ = 4;
	...
	/* Result TLV */
	wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Add Result TLV (status=SUCCESS)");
	result = wpabuf_put(buf, sizeof(*result));
	...
	/* PAC TLV */
	wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Add PAC TLV");
				   ...
				   struct wpabuf *plain, int piggyback)
{
	...
	if (data->ssl.tls_out && piggyback) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Piggyback Phase 2 data "
		...
		if (wpabuf_resize(&data->ssl.tls_out, wpabuf_len(encr)) < 0) {
			wpa_printf_dbg(MSG_WARNING, "EAP-FAST: Failed to resize "
				   "output buffer");
			wpabuf_free(encr);
			return -1;
		}
...
static struct wpabuf * eap_fast_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	case PHASE1:
		if (tls_connection_established(sm->ssl_ctx, data->ssl.conn)) {
			...
			if (data->state == PHASE2_START) {
				 ...
				 * roundtrip.
				 */
				wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Try to start "
					   "Phase 2");
	...
	case REQUEST_PAC:
		...
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: %s - unexpected state %d",
			   __func__, data->state);
		return NULL;
	}
			      ...
			      struct wpabuf *respData)
{
	...
	if (pos == NULL || len < 1) {
		wpa_printf_dbg(MSG_INFO, "EAP-FAST: Invalid frame");
		return TRUE;
	}
					     ...
					     u8 *in_data, size_t in_len)
{
	...
	if (priv == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: %s - Phase2 not "
	...
	if (in_len > sizeof(*hdr) && *pos == EAP_TYPE_NAK) {
		...
		if (m && m->vendor == EAP_VENDOR_IETF &&
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Peer Nak'ed required "
				   "TNC negotiation");
		...
		if (sm->user && sm->user_eap_method_index < EAP_MAX_METHODS &&
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: try EAP type %d",
				   next_type);
		} else {
			next_type = eap_fast_req_failure(sm, data);
		}
	...
	if (m->check(sm, priv, &buf)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Phase2 check() asked to "
			   "ignore the packet");
	...
	if (!m->isSuccess(sm, priv)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Phase2 method failed");
		next_type = eap_fast_req_failure(sm, data);
		eap_fast_phase2_init(sm, data, next_type);
		return;
	}
	...
	case PHASE2_ID:
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: try EAP type %d", next_type);
		break;
	case PHASE2_METHOD:
	case CRYPTO_BINDING:
		...
		if (sm->tnc && !data->tnc_started) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Initialize TNC");
					...
					u8 *in_data, size_t in_len)
{
	...
	if (in_len < (int) sizeof(*hdr)) {
		wpa_printf_dbg(MSG_INFO, "EAP-FAST: Too short Phase 2 "
	...
	if (len > in_len) {
		wpa_printf_dbg(MSG_INFO, "EAP-FAST: Length mismatch in "
			   "Phase 2 EAP frame (len=%lu hdr->length=%lu)",
			   (unsigned long) in_len, (unsigned long) len);
		eap_fast_req_failure(sm, data);
		return;
	}
	wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Received Phase 2: code=%d "
	...
	case EAP_CODE_RESPONSE:
		...
		break;
	default:
		wpa_printf_dbg(MSG_INFO, "EAP-FAST: Unexpected code=%d in "
			       ...
			       struct eap_fast_tlv_parse *tlv)
{
	...
	while (end - pos > 4) {
		...
		if (len > (size_t) (end - pos)) {
			wpa_printf_dbg(MSG_INFO, "EAP-FAST: TLV overflow");
			return -1;
		}
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Received Phase 2: "
		...
		if (res < 0) {
			if (mandatory) {
				wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Nak unknown "
					   "mandatory TLV type %d", tlv_type);
				/* TODO: generate Nak TLV */
				break;
			} else {
				wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Ignored "
	...
	size_t bind_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Reply Crypto-Binding TLV: "
	...
	if (b->version != EAP_FAST_VERSION ||
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Unexpected version "
	...
	if (b->subtype != EAP_TLV_CRYPTO_BINDING_SUBTYPE_RESPONSE) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Unexpected subtype in "
	...
	if (os_memcmp_const(data->crypto_binding_nonce, b->nonce, 31) != 0 ||
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Invalid nonce in "
			   "Crypto-Binding");
		return -1;
	}
	...
	if (os_memcmp_const(cmac, b->compound_mac, sizeof(cmac)) != 0) {
		...
		wpa_printf_dbg(MSG_INFO, "EAP-FAST: Compound MAC did not "
			   "match");
		return -1;
	}
					 ...
					 struct wpabuf *in_data)
{
	...
	if (eap_fast_parse_tlvs(in_data, &tlv) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Failed to parse received "
			   "Phase 2 TLVs");
		return;
	}

	if (tlv.result == EAP_TLV_RESULT_FAILURE) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Result TLV indicated "
			   "failure");
		eap_fast_state(data, FAILURE);
		return;
	}
	...
	if (data->state == REQUEST_PAC) {
		...
		if (tlv.pac == NULL || tlv.pac_len < 6) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: No PAC "
		...
		if (type != PAC_TYPE_PAC_ACKNOWLEDGEMENT || len != 2 ||
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: PAC TLV did not "
				   "contain acknowledgement");
			eap_fast_state(data, FAILURE);
			return;
		}

		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: PAC-Acknowledgement received "
	...
	if (check_crypto_binding) {
		if (tlv.crypto_binding == NULL) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: No Crypto-Binding "
				   "TLV received");
			eap_fast_state(data, FAILURE);
			return;
		}
		...
		if (data->final_result &&
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Crypto-Binding TLV "
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Valid Crypto-Binding TLV "
			   "received");
		if (data->final_result) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Authentication "
		...
		if (data->anon_provisioning &&
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Client is trying to "
		...
		if (data->anon_provisioning ||
		    (tlv.request_action == EAP_TLV_ACTION_PROCESS_TLV &&
		     eap_fast_pac_type(tlv.pac, tlv.pac_len,
				       PAC_TYPE_TUNNEL_PAC))) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Requested a new "
				   "Tunnel PAC");
			eap_fast_state(data, REQUEST_PAC);
		} else if (data->send_new_pac) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Server triggered "
				    ...
				    struct wpabuf *in_buf)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Received %lu bytes encrypted data for"
		   " Phase 2", (unsigned long) wpabuf_len(in_buf));

	if (data->pending_phase2_resp) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Pending Phase 2 response - "
	...
	if (in_decrypted == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-FAST: Failed to decrypt Phase 2 "
			   "data");
		eap_fast_state(data, FAILURE);
		return;
	}
	...
	if (sm->method_pending == METHOD_PENDING_WAIT) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Phase2 method is in "
				    ...
				    int peer_version)
{
	...
	if (data->force_version >= 0 && peer_version != data->force_version) {
		wpa_printf_dbg(MSG_INFO, "EAP-FAST: peer did not select the forced"
	...
	if (peer_version < data->fast_version) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: peer ver=%d, own ver=%d; "
			   "use version %d",
				   ...
				   struct eap_fast_data *data)
{
	if (eap_server_tls_phase1(sm, &data->ssl) < 0) {
		wpa_printf_dbg(MSG_INFO, "EAP-FAST: TLS processing failed");
		eap_fast_state(data, FAILURE);
		return -1;
	}
					 ...
					 struct eap_fast_data *data)
{
	...
	if (data->identity) {
		...
		if (eap_user_get(sm, sm->identity, sm->identity_len, 1) != 0) {
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Identity already "
				 ...
				 const struct wpabuf *respData)
{
	...
	case REQUEST_PAC:
		...
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-FAST: Unexpected state %d in %s",
			   data->state, __func__);
		break;
	}
}
