				       ...
				       struct eap_psk_data *data, u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: PSK-1 (sending)");
				       ...
				       struct eap_psk_data *data, u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: PSK-3 (sending)");
...
static struct wpabuf * eap_psk_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: Unknown state %d in buildReq",
			   data->state);
		break;
	}
	return NULL;
			     ...
			     struct wpabuf *respData)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: received frame: T=%d", t);

	if (data->state == PSK_1 && t != 1) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: Expected PSK-2 - "
			   "ignore T=%d", t);
		return TRUE;
	}

	if (data->state == PSK_3 && t != 3) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: Expected PSK-4 - "
			   "ignore T=%d", t);
		return TRUE;
	}
	...
	if ((t == 1 && len < sizeof(struct eap_psk_hdr_2)) ||
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: Too short frame");
		return TRUE;
	}
			      ...
			      struct wpabuf *respData)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: Received PSK-2");
			      ...
			      struct wpabuf *respData)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: Received PSK-4");
	...
	if (pos[0] == 0 && pos[1] == 0 && pos[2] == 0 && pos[3] == 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PSK: Nonce did not increase");
		return;
	}
