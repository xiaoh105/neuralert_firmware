...
static struct tncs_data * tncs_get_conn(TNC_ConnectionID connectionID)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "TNC: Connection ID %lu not found",
	...
	TNC_UInt32 typeCount)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_TNCS_ReportMessageTypes(imvID=%lu "
		   "typeCount=%lu)",
	...
	TNC_MessageType messageType)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_TNCS_SendMessage(imvID=%lu "
...
{
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_TNCS_RequestHandshakeRetry");
	/* TODO */
	return TNC_RESULT_SUCCESS;
}
	...
	TNC_IMV_Evaluation_Result evaluation)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_TNCS_ProvideRecommendation(imvID=%lu "
...
{
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_TNCS_GetAttribute");
	/* TODO */
	return TNC_RESULT_SUCCESS;
}
...
static TNC_Result TNC_TNCS_SetAttribute(
	...
	TNC_BufferReference buffer)
{
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_TNCS_SetAttribute");
	/* TODO */
	return TNC_RESULT_SUCCESS;
}
...
{
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_TNCS_BindFunction(imcID=%lu, "
...
static int tncs_imv_initialize(struct tnc_if_imv *imv)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "TNC: Calling TNC_IMV_Initialize for IMV '%s'",
		   imv->name);
	res = imv->Initialize(imv->imvID, TNC_IFIMV_VERSION_1,
			      TNC_IFIMV_VERSION_1, &imv_ver);
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_IMV_Initialize: res=%lu imv_ver=%lu",
...
static int tncs_imv_terminate(struct tnc_if_imv *imv)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "TNC: Calling TNC_IMV_Terminate for IMV '%s'",
		   imv->name);
	res = imv->Terminate(imv->imvID);
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_IMV_Terminate: %lu",
		   (unsigned long) res);
...
static int tncs_imv_provide_bind_function(struct tnc_if_imv *imv)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "TNC: Calling TNC_IMV_ProvideBindFunction for "
		   "IMV '%s'", imv->name);
	res = imv->ProvideBindFunction(imv->imvID, TNC_TNCS_BindFunction);
	wpa_printf_dbg(MSG_DEBUG, "TNC: TNC_IMV_ProvideBindFunction: res=%lu",
		   (unsigned long) res);
					     ...
					     TNC_ConnectionState state)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "TNC: Calling TNC_IMV_NotifyConnectionChange(%d)"
...
{
	if (imv->path == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "TNC: No IMV configured");
		return -1;
	}

	wpa_printf_dbg(MSG_DEBUG, "TNC: Opening IMV: %s (%s)",
		   imv->name, imv->path);
			      ...
			      const u8 *msg, size_t len)
{
	...
	for (imv = tncs->imv; imv; imv = imv->next) {
		...
		wpa_printf_dbg(MSG_DEBUG, "TNC: Call ReceiveMessage for IMV '%s'",
			   imv->name);
...
static void tncs_batch_ending(struct tncs_data *tncs)
{
	...
	for (imv = tncs->imv; imv; imv = imv->next) {
		...
		wpa_printf_dbg(MSG_DEBUG, "TNC: Call BatchEnding for IMV '%s'",
			   imv->name);
		res = imv->BatchEnding(imv->imvID, tncs->connectionID);
		wpa_printf_dbg(MSG_DEBUG, "TNC: BatchEnding: %lu",
			   (unsigned long) res);
	}
}
...
static void tncs_solicit_recommendation(struct tncs_data *tncs)
{
	...
	for (imv = tncs->imv; imv; imv = imv->next) {
		...
		wpa_printf_dbg(MSG_DEBUG, "TNC: Call SolicitRecommendation for "
			   "IMV '%s'", imv->name);
		res = imv->SolicitRecommendation(imv->imvID,
						 tncs->connectionID);
		wpa_printf_dbg(MSG_DEBUG, "TNC: SolicitRecommendation: %lu",
			   (unsigned long) res);
	}
}
...
static enum tncs_process_res tncs_derive_recommendation(struct tncs_data *tncs)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "TNC: No more messages from IMVs");
	...
	wpa_printf_dbg(MSG_DEBUG, "TNC: Recommendation: %d", rec);
					  ...
					  int *error)
{
	...
	if (id >= TNC_MAX_IMV_ID) {
		wpa_printf_dbg(MSG_DEBUG, "TNC: Too many IMVs");
		return NULL;
	}
	...
	pos = start;
	wpa_printf_dbg(MSG_DEBUG, "TNC: Configured IMV: %s", pos);
	if (pos + 1 >= end || *pos != '"') {
	...
	*pos2 = '\0';
	...
	pos++;
	wpa_printf_dbg(MSG_DEBUG, "TNC: IMV file: '%s'", pos);
