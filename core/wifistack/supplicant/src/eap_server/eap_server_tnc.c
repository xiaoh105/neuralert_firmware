...
{
	wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: %s -> %s",
						    ...
						    struct eap_tnc_data *data)
{
	...
	case NO_RECOMMENDATION:
		...
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Unknown recommendation");
		return NULL;
	}
...
static struct wpabuf * eap_tnc_build_frag_ack(u8 id, u8 code)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Send fragment ack");
...
static struct wpabuf * eap_tnc_build_msg(struct eap_tnc_data *data, u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Generating Request");
	...
	if (data->out_used == wpabuf_len(data->out_buf)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Sending out %lu bytes "
...
static struct wpabuf * eap_tnc_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	case CONTINUE:
		if (data->out_buf == NULL) {
			...
			if (data->out_buf == NULL) {
				wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Failed to "
					   "generate message");
				return NULL;
			}
			data->out_used = 0;
			     ...
			     struct wpabuf *respData)
{
	...
	if ((*pos & EAP_TNC_VERSION_MASK) != EAP_TNC_VERSION) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Unsupported version %d",
			   *pos & EAP_TNC_VERSION_MASK);
		return TRUE;
	}

	if (*pos & EAP_TNC_FLAGS_START) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Peer used Start flag");
		return TRUE;
	}
...
static void tncs_process(struct eap_tnc_data *data, struct wpabuf *inbuf)
{
	...
	case TNCCS_RECOMMENDATION_ALLOW:
		wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: TNCS allowed access");
		eap_tnc_set_state(data, RECOMMENDATION);
		data->recommendation = ALLOW;
		break;
	case TNCCS_RECOMMENDATION_NO_RECOMMENDATION:
		wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: TNCS has no recommendation");
		eap_tnc_set_state(data, RECOMMENDATION);
		data->recommendation = NO_RECOMMENDATION;
		break;
	case TNCCS_RECOMMENDATION_ISOLATE:
		wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: TNCS requested isolation");
		eap_tnc_set_state(data, RECOMMENDATION);
		data->recommendation = ISOLATE;
		break;
	case TNCCS_RECOMMENDATION_NO_ACCESS:
		wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: TNCS rejected access");
		eap_tnc_set_state(data, RECOMMENDATION);
		data->recommendation = NO_ACCESS;
		break;
	case TNCCS_PROCESS_ERROR:
		wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: TNCS processing error");
		eap_tnc_set_state(data, FAIL);
		break;
	default:
		break;
				...
				const u8 *buf, size_t len)
{
	...
	if (len > wpabuf_tailroom(data->in_buf)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Fragment overflow");
		eap_tnc_set_state(data, FAIL);
		return -1;
	}

	wpabuf_put_data(data->in_buf, buf, len);
	wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Received %lu bytes, waiting for %lu "
				    ...
				    const u8 *buf, size_t len)
{
	...
	if (data->in_buf == NULL && !(flags & EAP_TNC_FLAGS_LENGTH_INCLUDED)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: No Message Length field in a "
			   "fragmented packet");
		return -1;
	}
	...
	if (data->in_buf == NULL) {
		...
		if (data->in_buf == NULL) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: No memory for "
				   "message");
			return -1;
		}
		wpabuf_put_data(data->in_buf, buf, len);
		wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Received %lu bytes in first "
			    ...
			    struct wpabuf *respData)
{
	...
	if (len == 1 && (data->state == DONE || data->state == FAIL)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Peer acknowledged the last "
			   "message");
		return;
	}
	...
	if (flags & EAP_TNC_FLAGS_LENGTH_INCLUDED) {
		if (end - pos < 4) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Message underflow");
			eap_tnc_set_state(data, FAIL);
			return;
		}
		...
		if (message_length < (u32) (end - pos) ||
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Invalid Message "
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Received packet: Flags 0x%x "
		   "Message Length %u", flags, message_length);

	if (data->state == WAIT_FRAG_ACK) {
		if (len > 1) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Unexpected payload "
				   "in WAIT_FRAG_ACK state");
			eap_tnc_set_state(data, FAIL);
			return;
		}
		wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: Fragment acknowledged");
		eap_tnc_set_state(data, CONTINUE);
		return;
	}
	...
	if (flags & EAP_TNC_FLAGS_MORE_FRAGMENTS) {
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-TNC: All fragments received");
		eap_tnc_set_state(data, CONTINUE);
	}
