...
{
	wpa_printf_dbg(MSG_DEBUG, "EAP-TLS: %s -> %s",
...
static struct wpabuf * eap_tls_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	case CONTINUE:
		...
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-TLS: %s - unexpected state %d",
			   __func__, data->state);
		return NULL;
	}
...
check_established:
	if (data->established && data->ssl.state != WAIT_FRAG_ACK) {
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-TLS: Done");
				...
				const struct wpabuf *respData)
{
	...
	if (data->state == SUCCESS && wpabuf_len(data->ssl.tls_in) == 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-TLS: Client acknowledged final TLS "
			   "handshake message");
		return;
	}
...
static u8 * eap_tls_getKey(struct eap_sm *sm, void *priv, size_t *len)
{
	...
	if (eapKeyData) {
		*len = EAP_TLS_KEY_LEN;
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-TLS: Failed to derive key");
	}
...
static u8 * eap_tls_get_emsk(struct eap_sm *sm, void *priv, size_t *len)
{
	...
	if (emsk) {
		*len = EAP_EMSK_LEN;
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-TLS: Failed to derive EMSK");
	}
