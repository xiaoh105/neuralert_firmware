...
{
	wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: %s -> %s",
...
static void * eap_peap_init(struct eap_sm *sm)
{
	...
	if (sm->user && sm->user->force_version >= 0) {
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: forcing version %d",
			   data->force_version);
	...
	if (eap_server_tls_ssl_init(sm, &data->ssl, 0, EAP_TYPE_PEAP)) {
		wpa_printf_dbg(MSG_INFO, "EAP-PEAP: Failed to initialize SSL.");
					    ...
					    struct eap_peap_data *data, u8 id)
{
	...
	req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_PEAP, 1,
			    EAP_CODE_REQUEST, id);
	if (req == NULL) {
		wpa_printf_dbg(MSG_ERROR, "EAP-PEAP: Failed to allocate memory for"
			   " request");
		eap_peap_state(data, FAILURE);
		return NULL;
	}
						 ...
						 u8 id)
{
	...
	if (data->phase2_method == NULL || data->phase2_priv == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Phase 2 method not ready");
		return NULL;
	}
...
#ifdef EAP_SERVER_TNC
		if (data->soh_response) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Adding MS-SOH "
				   "Response TLV");
...
static struct wpabuf * eap_peap_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	case PHASE1_ID2:
		if (tls_connection_established(sm->ssl_ctx, data->ssl.conn)) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Phase1 done, "
				   "starting Phase2");
	...
	case FAILURE_REQ:
		...
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: %s - unexpected state %d",
			   __func__, data->state);
		return NULL;
	}
			      ...
			      struct wpabuf *respData)
{
	...
	if (pos == NULL || len < 1) {
		wpa_printf_dbg(MSG_INFO, "EAP-PEAP: Invalid frame");
		return TRUE;
	}
					  ...
					  size_t crypto_tlv_len)
{
	...
	if (crypto_tlv_len != 4 + 56) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Invalid cryptobinding TLV "
	...
	if (pos[1] != data->peap_version) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Cryptobinding TLV Version "
	...
	if (pos[3] != 1) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Unexpected Cryptobinding TLV "
			   "SubType %d", pos[3]);
		return -1;
	}
	pos += 4;
	...
	if (os_memcmp_const(mac, pos, SHA1_MAC_LEN) != 0) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Invalid Compound_MAC in "
			   "cryptobinding TLV");
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Valid cryptobinding TLV received");
					...
					struct wpabuf *in_data)
{
	...
	if (pos == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Invalid EAP-TLV header");
		return;
	}
	...
	while (left >= 4) {
		...
		if ((size_t) tlv_len > left) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: TLV underrun "
		...
		case EAP_TLV_CRYPTO_BINDING_TLV:
			...
			break;
		default:
			wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Unsupported TLV Type "
				   "%d%s", tlv_type,
				   mandatory ? " (mandatory)" : "");
			if (mandatory) {
				eap_peap_state(data, FAILURE);
	...
	if (left) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Last TLV too short in "
	...
	if (crypto_tlv && data->crypto_binding_sent) {
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: No cryptobinding TLV");
		eap_peap_state(data, FAILURE);
		return;
	}
	...
	if (result_tlv) {
		...
		if (result_tlv_len < 2) {
			wpa_printf_dbg(MSG_INFO, "EAP-PEAP: Too short Result TLV "
				   "(len=%lu)",
		...
		if (status == EAP_TLV_RESULT_SUCCESS) {
			wpa_printf_dbg(MSG_INFO, "EAP-PEAP: TLV Result - Success "
			...
			wpa_printf_dbg(MSG_INFO, "EAP-PEAP: TLV Result - Failure "
				   "- requested %s", requested);
			eap_peap_state(data, FAILURE);
		} else {
			wpa_printf_dbg(MSG_INFO, "EAP-PEAP: Unknown TLV Result "
				   "Status %d", status);
			eap_peap_state(data, FAILURE);
		}
	}
					...
					struct wpabuf *in_data)
{
	...
	if (pos == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Not a valid SoH EAP "
	...
	while (left >= 4) {
		...
		case EAP_TLV_VENDOR_SPECIFIC_TLV:
			if (tlv_len < 4) {
				wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Too short "
			...
			if (vpos + vtlv_len > pos + left) {
				wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Vendor TLV "
					   "underrun");
				eap_peap_state(data, FAILURE);
				return;
			}
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Unsupported MS-TLV "
				   "Type %d%s", tlv_type,
				   mandatory ? " (mandatory)" : "");
			if (mandatory) {
				eap_peap_state(data, FAILURE);
	...
	if (soh_tlv) {
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: No SoH TLV received");
		eap_peap_state(data, FAILURE);
		return;
	}
...
auth_method:
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: try EAP vendor %d type %d",
					     ...
					     struct wpabuf *in_data)
{
	...
	if (data->phase2_priv == NULL) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: %s - Phase2 not "
	...
	if (wpabuf_len(in_data) > sizeof(*hdr) && *pos == EAP_TYPE_NAK) {
		...
		if (sm->user && sm->user_eap_method_index < EAP_MAX_METHODS &&
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: try EAP type %d",
				   next_type);
		} else {
	...
	if (data->phase2_method->check(sm, data->phase2_priv, in_data)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Phase2 check() asked to "
			   "ignore the packet");
		return;
	}
	...
	if (sm->method_pending == METHOD_PENDING_WAIT) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Phase2 method is in "
	...
	if (!data->phase2_method->isSuccess(sm, data->phase2_priv)) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Phase2 method failed");
		eap_peap_req_failure(sm, data);
		next_vendor = EAP_VENDOR_IETF;
		next_type = EAP_TYPE_NONE;
		eap_peap_phase2_init(sm, data, next_vendor, next_type);
	...
	if (data->phase2_method->getKey) {
		...
		if (data->phase2_key == NULL) {
			wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Phase2 getKey "
				   "failed");
			eap_peap_req_failure(sm, data);
	...
	case PHASE2_SOH:
		...
		if (data->state != PHASE2_SOH && sm->tnc &&
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Try to initialize "
				   "TNC (NAP SOH)");
			next_vendor = EAP_VENDOR_IETF;
			next_type = EAP_TYPE_NONE;
			break;
		...
		next_type = sm->user->methods[0].method;
		sm->user_eap_method_index = 1;
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: try EAP type %d", next_type);
		break;
	case PHASE2_METHOD:
				    ...
				    struct wpabuf *in_buf)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: received %lu bytes encrypted data for"
	...
	if (in_decrypted == NULL) {
		wpa_printf_dbg(MSG_INFO, "EAP-PEAP: Failed to decrypt Phase 2 "
			   "data");
		eap_peap_state(data, FAILURE);
		return;
	}
	...
	if (wpabuf_len(in_decrypted) < (int) sizeof(*hdr)) {
		wpa_printf_dbg(MSG_INFO, "EAP-PEAP: Too short Phase 2 "
			   "EAP frame (len=%lu)",
	...
	if (len > wpabuf_len(in_decrypted)) {
		wpa_printf_dbg(MSG_INFO, "EAP-PEAP: Length mismatch in "
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: received Phase 2: code=%d "
	...
	case EAP_CODE_SUCCESS:
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Phase 2 Success");
	...
	case EAP_CODE_FAILURE:
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Phase 2 Failure");
		eap_peap_state(data, FAILURE);
		break;
	default:
		wpa_printf_dbg(MSG_INFO, "EAP-PEAP: Unexpected code=%d in "
				    ...
				    int peer_version)
{
	...
	if (data->force_version >= 0 && peer_version != data->force_version) {
		wpa_printf_dbg(MSG_INFO, "EAP-PEAP: peer did not select the forced"
			   " version (forced=%d peer=%d) - reject",
			   data->force_version, peer_version);
		return -1;
	}
	if (peer_version < data->peap_version) {
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: peer ver=%d, own ver=%d; "
			   "use version %d",
				 ...
				 const struct wpabuf *respData)
{
	...
	case FAILURE_REQ:
		...
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Unexpected state %d in %s",
			   data->state, __func__);
		break;
	}
}
...
static u8 * eap_peap_getKey(struct eap_sm *sm, void *priv, size_t *len)
{
	...
	if (data->crypto_binding_used) {
		...
		if (eapKeyData) {
			...
			*len = EAP_TLS_KEY_LEN;
			...
			wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Failed to derive "
				   "key");
		}
	...
	if (eapKeyData) {
		*len = EAP_TLS_KEY_LEN;
		...
		wpa_printf_dbg(MSG_DEBUG, "EAP-PEAP: Failed to derive key");
	}
