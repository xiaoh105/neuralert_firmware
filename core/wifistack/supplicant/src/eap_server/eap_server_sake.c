...
{
	wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: %s -> %s",
...
static void * eap_sake_init(struct eap_sm *sm)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Initialized Session ID %d",
		   data->session_id);
					       ...
					       u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Request/Identity");
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: * AT_PERM_ID_REQ");
	eap_sake_add_attr(msg, EAP_SAKE_AT_PERM_ID_REQ, NULL, 2);

	wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: * AT_SERVERID");
						...
						u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Request/Challenge");
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: * AT_RAND_S");
					      ...
					      u8 id)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Request/Confirm");
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: * AT_MIC_S");
...
static struct wpabuf * eap_sake_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	default:
		wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Unknown state %d in buildReq",
			   data->state);
		break;
	}
	return NULL;
			      ...
			      struct wpabuf *respData)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Received frame: subtype=%d", subtype);
				      ...
				      const u8 *payload, size_t payloadlen)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Received Response/Identity");
				       ...
				       const u8 *payload, size_t payloadlen)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Received Response/Challenge");
				     ...
				     const u8 *payload, size_t payloadlen)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Received Response/Confirm");
...
static void eap_sake_process_auth_reject(struct eap_sm *sm,
					 ...
					 const u8 *payload, size_t payloadlen)
{
	wpa_printf_dbg(MSG_DEBUG, "EAP-SAKE: Received Response/Auth-Reject");
	eap_sake_state(data, FAILURE);
}
