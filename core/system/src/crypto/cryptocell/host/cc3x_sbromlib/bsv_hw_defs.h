...
#define CC_BSV_READ_OTP_WORD(hwBaseAddress, otpAddr, otpData)                           \
    do {                                                                                \
        SB_HAL_READ_REGISTER((DA16X_ACRYPT_BASE + CC_OTP_BASE_ADDR + otpAddr), otpData);    \
    }while(0)
#else
#define CC_BSV_READ_OTP_WORD(hwBaseAddress, otpAddr, otpData)					\
	{																			\
		otp_mem_create();														\
		otp_mem_read((otpAddr>>2), otpData);									\
		otp_mem_close();														\
	}
#endif
...
#define CC_BSV_WRITE_OTP_WORD(hwBaseAddress, otpAddr, otpData)                          \
    do {                                                                                \
        SB_HAL_WRITE_REGISTER((DA16X_ACRYPT_BASE + CC_OTP_BASE_ADDR + otpAddr), otpData);   \
        CC_BSV_WAIT_ON_AIB_ACK_BIT(hwBaseAddress);                                      \
    }while(0)
#else
#define CC_BSV_WRITE_OTP_WORD(hwBaseAddress, otpAddr, otpData)							\
	{																			\
		extern int debug_otp_mem_write(UINT32 offset, UINT32 data);			\
		otp_mem_create();														\
		debug_otp_mem_write((otpAddr>>2), otpData);								\
		otp_mem_close();														\
	}
#endif
