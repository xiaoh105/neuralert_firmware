...
{
    int err = 0;
    uint32_t n_size_bits, a_size_bits, e_size_bits;
    ...
    e_size_bits = mbedtls_mpi_bitlen( E );
    ...
    // Reduce E ( E mod N - 1). N must be prime.
    if( e_size_bits > n_size_bits ) {
        err = mbedtls_mpi_sub_int(&_N, N, 1);
        if( err != 0 ) {
            goto End;
        }

        err = mbedtls_mpi_mod_mpi(&_E, E, &_N);
        if( err != 0 ) {
            goto End;
        }
    } else {
        err = mbedtls_mpi_copy(&_E, E);
        if( err != 0 ) {
            goto End;
        }
    }
    ...
    if( a_size_bits > n_size_bits ) {
        ...
        err = PkiExecModExpLeW( X->p/*out*/, T.p/*in*/, n_len_words, N->p/*modulus*/, n_size_bits, _E.p, _E.n );
        if ( err != 0 ) {
            goto End;
        }
    }
    /* exponentiation without reduction */
        ...
        /* execute exponentiation on PKA HW */
        err = PkiExecModExpLeW( X->p/*out*/, A->p/*in*/, a_len_words, N->p/*modulus*/, n_size_bits, _E.p, _E.n );
        if ( err != 0 ) {
            goto End;
        }
    }
                     ...
                     void *p_rng )
{
    ...
    if( ctx == NULL || output == NULL || f_rng == NULL || p_rng ==NULL || olen < 1 || olen > ctx->len )
        return( MBEDTLS_ERR_DHM_BAD_INPUT_DATA );
    if( x_size > (int)ctx->len )
        return( MBEDTLS_ERR_DHM_BAD_INPUT_DATA );
