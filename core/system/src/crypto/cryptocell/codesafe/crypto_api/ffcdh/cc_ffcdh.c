...
)
{
        ...
        DhContext_t *pDhCtx;
        size_t hashSize, minHashSize;
        size_t ffcPrimeSize, ffcOrderSize;
...
// ??? TBD which size is needed to save in Ctx ?
...
//  uint32_t publKeyDataSize = 0; //, privKeySize;
    ...
    } else if(schemeInfo->doUserEphemKey) {
    ...
    } else if(schemeInfo->doPartnerEphemKey) {
