...
#ifdef	ISSUE_IAR_WEAK_POINT
	// Skip
#else	//ISSUE_IAR_WEAK_POINT
        CCRndGenerateVectWorkFunc_t RndGenerateVectFunc;
#endif //ISSUE_IAR_WEAK_POINT
        /* the pointer to EC domain (curve). */
        CCEcMontDomain_t *pEcDomain;
        ...
        /* get domain */
        pEcDomain = (CCEcMontDomain_t *)EcMontGetCurve25519Domain();
...
#ifdef	ISSUE_IAR_WEAK_POINT
        if(pRndContext->rndGenerateVectFunc == NULL)
                return CC_RND_GEN_VECTOR_FUNC_ERROR;

        err = pRndContext->rndGenerateVectFunc(
