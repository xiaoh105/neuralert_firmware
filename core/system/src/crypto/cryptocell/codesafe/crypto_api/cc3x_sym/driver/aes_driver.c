...
drvError_t ProcessAesDrv(AesContext_t *aesCtx, CCBuffInfo_t *pInputBuffInfo, CCBuffInfo_t *pOutputBuffInfo, uint32_t blockSize)
{
        ...
        /* set dma completion bit in irr */
        CC_REG_FLD_SET(HOST_RGF, HOST_IRR, SYM_DMA_COMPLETED, irrVal, 1);
#if 0	/* workaround - redmine #1339 */
    drvRc = CC_HalWaitInterrupt(irrVal);
#else
#define IRQ_WAIT_TIMEOUT		300 // RTC clock around 10ms
    {
        uint32_t irr = 0;
        unsigned long long old_time;
        old_time = RTC_GET_COUNTER() + IRQ_WAIT_TIMEOUT;
        ...
        do {
            ...
            if( CC_REG_FLD_GET(0, HOST_IRR, AHB_ERR_INT, irr) == CC_TRUE){
                ...
                CC_REG_FLD_SET(HOST_RGF, HOST_ICR, AXI_ERR_CLEAR, irrVal , 1);
                break;
            }
        } while ((!(irr & irrVal)) && (old_time > RTC_GET_COUNTER()) );

        /* clear interrupt */
        CC_HAL_WRITE_REGISTER(CC_REG_OFFSET(HOST_RGF, HOST_ICR), irrVal);
        drvRc = CC_OK;
    }
#endif
    if (drvRc != 0){
