                    ...
                    uint32_t ccmMode)
{
    ...
    pAesCcmCtx = (AesCcmContext_t *)( &(ctx->buf[0]) );
#else	//CCM_FCI_OPTIMIZE
...
static int ccm_text_data(mbedtls_ccm_context *ctx, const uint8_t *pTextDataIn, size_t textDataSize, uint8_t *pTextDataOut)
{
    ...
    /* check overlapping of input-output buffers: overlapping is not allowed, excluding
     in placement, i.e. pTextDataIn = pTextDataOut */
    if (pTextDataIn != pTextDataOut)
    {
        if (pTextDataIn > pTextDataOut)
        {
            if (pTextDataOut + textDataSize > pTextDataIn)
            {
                return MBEDTLS_ERR_CCM_BAD_INPUT;
            }
        }
        else if (pTextDataIn + textDataSize > pTextDataOut)
        {
            return MBEDTLS_ERR_CCM_BAD_INPUT;
        }
    }
...
#ifdef	CCM_FCI_OPTIMIZE
    aes_ccm_ctx = (AesCcmContext_t *)( &(ctx->buf[0]) );
#else	//CCM_FCI_OPTIMIZE
