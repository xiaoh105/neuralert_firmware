...
*               PUBLIC FUNCTIONS
******************************************************************************/
drvError_t InitChachaExtDma(uint32_t *pNonceBuf, chachaNonceSize_t nonceSizeFlag, uint32_t *keyBuf, uint32_t initialCounter)
{
    uint32_t irrVal = 0;
    ...
    if (pNonceBuf == NULL || keyBuf == NULL) {
        rc = CHACHA_DRV_INVALID_USER_CONTEXT_POINTER_ERROR;
        goto InitExit;
    }
