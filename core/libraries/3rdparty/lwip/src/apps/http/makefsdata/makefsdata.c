...
int main(int argc, char *argv[])
{
  ...
  if (ssi_file_buffer) {
    vPortFree(ssi_file_buffer);
  }
  if (ssi_file_lines) {
    vPortFree(ssi_file_lines);
  }
  ...
  if (deflateNonSsiFiles) {
    if (can_be_compressed) {
      if (fsize < OUT_BUF_SIZE) {
        ...
        if (out_bytes < fsize) {
          ret_buf = (u8_t *)pvPortMalloc(out_bytes);
...
static void fix_filename_for_c(char *qualifiedName, size_t max_len)
{
  ...
  char *new_name = (char *)pvPortMalloc(len + 2);
  int filename_ok;
  int cnt = 0;
  size_t i;
  if (len + 3 == max_len) {
...
{
  struct file_entry *fe = (struct file_entry *)pvPortMalloc(sizeof(struct file_entry));
...
static int checkSsiByFilelist(const char* filename_listfile)
{
  ...
  if (f != NULL) {
    ...
    lines = (char**)pvPortMalloc(sizeof(char*) * num_lines);
    if (!lines) {
